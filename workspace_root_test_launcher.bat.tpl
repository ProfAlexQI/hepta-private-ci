...
if defined HEPTA_BAZEL_TEST_SKIP_FILTERS (
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$skipFilters = @();" ^
  "$rawSkipFilters = if (-not [string]::IsNullOrEmpty($env:HEPTA_BAZEL_TEST_SKIP_FILTERS)) { $env:HEPTA_BAZEL_TEST_SKIP_FILTERS } else { $env:CODEX_BAZEL_TEST_SKIP_FILTERS };" ^
  "if (-not [string]::IsNullOrEmpty($rawSkipFilters)) { $skipFilters = @($rawSkipFilters -split ',' | Where-Object { $_ -ne '' }) };" ^
