    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if let Event::Actions(actions) = event {
            for (index, wr) in portal_list.items_with_actions(actions) {
                ...
                if wr.button(cx, ids!(hepta_actions.inspect_button)).clicked(actions) {
                    self.handle_hepta_inspect_event(cx, index);
                }
                if wr.button(cx, ids!(hepta_actions.approve_button)).clicked(actions) {
                    self.handle_hepta_approval_decision(cx, index, true);
                }
                if wr.button(cx, ids!(hepta_actions.reject_button)).clicked(actions) {
                                    ...
                                    && msg_like_content.thread_root.is_some()
                                {
                                    ...
                                    match &msg_like_content.kind {
                                        ...
                                        MsgLikeKind::Other(other)
                                            if is_hepta_event_type(&other.event_type().to_string()) =>
                                        {
...
impl RoomScreen {
    ...
    fn handle_hepta_approval_decision(&self, cx: &mut Cx, index: usize, approved: bool) {
        ...
        let envelope_id = envelope
            ...
            .or_else(|| event_tl_item.event_id().map(|event_id| event_id.to_string()))
    ...
    fn apply_hepta_fixture_timeline_visibility(&mut self, cx: &mut Cx, visible: bool) {
        ...
        self.view.view(cx, ids!(hepta_fixture_timeline)).set_visible(cx, visible);
        self.view.view(cx, ids!(typing_notice)).set_visible(cx, false);
...
fn populate_hepta_event_card(
    ...
    item.label(cx, ids!(header.eyebrow)).set_text(cx, &card.eyebrow);
    item.label(cx, ids!(header.status)).set_text(cx, &card.status);
...
fn hepta_policy_badge_text(envelope: Option<&HeptaEventEnvelope>) -> String {
    ...
    match envelope.event_kind.as_str() {
        ...
        "task" | "agent_run" => {
            "policy: progress event · read-only timeline evidence".to_string()
        }
