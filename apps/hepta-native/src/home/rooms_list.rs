...
use crate::{
    ...
    home::{
        ...
        room_screen::invalidate_timeline_state,
...
impl RoomsList {
    ...
    fn set_current_active_room(&mut self, cx: &mut Cx, selected_room: Option<SelectedRoom>) {
        if self.current_active_room == selected_room {
            return;
        }
        ...
        for action in rooms_list_actions {
                ...
                action.as_widget_action().cast()
            {
                if !toggle_supported_rooms_header(
                    ...
                    error!(
                        "BUG: ignored toggle from an inactive rooms-list header category: {category:?}"
                    );
                    continue;
                }
                self.redraw(cx);
