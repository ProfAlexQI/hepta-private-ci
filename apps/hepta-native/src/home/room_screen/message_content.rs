...
pub(super) fn populate_image_message_content_with_fallback(
    ...
    let fallback = text_or_image_ref
        .status()
        .is_text()
        ...
        |cx: &mut Cx, media_source: MediaSource, image_info: &ImageInfo| {
            match media_cache.try_get_media_or_fetch(&media_source, MEDIA_THUMBNAIL_FORMAT.into()) {
                (MediaCacheEntry::Loaded(data), media_format) => {
                    ...
                    let variant = if matches!(media_format, MediaFormat::File) {
                        "full"
                    } else {
                        "thumb"
                    };
                    let cache_key = format!("{}#{variant}", media_source_mxc(&media_source));
                    let show_image_result =
                        text_or_image_ref.show_image(cx, Some(media_source), |cx, img| {
                            utils::load_image_with_cache_key(
                                &img,
                                cx,
                                std::path::Path::new(&cache_key),
                                Arc::clone(&data),
                            )
                ...
                (MediaCacheEntry::Requested, _media_format) => {
                    ...
                    if let (Some(blurhash), Some(width), Some(height)) = (
                        image_info.blurhash.as_deref(),
                        image_info.width,
                        image_info.height,
                    ) {
                        let show_image_result =
                    ...
                    if should_fetch_full_size && status_code != StatusCode::NOT_FOUND =>
                {
                    match media_cache.try_get_media_or_fetch(&media_source, MediaFormat::File) {
                        (MediaCacheEntry::Loaded(data), _) => {
                            ...
                            let res = text_or_image_ref.show_image(
                                cx,
                                Some(media_source.clone()),
                                |cx, img| {
                                    utils::load_image_with_cache_key(
                                        &img,
                                        cx,
...
pub(super) fn populate_file_message_content(
    ...
    let caption = file_content
        ...
        .or_else(|| {
            file_content
                .caption()
                .map(|c| format!("{}<br>", htmlize::escape_text(c)))
        })
        .unwrap_or_default();

    message_content_widget.show_html(cx, format!("<b>File: </b>{caption}{filename}{size}"));
    true
}
    ...
    if let Some(redacted_msg) = event_tl_item.latest_json() {
        ...
        ))) = redacted_msg.deserialize()
        {
            if let Ok(redacted_because) = redaction.unsigned.redacted_because.deserialize() {
                ...
                redactor_id_and_reason = Some((redacted_because.sender().to_owned(), reason));
            }
        }
    }
