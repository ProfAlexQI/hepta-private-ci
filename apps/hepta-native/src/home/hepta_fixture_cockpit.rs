...
mod tests {
    ...
    fn hepta_fixture_cockpit_has_a_card_for_each_sample_event() {
        assert!(
            sample_matrix_timeline_events().len() <= HEPTA_FIXTURE_COCKPIT_VISIBLE_CARD_CAPACITY
        );
    }
    ...
    fn hepta_fixture_route_shell_pages_match_chrome_routes_without_live_mutation() {
        ...
        assert!(
            pages
                .iter()
                .any(|page| page.route == "Actions"
                    && page.primary_surface.contains("pending steps"))
        );
        assert!(
            pages.iter().any(|page| page.route == "Approvals"
    ...
    fn hepta_fixture_route_states_drive_first_screen_content_without_live_mutation() {
        ...
        assert!(states.iter().any(|state| state.route == "Approvals"
    ...
    fn hepta_fixture_selected_row_variants_cover_each_route_row_without_live_mutation() {
        ...
        assert_eq!(
            fixture_selected_row_detail_for_row(HeptaFixtureRouteKey::Actions, 0)
                .selected_row_label,
            "Draft plan"
        );
        assert_eq!(
            fixture_selected_row_detail_for_row(HeptaFixtureRouteKey::Approvals, 2)
                .selected_row_label,
            "Decision"
        );
        assert!(
        ...
        for route in HEPTA_FIXTURE_ROUTE_KEYS {
            for selected_row_index in 0..HEPTA_FIXTURE_ROUTE_ROW_COUNT {
                ...
                assert!(
                    action_lines
                        .iter()
                        .any(|line| line.contains("enabled=false")
                            && line.contains("mutation=false"))
                );
            }
        }

        let approvals = fixture_selected_row_action_strip_for(
            &fixture_selected_row_detail_for_row(HeptaFixtureRouteKey::Approvals, 1),
        );
        assert!(approvals.inspect_action.contains("exact request"));
        let inspector = fixture_selected_row_action_strip_for(
            &fixture_selected_row_detail_for_row(HeptaFixtureRouteKey::Inspector, 1),
        );
