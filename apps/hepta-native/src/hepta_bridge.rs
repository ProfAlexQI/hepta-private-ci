//! Local Hepta-runtime-to-Matrix event shape helpers for Hepta Native.
//!
//! This does not talk to the OpenClaw Gateway and does not send Matrix events.
//! It defines the narrow, testable payload shape that Hepta's own native
//! OpenClaw-parity runtime can project into Robrix's Matrix timeline heart as
//! `m.hepta.*` custom events.

use crate::hepta_event::{
    event_type_for_event_kind, HeptaEventEnvelope, HeptaEventStatus, HeptaRedactionState,
    HEPTA_SCHEMA_V1,
};
...
mod tests {
    use super::*;
    use crate::hepta_event::{HeptaEventEnvelope, EVENT_APPROVAL_REQUEST, EVENT_TOOL_RESULT};
