...
use matrix_sdk::{
    ruma::{
        ...
        events::{
            ...
            AnyRedactionEvent, AnySyncMessageLikeEvent, AnySyncTimelineEvent,
            StateEventContentChange, SyncMessageLikeEvent,
        },
        serde::Raw,
        UserId,
    },
    ...
    if let Some(redacted_msg) = latest_json {
        ...
        ))) = redacted_msg.deserialize()
        {
            if let Ok(redacted_because) = redaction.unsigned.redacted_because.deserialize() {
                ...
                redactor_and_reason = Some((redacted_because.sender().to_owned(), reason));
            }
        }
    }
