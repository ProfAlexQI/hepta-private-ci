...
pub enum HeptaAndroidTonalRole {
    Solid,
    Chrome,
    Sheet,
}
...
impl HeptaAndroidActivityIdentity {
    pub const fn is_valid(self) -> bool {
        self.activity_instance_id != 0
            && self.chrome_view_id > 0
            && self.sheet_view_id > 0
            && self.chrome_view_id != self.sheet_view_id
    }
}
...
impl HeptaAndroidMaterial3Environment {
    pub fn is_valid(self) -> bool {
        self.verified
            && self.animator_duration_scale.is_finite()
            && (0.0..=10.0).contains(&self.animator_duration_scale)
    }
}
...
pub trait HeptaAndroidMaterial3ActivityApi {
    ...
    ) -> Result<HeptaAndroidMaterial3Environment, i32>;
...
pub struct HeptaAndroidMaterial3Adapter<A> {
    identity: HeptaAndroidActivityIdentity,
    api: A,
    bound: bool,
}
impl<A> HeptaAndroidMaterial3Adapter<A> {
    pub const fn new(identity: HeptaAndroidActivityIdentity, api: A) -> Self {
        Self {
            identity,
            api,
            bound: false,
    ...
    where
        A: HeptaAndroidMaterial3ActivityApi,
    {
        let _ = self
            .api
            .apply_role(self.identity.sheet_view_id, HeptaAndroidTonalRole::Solid);
        let _ = self
            .api
            .apply_role(self.identity.chrome_view_id, HeptaAndroidTonalRole::Solid);
        self.bound = false;
    }
}
impl<A: HeptaAndroidMaterial3ActivityApi> HeptaSystemMaterialAdapter
    for HeptaAndroidMaterial3Adapter<A>
{
    ...
    fn bind(
        ...
        if profile.content != HeptaMaterialRenderer::Solid
            || profile.chrome != HeptaMaterialRenderer::AndroidTonalChrome
            || profile.transient != HeptaMaterialRenderer::AndroidTonalSheet
        ...
        let env = self
            .api
            .environment(self.identity.activity_instance_id)
            .map_err(HeptaSystemMaterialError::SystemCallFailed)?;
        if !env.is_valid() {
            self.rollback();
            return Err(HeptaSystemMaterialError::SystemApiUnavailable);
        }
        if !env.dynamic_color_available {
            self.rollback();
            return Err(HeptaSystemMaterialError::UnsupportedPlatform);
        }
        self.api
            .apply_dynamic_colors(self.identity.activity_instance_id)
            .map_err(HeptaSystemMaterialError::SystemCallFailed)?;
        self.api
            .apply_role(self.identity.chrome_view_id, HeptaAndroidTonalRole::Chrome)
            .map_err(HeptaSystemMaterialError::SystemCallFailed)?;
        if let Err(error) = self
            .api
            .apply_role(self.identity.sheet_view_id, HeptaAndroidTonalRole::Sheet)
        {
            self.rollback();
            return Err(HeptaSystemMaterialError::SystemCallFailed(error));
        }
        let exact = self.api.observe_role(self.identity.chrome_view_id).ok()
            == Some(HeptaAndroidTonalRole::Chrome)
            && self.api.observe_role(self.identity.sheet_view_id).ok()
                == Some(HeptaAndroidTonalRole::Sheet);
        if !exact {
            self.rollback();
        ...
        Ok(HeptaSystemMaterialReceipt {
            platform: HeptaPlatform::Android,
            chrome: HeptaMaterialRenderer::AndroidTonalChrome,
            transient: HeptaMaterialRenderer::AndroidTonalSheet,
...
mod tests {
    ...
    #[derive(Default)]
    struct Fake {
        env: Option<HeptaAndroidMaterial3Environment>,
        chrome: Option<HeptaAndroidTonalRole>,
        sheet: Option<HeptaAndroidTonalRole>,
        fail_sheet: bool,
    }
    impl HeptaAndroidMaterial3ActivityApi for Fake {
        fn environment(&mut self, _: u64) -> Result<HeptaAndroidMaterial3Environment, i32> {
            self.env.ok_or(-1)
        }
        fn apply_dynamic_colors(&mut self, _: u64) -> Result<(), i32> {
            Ok(())
        }
        fn apply_role(&mut self, id: i32, role: HeptaAndroidTonalRole) -> Result<(), i32> {
            if id == 12 && self.fail_sheet {
                return Err(-2);
            }
            if id == 11 {
                self.chrome = Some(role)
            } else if id == 12 {
                self.sheet = Some(role)
            } else {
                return Err(-3);
            }
            Ok(())
        }
        fn observe_role(&mut self, id: i32) -> Result<HeptaAndroidTonalRole, i32> {
            match id {
                11 => self.chrome.ok_or(-4),
                12 => self.sheet.ok_or(-4),
                _ => Err(-5),
            }
        }
    }
    fn env(dynamic: bool) -> HeptaAndroidMaterial3Environment {
        HeptaAndroidMaterial3Environment {
            dynamic_color_available: dynamic,
    ...
    #[test]
    fn android_binds_tonal_roles_without_blur() {
        let mut a = HeptaAndroidMaterial3Adapter::new(
            HeptaAndroidActivityIdentity {
        ...
        assert!(a.bind(profile()).unwrap().system_material_bound);
        a.unbind();
        assert!(!a.is_bound())
    }
    #[test]
    fn unavailable_dynamic_color_and_partial_apply_fail_closed() {
        let mut a = HeptaAndroidMaterial3Adapter::new(
            HeptaAndroidActivityIdentity {
                activity_instance_id: 7,
                chrome_view_id: 11,
        ...
        assert_eq!(
            a.bind(profile()),
            Err(HeptaSystemMaterialError::UnsupportedPlatform)
        );
        let mut a = HeptaAndroidMaterial3Adapter::new(
            HeptaAndroidActivityIdentity {
                activity_instance_id: 7,
