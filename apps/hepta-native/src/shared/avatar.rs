...
impl Avatar {
    ...
    pub fn set_avatar_and_get_username(
        ...
        let (avatar_img_data_opt, profile_drawn) = match avatar_state {
            ...
            // Avatar widget read evidence: known MXC avatar URIs reuse the
            // avatar_cache FetchAvatar read path; Unknown, Known(None),
            // Failed, and Requested states stay local fallback UI without
            // SetAvatar, FetchMedia, profile/account mutation, membership,
            // message, or room-state requests from the widget.
