...
pub enum HeptaIosUIKitMaterial {
    Solid,
    NavigationGlass,
    SystemSheet,
}
...
impl HeptaIosUIKitHostIdentity {
    pub const fn is_valid(self) -> bool {
        self.navigation_view != 0
            && self.sheet_view_controller != 0
            && self.navigation_view != self.sheet_view_controller
    }
}
...
pub struct HeptaIosUIKitAdapter<A> {
    host: HeptaIosUIKitHostIdentity,
    api: A,
    bound: bool,
}
impl<A> HeptaIosUIKitAdapter<A> {
    pub const fn new(host: HeptaIosUIKitHostIdentity, api: A) -> Self {
        Self {
            host,
            api,
            bound: false,
        ...
        A: HeptaIosUIKitApi,
    {
        let _ = self.api.apply_material(
            self.host.sheet_view_controller,
            HeptaIosUIKitMaterial::Solid,
        );
        let _ = self
            .api
            .apply_material(self.host.navigation_view, HeptaIosUIKitMaterial::Solid);
        self.bound = false;
    }
}
...
impl<A: HeptaIosUIKitApi> HeptaSystemMaterialAdapter for HeptaIosUIKitAdapter<A> {
    ...
    fn bind(
        ...
        if !environment.permits_material() {
            self.rollback();
            return Err(HeptaSystemMaterialError::UserTransparencyDisabled);
        }
        self.api
            .apply_material(
                self.host.navigation_view,
                HeptaIosUIKitMaterial::NavigationGlass,
            )
            .map_err(HeptaSystemMaterialError::SystemCallFailed)?;
        if let Err(error) = self.api.apply_material(
            self.host.sheet_view_controller,
            HeptaIosUIKitMaterial::SystemSheet,
        ) {
            self.rollback();
            return Err(HeptaSystemMaterialError::SystemCallFailed(error));
        }
        let exact = self.api.observe_material(self.host.navigation_view).ok()
            == Some(HeptaIosUIKitMaterial::NavigationGlass)
            && self
                .api
                .observe_material(self.host.sheet_view_controller)
                .ok()
                == Some(HeptaIosUIKitMaterial::SystemSheet);
        if !exact {
            self.rollback();
        ...
        Ok(HeptaSystemMaterialReceipt {
            platform: HeptaPlatform::Ios,
            chrome: HeptaMaterialRenderer::IosSystemGlass,
            transient: HeptaMaterialRenderer::IosSystemSheet,
...
mod tests {
    ...
    #[derive(Default)]
    struct FakeApi {
        nav: Option<HeptaIosUIKitMaterial>,
        sheet: Option<HeptaIosUIKitMaterial>,
        env: Option<HeptaIosUIKitEnvironment>,
        fail_sheet: bool,
    }
    impl HeptaIosUIKitApi for FakeApi {
        ...
        fn apply_material(
            &mut self,
            host: isize,
            material: HeptaIosUIKitMaterial,
        ) -> Result<(), i32> {
            if host == 12 && self.fail_sheet {
                return Err(-2);
            };
            if host == 11 {
                self.nav = Some(material)
            } else if host == 12 {
                self.sheet = Some(material)
            } else {
                return Err(-3);
            };
            Ok(())
    ...
    fn env() -> HeptaIosUIKitEnvironment {
        HeptaIosUIKitEnvironment {
            reduce_transparency: false,
            darker_system_colors: false,
            reduce_motion: false,
            dynamic_type_scale: 2.0,
            safe_area_top: 47.0,
    ...
    #[test]
    fn ios_binds_and_rolls_back() {
        let mut a = HeptaIosUIKitAdapter::new(
            HeptaIosUIKitHostIdentity {
                navigation_view: 11,
                sheet_view_controller: 12,
            },
            FakeApi {
                env: Some(env()),
                ..FakeApi::default()
    ...
    #[test]
    fn ios_accessibility_and_partial_bind_fail_closed() {
        let mut denied = env();
        denied.reduce_transparency = true;
        let mut a = HeptaIosUIKitAdapter::new(
            HeptaIosUIKitHostIdentity {
                navigation_view: 11,
                sheet_view_controller: 12,
