...
pub struct FileUploadModal {
    ...
    #[rust]
    preview_id: Option<FileUploadAttemptId>,
}
...
pub fn submit_attachment_upload(upload: AttachmentUpload) -> bool {
    ...
    submit_async_request(MatrixRequest::SendAttachment { upload_id, upload }).was_accepted()
}
...
impl FileUploadModal {
    ...
    fn set_upload(
        ...
        self.text_input(cx, ids!(caption_input))
            .set_text(cx, caption);
        self.label(cx, ids!(file_info_label))
            .set_text(cx, &file_info_text(file_data, false));
        self.label(cx, ids!(large_attachment_warning_label))
            .set_visible(
                cx,
                file_data.size > LARGE_ATTACHMENT_WARNING_THRESHOLD_BYTES,
            );
    ...
    fn set_preview(&mut self, cx: &mut Cx, preview_id: FileUploadAttemptId, preview: FilePreview) {
        ...
        if let Some(info) = self
            .upload
            .as_ref()
            .map(|u| file_info_text(&u.file_data, is_text))
        {
    ...
    fn reset(&mut self, cx: &mut Cx) {
        ...
        self.page_flip(cx, ids!(preview_flip))
            .set_active_page(cx, id!(loading_page));
    }
}

impl FileUploadModalRef {
    pub fn handle_file_previewer_action(
        &self,
        cx: &mut Cx,
        outer_modal: ModalRef,
        action: &FileUploadModalAction,
    ) {
        match action {
            ...
            FileUploadModalAction::PreviewReady {
                preview_id,
                preview,
            } => {
...
pub fn load_file_metadata(
    ...
    #[cfg(feature = "tsp")] sign_with_tsp: bool,
) -> Result<(AttachmentUpload, PreviewSource, FileUploadAttemptId), String> {
    let path = source.path();
    let metadata = std::fs::metadata(path).map_err(|e| format!("Unable to access file: {e}"))?;
    if !metadata.is_file() {
...
impl PreviewSource {
    ...
    pub fn build(self) -> FilePreview {
                ...
                Ok(data) => match decode_image_from_data(&data) {
                    ...
                    Err(e) => {
                        makepad_widgets::warning!(
                            "Unable to decode image preview for {path:?}: {e:?}"
                        );
        ...
        match read_text_preview(
            path,
            &self.mime_type,
            self.file_size,
            self.is_mime_guaranteed,
        ) {
...
fn read_text_preview(
    ...
    Some(TextPreview {
        content,
        truncated,
        is_code: crate::utils::is_code_file(path),
    })
}
