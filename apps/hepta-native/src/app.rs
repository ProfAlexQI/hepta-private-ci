    ...
    load_all_resources() do #(App::script_component(vm)) {
        ui: Root {
            main_window := Window {
                ...
                pass.clear_color: #F3FCFF
                caption_bar +: {
                    draw_bg.color: #xF8FEFFEA
                    caption_label +: {
                        label +: {
                            draw_text +: { color: #132332 }
                            text: "Hepta Native"
                        }
                    }
                }
...
impl MatchEvent for App {
    fn handle_startup(&mut self, cx: &mut Cx) {
        ...
        if crate::hepta_fixture::is_fixture_mode_enabled() {
            ...
            crate::hepta_fixture::enqueue_fixture_workspace_room();
            let fixture_room = crate::hepta_fixture::fixture_joined_room_info().room_name_id;
            self.app_state.selected_room = Some(SelectedRoom::JoinedRoom {
                room_name_id: fixture_room.clone(),
            });
            cx.widget_action(
                self.ui.widget_uid(),
                RoomsListAction::Selected(SelectedRoom::JoinedRoom {
                    room_name_id: fixture_room,
                }),
            );
            log!(
                "Hepta Native fixture mode enabled: {}",
                crate::hepta_fixture::fixture_summary()
            );
        }
        ...
        if crate::hepta_fixture::is_fixture_mode_enabled() {
            log!("Skipping Matrix SDK loop startup while Hepta fixture mode is active.");
            return;
        }
        ...
        for action in actions {
            ...
            match action.downcast_ref() {
                ...
                Some(DirectMessageRoomAction::DidNotExist { user_profile }) => {
                    ...
                    body_text.push_str(
                        "\n\nNo Matrix direct-message create request will be sent until you confirm.",
                    );
                    positive_confirmation_modal_inner.show(
                        ...
                        ConfirmationModalContent {
                            ...
                            on_cancel_clicked: Some(Box::new(move |_cx| {
                                enqueue_popup_notification(
                                    "Direct message creation canceled. No Matrix direct-message create request was sent."
                                        .to_string(),
                                    PopupKind::Info,
                                    Some(4.0),
                                );
                    ...
                    self.ui
                        ...
                        .open(cx);
                    enqueue_popup_notification(
                        "Direct message creation confirmation opened. No Matrix direct-message create request was sent before confirmation."
                            .to_string(),
                        PopupKind::Info,
                        Some(4.0),
                    );
...
impl App {
    ...
    fn install_macos_menu(&self, cx: &mut Cx) {
        cx.update_macos_menu(MacosMenu::Main {
            items: vec![
                MacosMenu::Sub {
                    name: "Hepta Native".into(),
                    items: vec![MacosMenu::Item {
                        name: "Quit Hepta Native".into(),
...
pub struct AppState {
    ...
    /// Whether a user is currently logged in to Hepta Native or not.
    pub logged_in: bool,
