...
script_mod! {
    ...
    mod.widgets.SettingsScreen = #(SettingsScreen::register_widget(vm)) {
        ...
        View {
            ...
            settings_close_evidence := Label {
                width: Fill, height: Fit
                margin: Inset{top: 0, left: 6, right: 6, bottom: 8}
                text: "Close restores previous tab locally; no account/profile request"
                draw_text +: {
...
impl Widget for SettingsScreen {
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if close_pane {
            // Settings close evidence: close button, Escape, back, and mouse
            // back only emit CloseSettings. HomeScreen restores the previous
            // selected tab locally; no logout, account/profile mutation,
            // message, room-state, membership, or live mutation request is
            // sent from this close path.
