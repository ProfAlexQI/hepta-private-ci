...
| `src/hepta_bridge/**` | typed bridge | Add a narrow, fail-closed, session-bound runtime/task/tool/approval presentation boundary plus an internal loopback GET-only snapshot seam; no production HTTP executor exists and the product facade stays disabled until an authoritative backend adapter lands. | bridge unit tests; origin/session/correlation/redirect/cache checks; no-live-receipt boundary check |
