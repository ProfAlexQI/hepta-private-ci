...
| `licenses/ATTRIBUTIONS.md` | third-party attribution | Record the Simple Icons 14.15.0 CC0-1.0 provenance and trademark boundary for the texture-free inline login-provider marks after retiring the unused upstream raster copies; the immutable upstream manifest retains their provenance. | attribution audit; login source inspection |
...
| `packaging/build-macos-unsigned-app.sh` | local packaging provenance | Build a resource-complete current-source macOS app with pinned disposable tools and an external Cargo target while retaining existing developer caches and explicit unsigned/public-release boundaries. | current package gate; repeated manifest comparison |
...
| `packaging/debian-copyright` | packaging identity and attribution | Update installed Hepta paths, preserve upstream notices, and declare the inline Simple Icons 14.15.0 geometry in the shipped binary as CC0-1.0 with a trademark boundary. | product-shell provenance check; package inventory; attribution audit |
...
| `packaging/native-fixture-contract-v1.json` | Native fixture contract | Pin the current product-shell fixture markers outside the historical visual archive so canonical source gates can validate the active login and shell contract without consuming archived evidence. | product-shell v2 gate; strict upstream sync accounting |
| `resources/android/res/**` | Android light system chrome | Override Makepad's base Android theme with Hepta's light environment colors, dark status/navigation icons, and matching launch surface. | Android emulator system-bar contrast probe; strict upstream sync accounting |
...
| `packaging/Robrix macOS dmg background.png` | retired upstream asset | Remove the unused Robrix-branded DMG background after the Hepta packaging recipe adopted its own canonical background. | product-shell v2 retired-asset check; strict upstream sync accounting |
| `resources/robrix_logo_alpha.png` | retired upstream asset | Remove the unreferenced legacy Robrix alpha logo while retaining its immutable upstream hash in the locked import manifest. | product-shell v2 retired-asset check; strict upstream sync accounting |
| `resources/img/*.png` | retired upstream assets | Remove six unreferenced legacy SSO raster marks after the login surface moved to attributed inline Simple Icons geometry. | product-shell v2 retired-asset check; login source/attribution audit; strict upstream sync accounting |
| `resources/icon*` | brand assets | Replace upstream product icons with the Hepta icon family while preserving frozen Robrix asset provenance in the locked import manifest. | image decode and dimension checks |
...
| `src/app.rs` | product identity, theme, lifecycle security, diagnostic capture, and mobile inset handling | Apply Hepta title/chrome to the real Robrix application root without replacing its desktop or mobile home shell, own the disabled Hepta facade and tear it down on login/session/logout state transitions, configure the root `KeyboardView` shift used with Android `adjustNothing`, request light Android system-bar icons over the platform's dark strip, and expose the delayed Makepad GPU-frame evidence hook only behind `developer-diagnostics`. | Native lib tests; `cargo check --locked`; product-shell v2 gate; default/all-feature checks; desktop/mobile GPU, IME, and status-bar contrast screenshots |
| `src/accessibility.rs` | accessibility contract | Publish a password-safe, focus-aware AccessKit login tree and an explicitly incomplete hierarchical post-login container skeleton with real navigation and composer focus/SetValue paths; visible PortalList room/message item enumeration remains blocked and platform/real-device claims stay fail-closed. | focused AccessKit hierarchy/value tests; feature-matrix compile; real per-item provider plus VoiceOver/TalkBack remains independently gated |
...
| `hepta-live-bridge-backend-contract-v1.json` | live bridge backend handoff | Record the canonical snapshot-only endpoint, strict BridgeUpdate metadata/session/correlation/provenance requirements, audited non-authoritative legacy GET shapes, wired lifecycle teardown, and the separately blocked executor/production-activation truth. | live-bridge contract gate/self-test; strict envelope validator; no-live-receipt boundary check |
| `src/hepta_bridge/**` | typed bridge | Add a narrow, fail-closed, session-bound runtime/task/tool/approval presentation boundary, a bounded loopback GET-only authorization-bearing HTTP client with an independent never-on-wire response HMAC key and total deadline, logout-start/unrecoverable lifecycle drop semantics, and a sealed backend-owned in-process activation entry with no production proof/key issuer yet; Matrix login alone cannot activate the facade. | bridge unit tests; real-socket executor/HMAC/deadline tests; lifecycle executor-drop and identity-mismatch tests; origin/session/correlation/redirect/cache checks; no-live-receipt boundary check |
...
| `src/profile/user_profile.rs` | dead-code cleanup | Remove the unconsumed `first_letter` helper after confirming all avatar/name callers use the current display-name path. | Native lib tests; strict upstream sync accounting |
...
| `src/tsp/mod.rs` | product identity and crash hardening | Apply the Hepta product user agent and user-facing TSP copy while retaining the upstream experimental TSP implementation; remove the unsubmitted `DeleteWallet` request variant whose sole worker arm panicked, leaving deletion fail-closed before execution. | `cargo check --locked`; TSP feature check; blocked-before-execution marker audit |
