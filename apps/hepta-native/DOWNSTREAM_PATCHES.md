...
| `src/hepta_bridge/**` | typed bridge | Add a narrow, fail-closed, session-bound runtime/task/tool/approval presentation boundary, an internal loopback GET-only snapshot seam, lifecycle drop semantics, and a sealed backend-owned in-process activation entry with no production proof issuer yet; no production HTTP executor exists and Matrix login alone cannot activate the facade. | bridge unit tests; lifecycle executor-drop and identity-mismatch tests; origin/session/correlation/redirect/cache checks; no-live-receipt boundary check |
