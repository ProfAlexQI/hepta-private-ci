...
| `resources/android/AndroidManifest.xml.template` | packaging identity and security | Disable backup and cleartext transport, remove the tall-display aspect limit, retain lifecycle configuration changes, and register narrowly scoped Matrix/Hepta deep links without broad media permissions. The source contract does not claim runtime RTL, Dynamic Type, or IME completion. | `tests/android_manifest_contract.rb --static-only`; pinned cargo-makepad contract; compiled APK manifest audit |
