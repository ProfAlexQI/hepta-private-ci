...
async fn recorder_omits_ordinals_from_legacy_rollouts() -> std::io::Result<()> {
    ...
    let values = fs::read_to_string(recorder.rollout_path())?
        .lines()
...
#[tokio::test]
async fn resumed_paginated_rollout_continues_after_gap_and_invalid_tail() -> std::io::Result<()> {
    let home = TempDir::new().expect("temp dir");
    let config = test_config(home.path());
    let rollout_path = home.path().join("rollout.jsonl");
    write_paginated_rollout(&rollout_path, ThreadId::new(), &[4])?;
    let mut file = fs::OpenOptions::new().append(true).open(&rollout_path)?;
    write!(file, "{{\"timestamp\":\"unterminated\"")?;
    drop(file);
    ...
    let ordinals = fs::read_to_string(&rollout_path)?
        ...
        .collect::<Vec<_>>();
    assert_eq!(ordinals, vec![Some(0), Some(4), Some(5)]);
    recorder.shutdown().await
}

#[tokio::test]
async fn resumed_paginated_rollout_continues_after_valid_unterminated_tail() -> std::io::Result<()>
{
    let home = TempDir::new().expect("temp dir");
    let config = test_config(home.path());
    let rollout_path = home.path().join("rollout.jsonl");
    ...
    let recorder =
        RolloutRecorder::new(&config, RolloutRecorderParams::resume(rollout_path.clone())).await?;
    recorder
        .record_canonical_items(&[agent_message_item("after-tail")])
        .await?;
    recorder.flush().await?;
    assert_eq!(
...
async fn paginated_ordinal_overflow_fails_without_appending() -> std::io::Result<()> {
    ...
    recorder
        ...
        .await?;
    let err = recorder.flush().await.expect_err("ordinal overflow");
...
#[tokio::test]
async fn offline_append_assigns_next_paginated_ordinal() -> std::io::Result<()> {
    let home = TempDir::new().expect("temp dir");
    let rollout_path = home.path().join("rollout.jsonl");
    write_paginated_rollout(&rollout_path, ThreadId::new(), &[4])?;
