//! Persist Hepta session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
///
/// ```ignore
/// $ jq -C . ~/.hepta/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.hepta/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
    ...
    pub async fn new(
        ...
        let (file, deferred_log_file_info, rollout_path, meta, ordinal_state) = match params {
            ...
            RolloutRecorderParams::Resume { path } => {
                let (path, file, ordinal_state) = open_rollout_for_append(&path).await?;
...
fn precompute_log_file_info(
    ...
    // Resolve {runtime_home}/sessions/YYYY/MM/DD path.
...
pub async fn append_rollout_item_to_path(
    ...
    let (_path, file, ordinal_state) = open_rollout_for_append(rollout_path).await?;
    let ordinal = ordinal_state.current()?;
