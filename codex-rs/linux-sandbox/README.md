...
- a `codex-linux-sandbox` standalone executable for Linux that is bundled with the Hepta CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `codex-exec` CLI can check if its arg0 is `codex-linux-sandbox` and, if so, execute as if it were `codex-linux-sandbox`
  - this should also be true of the `codex` multitool CLI

On Linux, Hepta prefers the first `bwrap` found on `PATH`
...
with Hepta.
Hepta also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Hepta surfaces the same startup warning
...
it cannot create the required user namespaces, so Hepta rejects sandboxed shell
...
- If `bwrap` is missing, Hepta also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Hepta surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Hepta rejects sandboxed
