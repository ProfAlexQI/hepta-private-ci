...
impl std::fmt::Display for ContractError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Unavailable(message) => write!(formatter, "P1.1b component unavailable: {message}"),
...
pub(crate) fn usize_to_u32(value: usize, label: &'static str) -> Result<u32, ContractError> {
    u32::try_from(value)
        .map_err(|_| ContractError::Invalid(format!("{label} exceeds u32")))
}

pub(crate) fn usize_to_u64(value: usize, label: &'static str) -> Result<u64, ContractError> {
    u64::try_from(value)
        .map_err(|_| ContractError::Invalid(format!("{label} exceeds u64")))
}
