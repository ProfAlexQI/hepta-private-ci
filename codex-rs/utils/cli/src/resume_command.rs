//! Shared formatting for user-facing `hepta resume` command hints.
...
mod tests {
    ...
    fn prefers_name_over_id() {
        ...
        assert_eq!(command, Some("hepta resume my-thread".to_string()));
    }
    ...
    fn formats_thread_id_when_name_is_missing() {
        ...
        assert_eq!(
            command,
            Some("hepta resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    fn quotes_thread_names_when_needed() {
        ...
        assert_eq!(
            command,
            Some("hepta resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), /*thread_id*/ None);
        assert_eq!(command, Some("hepta resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), /*thread_id*/ None);
        assert_eq!(command, Some("hepta resume \"quote'case\"".to_string()));
    }
}
