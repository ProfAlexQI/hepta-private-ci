...
impl<'a> HostEvidenceResolverV1<'a> {
    pub(crate) fn new(
        ...
        for (ordinal, draft) in ranges.into_iter().enumerate() {
            ...
            let evidence_sha256 = Sha256Digest::for_bytes(&source_content.as_bytes()[start..end]);
        ...
        let segment_catalog_sha256 = segment_catalog_digest(&source_content_sha256, &segments);
        Ok(Self {
            source_content,
            source_content_sha256,
    ...
    fn resolve_fact_evidence(
        ...
        for pair in candidates.windows(2) {
            let previous = &pair[0];
            let current = &pair[1];
            if previous.start_byte == current.start_byte && previous.end_byte == current.end_byte {
                let kind = fact_kind.as_str();
                return Err(format!(
        ...
        match selector {
            ...
            EvidenceLocatorV4::SourceSegment(locator) => {
                ...
                let index = self
                    ...
                    .ok_or_else(|| {
                        let kind = fact_kind.as_str();
                        format!("{kind} fact `{fact_key}` references an unknown source segment")
                    })?;
                let segment = self
                    .segments
                    .get(index)
                    .ok_or_else(|| "host evidence segment index is corrupt".to_string())?;
...
pub(super) fn prepare_grounded_tool_v4_impl(
    ...
    HostEvidenceResolverV1::new(source_content, segment_drafts)?.prepare_grounded_tool_v4(input)
}
