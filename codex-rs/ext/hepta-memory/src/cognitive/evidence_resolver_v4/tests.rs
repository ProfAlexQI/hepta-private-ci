...
fn segment_ids_are_bound_to_the_exact_source() {
    ...
    let first = HostEvidenceResolverV1::new(
        first_source,
        &[SourceSegmentDraftV1::new(0, 20)],
    )
    ...
    let second = HostEvidenceResolverV1::new(
        second_source,
        &[SourceSegmentDraftV1::new(0, 21)],
    )
    .expect("second resolver");
    let error = second
        .prepare_grounded_tool_v4(entity_input(
            "Project Borealis",
            vec![segment(foreign_id)],
        ))
...
fn source_segment_catalog_is_deterministic_and_rejects_duplicate_ranges() {
    ...
    assert_eq!(ordered.segment_descriptors(), reversed.segment_descriptors());
    assert_eq!(
...
fn source_segment_ranges_require_utf8_boundaries() {
    ...
    let error = HostEvidenceResolverV1::new(
        source,
        &[SourceSegmentDraftV1::new(1, 4)],
    )
    .err()
