    ...
    pub(crate) fn validate(&self) -> Result<(), String> {
        ...
        for span in &self.resolved_spans {
            ...
            let per_fact_count = match previous {
                None => {
                    if span.evidence_ordinal != 0 {
                        return Err("host evidence resolver first evidence ordinal must be zero"
                            .to_string());
                    }
                    1
                }
            ...
            if per_fact_count > MAX_SPANS_PER_FACT {
                return Err(
                    "host evidence resolver receipt exceeds the per-fact span limit".to_string(),
                );
            }
            if span.locator_kind == ResolvedLocatorKindV1::SourceSegment && self.segment_count == 0
            {
                return Err(
                    "host evidence resolver segment binding has no segment catalog".to_string(),
                );
            }
            previous = Some((
                span.fact_kind,
...
mod tests {
    ...
    fn receipt_rejects_digest_tampering() {
        let mut receipt = receipt(vec![binding(0, 0, 8, ResolvedLocatorKindV1::ExactQuote)], 0);
