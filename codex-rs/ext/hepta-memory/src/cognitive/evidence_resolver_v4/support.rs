...
pub(super) fn validate_source_range(
    ...
    let start =
        usize::try_from(start_byte).map_err(|_| format!("{label} start byte exceeds usize"))?;
    let end = usize::try_from(end_byte).map_err(|_| format!("{label} end byte exceeds usize"))?;
...
pub(super) fn segment_catalog_digest(
    ...
    super::super::hash_part(&mut hasher, b"hepta:cognitive:source-segment-catalog:v1");
...
pub(super) fn evidence_resolution_receipt_digest(
    ...
    super::super::hash_part(&mut hasher, receipt.tool_input_sha256.as_str().as_bytes());
    super::super::hash_part(
        &mut hasher,
