...
//! does not retain every intermediate current-tip retry closure/outbox and
//! cannot prove that an Intent or Outbox was durably published before I/O.
//! It is therefore a terminal audit artifact, not executable bridge input.
//! It exports exact terminal claim, bundle, phase, and provider-receipt
//! identities without granting provider I/O, durable publication,
...
const INSTALL_EPOCH_DURABLE_PROJECTION_JOB_DOMAIN_V1: &[u8] =
    b"hepta_linux_v8_install_epoch_durable_projection_job_v1\0";
const INSTALL_EPOCH_DURABLE_PROJECTION_BINDING_DOMAIN_V1: &[u8] =
    b"hepta_linux_v8_install_epoch_durable_projection_binding_v1\0";
...
#[derive(Debug)]
pub(crate) struct InstallEpochDurableProjectionSourceV1 {
    pub(crate) active_query_bundle: Option<RawDurableBundleProjectionV1>,
    ...
    pub(crate) initial_query_claim: RawDurableNonceClaimProjectionV1,
    ...
    pub(crate) retry_query_claim: Option<RawDurableNonceClaimProjectionV1>,
...
#[derive(Debug)]
pub struct InstallEpochDurableProjectionV1 {
    active_query_bundle: Option<InstallEpochDurableBundleProjectionV1>,
    ...
    initial_query_claim: InstallEpochDurableNonceClaimProjectionV1,
    ...
    retry_query_claim: Option<InstallEpochDurableNonceClaimProjectionV1>,
    successor_record: ExternalWatermarkRecordV1,
    successor_tip_sha256: String,
}

impl InstallEpochDurableProjectionV1 {
    ...
    pub fn active_query_bundle(&self) -> Option<&InstallEpochDurableBundleProjectionV1> {
        self.active_query_bundle.as_ref()
    }

    pub fn active_query_claim(&self) -> &InstallEpochDurableNonceClaimProjectionV1 {
        self.retry_query_claim
            .as_ref()
            .unwrap_or(&self.initial_query_claim)
    }
...
) -> Result<InstallEpochDurableProjectionV1, QualificationError> {
    let source = verified.durable_projection_source_v1();
    let projection = projection_from_source_v1(source)?;
    drop(verified);
    Ok(projection)
}

fn projection_from_source_v1(
    source: InstallEpochDurableProjectionSourceV1,
) -> Result<InstallEpochDurableProjectionV1, QualificationError> {
    validate_source_v1(&source)?;
    let job_id_sha256 = projection_job_id_sha256_v1(&source);
    let projection_sha256 = projection_binding_sha256_v1(&source, &job_id_sha256);
    Ok(InstallEpochDurableProjectionV1 {
        active_query_bundle: source.active_query_bundle,
        ...
        epoch: source.epoch,
        initial_query_claim: source.initial_query_claim,
        job_id_sha256,
        ...
        retry_query_claim: source.retry_query_claim,
...
fn validate_source_v1(
    ...
    validate_claim_v1(
        "initial query",
        &source.initial_query_claim,
        EXPECTED_QUERY_CLAIM_SCOPE_V1,
    )?;
    if source.initial_query_nonce != source.initial_query_claim.nonce {
        return Err(invalid(
            "durable projection initial query nonce is not exact",
        ));
    }
    match (&source.retry_query_claim, &source.active_query_bundle) {
        (None, None) if source.active_query_sequence == 1 => {
            if source.current_tip_query_nonce != source.initial_query_claim.nonce {
                return Err(invalid(
                    "durable projection final query does not match the initial claim",
                ));
            }
        }
        (Some(claim), Some(bundle)) if source.active_query_sequence > 1 => {
            validate_claim_v1("retry query", claim, EXPECTED_QUERY_CLAIM_SCOPE_V1)?;
            validate_bundle_v1("active query", bundle)?;
            if source.current_tip_query_nonce != claim.nonce
                || bundle.id_sha256 != claim.binding_sha256
                || bundle.binding_sha256 != source.active_query_state_sha256
            {
                return Err(invalid(
                    "durable projection retry query claim and bundle are not exact",
                ));
            }
        }
        _ => {
            return Err(invalid(
                "durable projection retry claim/bundle disposition is inconsistent",
            ));
        }
    }
    let mut nonces = vec![
        ...
        source.initial_query_claim.nonce.as_str(),
    ];
    if let Some(retry) = &source.retry_query_claim {
        nonces.push(retry.nonce.as_str());
    }
    for (index, nonce) in nonces.iter().enumerate() {
        if nonces[..index].contains(nonce) {
...
fn projection_job_id_sha256_v1(source: &InstallEpochDurableProjectionSourceV1) -> String {
    let mut bytes = INSTALL_EPOCH_DURABLE_PROJECTION_JOB_DOMAIN_V1.to_vec();
    append_field_v1(
        &mut bytes,
        "schema",
        INSTALL_EPOCH_DURABLE_PROJECTION_SCHEMA_V1.as_bytes(),
    );
    append_field_v1(
        &mut bytes,
        "claim_domain_id",
...
fn projection_binding_sha256_v1(
    source: &InstallEpochDurableProjectionSourceV1,
    job_id_sha256: &str,
) -> String {
    let mut bytes = INSTALL_EPOCH_DURABLE_PROJECTION_BINDING_DOMAIN_V1.to_vec();
    append_field_v1(
        &mut bytes,
        "schema",
        INSTALL_EPOCH_DURABLE_PROJECTION_SCHEMA_V1.as_bytes(),
    ...
    append_claim_v1(&mut bytes, "initial_query", &source.initial_query_claim);
    match &source.retry_query_claim {
        Some(claim) => append_claim_v1(&mut bytes, "active_retry_query", claim),
        None => append_field_v1(&mut bytes, "active_retry_query", b"none"),
    }
    ...
    match &source.active_query_bundle {
        Some(bundle) => append_bundle_v1(&mut bytes, "active_query", bundle),
        None => append_field_v1(&mut bytes, "active_query_bundle", b"none"),
    }
    append_field_v1(
        &mut bytes,
