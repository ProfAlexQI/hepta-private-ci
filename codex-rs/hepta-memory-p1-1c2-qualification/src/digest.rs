...
impl Digest32 {
    ...
    pub fn for_bytes(input: &[u8]) -> Self {
        let bit_length = (input.len() as u128).saturating_mul(8);
        ...
        let encoded_length = u64::try_from(bit_length)
            .unwrap_or(u64::MAX)
            .to_be_bytes();
        padded.extend_from_slice(&encoded_length);
    ...
    pub fn from_hex(value: &str) -> Result<Self, ContractError> {
        if value.len() != 64 {
            return Err(ContractError::Invalid(
                "SHA-256 digest must contain exactly 64 hexadecimal characters".to_string(),
            ));
        }
        let mut output = [0_u8; 32];
...
fn compress(state: &mut [u32; 8], block: &[u8]) {
    ...
    for index in 0..64 {
        let upper_sigma_one = e.rotate_right(6) ^ e.rotate_right(11) ^ e.rotate_right(25);
        let choice = (e & f) ^ ((!e) & g);
        let first = h
        ...
        let upper_sigma_zero = a.rotate_right(2) ^ a.rotate_right(13) ^ a.rotate_right(22);
        let majority = (a & b) ^ (a & c) ^ (b & c);
        let second = upper_sigma_zero.wrapping_add(majority);
