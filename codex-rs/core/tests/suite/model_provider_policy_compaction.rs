...
async fn provider_policy_block_prevents_local_compaction_send_and_retry() -> Result<()> {
    ...
    assert_eq!(observations(&state), vec![local_compaction_observation()]);
    Ok(())
}
    ...
    loop {
        ...
        match timeout(remaining, codex.next_event()).await {
            ...
            Ok(Ok(event)) => assert!(
                ...
                "local compact replaced history before provider terminal acknowledgement"
            ),
        }
    }
}
...
fn local_compaction_observation() -> ProviderAttemptObservation {
