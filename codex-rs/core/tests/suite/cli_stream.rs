...
fn compat_cli_bin() -> std::path::PathBuf {
    #[expect(clippy::expect_used)]
    codex_utils_cargo_bin::cargo_bin("hepta-codex-compat")
        .expect("failed to resolve compatibility CLI")
}
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Hepta to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `hepta exec --profile ...` preserves the active profile when it
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the Hepta CLI and invoke `exec`, which is what records a session.
    let bin = compat_cli_bin();
    let mut cmd = AssertCommand::new(bin);
    cmd.timeout(Duration::from_secs(30));
    cmd.arg("exec")
