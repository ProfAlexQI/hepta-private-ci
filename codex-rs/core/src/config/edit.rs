...
pub enum ConfigEdit {
    ...
    /// Toggle the opt-out marker for Hepta-managed fast defaults.
...
pub fn model_availability_nux_count_edits(shown_count: &HashMap<String, u32>) -> Vec<ConfigEdit> {
    ...
    shown_count_entries.sort_unstable_by_key(|(model_slug, _)| *model_slug);
...
/// `$HEPTA_HOME/config.toml`; profile-v2 callers should resolve their target
