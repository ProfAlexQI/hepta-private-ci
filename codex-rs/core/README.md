...
This crate implements the business logic for Hepta. It is designed to be used by the various Hepta UIs written in Rust.
...
Expects the binary containing `codex-core` to run the equivalent of `hepta sandbox linux` (legacy alias: `codex debug landlock`) when `arg0` is `codex-linux-sandbox`. See the `codex-arg0` crate for details.
...
binary shipped with Hepta and Hepta surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Hepta also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Hepta rejects sandboxed shell commands that would enter the
