...
pub struct DeepTd8RetAckTdRbackAckTdPreviewReport {
    ...
    pub terminal_decision_surfaces: Vec<DeepTd8RetAckTdRbackAckTdItem>,
    pub non_promotion_denials: Vec<DeepTd8RetAckTdRbackAckTdItem>,
    pub authority_guards: Vec<DeepTd8RetAckTdRbackAckTdItem>,
    pub release_delivery_guards: Vec<DeepTd8RetAckTdRbackAckTdItem>,
    pub local_views: Vec<DeepTd8RetAckTdRbackAckTdItem>,
    pub invariants: Vec<DeepTd8RetAckTdRbackAckTdItem>,
    ...
    pub side_effects: DeepTd8RetAckTdRbackAckTdSideEffects,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct DeepTd8RetAckTdRbackAckTdItem {
    pub id: String,
...
pub struct DeepTd8RetAckTdRbackAckTdSideEffects {
...
pub fn deep_td8_ret_ack_td_rback_ack_td_base() -> String {
    let gate = crate::deep_td8_ret_ack_td_receipt_retention_readback_ack_terminal_decision_gate();
    gate.strip_suffix("_preview_gate")
        .unwrap_or(gate.as_str())
        .to_owned()
}

pub fn deep_td8_ret_ack_td_rback_ack_td_receipt_gate() -> String {
    format!(
        "{}_receipt_preview_gate",
        deep_td8_ret_ack_td_rback_ack_td_base()
    )
}

pub fn hepta_work_graph_deep_td8_ret_ack_td_rback_ack_td_preview_report()
-> DeepTd8RetAckTdRbackAckTdPreviewReport {
    ...
    DeepTd8RetAckTdRbackAckTdPreviewReport {
        ...
        required_prior_gates: deep_td8_ret_ack_td_rback_ack_td_required_prior_gates(),
        ...
        local_views,
        invariants,
        recommended_next_gate: deep_td8_ret_ack_td_rback_ack_td_receipt_gate(),
        ready_for_terminal_decision_receipt_preview: true,
        ready_for_operator_acceptance: false,
        ready_for_live_persistence: false,
        side_effects: DeepTd8RetAckTdRbackAckTdSideEffects::none(),
    }
}

pub fn deep_td8_ret_ack_td_rback_ack_td_required_prior_gates() -> Vec<String> {
    let mut gates =
        crate::deep_td8_ret_ack_td_receipt_retention_readback_ack_replay_required_prior_gates();
    gates.push(crate::deep_td8_ret_ack_td_receipt_retention_readback_ack_replay_gate());
    gates
}

pub fn deep_td8_ret_ack_td_rback_ack_td_surface_ids() -> Vec<String> {
    (0..6)
        .map(|index| format!("deep_td8_ret_ack_td_rback_ack_td_surface_{index}"))
        .collect()
}

fn terminal_decision_items(prefix: &str, count: usize) -> Vec<DeepTd8RetAckTdRbackAckTdItem> {
    let source_ids = crate::deep_td8_ret_ack_td_receipt_retention_readback_ack_ids()
        .into_iter()
        .map(str::to_owned)
        .collect::<Vec<_>>();
    (0..count)
        .map(|index| DeepTd8RetAckTdRbackAckTdItem {
...
impl DeepTd8RetAckTdRbackAckTdSideEffects {
    pub fn none() -> Self {
        Self {
...
mod tests {
    ...
    #[test]
    fn work_graph_deep_td8_ret_ack_td_rback_ack_td_requires_ack_replay_gate() {
        let report = hepta_work_graph_deep_td8_ret_ack_td_rback_ack_td_preview_report();

        assert_eq!(
            report.required_prior_gates.last(),
            Some(&crate::deep_td8_ret_ack_td_receipt_retention_readback_ack_replay_gate())
        );
    }

    #[test]
    fn work_graph_deep_td8_ret_ack_td_rback_ack_td_declares_local_surfaces() {
        let report = hepta_work_graph_deep_td8_ret_ack_td_rback_ack_td_preview_report();
    ...
    #[test]
    fn work_graph_deep_td8_ret_ack_td_rback_ack_td_blocks_promotion_paths() {
        let report = hepta_work_graph_deep_td8_ret_ack_td_rback_ack_td_preview_report();
    ...
    #[test]
    fn work_graph_deep_td8_ret_ack_td_rback_ack_td_keeps_authority_and_delivery_denied() {
        let report = hepta_work_graph_deep_td8_ret_ack_td_rback_ack_td_preview_report();
    ...
    #[test]
    fn work_graph_deep_td8_ret_ack_td_rback_ack_td_points_to_receipt_gate() {
        let report = hepta_work_graph_deep_td8_ret_ack_td_rback_ack_td_preview_report();

        assert_eq!(
            report.recommended_next_gate,
            deep_td8_ret_ack_td_rback_ack_td_receipt_gate()
        );
    ...
    #[test]
    fn work_graph_deep_td8_ret_ack_td_rback_ack_td_has_no_side_effects() {
        let report = hepta_work_graph_deep_td8_ret_ack_td_rback_ack_td_preview_report();
