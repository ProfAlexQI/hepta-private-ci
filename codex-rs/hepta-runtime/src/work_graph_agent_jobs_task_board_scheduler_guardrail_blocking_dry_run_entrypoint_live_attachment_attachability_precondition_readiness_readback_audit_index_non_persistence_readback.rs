...
-> WorkGraphAgentJobsTaskBoardSchedulerGuardrailBlockingDryRunEntrypointLiveAttachmentAttachabilityPreconditionReadinessReadbackAuditIndexNonPersistenceReadbackReport{
    ...
    let source_audit_index_no_persistence_confirmed =
        source.source_attachability_readback_no_persistence_confirmed
    ...
    let source_audit_index_no_live_confirmed = source.ready_for_non_persistence_readback
        ...
        && !source.audit_index_authorizes_attachability_readiness_recording
        && !source.audit_index_authorizes_attachability_readiness_persistence
    ...
    let source_audit_index_ready = source.gate
        == WORK_GRAPH_AGENT_JOBS_TASK_BOARD_SCHEDULER_GUARDRAIL_BLOCKING_DRY_RUN_ENTRYPOINT_LIVE_ATTACHMENT_ATTACHABILITY_PRECONDITION_READINESS_READBACK_AUDIT_INDEX_GATE
        && source.source_attachability_readback_ready
        && source.source_attachability_readback_no_persistence_confirmed
        && source.source_attachability_readback_no_live_confirmed
        && source.source_attachability_readback_ready_for_audit_index
        ...
        && source.audit_index_blocker_count == 56
        && source.required_prior_gate_count == 23
    ...
    let readback_blockers_complete =
        readback_blockers.len() == 59 && readback_blockers.iter().all(|blocker| blocker.blocked);
