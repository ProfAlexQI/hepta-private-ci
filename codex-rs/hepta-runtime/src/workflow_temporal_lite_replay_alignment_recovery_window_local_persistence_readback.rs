...
use crate::WorkflowTemporalLiteReplayAlignmentRollbackConsistencyLocalPersistenceReadbackEntry;
use crate::WorkflowTemporalLiteReplayAlignmentRollbackConsistencyLocalPersistenceReadbackReport;
use crate::hepta_workflow_temporal_lite_replay_alignment_rollback_consistency_local_persistence_readback_report;

pub const WORKFLOW_TEMPORAL_LITE_REPLAY_ALIGNMENT_RECOVERY_WINDOW_LOCAL_PERSISTENCE_READBACK_GATE: &str =
    "workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_gate";
pub const WORKFLOW_TEMPORAL_LITE_REPLAY_ALIGNMENT_RECOVERY_WINDOW_LOCAL_PERSISTENCE_READBACK_SCHEMA_VERSION:
    &str = "workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_v1";
pub const WORKFLOW_TEMPORAL_LITE_REPLAY_ALIGNMENT_RECOVERY_WINDOW_LOCAL_PERSISTENCE_READBACK_RECOMMENDED_NEXT_GATE:
    &str = "workflow_temporal_lite_replay_alignment_recovery_receipt_local_persistence_readback";

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackReport {
    ...
    pub recovery_window_local_persistence_readback_ready: bool,
    pub entries:
        Vec<WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackEntry>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackEntry {
...
pub struct WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackSideEffects {
...
pub fn hepta_workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_report()
-> WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackReport {
    ...
    workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_report_from_source(&source)
}

pub fn workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_report_from_source(
    source: &WorkflowTemporalLiteReplayAlignmentRollbackConsistencyLocalPersistenceReadbackReport,
) -> WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackReport {
    let entries =
    ...
    let sqlite_write_count = entries
        ...
        .count();
    let ready = source.rollback_consistency_local_persistence_readback_ready
    ...
    WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackReport {
        ...
        source_rollback_consistency_ready: source.rollback_consistency_local_persistence_readback_ready,
        ...
        recovery_window_contract_readback_materialized: ready,
        ...
        recovery_window_local_persistence_readback_ready: ready,
        entries,
        blockers: vec![
        ...
        side_effects:
            WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackSideEffects::none(),
    }
}

pub fn workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_entries(
    source: &WorkflowTemporalLiteReplayAlignmentRollbackConsistencyLocalPersistenceReadbackReport,
) -> Vec<WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackEntry> {
    workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_entries_from_rollback_consistency_entries(&source.entries)
}

pub fn workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_entries_from_rollback_consistency_entries(
    rollback_consistency_entries: &[WorkflowTemporalLiteReplayAlignmentRollbackConsistencyLocalPersistenceReadbackEntry],
) -> Vec<WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackEntry> {
    rollback_consistency_entries
        .iter()
        .map(|source_entry| {
            let recovery_window_key = keyed_recovery_window(
                "temporal-lite.local-replay-alignment.recovery-window-readback",
                source_entry.replay_order,
                &source_entry.event_contract_id,
            );
            let recovery_window_start_key = keyed_recovery_window(
                "temporal-lite.local-recovery-window.start-readback",
                source_entry.replay_order,
                &source_entry.event_contract_id,
            );
            let recovery_window_end_key = keyed_recovery_window(
                "temporal-lite.local-recovery-window.end-readback",
                source_entry.replay_order,
                &source_entry.event_contract_id,
            );
            let recovery_window_digest = format!(
                "temporal-lite.local-replay-alignment-recovery-window-digest.v1.{:03}.{}.{}.{}.{}",
                source_entry.replay_order,
                source_entry.event_contract_id,
            ...
            WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackEntry {
                ...
                readback_state:
                    "projected_from_sqlite_wal_local_persistence_readback_without_recovery_window_writes",
...
fn keyed_recovery_window(prefix: &str, replay_order: usize, event_contract_id: &str) -> String {
    format!("{prefix}.{replay_order:03}.{event_contract_id}")
}

impl WorkflowTemporalLiteReplayAlignmentRecoveryWindowLocalPersistenceReadbackSideEffects {
...
mod tests {
    ...
    #[test]
    fn local_recovery_window_projects_all_rollback_entries_without_writes() {
        let report =
        ...
        assert!(report.recovery_window_local_persistence_readback_ready);
    }

    #[tokio::test]
    async fn local_recovery_window_uses_reopened_sqlite_event_history() {
        ...
        assert_eq!(recovery_entries.len(), 9);
        assert!(recovery_entries.iter().enumerate().all(|(index, entry)| {
            entry.replay_order == index + 1
                && entry.local_sequence >= 1
                && entry
                    .recovery_window_key
                    .starts_with("temporal-lite.local-replay-alignment.recovery-window-readback.")
                && entry
                    .recovery_window_start_key
                    .starts_with("temporal-lite.local-recovery-window.start-readback.")
                && entry
                    .recovery_window_end_key
                    .starts_with("temporal-lite.local-recovery-window.end-readback.")
                && entry
                    .recovery_window_digest
                    .starts_with("temporal-lite.local-replay-alignment-recovery-window-digest.v1.")
                && entry.expected_recovery_projection_key == entry.recovery_window_end_key
                && entry.window_state
                    == "recovery_window_projected_from_local_persistence_not_written"
    ...
    #[test]
    fn local_recovery_window_side_effects_remain_closed() {
        let report =
            hepta_workflow_temporal_lite_replay_alignment_recovery_window_local_persistence_readback_report();
