...
pub const WORK_GRAPH_TERMINAL_TASK_RESULT_WRAPPER_SHADOW_ACTIVATION_DRIFT_BUDGET_PREVIEW_GATE:
    &str =
    "hepta_work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_gate";
pub const WORK_GRAPH_TERMINAL_TASK_RESULT_WRAPPER_SHADOW_ACTIVATION_DRIFT_BUDGET_SCHEMA_VERSION:
    &str = "work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_v1";
pub const WORK_GRAPH_TERMINAL_TASK_RESULT_WRAPPER_SHADOW_ACTIVATION_DRIFT_BUDGET_RECOMMENDED_NEXT_GATE:
    &str = "hepta_work_graph_terminal_task_result_wrapper_shadow_activation_promotion_precondition_preview_gate";

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct WorkGraphTerminalTaskResultWrapperShadowActivationDriftBudgetPreviewReport {
    ...
    pub drift_budgets: Vec<WorkGraphTaskResultWrapperShadowActivationDriftBudgetPreview>,
    pub operator_summaries:
        Vec<WorkGraphTaskResultWrapperShadowActivationDriftOperatorSummaryPreview>,
    pub activation_preconditions:
        Vec<WorkGraphTaskResultWrapperShadowActivationDriftPreconditionPreview>,
    pub blockers: Vec<WorkGraphTaskResultWrapperShadowActivationDriftBudgetBlockerPreview>,
    pub required_prior_gates: Vec<&'static str>,
    pub recommended_next_gate: &'static str,
    pub ready_for_shadow_activation_promotion_precondition_preview: bool,
    ...
    pub side_effects:
        WorkGraphTerminalTaskResultWrapperShadowActivationDriftBudgetPreviewSideEffects,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct WorkGraphTaskResultWrapperShadowActivationDriftBudgetPreview {
    pub id: &'static str,
    ...
    pub allows_shadow_readback_execution: bool,
    pub allows_activation: bool,
    pub allows_promotion_execution: bool,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct WorkGraphTaskResultWrapperShadowActivationDriftOperatorSummaryPreview {
    pub id: &'static str,
    pub drift_detector_id: &'static str,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct WorkGraphTaskResultWrapperShadowActivationDriftPreconditionPreview {
    pub id: &'static str,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct WorkGraphTaskResultWrapperShadowActivationDriftBudgetBlockerPreview {
    pub id: &'static str,
    pub severity: &'static str,
...
pub struct WorkGraphTerminalTaskResultWrapperShadowActivationDriftBudgetPreviewSideEffects {
    pub filesystem_written: bool,
    pub shadow_drift_budget_persisted: bool,
...
pub fn hepta_work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_report()
-> WorkGraphTerminalTaskResultWrapperShadowActivationDriftBudgetPreviewReport {
    let drift_budgets = work_graph_terminal_task_result_wrapper_shadow_activation_drift_budgets();
    let operator_summaries =
        work_graph_terminal_task_result_wrapper_shadow_activation_drift_operator_summaries();
    let activation_preconditions =
        work_graph_terminal_task_result_wrapper_shadow_activation_drift_preconditions();
    let blockers =
        work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_blockers();
    let required_prior_gates =
        work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_required_prior_gates(
        );
    ...
    WorkGraphTerminalTaskResultWrapperShadowActivationDriftBudgetPreviewReport {
        ...
        runtime: "hepta",
        status: "ready",
        gate: WORK_GRAPH_TERMINAL_TASK_RESULT_WRAPPER_SHADOW_ACTIVATION_DRIFT_BUDGET_PREVIEW_GATE,
        schema_version:
            WORK_GRAPH_TERMINAL_TASK_RESULT_WRAPPER_SHADOW_ACTIVATION_DRIFT_BUDGET_SCHEMA_VERSION,
        preview_mode:
            "read_only_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_no_execution",
        ...
        recommended_next_gate:
            WORK_GRAPH_TERMINAL_TASK_RESULT_WRAPPER_SHADOW_ACTIVATION_DRIFT_BUDGET_RECOMMENDED_NEXT_GATE,
        ready_for_shadow_activation_promotion_precondition_preview: true,
        ...
        side_effects:
            WorkGraphTerminalTaskResultWrapperShadowActivationDriftBudgetPreviewSideEffects::none(),
    }
}

pub fn work_graph_terminal_task_result_wrapper_shadow_activation_drift_budgets()
-> Vec<WorkGraphTaskResultWrapperShadowActivationDriftBudgetPreview> {
    vec![
        drift_budget(
...
pub fn work_graph_terminal_task_result_wrapper_shadow_activation_drift_operator_summaries()
-> Vec<WorkGraphTaskResultWrapperShadowActivationDriftOperatorSummaryPreview> {
    vec![
        operator_summary(
            "shadow_surface_state_drift_operator_summary",
            "detect_shadow_activation_surface_state_drift",
            "Shadow activation surface state drift must remain blocked before activation",
        ),
        operator_summary(
            "shadow_blocker_binding_drift_operator_summary",
            "detect_shadow_activation_blocker_binding_drift",
            "Shadow blocker binding drift must be reviewed before promotion checks",
        ),
        operator_summary(
            "shadow_enablement_satisfaction_drift_operator_summary",
            "detect_shadow_enablement_satisfaction_drift",
            "Shadow enablement satisfaction drift must remain unsatisfied before activation",
        ),
        operator_summary(
            "shadow_kill_switch_armament_drift_operator_summary",
            "detect_shadow_kill_switch_armament_drift",
            "Shadow kill switch armament drift must be reviewed before execution",
        ),
        operator_summary(
            "shadow_side_effect_lock_drift_operator_summary",
            "detect_shadow_side_effect_lock_drift",
            "Shadow side-effect lock drift must remain zero before any live surface",
        ),
    ]
}

pub fn work_graph_terminal_task_result_wrapper_shadow_activation_drift_preconditions()
-> Vec<WorkGraphTaskResultWrapperShadowActivationDriftPreconditionPreview> {
    vec![
...
pub fn work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_blockers()
-> Vec<WorkGraphTaskResultWrapperShadowActivationDriftBudgetBlockerPreview> {
    vec![
        blocker(
            "shadow_activation_drift_budget_not_executed",
            "critical",
            "shadow activation drift detectors have zero-tolerance budgets but no shadow readback execution has run",
        ),
        blocker(
            "shadow_operator_review_not_performed",
            "high",
            "shadow activation operator summaries are preview-only and have not been reviewed or persisted",
        ),
        blocker(
            "shadow_activation_preconditions_not_attached",
            "medium",
            "shadow activation preconditions are defined but not attached to runtime activation or promotion logic",
        ),
        blocker(
            "shadow_drift_persistence_disabled",
            "medium",
            "shadow activation drift state cannot be persisted until store enablement is explicitly approved later",
        ),
    ]
}

pub fn work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_required_prior_gates()
-> Vec<&'static str> {
    vec![
...
impl WorkGraphTerminalTaskResultWrapperShadowActivationDriftBudgetPreviewSideEffects {
    pub const fn none() -> Self {
        Self {
            filesystem_written: false,
            shadow_drift_budget_persisted: false,
...
) -> WorkGraphTaskResultWrapperShadowActivationDriftBudgetPreview {
    WorkGraphTaskResultWrapperShadowActivationDriftBudgetPreview {
        id,
        drift_detector_id,
        compared_fields,
        ...
        max_replay_lag_ms: 0,
        severity: "critical",
        block_level: "block_shadow_readback_activation_promotion_enforcement_store_and_live_execution",
        operator_summary_id,
        activation_precondition_ids,
        allows_shadow_readback_execution: false,
        allows_activation: false,
        allows_promotion_execution: false,
...
fn operator_summary(
    ...
    drift_detector_id: &'static str,
    title: &'static str,
) -> WorkGraphTaskResultWrapperShadowActivationDriftOperatorSummaryPreview {
    WorkGraphTaskResultWrapperShadowActivationDriftOperatorSummaryPreview {
        id,
        drift_detector_id,
        title,
...
) -> WorkGraphTaskResultWrapperShadowActivationDriftPreconditionPreview {
    WorkGraphTaskResultWrapperShadowActivationDriftPreconditionPreview {
        id,
        required_budget_ids,
...
fn blocker(
    ...
    severity: &'static str,
    reason: &'static str,
) -> WorkGraphTaskResultWrapperShadowActivationDriftBudgetBlockerPreview {
    WorkGraphTaskResultWrapperShadowActivationDriftBudgetBlockerPreview {
        id,
        severity,
        reason,
...
{
    work_graph_terminal_task_result_wrapper_shadow_activation_drift_budgets()
        .iter()
...
mod tests {
    ...
    #[test]
    fn shadow_activation_drift_budget_declares_zero_tolerance_budgets() {
        let report =
            hepta_work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_report();
        ...
        assert!(report.drift_budgets.iter().all(|budget| {
            budget.max_allowed_mismatches == 0
                ...
                && !budget.allows_shadow_readback_execution
                && !budget.allows_activation
                && !budget.allows_promotion_execution
    ...
    #[test]
    fn shadow_activation_drift_budget_requires_operator_review() {
        let report =
            hepta_work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_report();
    ...
    #[test]
    fn shadow_activation_drift_budget_blocks_activation_until_preconditions_are_reviewed() {
        let report =
            hepta_work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_report();
        ...
        assert!(report.activation_preconditions.iter().all(|precondition| {
            precondition.blocks_activation
                && precondition.blocks_promotion
                && !precondition.currently_satisfied
        }));
        assert!(report.ready_for_shadow_activation_promotion_precondition_preview);
        assert_eq!(
    ...
    #[test]
    fn shadow_activation_drift_budget_keeps_execution_and_persistence_disabled() {
        let report =
            hepta_work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_report();
    ...
    #[test]
    fn shadow_activation_drift_budget_requires_shadow_readback_prior() {
        let report =
            hepta_work_graph_terminal_task_result_wrapper_shadow_activation_drift_budget_preview_report();
