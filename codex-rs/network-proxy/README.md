...
`codex-network-proxy` is Hepta's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from Hepta's merged `config.toml` (via `codex-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $HEPTA_HOME/proxy/ (ca.pem + ca.key).
...
When `codex-network-proxy` is embedded in managed Hepta runtime, policy decisions emit structured
