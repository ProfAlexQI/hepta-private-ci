...
pub(super) fn parse(bytes: &[u8]) -> Result<Value, AcceptanceError> {
    let StrictValue(value) = serde_json::from_slice(bytes)
        .map_err(|error| AcceptanceError::Invalid(format!("invalid strict JSON: {error}")))?;
    Ok(value)
}
...
mod tests {
    ...
    fn duplicate_keys_are_rejected_recursively() {
        assert!(parse(br#"{"pass":false,"pass":true}"#).is_err());
        assert!(parse(br#"{"nested":{"value":1,"value":2}}"#).is_err());
        assert!(parse(br#"{"pass":true,"nested":{"value":2}}"#).is_ok());
    }
}
