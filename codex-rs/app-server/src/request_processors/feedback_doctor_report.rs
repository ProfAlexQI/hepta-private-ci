...
//! the configured Hepta executable as a subprocess, accepts only valid JSON from
//! `hepta doctor --json`, derives a small set of Sentry tags, and otherwise
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `hepta-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `hepta doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn Hepta, finish before the timeout, or parse JSON means the
