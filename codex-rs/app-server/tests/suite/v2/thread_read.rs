...
fn run_thread_read_test_with_stack<F>(name: &str, future: F) -> Result<()>
where
...
fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    run_thread_read_test_with_stack("thread turns list pathless store", async {
        let codex_home = TempDir::new()?;
        let thread_id =
        ...
        let client = in_process::start(InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "hepta-app-server-tests".to_string(),
                    title: None,
...
fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    run_thread_read_test_with_stack("thread read pathless store history", async {
        let codex_home = TempDir::new()?;
        let store_id = Uuid::new_v4().to_string();
        create_config_toml_with_thread_store(codex_home.path(), &store_id)?;
        let store = InMemoryThreadStore::for_id(store_id.clone());
...
fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    run_thread_read_test_with_stack("thread list pathless store", async {
        let codex_home = TempDir::new()?;
        let thread_id =
