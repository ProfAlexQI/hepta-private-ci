...
fn run_conversation_summary_test_with_stack<F>(name: &str, future: F) -> Result<()>
where
...
fn get_conversation_summary_by_thread_id_reads_pathless_store_thread() -> Result<()> {
    run_conversation_summary_test_with_stack("conversation summary pathless store thread", async {
        let codex_home = TempDir::new()?;
