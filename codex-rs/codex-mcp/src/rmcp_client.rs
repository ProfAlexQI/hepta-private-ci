...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools =
        hepta_mcp_pagination::collect_paginated("tools/list", timeout, |cursor| async move {
            let params = cursor
                .map(|next| rmcp::model::PaginatedRequestParams::default().with_cursor(Some(next)));
            let response = client
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams::new(
        capabilities,
        Implementation::new("codex-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Hepta"),
    )
