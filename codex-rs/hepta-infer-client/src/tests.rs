...
async fn start_scripted_server(
    ...
    tokio::spawn(async move {
        ...
        let request_length = usize::try_from(u32::from_be_bytes(length_bytes))
...
async fn exchange_timeout_is_fail_closed() {
    ...
    let task = start_scripted_server(&socket, ScriptedResponse::Delay(Duration::from_secs(1))).await;
