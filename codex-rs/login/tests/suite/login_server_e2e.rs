...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let tmp = tempdir()?;
    let hepta_home = tmp.path().to_path_buf();
    ...
    let server_home = hepta_home.clone();
    ...
    // Validate auth.json
    let auth_path = hepta_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_hepta_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let hepta_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Hepta"),
        "error body should clearly explain the Hepta access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Hepta to retry"),
    ...
    assert!(
        !body.contains("get access to Hepta"),
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_hepta_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        codex_home: first_hepta_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: codex_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_hepta_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        codex_home: second_hepta_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: codex_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
