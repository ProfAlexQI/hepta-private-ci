...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("hepta-tui"), true);
    assert_eq!(is_first_party_originator("hepta_vscode"), true);
    assert_eq!(is_first_party_originator("Hepta Something Else"), true);
    ...
    assert_eq!(is_first_party_originator("hepta_cli"), false);
...
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("hepta_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("hepta_chatgpt_desktop"),
        true
    );
    ...
    assert_eq!(is_first_party_chat_originator("hepta_vscode"), false);
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Hepta UA for that originator
    let expected_ua = get_hepta_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "hepta_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "hepta_cli_rs/0.0.0 (bad_suffix)"
    );
}
