...
const APP_TOOLTIP: &str =
    "Try the **Hepta App**. Run 'hepta app' to open the local app experience.";
const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP: &str =
    "*New* Build faster with the **Hepta App**. Run 'hepta app' to open the local app experience.";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with Hepta.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, Hepta is included in your plan for free – let’s build together.";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("hepta app") {
                return false;
            }
            true
        })
...
/// Pick a random tooltip to show to the user when starting Hepta.
...
pub(crate) mod announcement {
    ...
    pub(crate) fn prewarm() {
        let _ = ANNOUNCEMENT_TIP.get_or_init(|| None);
    }
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for Hepta TUI.
...
[[announcements]]
content = "Welcome to Hepta! Check out the new onboarding flow."
