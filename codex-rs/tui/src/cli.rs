...
pub struct Cli {
    ...
    // Internal controls set by the top-level `hepta resume` subcommand.
    // These are not exposed as user flags on the base `hepta` command.
    #[clap(skip)]
    ...
    /// top-level `hepta resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `hepta fork` subcommand.
    // These are not exposed as user flags on the base `hepta` command.
    #[clap(skip)]
    ...
    /// top-level `hepta fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
