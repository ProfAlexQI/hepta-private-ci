...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let codex_home = find_codex_home().wrap_err("failed to resolve HEPTA_HOME")?;
...
pub async fn run_main(
    ...
    // Hepta CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `hepta {action}` without an ID to choose from existing sessions."
            )),
        })
    };
...
mod tests {
    ...
    fn large_stack_test_runtime() -> color_eyre::Result<tokio::runtime::Runtime> {
    ...
    fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        let runtime = large_stack_test_runtime()?;
