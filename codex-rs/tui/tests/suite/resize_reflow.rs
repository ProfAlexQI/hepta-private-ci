...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = codex_utils_cargo_bin::cargo_bin("hepta") {
        return Ok(path);
    }

    let fallback = repo_root.join("codex-rs/target/debug/hepta");
    anyhow::ensure!(
        fallback.is_file(),
        "Hepta binary is unavailable; run `cargo build -p codex-cli` first"
    );
    Ok(fallback)
}
