...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct ChannelIngressEvidenceSummary {
    pub events: u64,
    pub receipts: u64,
...
pub struct EvidenceSummary {
    ...
    pub channel_ingress: ChannelIngressEvidenceSummary,
}

impl HeptaEvidenceStore {
    /// Reads every supported family from one SQLite read transaction. This is
    /// a diagnostic snapshot, not an authority or an anti-rollback root.
    pub async fn summary(&self) -> Result<EvidenceSummary, EvidenceError> {
        ...
        let summary = EvidenceSummary {
            ...
            channel_ingress: ChannelIngressEvidenceSummary {
                events: count(
                    &mut transaction,
                ...
                pending_events: count(
                    &mut transaction,
                    "SELECT COUNT(*)
                     FROM channel_ingress_events AS events
                     LEFT JOIN channel_ingress_receipts AS receipts
                       ON receipts.event_id = events.event_id
                     WHERE receipts.event_id IS NULL",
                )
                .await?,
                indeterminate_events: count(
                    &mut transaction,
                    "SELECT COUNT(*) FROM channel_ingress_receipts
                     WHERE terminal_kind = 'indeterminate'",
                )
