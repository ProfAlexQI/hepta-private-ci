...
fn ingress_event() -> ChannelIngressEvent {
...
async fn supported_families_project_exact_pending_and_terminal_records() {
    ...
    let ingress = ingress_event();
    store
        .claim_channel_ingress_event(&ingress)
        .await
        .expect("claim ingress");
    let ingress_selector = HistoricalEvidenceSelector::new(
        HistoricalEvidenceFamily::ChannelIngress,
        ingress.event_id.as_str(),
    )
    .expect("ingress selector");
    let pending_ingress = store
        .historical_record(&ingress_selector)
        .await
        .expect("ingress pending read")
        .expect("ingress record");
    assert_eq!(pending_ingress.state(), HistoricalEvidenceState::Pending);
    store
        ...
        .await
        .expect("append ingress receipt");
    let terminal_ingress = store
        .historical_record(&ingress_selector)
        .await
        .expect("ingress terminal read")
        .expect("ingress record");
    assert_eq!(terminal_ingress.state(), HistoricalEvidenceState::Accepted);
    terminal_ingress.validate().expect("valid ingress record");
    assert_eq!(
        terminal_ingress.evidence_sha256().as_str(),
        "c3970dfaff515fddcc34f84e6d96df66a1f12374a43482e5f4fe2b253d25972f"
    );
    assert_eq!(
        terminal_ingress.record_sha256().as_str(),
        "6ca9f7fdfe7afe0b52309a3a64213e5053262c2001c50006314c3180e5ec103b"
    );
...
async fn missing_exact_ids_return_none_without_cross_family_fallback() {
    ...
    for (family, record_id) in [
        ...
        (
            HistoricalEvidenceFamily::ChannelIngress,
            format!("channel-ingress:v1:{}", "c".repeat(64)),
        ),
    ] {
