...
mod tests {
    ...
    fn bootstrap_candidate_matching_helpers_score_surface_and_semantic_aliases() {
        ...
        assert_eq!(
            extract_bootstrap_semantic_hints_for_match(
                "adaptive recall context",
                &topic_session,
                4,
            ),
            vec!["recall", "context"]
        ...
        let make_route =
            |label: &str, existing_index: Option<usize>| BootstrapTopicRouteCandidate {
        ...
        let merged = select_bootstrap_merged_topic_route(
            &selection.routes,
            Some("merge"),
            |routes, marker| {
    ...
    fn learned_router_signals_apply_feedback_hints_to_scores() {
        ...
        assert_eq!(
            ...
            Some(
                "1 learned semantic router signal(s) applied by semantic-router:learned-feedback-v1"
            )
        );
