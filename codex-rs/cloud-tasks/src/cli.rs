...
pub enum Command {
    /// Submit a new Hepta Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Hepta Cloud task.
    Status(StatusCommand),
    /// List Hepta Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Hepta Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Hepta Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Hepta Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `hepta cloud` to browse).
    ...
    /// Git branch to run in Hepta Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Hepta Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Hepta Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Hepta Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
