...
fn external_migration_is_experimental_and_disabled_by_default() {
    ...
    assert_eq!(
        ...
        Some(
            "Show a startup prompt when Hepta detects migratable external agent config for this machine or project."
        )
    );
...
subagent_developer_instructions = "Use the governed backend lane."
...
"#,
    ...
    assert_eq!(
        ...
        Some(crate::FeatureToml::Config(crate::MultiAgentV2ConfigToml {
            ...
            subagent_developer_instructions: Some("Use the governed backend lane.".to_string()),
