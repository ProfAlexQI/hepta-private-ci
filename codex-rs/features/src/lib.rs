...
pub enum Feature {
    ...
    /// Enable MCP apps.
    EnableMcpApps,
    /// Allow an explicit compatibility path override for the host-owned apps MCP server.
    AppsMcpPathOverride,
    ...
    Goals,
    /// Add context-window budget metadata and model-owned defaults to resolved config.
    TokenBudget,
