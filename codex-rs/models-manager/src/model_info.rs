...
const DEFAULT_PERSONALITY_HEADER: &str = "You are Hepta, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
fn clear_instruction_messages(model: &mut ModelInfo) {
    if let Some(model_messages) = &mut model.model_messages {
        ...
        if model_messages.token_budget.is_none() {
