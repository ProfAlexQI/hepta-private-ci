...
pub(super) async fn replay_operation_pool(
    ...
    .map_err(unavailable)?
    .ok_or_else(|| {
        CognitiveStoreError::Invalid("intelligence mutation operation does not exist".to_string())
    })?;
...
fn replay_rows(
    ...
    for row in transitions {
        let sequence_i64: i64 = row.try_get("sequence").map_err(unavailable)?;
        let sequence = from_i64(sequence_i64, "transition sequence")?;
...
fn verify_transition_row(
    row: &SqliteRow,
    receipt: &super::IntelligenceMutationTransitionReceipt,
) -> Result<(), CognitiveStoreError> {
    ...
    if parse_phase(&stored_from)? != receipt.from_phase
        ...
        || stored_intent_appended != i64::from(receipt.durable_intent_appended)
        || stored_intent_settled != i64::from(receipt.durable_intent_settled)
    ...
    {
        return Err(CognitiveStoreError::Corrupt(
            "persisted intelligence mutation receipt failed exact replay"
                .to_string(),
        ));
    }
    Ok(())
