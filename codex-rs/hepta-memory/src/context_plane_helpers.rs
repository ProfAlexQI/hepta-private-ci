...
impl StoreSnapshot {
    ...
    /// Builds the offline temporal-graph shadow WAL replay evidence report
    /// without recording receipts, persisting graph facts, or activation state.
    pub fn context_memory_temporal_graph_shadow_replay_report(
        &self,
        request: &ContextRecallRequest,
    ) -> ContextMemoryTemporalGraphShadowReplayReport {
        ContextMemoryTemporalGraphShadowReplayReport::from_shadow_store(
            &self.context_memory_temporal_graph_shadow_store_report(request),
        )
    }
    ...
    /// Builds the offline temporal-graph traversal quality/SLO report without
    /// enabling graph traversal, reranking, or a production route.
    pub fn context_memory_temporal_graph_shadow_traversal_quality_report(
        &self,
        request: &ContextRecallRequest,
        ...
        ContextMemoryTemporalGraphShadowTraversalQualityReport::from_traversal_diff(
            &self.context_memory_temporal_graph_shadow_traversal_diff_report(request),
        )
    }
    ...
    /// Builds the offline temporal-graph retrieval rollback/kill-switch
    /// evidence report without opening a route or writing rollback state.
    pub fn context_memory_temporal_graph_shadow_retrieval_rollback_kill_switch_report(
        &self,
        request: &ContextRecallRequest,
        ...
        ContextMemoryTemporalGraphShadowRetrievalRollbackKillSwitchReport::from_retrieval_canary_guard(
            &self.context_memory_temporal_graph_shadow_retrieval_canary_guard_report(request),
        )
    }
