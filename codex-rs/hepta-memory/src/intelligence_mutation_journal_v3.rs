...
//!
//! This is the canonical compiled entry for the P0.4b source candidate. The
//! journal is opt-in and qualification-only: it does not change the default
//! cognitive-store open path, register a runtime caller, or grant production
...
#[path = "intelligence_mutation_journal_v3/digest.rs"]
mod digest;
#[path = "intelligence_mutation_journal_v3/replay.rs"]
mod replay;
#[path = "intelligence_mutation_journal_v3/schema.rs"]
mod schema;

#[cfg(test)]
#[path = "intelligence_mutation_journal_v3/tests.rs"]
mod tests;
    ...
    pub(crate) async fn begin_intelligence_mutation_journal(
        ...
        let existing = sqlx::query(
            "SELECT owner_agent_id, lease_id, lease_epoch, expected_revision,
                    starting_projection_generation, causal_root_sha256, binding_sha256
        ...
        let disposition = if let Some(existing) = existing {
            ...
            let operation_count: i64 = sqlx::query_scalar(
                "SELECT COUNT(*) FROM cognitive_intelligence_mutation_operations",
            )
            .fetch_one(&mut *transaction)
            .await
            .map_err(unavailable)?;
            if operation_count >= i64::try_from(MAX_JOURNALED_OPERATIONS).unwrap_or(i64::MAX) {
