...
#[command(
    version,
    override_usage = "hepta exec [OPTIONS] [PROMPT]\n       hepta exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Hepta outside a Git repository.
    ...
    /// Do not load `$HEPTA_HOME/config.toml`; auth still uses `HEPTA_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "hepta resume --last <prompt>" treats the positional as a prompt, not a session id.
