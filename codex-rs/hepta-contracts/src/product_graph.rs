...
pub const PRODUCT_GRAPH_SCHEMA_VERSION: u32 = 2;
...
impl ProductGraph {
    pub fn agent_local(authority: &AuthorityGrant) -> Result<Self, ProductGraphError> {
        ...
        let graph = Self {
            ...
            components: vec![
                ...
                // This adapter is part of the product topology but remains
                // inert in every current local authority profile. Presence in
                // the graph never grants provider dispatch or external effect.
                ProductComponentId::ProviderEffectAdapter,
            ],
            edges: vec![
                ProductEdge {
                    from: ProductComponentId::Agentd,
                    to: ProductComponentId::Supervisor,
                    kind: ProductEdgeKind::DependsOn,
                },
                ProductEdge {
                    from: ProductComponentId::Agentd,
                    to: ProductComponentId::MemoryRuntime,
                    kind: ProductEdgeKind::Composes,
                },
                ProductEdge {
                    from: ProductComponentId::Agentd,
                    to: ProductComponentId::AutomationRuntime,
                    kind: ProductEdgeKind::Composes,
                },
                ProductEdge {
                    from: ProductComponentId::Agentd,
                    to: ProductComponentId::AppServer,
                    kind: ProductEdgeKind::Hosts,
                },
                ProductEdge {
                    from: ProductComponentId::AppServer,
                    to: ProductComponentId::MemoryRuntime,
                    kind: ProductEdgeKind::ReadsFrom,
                },
                ProductEdge {
                    from: ProductComponentId::AutomationRuntime,
                    to: ProductComponentId::AppServer,
                    kind: ProductEdgeKind::SubmitsTo,
                },
                ProductEdge {
                    from: ProductComponentId::MatrixIngress,
                    to: ProductComponentId::Agentd,
                    kind: ProductEdgeKind::SubmitsTo,
                },
                ProductEdge {
                    from: ProductComponentId::AppServer,
            ...
            data_authorities: vec![
                DataAuthority {
                    domain: DataDomain::FleetRegistry,
                    writer: ProductComponentId::Supervisor,
                    store: DataStoreKind::JsonRegistry,
                },
                DataAuthority {
                    domain: DataDomain::AgentLifecycle,
                    writer: ProductComponentId::Supervisor,
                    store: DataStoreKind::JsonRegistry,
                },
                DataAuthority {
                    domain: DataDomain::ThreadSession,
                    writer: ProductComponentId::AppServer,
                    store: DataStoreKind::AgentPrivateSqlite,
                },
                DataAuthority {
                    domain: DataDomain::MemoryLedger,
                    writer: ProductComponentId::MemoryRuntime,
                    store: DataStoreKind::AgentPrivateSqlite,
                },
                DataAuthority {
                    domain: DataDomain::KnowledgeProjection,
                    writer: ProductComponentId::MemoryRuntime,
                    store: DataStoreKind::AgentPrivateSqlite,
                },
                DataAuthority {
                    domain: DataDomain::AutomationSchedule,
                    writer: ProductComponentId::AutomationRuntime,
                    store: DataStoreKind::AgentPrivateSqlite,
                },
                DataAuthority {
                    domain: DataDomain::IngressProjection,
                    writer: ProductComponentId::MatrixIngress,
                    store: DataStoreKind::AgentPrivateSqlite,
                },
                DataAuthority {
                    domain: DataDomain::RuntimeHealth,
                    writer: ProductComponentId::Agentd,
                    store: DataStoreKind::ProcessMemory,
                },
            ],
        };
        graph.validate()?;
    ...
    pub fn components(&self) -> &[ProductComponentId] {
        &self.components
    }
    ...
    pub fn data_authorities(&self) -> &[DataAuthority] {
    ...
    pub fn writer_for(&self, domain: DataDomain) -> Option<ProductComponentId> {
        self.data_authorities
            .iter()
            .find(|authority| authority.domain == domain)
            .map(|authority| authority.writer)
    }
    ...
    pub fn validate(&self) -> Result<(), ProductGraphError> {
        ...
        if component_set.len() != self.components.len() {
            return Err(ProductGraphError::DuplicateComponent);
        }
        if component_set.contains(&ProductComponentId::QualificationPlane) {
            return Err(ProductGraphError::QualificationComponentInProductGraph);
        }
        let mut edge_set = BTreeSet::new();
        for edge in &self.edges {
            if !component_set.contains(&edge.from) || !component_set.contains(&edge.to) {
                return Err(ProductGraphError::EdgeReferencesUnknownComponent);
            }
            if !edge_set.insert((edge.from, edge.to, edge.kind)) {
                return Err(ProductGraphError::DuplicateEdge);
            }
        }
        validate_acyclic(&component_set, &self.edges)?;
        let mut writers = BTreeMap::new();
        for authority in &self.data_authorities {
            if !component_set.contains(&authority.writer) {
                return Err(ProductGraphError::DataWriterNotInGraph);
            }
            if writers.insert(authority.domain, authority.writer).is_some() {
                return Err(ProductGraphError::DuplicateDataWriter(authority.domain));
            }
        }
        for domain in [
            DataDomain::FleetRegistry,
            DataDomain::AgentLifecycle,
            DataDomain::ThreadSession,
            DataDomain::MemoryLedger,
            DataDomain::KnowledgeProjection,
            DataDomain::AutomationSchedule,
            DataDomain::IngressProjection,
            DataDomain::RuntimeHealth,
        ] {
            if !writers.contains_key(&domain) {
                return Err(ProductGraphError::MissingDataWriter(domain));
            }
        }
        Ok(())
    }
...
fn validate_acyclic(
    components: &BTreeSet<ProductComponentId>,
    edges: &[ProductEdge],
) -> Result<(), ProductGraphError> {
    ...
    for edge in edges {
        ...
        if let Some(targets) = outgoing.get_mut(&edge.from) {
    ...
    if visited != components.len() {
        return Err(ProductGraphError::DependencyCycle);
    }
    Ok(())
}
...
mod tests {
    ...
    fn agent_product_graph_is_acyclic_and_single_writer() {
        let grant = AuthorityGrant::agent_local(agent_id(), 1)
            .unwrap_or_else(|error| panic!("grant must be valid: {error}"));
        let graph = ProductGraph::agent_local(&grant)
            .unwrap_or_else(|error| panic!("product graph must be valid: {error}"));
        assert!(graph.validate().is_ok());
        assert_eq!(
            graph.writer_for(DataDomain::MemoryLedger),
            Some(ProductComponentId::MemoryRuntime)
        );
        assert_eq!(
            graph.writer_for(DataDomain::ThreadSession),
            Some(ProductComponentId::AppServer)
        );
        assert!(
            graph
                .components()
    ...
    fn qualification_profile_changes_authority_not_topology() {
        let local = AuthorityGrant::agent_local(agent_id(), 4)
            .unwrap_or_else(|error| panic!("local grant must be valid: {error}"));
        let qualification = AuthorityGrant::qualification_cognitive_write(agent_id(), 4)
            .unwrap_or_else(|error| panic!("qualification grant must be valid: {error}"));
        let local_graph = ProductGraph::agent_local(&local)
            .unwrap_or_else(|error| panic!("local graph must be valid: {error}"));
        let qualification_graph = ProductGraph::agent_local(&qualification)
            .unwrap_or_else(|error| panic!("qualification graph must be valid: {error}"));
        assert_eq!(local_graph.components(), qualification_graph.components());
        assert_eq!(local_graph.edges(), qualification_graph.edges());
        assert_eq!(
            local_graph.data_authorities(),
            qualification_graph.data_authorities()
        );
        assert_ne!(
            local_graph.authority_grant_sha256(),
            qualification_graph.authority_grant_sha256()
        );
    }

    #[test]
    fn snapshot_profile_cannot_host_agent_product_graph() {
        let snapshot = AuthorityGrant::snapshot_read_only(agent_id(), 1)
            .unwrap_or_else(|error| panic!("snapshot grant must be valid: {error}"));
        assert!(matches!(
            ProductGraph::agent_local(&snapshot),
            Err(ProductGraphError::UnsupportedAuthorityProfile)
        ));
    }

    #[test]
    fn duplicate_data_writer_fails_closed() {
        let grant = AuthorityGrant::agent_local(agent_id(), 2)
            .unwrap_or_else(|error| panic!("grant must be valid: {error}"));
        let mut graph = ProductGraph::agent_local(&grant)
            .unwrap_or_else(|error| panic!("product graph must be valid: {error}"));
        graph.data_authorities.push(DataAuthority {
            domain: DataDomain::MemoryLedger,
            writer: ProductComponentId::Agentd,
            store: DataStoreKind::ProcessMemory,
        });
        assert!(matches!(
            graph.validate(),
            Err(ProductGraphError::DuplicateDataWriter(
                DataDomain::MemoryLedger
            ))
        ));
    }
