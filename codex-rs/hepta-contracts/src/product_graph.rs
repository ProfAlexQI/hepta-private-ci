...
mod tests {
    ...
    fn agent_product_graph_is_acyclic_and_single_writer() {
        let grant = match AuthorityGrant::agent_local(agent_id(), 1) {
            Ok(grant) => grant,
            Err(error) => panic!("grant must be valid: {error}"),
        };
        let graph = match ProductGraph::agent_local(&grant) {
            Ok(graph) => graph,
            Err(error) => panic!("product graph must be valid: {error}"),
        };
    ...
    fn qualification_profile_changes_authority_not_topology() {
        let local = match AuthorityGrant::agent_local(agent_id(), 4) {
            Ok(grant) => grant,
            Err(error) => panic!("local grant must be valid: {error}"),
        };
        let qualification = match AuthorityGrant::qualification_cognitive_write(agent_id(), 4) {
            Ok(grant) => grant,
            Err(error) => panic!("qualification grant must be valid: {error}"),
        };
        let local_graph = match ProductGraph::agent_local(&local) {
            Ok(graph) => graph,
            Err(error) => panic!("local graph must be valid: {error}"),
        };
        let qualification_graph = match ProductGraph::agent_local(&qualification) {
            Ok(graph) => graph,
            Err(error) => panic!("qualification graph must be valid: {error}"),
        };
    ...
    fn snapshot_profile_cannot_host_agent_product_graph() {
        let snapshot = match AuthorityGrant::snapshot_read_only(agent_id(), 1) {
            Ok(grant) => grant,
            Err(error) => panic!("snapshot grant must be valid: {error}"),
        };
        assert!(matches!(
    ...
    fn duplicate_data_writer_fails_closed() {
        let grant = match AuthorityGrant::agent_local(agent_id(), 2) {
            Ok(grant) => grant,
            Err(error) => panic!("grant must be valid: {error}"),
        };
        let mut graph = match ProductGraph::agent_local(&grant) {
            Ok(graph) => graph,
            Err(error) => panic!("product graph must be valid: {error}"),
        };
        ...
        assert!(matches!(
            graph.validate(),
            Err(ProductGraphError::DuplicateDataWriter(DataDomain::MemoryLedger))
        ));
    }
}
