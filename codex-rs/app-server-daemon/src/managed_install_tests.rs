...
use super::parse_hepta_version;

#[test]
fn parses_hepta_cli_version_output() {
    assert_eq!(
        parse_hepta_version("hepta 1.2.3\n").expect("version"),
        "1.2.3"
    );
}

#[test]
fn rejects_malformed_hepta_cli_version_output() {
    assert!(parse_hepta_version("hepta\n").is_err());
}
