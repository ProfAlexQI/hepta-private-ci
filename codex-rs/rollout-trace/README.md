...
> **Privacy:** Rollout tracing is not telemetry. Hepta does **not** upload or
...
during a Hepta session. It records raw runtime evidence into a local bundle on
...
Hot-path Hepta code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Hepta got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["Hepta runtime (codex-core)"]
...
Hepta session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `hepta debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Hepta rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
hepta debug trace-reduce <trace-bundle>
```
