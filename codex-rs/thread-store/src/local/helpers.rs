...
pub(super) fn rollout_path_is_archived(hepta_home: &Path, path: &Path) -> bool {
    path.starts_with(hepta_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) async fn resolve_thread_names(
    ...
    {
        // Legacy titles remain authoritative when present; the index only fills names whose
        // SQLite title is still derived from the preview.
