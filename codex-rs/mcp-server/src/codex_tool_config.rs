//! Configuration object accepted by the `hepta` MCP tool-call.
...
/// Client-supplied configuration for a `hepta` tool-call.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, Default)]
#[serde(rename_all = "kebab-case")]
pub struct HeptaToolCallParam {
    /// The *initial user prompt* to start the Hepta conversation.
    pub prompt: String,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub approval_policy: Option<HeptaToolCallApprovalPolicy>,

    /// Sandbox mode: `read-only`, `workspace-write`, or `danger-full-access`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sandbox: Option<HeptaToolCallSandboxMode>,

    /// Individual config settings that will override what is in
    /// HEPTA_HOME/config.toml.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
#[serde(rename_all = "kebab-case")]
pub enum HeptaToolCallApprovalPolicy {
    Untrusted,
    OnFailure,
    OnRequest,
    Never,
...
impl From<HeptaToolCallApprovalPolicy> for AskForApproval {
    fn from(value: HeptaToolCallApprovalPolicy) -> Self {
        match value {
            HeptaToolCallApprovalPolicy::Untrusted => AskForApproval::UnlessTrusted,
            HeptaToolCallApprovalPolicy::OnFailure => AskForApproval::OnFailure,
            HeptaToolCallApprovalPolicy::OnRequest => AskForApproval::OnRequest,
            HeptaToolCallApprovalPolicy::Never => AskForApproval::Never,
        }
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq)]
#[serde(rename_all = "kebab-case")]
pub enum HeptaToolCallSandboxMode {
    ReadOnly,
    WorkspaceWrite,
    DangerFullAccess,
}

impl From<HeptaToolCallSandboxMode> for SandboxMode {
    fn from(value: HeptaToolCallSandboxMode) -> Self {
        match value {
            HeptaToolCallSandboxMode::ReadOnly => SandboxMode::ReadOnly,
            HeptaToolCallSandboxMode::WorkspaceWrite => SandboxMode::WorkspaceWrite,
            HeptaToolCallSandboxMode::DangerFullAccess => SandboxMode::DangerFullAccess,
        }
    }
}

/// Compatibility alias for downstream code that still imports the upstream
/// Codex MCP tool-call parameter type name.
pub type CodexToolCallParam = HeptaToolCallParam;

/// Compatibility alias for downstream code that still imports the upstream
/// Codex MCP approval policy type name.
pub type CodexToolCallApprovalPolicy = HeptaToolCallApprovalPolicy;

/// Compatibility alias for downstream code that still imports the upstream
/// Codex MCP sandbox mode type name.
pub type CodexToolCallSandboxMode = HeptaToolCallSandboxMode;

/// Builds a `Tool` definition (JSON schema etc.) for the Hepta tool-call.
pub(crate) fn create_tool_for_hepta_tool_call_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        ...
        .into_generator()
        .into_root_schema_for::<HeptaToolCallParam>();
...
impl HeptaToolCallParam {
    /// Returns the initial user prompt to start the Hepta conversation and the
...
pub struct HeptaToolCallReplyParam {
    /// DEPRECATED: use threadId instead.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    conversation_id: Option<String>,

    /// The thread id for this Hepta session.
    ...
    /// The *next user prompt* to continue the Hepta conversation.
    pub prompt: String,
}

impl HeptaToolCallReplyParam {
...
/// Builds a `Tool` definition for the `hepta-reply` tool-call.
pub(crate) fn create_tool_for_hepta_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
        ...
        .into_generator()
        .into_root_schema_for::<HeptaToolCallReplyParam>();
...
/// Compatibility alias for downstream code that still imports the upstream
/// Codex MCP reply parameter type name.
pub type CodexToolCallReplyParam = HeptaToolCallReplyParam;
...
mod tests {
    ...
    #[test]
    fn verify_hepta_tool_json_schema() {
        let tool = create_tool_for_hepta_tool_call_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Run a Hepta session. Accepts configuration parameters matching the Hepta Config struct.",
          "inputSchema": {
            "properties": {
              "approval-policy": {
              ...
              "config": {
                ...
                "description": "Individual config settings that will override what is in HEPTA_HOME/config.toml.",
                "type": "object"
              },
              "cwd": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the Hepta conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    #[test]
    fn verify_hepta_tool_reply_json_schema() {
        let tool = create_tool_for_hepta_tool_call_reply_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a Hepta conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the Hepta conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this Hepta session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
