...
pub(crate) async fn handle_patch_approval_request(
    ...
    message_lines.push("Allow Hepta to apply proposed code changes?".to_string());
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_patch_approval_response(approval_id, on_response, hepta_thread).await;
        });
    }
}
...
pub(crate) async fn on_patch_approval_response(
    ...
    let value = match response {
        ...
        Err(err) => {
            ...
            if let Err(submit_err) = hepta_thread
