//! Custom CA handling for Hepta outbound HTTP and websocket clients.
//!
//! Hepta constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!   because Hepta itself commonly runs spawned test processes under seatbelt, so this is not just
...
/// Builds a reqwest client that honors Hepta custom CA environment variables.
///
...
/// easy mistake to make when adding a new outbound Hepta HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Hepta custom CA bundle is configured.
///
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Hepta custom
...
trait EnvSource {
    ...
    ///
    /// `CODEX_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the legacy-compatible Hepta override.
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Hepta CA overrides.
    ///
    /// This accepts a few common real-world variants so Hepta behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Hepta only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
