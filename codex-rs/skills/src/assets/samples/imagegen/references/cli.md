...
Set a stable path to the skill CLI (default `HEPTA_HOME` is `~/.hepta`):

```
export HEPTA_HOME="${HEPTA_HOME:-$HOME/.hepta}"
export IMAGE_GEN="$HEPTA_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$HEPTA_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
