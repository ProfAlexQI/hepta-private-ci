...
fn read_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "Read a Hepta memory file by relative path, optionally starting at a 1-indexed line offset and limiting the number of lines returned.",
        ),
...
fn search_tool() -> Tool {
    let mut tool = Tool::new(
        ...
        Cow::Borrowed(
            "Search Hepta memory files for substring matches, optionally normalizing separators or requiring all query substrings on the same line or within a line window.",
        ),
