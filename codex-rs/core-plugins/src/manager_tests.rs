...
fn config_layer_stack_with_requirements(
    ...
    let requirements_toml: ConfigRequirementsToml =
        toml::from_str(requirements).expect("parse requirements");
    let mut with_sources = ConfigRequirementsWithSources::default();
    with_sources.merge_unset_fields(RequirementSource::Unknown, requirements_toml);
  ...
  "apps": {
    ...
    assert_eq!(
        ...
        vec![LoadedPlugin {
            ...
            skill_discovery_mode: codex_utils_plugins::SkillDiscoveryMode::Recursive,
...
#[test]
fn refresh_curated_plugin_cache_activates_short_sha_and_retains_local_generation() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    write_curated_plugin_sha(tmp.path(), TEST_CURATED_PLUGIN_SHA);
    ...
    assert!(
        refresh_curated_plugin_cache(tmp.path(), TEST_CURATED_PLUGIN_SHA, &[plugin_id.clone()])
...
#[test]
fn refresh_curated_plugin_cache_activates_short_sha_and_retains_full_sha_generation() {
    let tmp = tempfile::tempdir().unwrap();
    let curated_root = curated_plugins_repo_path(tmp.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_id = PluginId::new(
...
#[test]
fn refresh_non_curated_plugin_cache_activates_manifest_version_and_retains_local_generation() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_root = tmp.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).unwrap();
    fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
