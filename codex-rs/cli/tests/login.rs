...
fn write_file_auth_config(hepta_home: &Path) -> Result<()> {
    std::fs::write(
        hepta_home.join("config.toml"),
        "cli_auth_credentials_store = \"file\"\n",
    )?;
    Ok(())
}

fn read_auth_json(hepta_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(hepta_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(hepta_home.path())?;
    ...
    let auth = read_auth_json(hepta_home.path())?;
