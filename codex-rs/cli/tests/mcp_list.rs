...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = hepta_command(hepta_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(hepta_home.path()).await?;
    ...
    ConfigEditsBuilder::new(hepta_home.path())
        .replace_mcp_servers(&servers)
        .apply_blocking()?;

    let mut list_cmd = hepta_command(hepta_home.path())?;
    ...
    let mut list_json_cmd = hepta_command(hepta_home.path())?;
    ...
    let mut get_cmd = hepta_command(hepta_home.path())?;
    ...
    assert!(stdout.contains("remove: hepta mcp remove docs"));

    let mut get_json_cmd = hepta_command(hepta_home.path())?;
    get_json_cmd
