...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = hepta_command(hepta_home.path())?;
    add_cmd
    ...
    let servers = load_global_mcp_servers(hepta_home.path()).await?;
    ...
    let mut remove_cmd = hepta_command(hepta_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = hepta_command(hepta_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!hepta_home.path().join(".credentials.json").exists());
    assert!(!hepta_home.path().join(".env").exists());
