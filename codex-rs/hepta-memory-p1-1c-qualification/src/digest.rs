...
impl Digest32 {
    ...
    pub fn for_bytes(input: &[u8]) -> Self {
        ...
        let encoded_len = u64::try_from(bit_len).unwrap_or(u64::MAX).to_be_bytes();
        padded.extend_from_slice(&encoded_len);
    ...
    pub fn from_hex(value: &str) -> Result<Self, ContractError> {
        if value.len() != 64 {
            return Err(ContractError::Invalid(
                "SHA-256 hex digest must contain exactly 64 characters".to_string(),
            ));
        }
        let mut bytes = [0_u8; 32];
...
fn decode_nibble(byte: u8) -> Result<u8, ContractError> {
    match byte {
        ...
        _ => Err(ContractError::Invalid(
            "SHA-256 digest contains non-hexadecimal characters".to_string(),
        )),
    }
}
