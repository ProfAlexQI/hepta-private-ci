...
import run_bazel_q031_direct_bazel as subject
...
class DirectBazelCasQualificationTest(unittest.TestCase):
    ...
    def test_bazelisk_is_rehashed_after_child_resolution(self) -> None:
        temporary, env, bazelisk, real_bazel = self.fixture()
        self.addCleanup(temporary.cleanup)
        subject.prepare_bazelisk_environment(env)
        good = self.digest(bazelisk)
        calls = 0

        def digest(path: Path) -> str:
            nonlocal calls
            if path == bazelisk.resolve():
                calls += 1
                return good if calls == 1 else "0" * 64
            return self.digest(path)

        with (
            patch.object(subject, "BAZELISK_WINDOWS_X86_64_SHA256", good),
            patch.object(
                subject,
                "BAZEL_WINDOWS_X86_64_SHA256",
                self.digest(real_bazel),
            ),
            self.assertRaisesRegex(ValueError, "changed during child resolution"),
        ):
            subject.resolve_verified_bazel_command(
                ["bazel", "build"],
                env,
                which=lambda *_args, **_kwargs: str(bazelisk),
                run=lambda *_args, **_kwargs: Completed(
                    stdout=f"PATH={real_bazel.parent};C:/Windows/System32\n"
                ),
                digest_file=digest,
            )
    ...
    def test_direct_bazel_is_rehashed_immediately_before_launch(self) -> None:
        ...
        with (
            ...
            patch.object(subject, "_validate_bazelisk_inputs"),
            patch.object(
                subject,
                "BAZEL_WINDOWS_X86_64_SHA256",
                self.digest(real_bazel),
