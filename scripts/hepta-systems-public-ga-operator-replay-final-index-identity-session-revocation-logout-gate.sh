...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-replay-final-index-identity-session-revocation-logout-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-replay-denial-final-index-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_REPLAY_FINAL_INDEX_IDENTITY_SESSION_REVOCATION_LOGOUT_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-replay-final-index-identity-session-revocation-logout-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session revocation logout attachment report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Public GA operator identity/session replay denial final index gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session revocation logout architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session revocation logout attachment report"
fi

grep -q 'Public GA Operator Identity/Session Revocation Logout Attachment' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Revocation Logout Attachment"
grep -q 'ready-but-blocked' "$DOC" \
  || fail "architecture note must document ready-but-blocked status"
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that attachment does not invoke revocation/logout gates"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_revocation_logout_attachment"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_replay_denial_final_index_surface == "public_ga_operator_identity_session_replay_denial_final_index"
  and .source_public_ga_operator_identity_session_replay_denial_final_index_ready == true
  and .source_public_ga_operator_identity_session_replay_denial_final_index_blocked == true
  ...
  and .public_ga_operator_identity_session_replay_denial_final_index_attached == true
  and .public_ga_operator_identity_session_revocation_logout_attachment_ready == true
  and .public_ga_operator_identity_session_revocation_logout_attachment_blocked == true
  ...
  and .operator_identity_session_revocation_logout_static_mention_count >= 10
  ...
  and .operator_identity_invalidation_recorded == false
  ...
  and .next_migration_step == "derive_public_ga_operator_identity_session_revocation_logout_readback_without_revocation_or_logout"
...
printf 'hepta-systems-public-ga-operator-replay-final-index-identity-session-revocation-logout-gate: PASS: Public GA operator identity/session revocation logout attachment is ready but blocked without replay acceptance\n'
