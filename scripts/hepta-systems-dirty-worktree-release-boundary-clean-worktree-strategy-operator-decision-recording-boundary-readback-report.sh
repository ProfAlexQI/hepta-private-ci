...
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/dirty_worktree_release_boundary_clean_worktree_strategy_operator_decision_recording_boundary_readback.rs"
...
fail() {
  printf 'hepta-systems-dirty-worktree-release-boundary-clean-worktree-strategy-operator-decision-recording-boundary-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$PACKET_READBACK_REPORT" ]] || fail "missing executable Phase 19 operator decision checklist packet readback report: $PACKET_READBACK_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing Phase 20 Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing Phase 20 architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the Phase 20 decision recording boundary readback report"
fi
...
"$PACKET_READBACK_REPORT" >"$tmpdir/packet_readback.json" \
  || fail "failed to render Phase 19 operator decision checklist packet readback report"
jq -e . "$tmpdir/packet_readback.json" >/dev/null \
  || fail "invalid JSON rendered by Phase 19 packet readback report"

lib_export_present=false
if grep -q 'dirty_worktree_release_boundary_clean_worktree_strategy_operator_decision_recording_boundary_readback_report' "$LIB_SOURCE"; then
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-dirty-worktree-release-boundary-clean-worktree-strategy-operator-decision-recording-boundary-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_DIRTY_WORKTREE_RELEASE_BOUNDARY_CLEAN_WORKTREE_STRATEGY_OPERATOR_DECISION_RECORDING_BOUNDARY_READBACK_2026-06-28.md" \
  '
  ...
  def boundary_entry:
    ...
    | {
      ...
      boundary_key:("dirty_worktree.decision_recording_boundary." + key_safe($entry.group_type) + "." + key_safe($entry.source_bucket)),
      boundary_route:("readback://release-boundary/dirty-worktree/clean-worktree-strategy/operator-decision-recording-boundary/" + route_prefix($entry.group_type) + "/" + route_safe($entry.source_bucket)),
  ...
  {
    runtime:"hepta",
    surface:"dirty_worktree_release_boundary_clean_worktree_strategy_operator_decision_recording_boundary_readback",
    status:(if $boundary_ready then "ready_blocked" else "blocked" end),
    gate:"dirty_worktree_release_boundary_clean_worktree_strategy_operator_decision_recording_boundary_readback_gate",
    schema_version:"dirty_worktree_release_boundary_clean_worktree_strategy_operator_decision_recording_boundary_readback_v1",
    ...
    recommended_next_gate:"phase21_dirty_worktree_release_boundary_clean_worktree_strategy_operator_approval_acceptance_boundary_readback_without_acceptance",
    next_actions:[
    ...
    next_migration_step:"phase21_dirty_worktree_release_boundary_clean_worktree_strategy_operator_approval_acceptance_boundary_readback_without_acceptance",
    local_gate:$gate,
