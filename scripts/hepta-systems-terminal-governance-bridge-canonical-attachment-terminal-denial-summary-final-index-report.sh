...
READBACK_REPORT="$ROOT/scripts/hepta-systems-terminal-governance-bridge-canonical-attachment-terminal-denial-summary-readback-report.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_TERMINAL_GOVERNANCE_BRIDGE_CANONICAL_ATTACHMENT_TERMINAL_DENIAL_SUMMARY_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-terminal-governance-bridge-canonical-attachment-terminal-denial-summary-final-index-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$READBACK_REPORT" ]] || fail "missing executable terminal denial summary attachment readback report: $READBACK_REPORT"
[[ -f "$DOC" ]] || fail "missing terminal denial summary attachment final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the terminal denial summary attachment final index report"
fi

jq -n \
  --slurpfile readback <("$READBACK_REPORT") \
  --arg gate "scripts/hepta-systems-terminal-governance-bridge-canonical-attachment-terminal-denial-summary-final-index-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_TERMINAL_GOVERNANCE_BRIDGE_CANONICAL_ATTACHMENT_TERMINAL_DENIAL_SUMMARY_FINAL_INDEX_2026-06-21.md" \
  '
  ...
  ($readback.terminal_denial_summary_attachment_readback_ready == true
    and $readback.terminal_denial_summary_attachment_readback_blocked == true
    and $readback.terminal_summary_source_probe_ready_count == 4
  ...
  {
    runtime:"hepta",
    surface:"terminal_governance_bridge_canonical_attachment_terminal_denial_summary_final_index",
    plugin_id:$readback.plugin_id,
    status:(if $final_index_ready then "ready_blocked" else "blocked" end),
    source_terminal_denial_summary_attachment_readback_surface:$readback.surface,
    source_terminal_denial_summary_attachment_readback_ready:$readback.terminal_denial_summary_attachment_readback_ready,
    source_terminal_denial_summary_attachment_readback_blocked:$readback.terminal_denial_summary_attachment_readback_blocked,
    terminal_denial_summary_attachment_final_index_ready:$final_index_ready,
    terminal_denial_summary_attachment_final_index_blocked:true,
    terminal_summary_source_probe_count:$readback.terminal_summary_source_probe_count,
    terminal_summary_source_probe_ready_count:$readback.terminal_summary_source_probe_ready_count,
    terminal_summary_gates_invoked:false,
    terminal_live_gates_invoked:false,
    canonical_gate_wrapper_invoked:false,
    wrapper_target_invoked:false,
    bridge_source_count:$readback.bridge_source_count,
    tool_execution_closure_attached:$readback.tool_execution_closure_attached,
    current_canonical_governance_terminal_index_attached:$readback.current_canonical_governance_terminal_index_attached,
    ...
    next_migration_step:"attach_terminal_denial_summary_final_index_to_terminal_governance_closure_summary_without_live_gate_invocation",
    local_gate:$gate,
    architecture_note:$doc,
    source_files:{
      terminal_denial_summary_attachment_readback_report:"scripts/hepta-systems-terminal-governance-bridge-canonical-attachment-terminal-denial-summary-readback-report.sh"
    },
