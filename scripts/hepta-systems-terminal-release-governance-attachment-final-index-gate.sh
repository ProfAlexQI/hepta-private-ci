...
REPORT="$ROOT/scripts/hepta-systems-terminal-release-governance-attachment-final-index-report.sh"
READBACK_GATE="$ROOT/scripts/hepta-systems-terminal-release-governance-attachment-readback-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_TERMINAL_RELEASE_GOVERNANCE_ATTACHMENT_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-terminal-release-governance-attachment-final-index-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable terminal release governance attachment final index report: $REPORT"
[[ -x "$READBACK_GATE" ]] || fail "missing executable terminal release governance attachment readback gate: $READBACK_GATE"
[[ -f "$DOC" ]] || fail "missing terminal release governance attachment final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the terminal release governance attachment final index report"
fi

grep -q 'Terminal Release Governance Attachment Final Index' "$DOC" \
  || fail "architecture note must document Terminal Release Governance Attachment Final Index"
...
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that final index does not invoke release or live gates"

"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "terminal_release_governance_attachment_final_index"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_terminal_release_governance_attachment_readback_surface == "terminal_release_governance_attachment_readback"
  and .source_terminal_release_governance_attachment_readback_ready == true
  and .source_terminal_release_governance_attachment_readback_blocked == true
  and .terminal_release_governance_attachment_final_index_ready == true
  and .terminal_release_governance_attachment_final_index_blocked == true
  ...
  and .next_migration_step == "attach_terminal_release_governance_attachment_final_index_to_terminal_release_artifact_non_write_lock_without_release_gate_invocation"
...
printf 'hepta-systems-terminal-release-governance-attachment-final-index-gate: PASS: terminal release governance attachment final index is ready but blocked without release/live invocation\n'
