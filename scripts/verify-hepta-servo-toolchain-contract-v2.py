#!/usr/bin/env python3
"""Canonical verifier for the Servo toolchain receipt contract."""
...
def verify_capture() -> None:
    ...
    for forbidden in (
        ...
        if forbidden in text:
            fail(f"toolchain capture contains forbidden surface {forbidden!r}")
...
def main() -> int:
    try:
        ...
        print(f"HEPTA_SERVO_TOOLCHAIN_CONTRACT_V2=FAIL: {error}", file=sys.stderr)
        return 1
    print(
        json.dumps(
            {
                "schema": "hepta.browser.servo_toolchain_contract_check.v2",
