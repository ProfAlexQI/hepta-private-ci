...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-agent-jobs-task-board-scheduler-guardrail-blocking-dry-run-entrypoint-hardening-terminal-no-enforcement-final-closeout-report.sh"
...
jq -e '
  ...
  and .runtime == "hepta"
  and .status == "ready"
  and .gate == "hepta_work_graph_agent_jobs_task_board_scheduler_guardrail_blocking_dry_run_entrypoint_hardening_terminal_no_enforcement_final_closeout_gate"
  and .schema_version == "work_graph_agent_jobs_task_board_scheduler_guardrail_blocking_dry_run_entrypoint_hardening_terminal_no_enforcement_final_closeout_v1"
  and .preview_mode == "scheduler_guardrail_blocking_dry_run_entrypoint_hardening_terminal_no_enforcement_final_closeout_report_only"
  and .source_non_persistence_readback_gate == "hepta_work_graph_agent_jobs_task_board_scheduler_guardrail_blocking_dry_run_entrypoint_hardening_readback_audit_index_non_persistence_readback_gate"
  and .source_readback_entry_count == 6
  and .source_readback_blocker_count == 33
  ...
  and .final_closeout_blocker_count == 36
  and .required_prior_gate_count == 15
  and .final_closeout_scope.source_surface_id == "work_graph_agent_jobs_task_board.scheduler_guardrail_blocking_dry_run_entrypoint_hardening.readback_audit_index_non_persistence_readback"
  and .final_closeout_scope.closeout_mode == "scheduler_guardrail_blocking_dry_run_entrypoint_hardening_terminal_no_enforcement_final_closeout_report_only"
  ...
  and .recommended_next_gate == "hepta_work_graph_agent_jobs_task_board_scheduler_guardrail_blocking_dry_run_entrypoint_live_attachment_precondition_matrix_gate"
  and .terminal_no_enforcement_branch_closed == true
  ...
  and .terminal_no_enforcement_final_closeout_preconditions_complete == true
  ...
  and .ready_for_live_attachment_precondition_matrix == true
  and .ready_for_live_execution == false
  and .source_probes.non_persistence_readback_report_gate == "hepta_work_graph_agent_jobs_task_board_scheduler_guardrail_blocking_dry_run_entrypoint_hardening_readback_audit_index_non_persistence_readback_gate"
  ...
  and .source_probes.non_persistence_readback_no_live_confirmed == true
  and .source_probes.non_persistence_readback_side_effects_all_false == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

tests=(
  scheduler_guardrail_terminal_no_enforcement_final_closeout_derives_from_readback
  scheduler_guardrail_terminal_no_enforcement_final_closeout_is_visible_only
  scheduler_guardrail_terminal_no_enforcement_final_closeout_blocks_live_paths
  scheduler_guardrail_terminal_no_enforcement_final_closeout_links_priors_and_side_effects
)
...
echo "Hepta WorkGraph agent_jobs + task_board scheduler guardrail blocking dry-run entrypoint hardening terminal no-enforcement final closeout gate passed"
