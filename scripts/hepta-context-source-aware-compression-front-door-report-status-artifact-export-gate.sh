...
export_gate_script="hepta-context-source-aware-compression-front-door-report-status-artifact-export-gate.sh"
...
tmp_dir="$(mktemp -d -t hepta-context-source-aware-status-artifact-out.XXXXXX)"
...
fail() {
  echo "hepta-context-source-aware-compression-front-door-report-status-artifact-export-gate: $*" >&2
...
for term in "${required_contract_terms[@]}"; do
  assert_file_contains \
    "$context_contracts" \
    "$term" \
    "source-aware compression persisted status artifact export contract"
done

assert_file_contains \
  "$front_door_gate" \
  "$export_gate_script" \
  "source-aware compression status artifact export front-door wiring"

assert_file_contains \
  "$debug_gate" \
  "$export_gate_script" \
  "source-aware compression status artifact export debug gate"

assert_file_contains \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export" \
  "source-aware compression status artifact export preflight stage"

assert_line_before \
  "$preflight_script" \
  "source-aware compression front-door report status artifact consumer" \
  "source-aware compression front-door report persisted status artifact export" \
  "source-aware compression status artifact export preflight stage order"
...
if ! env "$artifact_out_env=$artifact_out" bash "$artifact_gate" >"$gate_output" 2>&1; then
  fail "status artifact gate failed with persisted artifact output enabled"
fi
...
echo "Hepta context source-aware compression persisted status artifact export gate passed"
