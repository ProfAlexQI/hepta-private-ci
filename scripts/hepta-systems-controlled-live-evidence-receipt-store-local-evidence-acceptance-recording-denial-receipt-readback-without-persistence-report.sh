...
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_readback_without_persistence.rs"
...
fail() {
  printf 'hepta-systems-controlled-live-evidence-receipt-store-local-evidence-acceptance-recording-denial-receipt-readback-without-persistence-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable local evidence acceptance recording boundary report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing local evidence acceptance recording denial receipt Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the local evidence acceptance recording denial receipt report"
fi
...
if grep -q 'controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_readback_without_persistence_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_RECORDING_BOUNDARY_JSON:-}"
source_cache_input_present=false
source_report_render_count=0
if [[ -n "$source_json" ]]; then
  [[ -f "$source_json" ]] || fail "missing cached local evidence acceptance recording boundary report: $source_json"
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-controlled-live-evidence-receipt-store-local-evidence-acceptance-recording-denial-receipt-readback-without-persistence-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_RECORDING_DENIAL_RECEIPT_READBACK_WITHOUT_PERSISTENCE_2026-07-07.md" \
  --arg collection_id "controlled-live-evidence-receipt-store-local-evidence-acceptance-recording-denial-receipts" \
  --arg collection_route "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/recording-denial-receipts" \
  --arg denial_schema_version "controlled_live_local_evidence_acceptance_source_recording_denial_receipt_v1" \
  --arg denial_reason "local_evidence_acceptance_source_recording_disabled_open_preconditions_missing" \
  '
  def hyphen_id($id):
    $id | gsub("_"; "-");
  def denial_receipt_id($id):
    "local-evidence-acceptance-recording-denial-receipt:controlled-live-evidence-receipt-store:" + $id + ":not-persisted";
  def denial_digest($id):
    "sha256:controlled-live-evidence-receipt-store-local-evidence-acceptance-recording-denial:" + $id + ":not-persisted";
  def denial_idempotency($id):
    "controlled-live-evidence-receipt-store.local-evidence-acceptance-recording-denial-receipt.idempotency." + $id;
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_without_persistence_" + .source_blocker_id),
    source_blocker_id,
    ...
    source_acceptance_source_record_id:.acceptance_source_record_id,
    source_acceptance_source_record_idempotency_key:.acceptance_source_record_idempotency_key,
    ...
    operator_status,
    observed_state:"local_evidence_acceptance_recording_denial_receipt_projected_without_persistence",
    previous_state,
    current_state,
    state_delta,
    owner,
    ...
    source_acceptance_source_record_id_attached:true,
    source_denial_receipt_id_attached:.denial_receipt_projected,
    recording_denial_reason_projected:true,
    recording_precondition_missing,
    acceptance_source_recording_disabled:(.acceptance_source_recording_allowed == false),
  ...
  ($entries | map(select(.source_acceptance_source_record_id_attached == true)) | length) as $source_acceptance_source_record_id_attached_count |
  ($entries | map(select(.source_denial_receipt_id_attached == true)) | length) as $source_denial_receipt_id_attached_count |
  ...
  ($entries | map(select(.acceptance_source_recording_disabled == true)) | length) as $acceptance_source_recording_disabled_count |
  ...
  ($src.local_evidence_acceptance_recording_boundary_readback_ready == true
    and $src.boundary_entry_count == 7
    and $src.boundary_projected_count == 7
    and $src.boundary_ready_count == 7
    and $src.source_acceptance_source_attached_count == 7
    and $src.record_schema_projected_count == 7
    ...
    and ($entries | all(.observed_state == "local_evidence_acceptance_recording_denial_receipt_projected_without_persistence"
  ...
  {
    runtime:"hepta",
    surface:"controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_readback_without_persistence",
    status:(if $ready then "ready_blocked" else "blocked" end),
    gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_readback_without_persistence_gate",
    schema_version:"controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_readback_without_persistence_v1",
    plugin_id:"hepta-system@hepta-local",
    source_recording_boundary_ready:$src.local_evidence_acceptance_recording_boundary_readback_ready,
    ...
    source_acceptance_source_attached_count:$src.source_acceptance_source_attached_count,
    source_record_schema_projected_count:$src.record_schema_projected_count,
    ...
    source_acceptance_source_record_id_attached_count:$source_acceptance_source_record_id_attached_count,
    source_denial_receipt_id_attached_count:$source_denial_receipt_id_attached_count,
    ...
    acceptance_source_recording_disabled_count:$acceptance_source_recording_disabled_count,
    ...
    local_evidence_acceptance_recording_denial_receipt_readback_ready:$ready,
    acceptance_source_recording_allowed:false,
    acceptance_source_recorded:false,
    acceptance_source_persisted:false,
    denial_receipt_persistence_allowed:false,
    ...
    next_actions:[
      ...
      "keep_local_evidence_acceptance_recording_denial_receipts_projected_without_persistence_until_local_store_is_open"
    ],
    recommended_next_gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_retention_replay_readback_without_persistence",
    local_gate:$gate,
