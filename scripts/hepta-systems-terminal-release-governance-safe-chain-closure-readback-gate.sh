...
REPORT="$ROOT/scripts/hepta-systems-terminal-release-governance-safe-chain-closure-readback-report.sh"
CLOSURE_GATE="$ROOT/scripts/hepta-systems-terminal-release-governance-safe-chain-closure-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_TERMINAL_RELEASE_GOVERNANCE_SAFE_CHAIN_CLOSURE_READBACK_2026-06-21.md"

fail() {
  printf 'hepta-systems-terminal-release-governance-safe-chain-closure-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable terminal release governance safe chain closure readback report: $REPORT"
[[ -x "$CLOSURE_GATE" ]] || fail "missing executable terminal release governance safe chain closure gate: $CLOSURE_GATE"
[[ -f "$DOC" ]] || fail "missing terminal release governance safe chain closure readback architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the terminal release governance safe chain closure readback report"
fi

grep -q 'Terminal Release Governance Safe Chain Closure Readback' "$DOC" \
  || fail "architecture note must document Terminal Release Governance Safe Chain Closure Readback"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "terminal_release_governance_safe_chain_closure_readback"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .readback_mode == "static_terminal_release_governance_safe_chain_closure_snapshot_only"
  and .source_terminal_release_governance_safe_chain_closure_surface == "terminal_release_governance_safe_chain_closure"
  and .source_terminal_release_governance_safe_chain_closure_report_reexecuted == false
  and .source_terminal_release_governance_safe_chain_closure_ready == true
  and .source_terminal_release_governance_safe_chain_closure_blocked == true
  and .terminal_release_governance_safe_chain_closure_readback_ready == true
  and .terminal_release_governance_safe_chain_closure_readback_blocked == true
  ...
  and .next_migration_step == "derive_terminal_release_governance_safe_chain_closure_final_index_without_operator_gate_invocation"
...
printf 'hepta-systems-terminal-release-governance-safe-chain-closure-readback-gate: PASS: terminal release governance safe chain closure readback is static and blocked without operator/release/live invocation\n'
