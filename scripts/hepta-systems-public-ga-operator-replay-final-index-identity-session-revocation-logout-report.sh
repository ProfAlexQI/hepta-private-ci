...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-replay-denial-final-index-report.sh"
REVOCATION_GATE="$ROOT/scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-denial-gate.sh"
REVOCATION_DOC="$ROOT/docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_DENIAL_GATE.md"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session replay denial final index report: $SOURCE_REPORT" >&2
  exit 1
}
[[ -f "$REVOCATION_GATE" ]] || {
  echo "missing operator identity/session revocation logout denial gate: $REVOCATION_GATE" >&2
  exit 1
}
[[ -f "$REVOCATION_DOC" ]] || {
  echo "missing operator identity/session revocation logout denial doc: $REVOCATION_DOC" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session revocation logout report" >&2
  exit 1
fi
...
jq -e '
  .surface == "public_ga_operator_identity_session_replay_denial_final_index"
...
revocation_static_mention_count="$(
  grep -Ec 'revocation|logout|reinstatement|session lifecycle|operator_identity|operator_session|session_' "$REVOCATION_GATE" || true
)"

jq -n \
  --argjson source "$source_json" \
  --argjson revocation_static_mention_count "$revocation_static_mention_count" \
  '{
    runtime: "hepta",
    surface: "public_ga_operator_identity_session_revocation_logout_attachment",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_identity_session_replay_denial_final_index_surface: $source.surface,
    source_public_ga_operator_identity_session_replay_denial_final_index_ready: $source.public_ga_operator_identity_session_replay_denial_final_index_ready,
    source_public_ga_operator_identity_session_replay_denial_final_index_blocked: $source.public_ga_operator_identity_session_replay_denial_final_index_blocked,
    ...
    public_ga_operator_identity_session_replay_denial_final_index_attached: true,
    public_ga_operator_identity_session_revocation_logout_attachment_ready: true,
    public_ga_operator_identity_session_revocation_logout_attachment_blocked: true,
    operator_identity_session_revocation_logout_denial_gate_present: true,
    operator_identity_session_revocation_logout_denial_doc_present: true,
    operator_identity_session_revocation_logout_static_mention_count: $revocation_static_mention_count,
    ...
    long_soak_required_by_source_revocation_gate: true,
    ...
    next_migration_step: "derive_public_ga_operator_identity_session_revocation_logout_readback_without_revocation_or_logout",
    local_gate: "scripts/hepta-systems-public-ga-operator-replay-final-index-identity-session-revocation-logout-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_REPLAY_FINAL_INDEX_IDENTITY_SESSION_REVOCATION_LOGOUT_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_replay_denial_final_index_report: "scripts/hepta-systems-public-ga-operator-identity-session-replay-denial-final-index-report.sh",
      operator_identity_session_revocation_logout_denial_gate: "scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-denial-gate.sh",
      operator_identity_session_revocation_logout_denial_doc: "docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_DENIAL_GATE.md"
    },
