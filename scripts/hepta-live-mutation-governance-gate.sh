...
case "$SOURCE_MODE" in
  ...
  offline_fixture)
    ...
    PROVIDER_PLAN_JSON="$(jq -c '.provider_channel_dry_run_plan' <<<"$HEPTA_ROUTE_PARITY_NATIVE_REPORTS_JSON")"
    RUNTIME_JSON="$(jq -c '.runtime_session_dry_run_inventory' <<<"$HEPTA_ROUTE_PARITY_NATIVE_REPORTS_JSON")"
    LOCAL_JSON="$(jq -c '.local_tooling_content_inventory' <<<"$HEPTA_ROUTE_PARITY_NATIVE_REPORTS_JSON")"
