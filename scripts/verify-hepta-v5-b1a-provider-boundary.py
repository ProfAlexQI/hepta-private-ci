...
EXPECTED_PARENT = "ad7845a8d67390299f86e931bab11d8b0ec13115"
...
FORMAT_VERIFIER = Path("scripts/verify-hepta-v5-b1a-format.py")
VERIFIER = Path("scripts/verify-hepta-v5-b1a-provider-boundary.py")
...
def main() -> int:
    ...
    rust_files = [LIB, SOURCE_MAIN, *SOURCE_PARTS, TEST_MAIN, *TEST_PARTS]
    paths = [
        *rust_files,
        STATUS,
        INVENTORY,
    ...
    if args.require_direct_parent:
        require(
            ...
            "source head is not a direct child of current B0",
        )
        branch = (
        ...
        expected_diff = {str(path) for path in [WORKFLOW, *paths]}
        actual_diff = set(
            filter(None, git("diff", "--name-only", EXPECTED_PARENT, "HEAD").splitlines())
        )
        require(actual_diff == expected_diff, "exact B1a changed-file scope drift")
    ...
    for path in SOURCE_PARTS:
        require(
            f'include!("checked_provider_operation_parts/{path.name}");' in main_source,
            f"missing include {path.name}",
        )
    for path in TEST_PARTS:
        require(
            f'include!("provider_verified_use_boundary_parts/{path.name}");' in test_main,
            f"missing test include {path.name}",
        )

    require("mod provider_effect;" in lib_source, "raw provider module is not private")
    require(
        "pub use checked_provider_operation::ProviderOperationCoordinator;" in lib_source,
        "checked provider boundary is not root-exported",
    )
    require(
        "pub use provider_effect::ProviderEffectCoordinator;" not in lib_source,
        "raw provider coordinator is publicly root-exported",
    )
    require(
        "pub(crate) use provider_effect::ProviderEffectCoordinator;" in lib_source,
        "crate-private raw provider coordinator import is missing",
    )
    ...
    for needle in [
        "pub fn with_existing_provider(",
        "pub fn provider(&self)",
        "pub fn into_parts(",
    ]:
        require(needle not in source, f"public raw-provider escape exposed: {needle}")
    require(
        source.count("ProviderEffectCoordinator::new(") == 1,
        "raw coordinator construction escaped the checked constructor",
    )
    require(
    ...
    expected_files = [str(path) for path in rust_files]
    status = load_json(STATUS)
    require(status["stackParent"]["commit"] == EXPECTED_PARENT, "status parent commit drift")
    require(
        status["stackParent"]["branch"]
        == "codex/hepta-architecture-v5-b0-exact-restack-20260831",
    ...
    require(
        status["source"]["rawProviderCoordinatorRootExported"] is False,
        "status permits raw coordinator export",
    )
    require(
        status["source"]["rawProviderEscapeApiPublic"] is False,
        "status permits raw provider escape API",
    )
    ...
    require(
        inventory["runtimeRegistered"] is False and inventory["qualified"] is False,
        "runtime claim crossed",
    )
    require(all(value is False for value in inventory["authority"].values()), "positive authority in inventory")
    require(
        set(inventory["allowedRustReferences"]) == set(expected_files),
        "Rust reference inventory drift",
    )
    require(
        inventory["rawProviderCoordinator"]["rootExported"] is False,
        "inventory permits raw coordinator export",
    )
    require(
        inventory["checkedBoundary"]["publicRawProviderEscape"] is False,
        "inventory permits checked-boundary escape",
    )

    for needle in [
        "current broad-capability verification",
        "raw provider coordinator is not root-exported",
        "no public raw-provider accessor, restoration constructor or extraction method",
    ]:
        require(needle in contract, f"contract missing no-bypass clause: {needle}")
    require(EXPECTED_BRANCH in workflow, "workflow branch drift")
    require(str(LIB) in workflow, "workflow does not trigger on root export changes")
    require(
