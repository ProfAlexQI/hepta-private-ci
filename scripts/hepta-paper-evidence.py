...
def git_text(*args: str) -> str:
    proc = subprocess.run(["git", "-C", str(ROOT), *args], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if proc.returncode: die("git " + " ".join(args) + ": " + proc.stderr.strip())
    return proc.stdout.strip()

def git_bytes(*args: str) -> bytes:
    proc = subprocess.run(["git", "-C", str(ROOT), *args], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if proc.returncode: die("git " + " ".join(args) + ": " + proc.stderr.decode(errors="replace").strip())
    return proc.stdout
...
def sentence_rows(text: str) -> list[dict[str, Any]]:
    ...
    return [
        {"sentenceIndex": i, "sha256": hashlib.sha256(s.encode("utf-8")).hexdigest(), "bytes": len(s.encode("utf-8"))}
        for i, s in enumerate(sentences, 1)
    ]

def verify_source(commit: str, row: dict[str, Any]) -> None:
    required = ["sourceId","path","kind","gitBlobSha","sha256","bytes"]
    need(all(k in row for k in required), row.get("sourceId", "source") + " required fields")
    spec = f"{commit}:{row['path']}"
    need(git_text("rev-parse", spec) == row["gitBlobSha"], row["sourceId"] + " Git blob")
    ...
    if row["kind"] == "official_publisher_abstract_snapshot":
        ...
        except UnicodeDecodeError as exc: die(row["sourceId"] + " UTF-8: " + str(exc))
        need(canonicalize(text).encode("utf-8") == raw, row["sourceId"] + " non-canonical snapshot")
        need(row.get("sentences") == sentence_rows(text), row["sourceId"] + " sentence digest replay")
    else:
        need("sentences" not in row, row["sourceId"] + " binary sentences")

def verify() -> int:
    binding = load_path(BINDING_PATH)
    need(binding.get("schema") == "hepta.paper-evidence-binding.v1" and binding.get("schemaVersion") == 1, "binding schema")
    ...
    need(git_text("rev-parse", commit + "^{tree}") == binding["evidenceTree"], "evidence tree")
    ...
    need(git_text("rev-parse", manifest_spec) == binding["manifestGitBlobSha"], "manifest Git blob")
    manifest = load_bytes(git_bytes("show", manifest_spec), "evidence manifest")
    need(manifest.get("schema") == "hepta.paper-source-lock-manifest.v2", "manifest schema")
    need(manifest.get("parentCommit") == binding["evidenceParentCommit"], "manifest parent")
    ...
    need(isinstance(sources, list) and len(sources) == binding["sourceArtifactCount"], "source count")
    need(isinstance(claims, list) and len(claims) == binding["semanticClaimCount"], "claim count")
    ...
    for row in sources: verify_source(commit, row)
    semantic = load_path(ROOT / binding["semanticRegistry"])
    papers = semantic.get("papers")
    need(isinstance(papers, list) and len(papers) == binding["semanticPaperCount"], "semantic paper count")
    ...
    abstract_rows = {k:v for k,v in source_map.items() if v["kind"] == "official_publisher_abstract_snapshot"}
    for row in claims:
        need(row["paperId"] in abstract_rows, "claim source missing " + row["paperId"])
        sentence_ids = {x["sentenceIndex"] for x in abstract_rows[row["paperId"]]["sentences"]}
        need(row["sentenceIndex"] in sentence_ids, "claim sentence missing " + row["claim"])
    policy = binding["verificationPolicy"]
    need(policy["changingRegistryAndVerifierConstantsTogetherMaySatisfyExternalEvidence"] is False, "self-certification policy")
    need(policy["missingEvidenceObjectMayAdvanceClaim"] is False, "missing evidence policy")
    print(json.dumps({
        "status":"PASS_HEPTA_PAPER_EVIDENCE_REPLAY",
        "evidenceCommit":commit,"evidenceTree":binding["evidenceTree"],
        "sources":len(sources),"semanticPapers":len(papers),"semanticClaims":len(claims),
...
def self_test() -> int:
    ...
    need(rows[0]["sha256"] == hashlib.sha256(b"One sentence.").hexdigest(), "digest fixture")
    try:
    ...
    need(hashlib.sha256(b"changed source").hexdigest() != good, "hostile source substitution fixture")
    print(json.dumps({"status":"PASS_HEPTA_PAPER_EVIDENCE_SELF_TEST","cases":["canonicalization","sentence_digest","duplicate_key","source_substitution"],"authorityGranted":False}, sort_keys=True))
    return 0
