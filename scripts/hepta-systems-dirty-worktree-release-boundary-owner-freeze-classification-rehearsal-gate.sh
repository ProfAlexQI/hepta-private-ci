...
REPORT="$ROOT/scripts/hepta-systems-dirty-worktree-release-boundary-owner-freeze-classification-rehearsal-report.sh"
...
fail() {
  printf 'hepta-systems-dirty-worktree-release-boundary-owner-freeze-classification-rehearsal-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable owner/freeze/classification rehearsal report: $REPORT"
...
if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the owner/freeze/classification rehearsal report"
fi

grep -q 'Owner Freeze Classification Rehearsal' "$DOC" \
  || fail "architecture note must document Owner Freeze Classification Rehearsal"
grep -q 'visible-only owner, freeze, and classification read model' "$DOC" \
  || fail "architecture note must document the visible-only read model"
grep -q 'no owner assignment persistence, freeze application, classification persistence, test probe execution, git mutation, cleanup, delete, evidence recording, approval acceptance, decision recording, package, release, Public GA, canary activation, live activation, or live execution' "$DOC" \
  || fail "architecture note must document the closed boundary"
grep -q 'temporal_lite_lease_idempotency_index_feature_gated_readback' "$DOC" \
  || fail "architecture note must document the Temporal-lite lease/idempotency next gate"
...
"$REPORT" | jq -e '
  ...
  and (.next_actions | index("temporal_lite_lease_idempotency_index_feature_gated_readback")) != null
  and .recommended_next_gate == "temporal_lite_lease_idempotency_index_feature_gated_readback"
