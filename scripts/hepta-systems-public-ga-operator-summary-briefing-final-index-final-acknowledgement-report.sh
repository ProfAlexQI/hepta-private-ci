...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-operator-facing-summary-briefing-final-index-report.sh"
FINAL_ACK_GATE="$ROOT/scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-final-operator-acknowledgement-non-acceptance-denial-gate.sh"
FINAL_ACK_DOC="$ROOT/docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_REPLAY_REINSTATEMENT_FINAL_OPERATOR_ACKNOWLEDGEMENT_NON_ACCEPTANCE_DENIAL_GATE.md"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session summary briefing final index report: $SOURCE_REPORT" >&2
  exit 1
}
[[ -f "$FINAL_ACK_GATE" ]] || {
  echo "missing operator identity/session final acknowledgement non-acceptance gate: $FINAL_ACK_GATE" >&2
  exit 1
}
[[ -f "$FINAL_ACK_DOC" ]] || {
  echo "missing operator identity/session final acknowledgement non-acceptance doc: $FINAL_ACK_DOC" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session final acknowledgement report" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    runtime: "hepta",
    surface: "public_ga_operator_identity_session_final_acknowledgement_non_acceptance_attachment",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_surface: $source.surface,
    source_public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_ready: $source.public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_ready,
    source_public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_blocked: $source.public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_blocked,
    public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_attached: true,
    public_ga_operator_identity_session_final_acknowledgement_non_acceptance_attachment_ready: true,
    public_ga_operator_identity_session_final_acknowledgement_non_acceptance_attachment_blocked: true,
    ...
    operator_identity_session_final_acknowledgement_non_acceptance_gate_present: true,
    operator_identity_session_final_acknowledgement_non_acceptance_doc_present: true,
    operator_identity_session_final_acknowledgement_static_mention_count: $final_acknowledgement_static_mention_count,
    ...
    long_soak_required_by_source_final_acknowledgement_gate: true,
    ...
    attachment_blocker_count: 44,
    manual_operator_live_cutover_approval_required: true,
    public_ga_claim_allowed: false,
    public_ga_claimed: false,
    public_release_published: false,
    rollback_execution_allowed: false,
    next_migration_step: "derive_public_ga_operator_identity_session_final_acknowledgement_readback_without_summary",
    local_gate: "scripts/hepta-systems-public-ga-operator-summary-briefing-final-index-final-acknowledgement-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_SUMMARY_BRIEFING_FINAL_INDEX_FINAL_ACKNOWLEDGEMENT_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_report: "scripts/hepta-systems-public-ga-operator-identity-session-operator-facing-summary-briefing-final-index-report.sh",
      operator_identity_session_final_acknowledgement_non_acceptance_gate: "scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-final-operator-acknowledgement-non-acceptance-denial-gate.sh",
      operator_identity_session_final_acknowledgement_non_acceptance_doc: "docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_REPLAY_REINSTATEMENT_FINAL_OPERATOR_ACKNOWLEDGEMENT_NON_ACCEPTANCE_DENIAL_GATE.md"
    },
