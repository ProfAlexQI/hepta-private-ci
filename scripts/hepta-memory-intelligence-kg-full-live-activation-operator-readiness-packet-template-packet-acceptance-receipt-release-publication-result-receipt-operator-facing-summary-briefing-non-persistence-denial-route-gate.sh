...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator readiness packet template packet-acceptance receipt release publication result receipt operator-facing summary/briefing route source text: $label" >&2
    exit 1
  fi
}
...
OPERATOR_SUMMARY_BRIEFING_JSON="$(
  capture_json_report \
    "hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-operator-facing-summary-briefing-non-persistence-denial-gate" \
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes packet acceptance receipt release publication result receipt operator-facing summary/briefing route"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt operator-facing summary/briefing endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt operator-facing summary/briefing endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt operator-facing summary/briefing source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt operator-facing summary/briefing report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "packet acceptance receipt release publication result receipt operator-facing summary/briefing route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "packet acceptance receipt release publication result receipt operator-facing summary/briefing surface count emitted"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"release_publication_result_receipt_operator_summary_recorded_count": 0' \
  "packet acceptance receipt release publication result receipt operator summary recording denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"release_publication_result_receipt_operator_briefing_recorded_count": 0' \
  "packet acceptance receipt release publication result receipt operator briefing recording denied"

TEST_LOG="$(mktemp /tmp/hepta-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-operator-summary-briefing-route-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_operator_facing_summary_briefing_endpoint_blocks_delivery \
  -- --nocapture >"$TEST_LOG"

LIVE_ROUTE_JSON='{}'
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-operator-facing-summary-briefing-non-persistence-denial"
  )"
  jq -e '
    .status == "ready"
    and .route_count == 136
    ...
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_operator_facing_summary_briefing_non_persistence_denial_route_enabled == true
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_operator_facing_summary_briefing_non_persistence_denial_ready == true
    and .release_publication_result_receipt_operator_facing_summary_briefing_surface_count == 18
    and .release_publication_result_receipt_operator_summary_recorded_count == 0
    and .release_publication_result_receipt_operator_summary_persisted_count == 0
...
jq -n \
  ...
  --arg gate "hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_operator_facing_summary_briefing_non_persistence_denial_route_gate" \
  --arg endpoint "/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-operator-facing-summary-briefing-non-persistence-denial" \
  --arg source_command "/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-operator-facing-summary-briefing-non-persistence-denial --json" \
  ...
  '{
    ...
    activation_mode:"full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_operator_facing_summary_briefing_non_persistence_denial_native_route_status",
    side_effect_free:true,
    native_route:true,
    route_enabled:true,
    source_operator_summary_briefing_gate_ready:true,
    ...
    source_packet_acceptance_receipt_release_publication_result_receipt_export_query_observability_ready:true,
    release_publication_result_receipt_operator_facing_summary_briefing_surface_count:18,
    release_publication_result_receipt_operator_summary_recorded_count:0,
    release_publication_result_receipt_operator_briefing_recorded_count:0,
    ...
    terminal_required_marker_count:276,
    terminal_present_required_marker_count:276,
...
echo "Hepta memory/intelligence/KG full live activation operator readiness packet template packet acceptance receipt release/publication result receipt operator-facing summary/briefing non-persistence denial route gate passed"
