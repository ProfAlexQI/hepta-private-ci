...
precheck_gate_script="hepta-context-source-aware-compression-front-door-report-status-artifact-export-precheck-gate.sh"
artifact_out_env="HEPTA_CONTEXT_SOURCE_AWARE_COMPRESSION_STATUS_ARTIFACT_OUT"
tmp_dir="$(mktemp -d -t hepta-context-source-aware-status-artifact-precheck.XXXXXX)"
...
fail() {
  echo "hepta-context-source-aware-compression-front-door-report-status-artifact-export-precheck-gate: $*" >&2
  exit 1
}
...
run_precheck_failure() {
  ...
  if grep -F "front-door:" "$output_file" >/dev/null; then
    echo "$label output:" >&2
    cat "$output_file" >&2
    fail "$label must fail before running the front-door report"
  fi

  if grep -F "source-aware-contracts=" "$output_file" >/dev/null; then
    echo "$label output:" >&2
    cat "$output_file" >&2
    fail "$label must fail before emitting machine-readable source-aware-contracts status"
  fi

  if grep -F "source-aware-contracts.runtime-activation=enabled" "$output_file" >/dev/null; then
    fail "$label must not enable runtime activation"
  fi
}
...
for term in "${required_contract_terms[@]}"; do
  assert_file_contains \
    "$context_contracts" \
    "$term" \
    "source-aware compression persisted status artifact export precheck contract"
done

assert_file_contains \
  "$front_door_gate" \
  "$precheck_gate_script" \
  "source-aware compression status artifact export precheck front-door wiring"

assert_file_contains \
  "$debug_gate" \
  "$precheck_gate_script" \
  "source-aware compression status artifact export precheck debug gate"

assert_file_contains \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export precheck" \
  "source-aware compression status artifact export precheck preflight stage"

assert_line_before \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export negative matrix" \
  "source-aware compression front-door report persisted status artifact export precheck" \
  "source-aware compression status artifact export precheck preflight stage order"
...
assert_line_before \
  ...
  "source-aware compression status artifact export precheck happens before front-door report"
...
if [ -e "$missing_parent_path" ]; then
  fail "missing parent directory precheck must not create an artifact: $missing_parent_path"
fi

echo "Hepta context source-aware compression persisted status artifact export precheck gate passed"
