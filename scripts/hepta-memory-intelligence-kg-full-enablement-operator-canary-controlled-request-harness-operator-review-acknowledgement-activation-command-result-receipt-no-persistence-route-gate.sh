...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator canary activation command result receipt no-persistence route source text: $label" >&2
    exit 1
  fi
}

ACTIVATION_COMMAND_ROUTE_JSON="$(
...
jq -e '
  ...
  and .current_live_enabled_lane_count == 17
  and .enablement_lane_count == 20
  and .ready_enablement_lane_count == 20
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$ACTIVATION_COMMAND_ROUTE_JSON"
...
jq -e '
  ...
  and .source_activation_command_performed_count == 0
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command result receipt no-persistence endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command result receipt no-persistence endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command result receipt no-persistence source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command result receipt no-persistence report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "activation command result receipt no-persistence route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"activation_command_result_receipt_recorded": false' \
  "activation command result receipt recording denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"activation_command_result_receipt_persisted": false' \
  "activation command result receipt persistence denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"activation_command_completion_ack_recorded": false' \
  "activation command completion acknowledgement recording denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"activation_from_receipt_allowed": false' \
  "activation from result receipt denied"

TEST_LOG="$(mktemp /tmp/hepta-operator-canary-activation-command-result-receipt-no-persistence-route-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_enablement_operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_result_receipt_no_persistence_endpoint_blocks_receipts \
  -- --nocapture >"$TEST_LOG"

if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-enablement-operator-canary-controlled-request-harness-operator-review-acknowledgement-activation-command-result-receipt-no-persistence"
  )"
  jq -e '
    .status == "ready"
    and .route_count == 122
    ...
    and .secret_file_read_count == 0
    and .channel_send_performed_count == 0
    and .current_live_enabled_lane_count == 18
    and .enablement_lane_count == 21
    and .ready_enablement_lane_count == 21
    and (.side_effects | to_entries | all(.value == false))
...
report="$(
  jq -n \
    ...
    --arg gate "hepta_memory_intelligence_kg_full_enablement_operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_result_receipt_no_persistence_route_gate" \
    --arg endpoint "/api/hepta-memory-intelligence-kg-full-enablement-operator-canary-controlled-request-harness-operator-review-acknowledgement-activation-command-result-receipt-no-persistence" \
    --arg source_command "/hepta-memory-intelligence-kg-full-enablement-operator-canary-controlled-request-harness-operator-review-acknowledgement-activation-command-result-receipt-no-persistence --json" \
    --arg native_gateway_source "$NATIVE_GATEWAY_SOURCE" \
    --arg native_gateway_sha256 "$(sha256_file "$NATIVE_GATEWAY_SOURCE")" \
    --arg test_log "$TEST_LOG" \
    --argjson activation_command_route "$ACTIVATION_COMMAND_ROUTE_JSON" \
    --argjson result_receipt "$RESULT_RECEIPT_JSON" \
    ...
    '{
      ...
      activation_mode:"operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_result_receipt_no_persistence_native_route_status",
      source_activation_command_route_ready:($activation_command_route.status == "ready"),
      source_activation_command_result_receipt_no_persistence_gate:$result_receipt.gate,
      source_activation_command_result_receipt_no_persistence_gate_ready:$result_receipt.operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_result_receipt_no_persistence_ready,
      source_activation_command_result_receipt_no_persistence_gate_status:$result_receipt.operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_result_receipt_no_persistence_status,
      ...
      source_contract:{
        ...
        test_log:$test_log,
        contract:"hepta-native-gateway-operator-canary-controlled-request-harness-operator-review-acknowledgement-activation-command-result-receipt-no-persistence-route-v1",
        source_pattern_present:true,
        compile_checked_by_tests:true,
        route_handler_present:true,
        registers_result_receipt:false,
        records_result_receipt:false,
        persists_result_receipt:false,
        accepts_result_receipt:false,
        exports_result_receipt:false,
        registers_result_receipt_query:false,
...
jq -e '
  .runtime == "hepta"
  and .status == "ready"
  and .gate == "hepta_memory_intelligence_kg_full_enablement_operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_result_receipt_no_persistence_route_gate"
  and .source_activation_command_route_ready == true
  and .source_activation_command_result_receipt_no_persistence_gate_ready == true
  and .source_activation_command_result_receipt_no_persistence_gate_status == "blocked"
...
printf '%s\n' "$report"
echo "Hepta Memory/Intelligence/KG operator canary controlled request harness operator review acknowledgement activation command result receipt no-persistence route gate passed"
