...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing memory write execution activation command result receipt terminal operator decision public-claim boundary source text: $label" >&2
    exit 1
  fi
}
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes memory terminal operator decision public-claim boundary"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory terminal operator decision public-claim boundary endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" "$ENDPOINT" \
  "memory terminal operator decision public-claim boundary endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" "$SOURCE_COMMAND" \
  "memory terminal operator decision public-claim boundary source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory terminal operator decision public-claim boundary report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory terminal operator decision public-claim boundary ready field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"denied_by_activation_command_result_receipt_terminal_operator_decision_public_claim_count\"" \
  "memory terminal operator decision public-claim denied count"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  ...
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_write_execution_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_boundary_ready == true
    and .activation_command_result_receipt_terminal_operator_decision_public_claim_mode == "memory_write_execution_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial"
    and .source_activation_command_result_receipt_final_operator_acknowledgement_boundary_ready == true
    and .source_activation_command_result_receipt_final_operator_acknowledgement_ready == true
    and .source_activation_command_result_receipt_final_operator_acknowledgement_report_sha256 != ""
    and .source_final_operator_acknowledgement_fixture_count == 10
    and .source_blocked_final_operator_acknowledgement_fixture_count == 10
    and .source_accepted_final_operator_acknowledgement_fixture_count == 0
    and .source_final_operator_acknowledgement_performed_count == 0
    and .source_final_operator_acknowledgement_denial_count == 37
    ...
    and .activation_command_result_receipt_public_claim_requested == false
    and .activation_command_result_receipt_public_claim_accepted == false
    ...
    and (.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures | all((.terminal_operator_decision_status | startswith("blocked")) and .terminal_decision_recorded == false and .terminal_decision_persisted == false and .terminal_decision_materialized == false and .terminal_decision_filesystem_written == false and .terminal_decision_delivered == false and .terminal_decision_accepted == false and .terminal_decision_final_state_promoted == false and .public_claim_promoted == false and .public_release_published == false and .public_ga_claimed == false and .telegram_send_performed == false and .receipt_recorded == false and .receipt_persisted == false and .receipt_accepted == false and .activation_allowed == false and .live_mutation_execution_performed == false and .memory_store_write_performed == false and .memory_store_mutated == false and .rollback_executed == false and .terminal_operator_decision_noop_confirmed == true))
    ...
    and ([.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures[] | select(.public_claim_requested == true and .public_claim_promotion_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures[] | select(.public_ga_claim_requested == true and .public_release_publish_requested == true and .release_artifact_write_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures[] | select(.activation_from_terminal_decision_requested == true and .memory_write_terminal_decision_requested == true and .provider_prompt_terminal_decision_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures[] | select(.external_send_decision_requested == true and .release_artifact_decision_requested == true and .install_decision_requested == true and .active_binary_decision_requested == true)] | length) == 1
    and .denied_by_activation_command_result_receipt_terminal_operator_decision_public_claim_count == 55
    and (.denied_by_activation_command_result_receipt_terminal_operator_decision_public_claim | length) == 55
    and .terminal_operator_decision_acceptance_forbidden == true
    and .public_claim_promotion_forbidden == true
    and .public_release_publication_forbidden == true
    and .release_artifact_publication_forbidden == true
    and .allowed_next_actions[0].action == "run_memory_write_execution_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_boundary_require_live_gate"
    and .allowed_next_actions[1].action == "prepare_memory_write_execution_activation_command_result_receipt_release_artifact_publication_denial_boundary"
...
jq -n \
  ...
  '{
    ...
    runtime: $runtime,
    status: "ready",
    gate: "hepta_memory_live_mutation_operator_write_execution_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_boundary_route_gate",
    endpoint: $endpoint,
