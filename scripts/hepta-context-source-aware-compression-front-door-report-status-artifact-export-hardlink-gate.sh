...
hardlink_gate_script="hepta-context-source-aware-compression-front-door-report-status-artifact-export-hardlink-gate.sh"
...
tmp_dir="$(mktemp -d -t hepta-context-source-aware-status-artifact-hardlink.XXXXXX)"
artifact_out="$tmp_dir/source-aware-contracts.status"
victim_file="$tmp_dir/hardlink-victim.txt"
expected_victim="$tmp_dir/expected-hardlink-victim.txt"
...
fail() {
  echo "hepta-context-source-aware-compression-front-door-report-status-artifact-export-hardlink-gate: $*" >&2
  if [ -e "$artifact_out" ]; then
    echo "persisted source-aware status artifact path:" >&2
    ls -li "$artifact_out" >&2 || true
    if [ -f "$artifact_out" ]; then
      cat "$artifact_out" >&2
    fi
  fi
  if [ -f "$victim_file" ]; then
    echo "hardlink victim file:" >&2
    ls -li "$victim_file" >&2 || true
    cat "$victim_file" >&2
  fi
...
assert_persisted_artifact_contract() {
  ...
  if grep -F "hardlink-victim-should-not-change" "$artifact_out" >/dev/null; then
    fail "persisted status artifact must replace hardlinked stale caller-provided content"
  fi
...
for term in "${required_contract_terms[@]}"; do
  assert_file_contains \
    "$context_contracts" \
    "$term" \
    "source-aware compression persisted status artifact export hardlink replacement contract"
done

assert_file_contains \
  "$front_door_gate" \
  "$hardlink_gate_script" \
  "source-aware compression status artifact export hardlink front-door wiring"

assert_file_contains \
  "$debug_gate" \
  "$hardlink_gate_script" \
  "source-aware compression status artifact export hardlink debug gate"

assert_file_contains \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export hardlink replacement" \
  "source-aware compression status artifact export hardlink preflight stage"

assert_line_before \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export symlink replacement" \
  "source-aware compression front-door report persisted status artifact export hardlink replacement" \
  "source-aware compression status artifact export hardlink preflight stage order"
...
{
  echo "front-door: stale-source-aware-status hardlink victim diagnostic"
  echo "source-aware-contracts.runtime-activation=enabled"
  echo "hardlink-victim-should-not-change"
} >"$victim_file"
cp "$victim_file" "$expected_victim"

if ! ln "$victim_file" "$artifact_out"; then
  fail "failed to create hardlink output path"
fi

if ! env "$artifact_out_env=$artifact_out" bash "$artifact_gate" >"$gate_output" 2>&1; then
  fail "status artifact gate failed with hardlink output path"
fi

assert_persisted_artifact_contract

if ! cmp -s "$victim_file" "$expected_victim"; then
  fail "status artifact export must not mutate the other hardlink target"
fi

echo "Hepta context source-aware compression persisted status artifact export hardlink gate passed"
