...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-packet-non-send-readback-final-index-report.sh"
STATIC_READBACK_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-packet-non-send-static-readback-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_PACKET_NON_SEND_READBACK_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-packet-non-send-readback-final-index-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator packet non-send readback final index report: $REPORT"
[[ -x "$STATIC_READBACK_GATE" ]] || fail "missing executable Public GA operator packet non-send static readback gate: $STATIC_READBACK_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator packet non-send readback final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator packet non-send readback final index report"
fi

grep -q 'Public GA Operator Packet Non-Send Readback Final Index' "$DOC" \
  || fail "architecture note must document Public GA Operator Packet Non-Send Readback Final Index"
grep -q 'ready-but-blocked' "$DOC" \
  || fail "architecture note must document ready-but-blocked status"
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that final index does not invoke the Public GA operator packet"
grep -q 'canonical terminal closure backfeed' "$DOC" \
  || fail "architecture note must document canonical terminal closure backfeed"

"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_packet_non_send_readback_final_index"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_packet_non_send_static_readback_surface == "public_ga_operator_packet_non_send_static_readback"
  and .source_public_ga_operator_packet_non_send_static_readback_ready == true
  and .source_public_ga_operator_packet_non_send_static_readback_blocked == true
  ...
  and .public_ga_operator_packet_non_send_static_readback_attached == true
  ...
  and .next_migration_step == "attach_public_ga_operator_packet_non_send_readback_final_index_to_operator_approval_non_acceptance_without_packet_send"
  and .side_effect_free == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null

"$STATIC_READBACK_GATE" >/dev/null

printf 'hepta-systems-public-ga-operator-packet-non-send-readback-final-index-gate: PASS: Public GA operator packet non-send readback final index is ready but blocked\n'
