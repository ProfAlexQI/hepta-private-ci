...
REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-recording-denial-persistence-denial-report.sh"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-recording-denial-persistence-denial-gate: FAIL: %s\n' "$1" >&2
  exit 1
}
...
target_json="$tmpdir/persistence-denial.json"
...
HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_RECORDING_DENIAL_RECEIPT_POSITIVE_PRECONDITIONS_JSON="$source_json" \
...
rg -q 'Controlled Live Evidence Receipt Store Local Evidence Acceptance Recording Denial Receipt Persistence Denial Readback Without Persistence' "$DOC" \
...
jq -e '
  .runtime == "hepta"
  and .surface == "controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_readback_without_persistence"
  and .status == "ready_blocked"
  and .gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_readback_without_persistence_gate"
  and .schema_version == "controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_readback_without_persistence_v1"
  ...
  and .persistence_denial_route == "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/recording-denial-receipts/persistence-denial"
  ...
  and (.entries | all(
    (.id | startswith("evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_without_persistence_"))
    ...
    and (.persistence_denial_id | startswith("local-evidence-acceptance-recording-denial-receipt-persistence-denial:"))
    and (.persistence_denial_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/recording-denial-receipts/persistence-denial/"))
    and .persistence_denial_reason == "local_evidence_acceptance_recording_denial_receipt_persistence_disabled_positive_preconditions_missing"
    and .observed_state == "local_evidence_acceptance_recording_denial_receipt_persistence_denied_without_persistence"
  ...
  and any(.entries[]; .source_blocker_id == "dirty_worktree_boundary" and .persistence_denial_route == "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/recording-denial-receipts/persistence-denial/dirty-worktree-boundary")
  and .recommended_next_gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_retention_replay_readback_without_persistence"
...
(
  cd "$ROOT/codex-rs"
  cargo test -p hepta-runtime local_evidence_acceptance_recording_denial_receipt_persistence_denial --lib
)

printf 'hepta-systems-cler-store-local-evidence-acceptance-recording-denial-persistence-denial-gate: PASS: local evidence acceptance recording denial receipt persistence denial is read back without persistence or live execution\n'
