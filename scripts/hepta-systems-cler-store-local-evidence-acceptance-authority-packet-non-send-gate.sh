...
REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-packet-non-send-report.sh"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-packet-non-send-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable local evidence acceptance authority packet non-send report: $REPORT"
...
rg -q 'Controlled Live Evidence Receipt Store Local Evidence Acceptance Authority Packet Non-Send Readback' "$DOC" \
  || fail "architecture note must document the local evidence acceptance authority packet non-send surface"
rg -q 'controlled live evidence receipt store local evidence acceptance authority packet non-send readback' "$DOC" \
  || fail "architecture note must document the plain-language local evidence acceptance authority packet non-send boundary"
rg -q 'no operator packet send, send attempt record, operator packet persistence, local evidence acceptance authority, authority decision recording, non-authority receipt persistence, local evidence acceptance, local evidence acceptance recording, evidence acceptance recording, evidence recording, receipt-store write-attempt recording, receipt store write, receipt persistence, ledger write, event-log write, SQLite write, credential read, Native POST mutation, Telegram transport mutation, gateway/auth mutation, channel send, provider call, model call, replay execution, rollback, kill-switch rehearsal execution, kill-switch mutation, package, release, Public GA promotion, or live execution' "$DOC" \
  || fail "architecture note must document the closed local non-send boundary"

tmpdir="$(mktemp -d)"
trap 'rm -rf "$tmpdir"' EXIT

source_json="$tmpdir/local-evidence-acceptance-authority-packet.json"
target_json="$tmpdir/local-evidence-acceptance-authority-packet-non-send.json"
...
HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_PACKET_JSON="$source_json" \
  "$REPORT" >"$target_json" || fail "failed to render local evidence acceptance authority packet non-send report from cached source"
...
jq -e '
  ...
  and .gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_non_send_readback_gate"
  and .schema_version == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_non_send_readback_v1"
  and .plugin_id == "hepta-system@hepta-local"
  and .source_authority_packet_ready == true
  and .source_packet_entry_count == 7
  and .source_packet_ready_count == 7
  ...
  and (.entries | all(
    (.id | startswith("evidence_receipt_store_local_evidence_acceptance_authority_packet_non_send_readback_"))
    ...
    and (.source_authority_decision_request_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/decision-request/"))
    ...
    and (.source_non_authority_receipt_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/non-authority-receipts/"))
    and (.packet_non_send_readback_id | startswith("local-evidence-acceptance-authority-packet-non-send:controlled-live-evidence-receipt-store:"))
    and (.packet_non_send_readback_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/non-send/"))
    and .packet_send_denial_reason == "operator_packet_send_disabled_local_evidence_acceptance_authority_missing_local_acceptance_disabled"
    and .observed_state == "local_evidence_acceptance_authority_packet_confirmed_unsent_without_acceptance"
  ...
  and any(.entries[]; .source_blocker_id == "operator_live_approval_missing" and .packet_non_send_readback_route == "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/non-send/operator-live-approval-missing")
  and (.next_actions | index("controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_readback_without_persistence")) != null
  and .recommended_next_gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_readback_without_persistence"
...
(
  cd "$ROOT/codex-rs"
  cargo test -p hepta-runtime local_evidence_acceptance_authority_packet_non_send --lib
)

printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-packet-non-send-gate: PASS: local evidence acceptance authority packet remains unsent without acceptance, persistence, or live execution\n'
