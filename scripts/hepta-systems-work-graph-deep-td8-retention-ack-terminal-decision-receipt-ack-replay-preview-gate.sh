...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-deep-td8-retention-ack-terminal-decision-receipt-ack-replay-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-deep-td8-retention-ack-terminal-decision-receipt-ack-replay-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and (.monotonicity_checks | all(.blocks_out_of_order_replay == true))
  ...
  and .source_probes.deep_td8_retention_ack_terminal_decision_receipt_ack_replay.rust_module_present == true
  and .source_probes.prior_deep_td8_retention_ack_terminal_decision_receipt_ack.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_retention_ack_terminal_decision_receipt_ack_replay_preview --lib

echo "Hepta WorkGraph deep td8 terminal decision receipt acknowledgement replay idempotency preview gate passed"
