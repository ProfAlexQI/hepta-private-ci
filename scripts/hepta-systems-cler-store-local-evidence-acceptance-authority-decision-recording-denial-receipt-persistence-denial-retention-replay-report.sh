...
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay.rs"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-retention-replay-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable authority decision recording denial receipt persistence denial report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing authority decision recording denial receipt persistence denial retention/replay Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the authority decision recording denial receipt persistence denial retention/replay report"
fi
...
if grep -q 'controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_readback_without_persistence_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_DENIAL_RECEIPT_PERSISTENCE_DENIAL_JSON:-}"
source_cache_input_present=false
source_report_render_count=0
if [[ -n "$source_json" ]]; then
  [[ -f "$source_json" ]] || fail "missing cached authority decision recording denial receipt persistence denial report: $source_json"
  ...
  "$SOURCE_REPORT" >"$source_json" || fail "failed to render authority decision recording denial receipt persistence denial report"
  source_report_render_count=1
fi
jq -e . "$source_json" >/dev/null || fail "authority decision recording denial receipt persistence denial report did not render valid JSON"
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-retention-replay-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_DENIAL_RECEIPT_PERSISTENCE_DENIAL_RETENTION_REPLAY_2026-07-07.md" \
  --arg collection_id "controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-retention-replay" \
  --arg collection_route "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-decision-recording-denial-receipts/persistence-denial/retention-replay" \
  '
  def hyphen_id($id):
    $id | gsub("_"; "-");
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_without_persistence_" + .source_blocker_id),
    source_blocker_id,
    ...
    retention_policy_id:("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-retention-policy:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    retention_policy_route:($collection_route + "/retention/" + hyphen_id(.source_blocker_id)),
    expiry_guard_id:("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-expiry-guard:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    replay_key:("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-replay-key:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    replay_idempotency_key:("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-replay-idempotency:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    ...
    garbage_collection_denial_id:("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-gc-denial:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    supersession_guard_id:("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-supersession-guard:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    zero_effect_digest:("sha256:local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-retention-replay-zero-effect:" + .source_blocker_id),
    retention_state:"projected_not_persisted",
    replay_state:"projected_not_written",
    observed_state:"local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_projected_without_persistence",
    ...
    source_denial_receipt_persistence_denied:.denial_receipt_persistence_denied,
  ...
  ($src.persistence_denial_readback_ready == true
    ...
    and $src.denial_receipt_persistence_denied_count == 7
  ...
  {
    runtime:"hepta",
    surface:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_readback_without_persistence",
    status:(if $ready then "ready_blocked" else "blocked" end),
    gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_readback_without_persistence_gate",
    schema_version:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_readback_without_persistence_v1",
    ...
    source_denial_receipt_persistence_denied_count:$src.denial_receipt_persistence_denied_count,
    ...
    entries:$entries,
    next_actions:[
      "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback",
      "keep_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_unpersisted"
    ],
    recommended_next_gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback",
    local_gate:$gate,
