    ...
    'def finding: (keys|sort)==["advisory_id","kind","package","version"] and (.advisory_id==null or (.advisory_id|test("^RUSTSEC-[0-9]{4}-[0-9]{4}$"))) and (.kind|IN("vulnerability","unmaintained","unsound","yanked")) and (.package|type=="string" and length>0) and (.version|type=="string" and length>0);
      ...
      and (
        (
          ...
          and $g["gate-compat-debt"].receipt == {"captured_shell_pair_count":1239,"compatibility_payload_count":548,"counts_may_only_improve":true,"declarative_pair_count":35,"entrypoint_manifest_verified":true,"legacy_workgraph_pair_count":8,"logical_compatibility_payload_count":2512,"logical_virtual_entrypoint_count":2399,"normalized_payload_bundle_verified":true,"pair_count":1282,"payload_bundle_verified":true,"physical_entrypoint_count":165,"schema":"hepta_gate_compat_debt_receipt_v3","status":"ready","unregistered_payload_count":26,"virtual_entrypoint_count":2}
          and $g["gate-compat-debt"].self_test_receipt == {schema:"hepta_gate_compat_debt_self_test_v3",status:"ready",negative_fixtures:5}
        )
        or
        (
...
hepta_release_fixture_bound_gate_receipts_json() {
  ...
  compat_self='{"negative_fixtures":5,"schema":"hepta_gate_compat_debt_self_test_v3","status":"ready"}'
