...
for sample in $(seq 1 "$SAMPLES"); do
  ...
  operator="$(curl -fsS "$BASE_URL/api/operator-security" | jq -r '.status + ":" + (.legacy_owner_coexistence_ready|tostring) + ":" + (.attention_reason // "")' 2>/dev/null || echo fail)"

  if [[ "$health" == "ready" \
    && "$route" == "ready:0" \
    && "$owner" == "legacy_openclaw:false:false" \
    && "$poll" == "gated:false:false" \
    && "$post" == "false:false:false" \
    && "$stores" == "true:true" \
    && "$operator" == "attention:true:telegram_replacement_not_requested" ]]; then
    ok=$((ok + 1))
  else
    fail=$((fail + 1))
...
jq -n \
  ...
  --argjson ok "$ok" \
  --argjson fail "$fail" \
  '{product:$product,runtime:$runtime,base_url:$base_url,status:(if $fail == 0 then "ready" else "failed" end),samples:$samples,ok:$ok,fail:$fail,legacy_owner_preserved:true,telegram_live_send_enabled:false,native_post_real_activation_enabled:false}'
