...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-terminal-decision-status-readback-report.sh"
ATTACHMENT_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-final-acknowledgement-final-index-terminal-decision-status-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_TERMINAL_DECISION_STATUS_READBACK_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-identity-session-terminal-decision-status-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session terminal decision/status readback report: $REPORT"
[[ -x "$ATTACHMENT_GATE" ]] || fail "missing executable Public GA operator identity/session terminal decision/status attachment gate: $ATTACHMENT_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session terminal decision/status readback architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session terminal decision/status readback report"
fi

grep -q 'Public GA Operator Identity/Session Terminal Decision/Status Promotion Readback' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Terminal Decision/Status Promotion Readback"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_terminal_decision_status_promotion_readback"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_surface == "public_ga_operator_identity_session_terminal_decision_status_promotion_attachment"
  and .source_public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_ready == true
  and .source_public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_blocked == true
  ...
  and .public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_attached == true
  and .public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_attached == true
  and .readback_mode == "static_operator_identity_session_terminal_decision_status_snapshot_only"
  and .readback_check_count == 52
  ...
  and .readback_blocker_count == 46
  and .next_migration_step == "derive_public_ga_operator_identity_session_terminal_decision_status_promotion_final_index_without_acknowledgement"
...
printf 'hepta-systems-public-ga-operator-identity-session-terminal-decision-status-readback-gate: PASS: Public GA operator identity/session terminal decision/status readback is ready but blocked\n'
