...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator readiness packet template packet-acceptance receipt release publication result receipt ordering/monotonicity route source text: $label" >&2
    exit 1
  fi
}
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt ordering/monotonicity endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt ordering/monotonicity endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt ordering/monotonicity source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt ordering/monotonicity report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "packet acceptance receipt release publication result receipt ordering/monotonicity route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"release_publication_result_receipt_ordering_surface_count": release_publication_result_receipt_ordering_surface_count' \
  "packet acceptance receipt release publication result receipt ordering surface count emitted"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"packet_acceptance_receipt_release_publication_result_receipt_sequence_cursor_recorded"' \
  "packet acceptance receipt release publication result receipt sequence cursor recording denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"packet_acceptance_receipt_release_publication_result_receipt_latest_wins_overwrite_accepted"' \
  "packet acceptance receipt release publication result receipt latest-wins overwrite denied"

TEST_LOG="$(mktemp /tmp/hepta-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-ordering-monotonicity-route-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_ordering_monotonicity_endpoint_blocks_ordering_and_authority \
  -- --nocapture >"$TEST_LOG"

LIVE_ROUTE_JSON='{}'
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-ordering-monotonicity-denial"
  )"
  jq -e '
    .status == "ready"
    and .route_count == 137
...
source_ordering_gate_sha256="$(printf '%s' "$ORDERING_JSON" | shasum -a 256 | awk '{print $1}')"
...
jq -n \
  ...
  --arg endpoint "/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-ordering-monotonicity-denial" \
  --arg source_command "/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-ordering-monotonicity-denial --json" \
  ...
  '{
    ...
    status:"ready",
    base_url:$base_url,
    gate:"hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_ordering_monotonicity_denial_route_gate",
    endpoint:$endpoint,
    source_command:$source_command,
    activation_mode:"full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_ordering_monotonicity_denial_native_route_status",
    ...
    release_publication_result_receipt_sequence_cursor_recorded_count:$source.release_publication_result_receipt_sequence_cursor_recorded_count,
    release_publication_result_receipt_monotonicity_state_recorded_count:$source.release_publication_result_receipt_monotonicity_state_recorded_count,
    release_publication_result_receipt_latest_wins_overwrite_accepted_count:$source.release_publication_result_receipt_latest_wins_overwrite_accepted_count,
    release_publication_result_receipt_completion_ack_ordering_accepted_count:$source.release_publication_result_receipt_completion_ack_ordering_accepted_count,
    release_publication_result_receipt_ordering_activation_authority_derived_count:$source.release_publication_result_receipt_ordering_activation_authority_derived_count,
    release_publication_result_receipt_ordering_live_execution_allowed_count:$source.release_publication_result_receipt_ordering_live_execution_allowed_count,
...
echo "Hepta memory/intelligence/KG full live activation operator readiness packet template packet acceptance receipt release/publication result receipt ordering/monotonicity denial route gate passed"
