...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing memory write execution post-write validation dry-run boundary source text: $label" >&2
    exit 1
  fi
}
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write execution post-write validation dry-run boundary report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"memory_write_execution_post_write_validation_dry_run_boundary_ready\"" \
  "memory write execution post-write validation dry-run boundary ready field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"denied_by_post_write_validation_count\", 14" \
  "memory write execution post-write validation dry-run denied count"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  ...
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_write_execution_post_write_validation_dry_run_boundary_ready == true
    and .post_write_validation_mode == "memory_write_execution_post_write_validation_dry_run_non_activation"
    and .source_memory_write_execution_write_enable_fixture_boundary_ready == true
    ...
    and .denied_by_post_write_validation_count == 14
    ...
    and .allowed_next_actions[0].action == "run_memory_write_execution_post_write_validation_dry_run_boundary_require_live_gate"
    and .allowed_next_actions[1].action == "prepare_memory_write_execution_post_write_operator_acceptance_denial_boundary"
...
jq -n \
  ...
  '{
    ...
    status:"ready",
    base_url:$base_url,
    gate:"hepta_memory_live_mutation_operator_write_execution_post_write_validation_dry_run_boundary_route_gate",
    endpoint:$endpoint,
    ...
    memory_write_execution_post_write_validation_dry_run_boundary_ready:true,
