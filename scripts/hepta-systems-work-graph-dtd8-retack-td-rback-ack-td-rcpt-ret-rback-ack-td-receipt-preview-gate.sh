...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-retack-td-rback-ack-td-rcpt-ret-rback-ack-td-receipt-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-dtd8-retack-td-rback-ack-td-rcpt-ret-rback-ack-td-receipt-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .source_probes.deep_td8_ret_ack_td_rback_ack_td_receipt_retention_readback_ack_td_receipt.rust_module_present == true
  and .source_probes.prior_ret_ack_td_rback_ack_td.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_ret_ack_td_rback_ack_td_receipt_retention_readback_ack_td_receipt_preview --lib
