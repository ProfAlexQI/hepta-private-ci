...
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/dirty_worktree_release_boundary_clean_worktree_strategy_operator_evidence_recording_boundary_readback.rs"
...
fail() {
  printf 'hepta-systems-dirty-worktree-release-boundary-clean-worktree-strategy-operator-evidence-recording-boundary-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable Phase 21 approval acceptance boundary readback report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing Phase 22 Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing Phase 22 architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the Phase 22 evidence recording boundary readback report"
fi
...
"$SOURCE_REPORT" >"$tmpdir/source.json" \
  || fail "failed to render Phase 21 approval acceptance boundary readback report"
jq -e . "$tmpdir/source.json" >/dev/null \
  || fail "invalid JSON rendered by Phase 21 approval acceptance boundary readback report"

lib_export_present=false
if grep -q 'dirty_worktree_release_boundary_clean_worktree_strategy_operator_evidence_recording_boundary_readback_report' "$LIB_SOURCE"; then
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-dirty-worktree-release-boundary-clean-worktree-strategy-operator-evidence-recording-boundary-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_DIRTY_WORKTREE_RELEASE_BOUNDARY_CLEAN_WORKTREE_STRATEGY_OPERATOR_EVIDENCE_RECORDING_BOUNDARY_READBACK_2026-06-28.md" \
  '
  ...
  def evidence_entry:
    ...
    | {
      ...
      evidence_boundary_key:("dirty_worktree.evidence_recording_boundary." + key_safe($entry.group_type) + "." + key_safe($entry.source_bucket)),
      evidence_boundary_route:("readback://release-boundary/dirty-worktree/clean-worktree-strategy/operator-evidence-recording-boundary/" + route_prefix($entry.group_type) + "/" + route_safe($entry.source_bucket)),
  ...
  {
    runtime:"hepta",
    surface:"dirty_worktree_release_boundary_clean_worktree_strategy_operator_evidence_recording_boundary_readback",
    status:(if $boundary_ready then "ready_blocked" else "blocked" end),
    gate:"dirty_worktree_release_boundary_clean_worktree_strategy_operator_evidence_recording_boundary_readback_gate",
    schema_version:"dirty_worktree_release_boundary_clean_worktree_strategy_operator_evidence_recording_boundary_readback_v1",
    ...
    recommended_next_gate:"phase23_dirty_worktree_release_boundary_release_risk_snapshot_without_git_mutation",
    next_actions:[
