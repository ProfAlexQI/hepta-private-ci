...
READBACK_REPORT="$ROOT/scripts/hepta-systems-terminal-non-activation-release-claim-index-attachment-readback-report.sh"
...
fail() {
  printf 'hepta-systems-terminal-non-activation-release-claim-index-attachment-final-index-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$READBACK_REPORT" ]] || fail "missing executable terminal non-activation release claim attachment readback report: $READBACK_REPORT"
[[ -f "$DOC" ]] || fail "missing terminal non-activation release claim attachment final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the terminal non-activation release claim attachment final index report"
fi

jq -n \
  --slurpfile readback <("$READBACK_REPORT") \
  --arg gate "scripts/hepta-systems-terminal-non-activation-release-claim-index-attachment-final-index-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_TERMINAL_NON_ACTIVATION_RELEASE_CLAIM_INDEX_ATTACHMENT_FINAL_INDEX_2026-06-21.md" \
  '
  ...
  ($readback.terminal_non_activation_release_claim_index_attachment_readback_ready == true
    and $readback.terminal_non_activation_release_claim_index_attachment_readback_blocked == true
    and $readback.terminal_non_activation_release_claim_index_gate_present == true
  ...
  {
    runtime:"hepta",
    surface:"terminal_non_activation_release_claim_index_attachment_final_index",
    plugin_id:$readback.plugin_id,
    status:(if $final_index_ready then "ready_blocked" else "blocked" end),
    source_terminal_non_activation_release_claim_index_attachment_readback_surface:$readback.surface,
    source_terminal_non_activation_release_claim_index_attachment_readback_ready:$readback.terminal_non_activation_release_claim_index_attachment_readback_ready,
    source_terminal_non_activation_release_claim_index_attachment_readback_blocked:$readback.terminal_non_activation_release_claim_index_attachment_readback_blocked,
    terminal_non_activation_release_claim_index_attachment_final_index_ready:$final_index_ready,
    terminal_non_activation_release_claim_index_attachment_final_index_blocked:true,
    ...
    next_migration_step:"attach_terminal_non_activation_release_claim_index_attachment_final_index_to_terminal_operator_readiness_non_approval_index_without_release_claim_gate_invocation",
    local_gate:$gate,
    architecture_note:$doc,
    source_files:{
      terminal_non_activation_release_claim_index_attachment_readback_report:"scripts/hepta-systems-terminal-non-activation-release-claim-index-attachment-readback-report.sh"
    },
