...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator readiness packet template packet-acceptance receipt release publication result receipt replay/idempotency route source text: $label" >&2
    exit 1
  fi
}
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt replay/idempotency endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt replay/idempotency endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt replay/idempotency source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt replay/idempotency report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "packet acceptance receipt release publication result receipt replay/idempotency route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"release_publication_result_receipt_replay_surface_count": release_publication_result_receipt_replay_surface_count' \
  "packet acceptance receipt release publication result receipt replay surface count emitted"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"packet_acceptance_receipt_release_publication_result_receipt_idempotency_cache_written"' \
  "packet acceptance receipt release publication result receipt idempotency cache write denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"packet_acceptance_receipt_publication_completion_ack_replayed"' \
  "packet acceptance receipt publication completion acknowledgement replay denied"

TEST_LOG="$(mktemp /tmp/hepta-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-replay-idempotency-route-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_replay_idempotency_endpoint_blocks_replay_and_authority \
  -- --nocapture >"$TEST_LOG"

LIVE_ROUTE_JSON='{}'
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-replay-idempotency-denial"
  )"
  jq -e '
    .status == "ready"
    and .route_count == 136
    ...
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_replay_idempotency_denial_route_enabled == true
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_replay_idempotency_denial_ready == true
    and .source_packet_acceptance_receipt_release_publication_result_receipt_ready == true
    and .release_publication_result_receipt_replay_surface_count == 14
    and .release_publication_result_receipt_replay_attempt_count == 14
...
jq -n \
  ...
  --arg endpoint "/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-replay-idempotency-denial" \
  --arg source_command "/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-replay-idempotency-denial --json" \
  ...
  '{
    ...
    status:"ready",
    base_url:$base_url,
    gate:"hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_replay_idempotency_denial_route_gate",
    endpoint:$endpoint,
    source_command:$source_command,
    activation_mode:"full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_replay_idempotency_denial_native_route_status",
    ...
    source_packet_acceptance_receipt_release_publication_result_receipt_ready:$source.source_packet_acceptance_receipt_release_publication_result_receipt_ready,
    release_publication_result_receipt_replay_surface_count:$source.release_publication_result_receipt_replay_surface_count,
    release_publication_result_receipt_replay_recorded_count:$source.release_publication_result_receipt_replay_recorded_count,
    release_publication_result_receipt_idempotency_key_registered_count:$source.release_publication_result_receipt_idempotency_key_registered_count,
    release_publication_result_receipt_idempotency_cache_written_count:$source.release_publication_result_receipt_idempotency_cache_written_count,
    publication_completion_ack_replayed_count:$source.publication_completion_ack_replayed_count,
    release_publication_result_receipt_replay_activation_authority_derived_count:$source.release_publication_result_receipt_replay_activation_authority_derived_count,
    release_publication_result_receipt_replay_live_execution_allowed_count:$source.release_publication_result_receipt_replay_live_execution_allowed_count,
...
echo "Hepta memory/intelligence/KG full live activation operator readiness packet template packet acceptance receipt release/publication result receipt replay/idempotency denial route gate passed"
