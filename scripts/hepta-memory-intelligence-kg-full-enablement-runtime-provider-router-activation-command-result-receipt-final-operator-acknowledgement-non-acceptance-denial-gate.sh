...
SUMMARY_BRIEFING_JSON="$(
  HEPTA_LIVE_URL="$BASE_URL" \
    ...
    capture_json_report "hepta-memory-intelligence-kg-full-enablement-runtime-provider-router-activation-command-result-receipt-operator-facing-summary-briefing-non-persistence-denial-gate" \
...
jq -n -e \
  ...
  '
    ...
    and $source.gate == "hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial_gate"
    and $source.activation_command_result_receipt_operator_facing_summary_briefing_mode == "runtime_provider_router_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial_no_summary_no_briefing_no_delivery"
    ...
    and ($source.operator_facing_summary_briefing_fixtures | all((.operator_summary_briefing_status == "blocked_noop" or .operator_summary_briefing_status == "blocked_summary_noop" or .operator_summary_briefing_status == "blocked_briefing_noop" or .operator_summary_briefing_status == "blocked_delivery_noop") and .operator_summary_recorded == false and .operator_summary_persisted == false and .operator_summary_materialized == false and .operator_summary_filesystem_written == false and .operator_summary_delivered == false and .operator_briefing_recorded == false and .operator_briefing_persisted == false and .operator_briefing_materialized == false and .operator_briefing_filesystem_written == false and .operator_briefing_delivered == false and .telegram_send_performed == false and .activation_command_result_receipt_recorded == false and .activation_command_result_receipt_persisted == false and .activation_command_result_receipt_accepted == false and .activation_activated == false and .runtime_router_mutated == false and .provider_invoked == false and .model_invoked == false and .memory_store_write_performed == false and .memory_store_mutated == false and .live_kg_write_performed == false and .rollback_executed == false and .summary_briefing_noop_confirmed == true))
...
report="$(jq -n \
  ...
  --arg gate "hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_activation_command_result_receipt_final_operator_acknowledgement_non_acceptance_denial_gate" \
  ...
  [
    blocked_fixture("provider-router-activation-result-receipt-final-ack-missing-source-summary-briefing"; "blocked_noop"; "source_summary_briefing_report_required"; {source_summary_briefing_present:false, source_summary_briefing_ready:false, final_operator_acknowledgement_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-request"; "blocked_ack_noop"; "final_operator_acknowledgement_request_shape_denied"; {final_operator_acknowledgement_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-acceptance-request"; "blocked_acceptance_noop"; "acknowledgement_acceptance_denied"; {final_operator_acknowledgement_requested:true, acknowledgement_acceptance_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-recording-request"; "blocked_ack_noop"; "acknowledgement_recording_denied"; {final_operator_acknowledgement_requested:true, acknowledgement_recording_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-persistence-filesystem-write-request"; "blocked_ack_noop"; "acknowledgement_persistence_filesystem_write_denied"; {final_operator_acknowledgement_requested:true, acknowledgement_persistence_requested:true, acknowledgement_filesystem_write_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-identity-signature-request"; "blocked_acceptance_noop"; "operator_identity_signature_acknowledgement_acceptance_denied"; {final_operator_acknowledgement_requested:true, operator_identity_acceptance_requested:true, operator_signature_acceptance_requested:true, operator_timestamp_acceptance_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-delivery-request"; "blocked_delivery_noop"; "acknowledgement_delivery_denied"; {final_operator_acknowledgement_requested:true, acknowledgement_delivery_requested:true, telegram_send_requested:true, channel_delivery_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-state-promotion-request"; "blocked_promotion_noop"; "final_state_completion_promotion_denied"; {final_operator_acknowledgement_requested:true, final_state_promotion_requested:true, completion_promotion_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-activation-memory-kg-provider-request"; "blocked_ack_noop"; "activation_memory_kg_rollback_secret_provider_acknowledgement_denied"; {final_operator_acknowledgement_requested:true, activation_from_acknowledgement_requested:true, memory_store_acknowledgement_requested:true, live_kg_acknowledgement_requested:true, rollback_acknowledgement_requested:true, secret_material_acknowledgement_requested:true, provider_prompt_acknowledgement_requested:true}),
    blocked_fixture("provider-router-activation-result-receipt-final-ack-external-public-install-request"; "blocked_delivery_noop"; "external_public_install_restart_active_binary_acknowledgement_denied"; {final_operator_acknowledgement_requested:true, external_send_acknowledgement_requested:true, public_claim_acknowledgement_requested:true, release_artifact_acknowledgement_requested:true, install_acknowledgement_requested:true, service_restart_acknowledgement_requested:true, active_binary_acknowledgement_requested:true})
  ] as $fixtures
  | {
    product:$product,
    runtime:$runtime,
    ...
    base_url:$base_url,
    gate:$gate,
    activation_command_result_receipt_final_operator_acknowledgement_mode:"runtime_provider_router_activation_command_result_receipt_final_operator_acknowledgement_non_acceptance_denial",
    ...
    source_runtime_provider_router_context_attachment_report_sha256:$source.source_runtime_provider_router_context_attachment_report_sha256,
...
printf '%s\n' "$report" | jq .
echo "Hepta memory/intelligence/KG full enablement runtime provider-router activation command result receipt final operator acknowledgement non-acceptance denial gate passed"
