...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-plugin-tool-invocation-read-only-status-dry-run-operator-packet-readback-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/hepta_systems_plugin_tool_invocation_read_only_status_dry_run_acceptance_recording_boundary_readback.rs"
LIB_SOURCE="$ROOT/codex-rs/hepta-runtime/src/lib.rs"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PLUGIN_TOOL_INVOCATION_READ_ONLY_STATUS_DRY_RUN_ACCEPTANCE_RECORDING_BOUNDARY_READBACK_2026-06-30.md"

fail() {
  printf 'hepta-systems-plugin-tool-invocation-read-only-status-dry-run-acceptance-recording-boundary-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable read-only status dry-run operator packet report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing read-only status dry-run acceptance recording boundary Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing acceptance recording boundary architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the acceptance recording boundary report"
fi

tmpdir="$(mktemp -d)"
trap 'rm -rf "$tmpdir"' EXIT

"$SOURCE_REPORT" >"$tmpdir/source.json" || fail "failed to render read-only status dry-run operator packet report"
jq -e . "$tmpdir/source.json" >/dev/null || fail "operator packet report did not render valid JSON"

lib_export_present=false
if grep -q 'hepta_systems_plugin_tool_invocation_read_only_status_dry_run_acceptance_recording_boundary_readback_report' "$LIB_SOURCE"; then
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-plugin-tool-invocation-read-only-status-dry-run-acceptance-recording-boundary-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_PLUGIN_TOOL_INVOCATION_READ_ONLY_STATUS_DRY_RUN_ACCEPTANCE_RECORDING_BOUNDARY_READBACK_2026-06-30.md" \
  '
  def suffix($kind):
  ...
  def acceptance_record_id($kind):
    "operator-acceptance-record:hepta-system:" + suffix($kind) + ":" + selected_suffix($kind);
  def non_recording_denial_receipt_id($kind):
    "operator-acceptance-non-recording-denial:hepta-system:" + suffix($kind) + ":" + selected_suffix($kind);
  def ledger_preview_anchor_id($kind):
    if selected($kind) then "acceptance-ledger-preview-anchor:hepta-system:local-mcp:dry-run-read-only-denied"
    else "acceptance-ledger-preview-anchor:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def receipt_preview_anchor_id($kind):
    if selected($kind) then "acceptance-receipt-preview-anchor:hepta-system:local-mcp:read-only-denied"
    else "acceptance-receipt-preview-anchor:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def operator_checklist_closure_id($kind):
    "operator-checklist-closure:hepta-system:" + suffix($kind) + ":" + selected_suffix($kind);
  def acceptance_idempotency_key($kind):
    "operator-acceptance-recording-idempotency:hepta-system:" + suffix($kind) + ":" + selected_suffix($kind);
  def checklist_closure_items:
    [
  ...
  def entry($source_entry):
    ($source_entry.contribution_kind) as $kind |
    {
      ...
      source_operator_packet_id:$source_entry.operator_packet_id,
      source_operator_checklist_id:$source_entry.operator_checklist_id,
      source_non_acceptance_receipt_id:$source_entry.non_acceptance_receipt_id,
      source_ledger_preview_link_id:$source_entry.ledger_preview_link_id,
      source_receipt_preview_link_id:$source_entry.receipt_preview_link_id,
      source_policy_denial_anchor_link_id:$source_entry.policy_denial_anchor_link_id,
      source_approval_denial_anchor_link_id:$source_entry.approval_denial_anchor_link_id,
      source_operator_packet_idempotency_key:$source_entry.operator_packet_idempotency_key,
      acceptance_record_id:acceptance_record_id($kind),
      non_recording_denial_receipt_id:non_recording_denial_receipt_id($kind),
      ledger_preview_anchor_id:ledger_preview_anchor_id($kind),
      receipt_preview_anchor_id:receipt_preview_anchor_id($kind),
      operator_checklist_closure_id:operator_checklist_closure_id($kind),
      operator_checklist_closure_items:checklist_closure_items,
      acceptance_idempotency_key:acceptance_idempotency_key($kind),
      first_acceptance_record_id:acceptance_record_id($kind),
      second_acceptance_record_id:acceptance_record_id($kind),
      first_non_recording_denial_receipt_id:non_recording_denial_receipt_id($kind),
      second_non_recording_denial_receipt_id:non_recording_denial_receipt_id($kind),
      first_acceptance_idempotency_key:acceptance_idempotency_key($kind),
      second_acceptance_idempotency_key:acceptance_idempotency_key($kind),
  ...
  ($entries | map(select(.acceptance_record_id_projected == true)) | length) as $acceptance_record_id_projected_count |
  ($entries | map(select(.non_recording_denial_receipt_projected == true)) | length) as $non_recording_denial_receipt_projected_count |
  ($entries | map(select(.ledger_preview_anchor_projected == true)) | length) as $ledger_preview_anchor_projected_count |
  ($entries | map(select(.receipt_preview_anchor_projected == true)) | length) as $receipt_preview_anchor_projected_count |
  ($entries | map(select(.operator_checklist_closure_projected == true)) | length) as $operator_checklist_closure_projected_count |
  ($entries | map(.operator_checklist_closure_items | length) | add) as $operator_checklist_closure_item_count |
  ($entries | map(select(.acceptance_idempotency_key_projected == true)) | length) as $acceptance_idempotency_key_projected_count |
  ($entries | map(select(.stable_acceptance_record == true)) | length) as $stable_acceptance_record_count |
  ($entries | map(.first_acceptance_record_id) | unique | length) as $unique_acceptance_record_count |
  ($entries | map(select(.stable_non_recording_denial_receipt == true)) | length) as $stable_non_recording_denial_receipt_count |
  ($entries | map(.first_non_recording_denial_receipt_id) | unique | length) as $unique_non_recording_denial_receipt_count |
  ($entries | map(select(.stable_acceptance_idempotency_key == true)) | length) as $stable_acceptance_idempotency_key_count |
  ($entries | map(.first_acceptance_idempotency_key) | unique | length) as $unique_acceptance_idempotency_key_count |
  ($entries | map(select(.stable_acceptance_record == false)) | length) as $acceptance_record_mismatch_count |
  ($boundary_entry_count - $unique_acceptance_record_count) as $duplicate_acceptance_record_count |
  ($entries | map(select(.stable_non_recording_denial_receipt == false)) | length) as $non_recording_denial_receipt_mismatch_count |
  ($boundary_entry_count - $unique_non_recording_denial_receipt_count) as $duplicate_non_recording_denial_receipt_count |
  ($entries | map(select(.stable_acceptance_idempotency_key == false)) | length) as $acceptance_idempotency_mismatch_count |
  ($boundary_entry_count - $unique_acceptance_idempotency_key_count) as $duplicate_acceptance_idempotency_key_count |
  ...
  ($entries | map(select(.acceptance_record_persisted == true)) | length) as $acceptance_record_persisted_count |
  ($entries | map(select(.operator_acceptance_recorded == true)) | length) as $operator_acceptance_recorded_count |
  ($entries | map(select(.non_recording_denial_receipt_persisted == true)) | length) as $non_recording_denial_receipt_persisted_count |
  ($entries | map(select(.operator_checklist_closure_persisted == true)) | length) as $operator_checklist_closure_persisted_count |
  ...
  ($source_report.operator_packet_readback_ready == true
    and $source_report.candidate_count == 2
    and $source_report.packet_entry_count == 2
    and $source_report.selected_read_only_status_tool_count == 1
    and $source_report.non_selected_preflight_boundary_count == 1
    and $source_report.operator_packet_id_projected_count == 2
    and $source_report.operator_checklist_projected_count == 2
    and $source_report.non_acceptance_receipt_projected_count == 2
    and $source_report.ledger_preview_link_projected_count == 2
    and $source_report.receipt_preview_link_projected_count == 2
    and $source_report.stable_operator_packet_count == 2
    and $source_report.unique_operator_packet_count == 2
    and $source_report.operator_acceptance_recorded_count == 0
    and $source_report.operator_packet_sent_count == 0
    and $source_report.operator_packet_persisted_count == 0
    ...
    and $acceptance_record_id_projected_count == 2
    and $non_recording_denial_receipt_projected_count == 2
    and $ledger_preview_anchor_projected_count == 2
    and $receipt_preview_anchor_projected_count == 2
    and $operator_checklist_closure_projected_count == 2
    and $operator_checklist_closure_item_count == 10
    and $acceptance_idempotency_key_projected_count == 2
    and $stable_acceptance_record_count == 2
    and $unique_acceptance_record_count == 2
    and $stable_non_recording_denial_receipt_count == 2
    and $unique_non_recording_denial_receipt_count == 2
    and $stable_acceptance_idempotency_key_count == 2
    and $unique_acceptance_idempotency_key_count == 2
    and $acceptance_record_mismatch_count == 0
    and $duplicate_acceptance_record_count == 0
    and $non_recording_denial_receipt_mismatch_count == 0
    and $duplicate_non_recording_denial_receipt_count == 0
    and $acceptance_idempotency_mismatch_count == 0
    and $duplicate_acceptance_idempotency_key_count == 0
    ...
    and $operator_checklist_closure_persisted_count == 0
  ...
  {
    runtime:"hepta",
    surface:"hepta_systems_plugin_tool_invocation_read_only_status_dry_run_acceptance_recording_boundary_readback",
    ...
    schema_version:"hepta_systems_plugin_tool_invocation_read_only_status_dry_run_acceptance_recording_boundary_readback_v1",
    plugin_id:"hepta-system@hepta-local",
    manifest_name:$source_report.manifest_name,
    manifest_version:$source_report.manifest_version,
    source_operator_packet_readback_ready:$source_report.operator_packet_readback_ready,
    ...
    acceptance_record_id_projected_count:$acceptance_record_id_projected_count,
    non_recording_denial_receipt_projected_count:$non_recording_denial_receipt_projected_count,
    ledger_preview_anchor_projected_count:$ledger_preview_anchor_projected_count,
    receipt_preview_anchor_projected_count:$receipt_preview_anchor_projected_count,
    operator_checklist_closure_projected_count:$operator_checklist_closure_projected_count,
    operator_checklist_closure_item_count:$operator_checklist_closure_item_count,
    acceptance_idempotency_key_projected_count:$acceptance_idempotency_key_projected_count,
    stable_acceptance_record_count:$stable_acceptance_record_count,
    unique_acceptance_record_count:$unique_acceptance_record_count,
    stable_non_recording_denial_receipt_count:$stable_non_recording_denial_receipt_count,
    unique_non_recording_denial_receipt_count:$unique_non_recording_denial_receipt_count,
    stable_acceptance_idempotency_key_count:$stable_acceptance_idempotency_key_count,
    unique_acceptance_idempotency_key_count:$unique_acceptance_idempotency_key_count,
    acceptance_record_mismatch_count:$acceptance_record_mismatch_count,
    duplicate_acceptance_record_count:$duplicate_acceptance_record_count,
    non_recording_denial_receipt_mismatch_count:$non_recording_denial_receipt_mismatch_count,
    duplicate_non_recording_denial_receipt_count:$duplicate_non_recording_denial_receipt_count,
    acceptance_idempotency_mismatch_count:$acceptance_idempotency_mismatch_count,
    duplicate_acceptance_idempotency_key_count:$duplicate_acceptance_idempotency_key_count,
    ...
    non_recording_denial_receipt_persisted_count:$non_recording_denial_receipt_persisted_count,
    operator_checklist_closure_persisted_count:$operator_checklist_closure_persisted_count,
    ...
    acceptance_recording_boundary_readback_ready:$ready,
    ...
    recommended_next_gate:"hepta_systems_plugin_tool_invocation_read_only_status_dry_run_execution_open_preconditions_readback",
