...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-export-query-observability-final-index-report.sh"
SUMMARY_BRIEFING_GATE="$ROOT/scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-operator-facing-summary-briefing-non-persistence-denial-gate.sh"
SUMMARY_BRIEFING_DOC="$ROOT/docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_REPLAY_REINSTATEMENT_OPERATOR_FACING_SUMMARY_BRIEFING_NON_PERSISTENCE_DENIAL_GATE.md"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session export query observability final index report: $SOURCE_REPORT" >&2
  exit 1
}
[[ -f "$SUMMARY_BRIEFING_GATE" ]] || {
  echo "missing operator identity/session summary briefing non-persistence gate: $SUMMARY_BRIEFING_GATE" >&2
  exit 1
}
[[ -f "$SUMMARY_BRIEFING_DOC" ]] || {
  echo "missing operator identity/session summary briefing non-persistence doc: $SUMMARY_BRIEFING_DOC" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session summary briefing report" >&2
  exit 1
fi
...
jq -e '
  .surface == "public_ga_operator_identity_session_export_query_observability_final_index"
...
jq -n \
  ...
  '{
    runtime: "hepta",
    surface: "public_ga_operator_identity_session_operator_facing_summary_briefing_attachment",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_identity_session_export_query_observability_final_index_surface: $source.surface,
    source_public_ga_operator_identity_session_export_query_observability_final_index_ready: $source.public_ga_operator_identity_session_export_query_observability_final_index_ready,
    source_public_ga_operator_identity_session_export_query_observability_final_index_blocked: $source.public_ga_operator_identity_session_export_query_observability_final_index_blocked,
    public_ga_operator_identity_session_export_query_observability_final_index_attached: true,
    public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_ready: true,
    public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_blocked: true,
    ...
    operator_identity_session_operator_facing_summary_briefing_non_persistence_gate_present: true,
    operator_identity_session_operator_facing_summary_briefing_non_persistence_doc_present: true,
    operator_identity_session_operator_facing_summary_briefing_static_mention_count: $summary_briefing_static_mention_count,
    ...
    long_soak_required_by_source_summary_briefing_gate: true,
    ...
    attachment_blocker_count: 42,
    manual_operator_live_cutover_approval_required: true,
    public_ga_claim_allowed: false,
    public_ga_claimed: false,
    public_release_published: false,
    rollback_execution_allowed: false,
    next_migration_step: "derive_public_ga_operator_identity_session_operator_facing_summary_briefing_readback_without_export",
    local_gate: "scripts/hepta-systems-public-ga-operator-export-query-observability-final-index-summary-briefing-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_EXPORT_QUERY_OBSERVABILITY_FINAL_INDEX_SUMMARY_BRIEFING_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_export_query_observability_final_index_report: "scripts/hepta-systems-public-ga-operator-identity-session-export-query-observability-final-index-report.sh",
      operator_identity_session_operator_facing_summary_briefing_non_persistence_gate: "scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-operator-facing-summary-briefing-non-persistence-denial-gate.sh",
      operator_identity_session_operator_facing_summary_briefing_non_persistence_doc: "docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_REPLAY_REINSTATEMENT_OPERATOR_FACING_SUMMARY_BRIEFING_NON_PERSISTENCE_DENIAL_GATE.md"
    },
