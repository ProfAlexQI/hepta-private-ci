...
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_local_evidence_acceptance_positive_preconditions.rs"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-positive-preconditions-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable terminal no-persistence source report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing local evidence acceptance positive preconditions Rust source: $RUST_SOURCE"
...
if grep -q 'controlled_live_evidence_receipt_store_local_evidence_acceptance_positive_preconditions_readback_without_acceptance_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_RECORDING_DENIAL_RECEIPT_PERSISTENCE_DENIAL_TERMINAL_NO_PERSISTENCE_JSON:-}"
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-cler-store-local-evidence-acceptance-positive-preconditions-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_POSITIVE_PRECONDITIONS_2026-07-07.md" \
  --arg precondition_route "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/positive-preconditions" \
  '
  def hyphen_id($id): $id | gsub("_"; "-");
  def count_true($entries; $key): $entries | map(select(.[$key] == true)) | length;
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_local_evidence_acceptance_positive_preconditions_without_acceptance_" + .source_blocker_id),
    source_blocker_id,
    ...
    positive_precondition_set_id:("local-evidence-acceptance-positive-preconditions:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    positive_precondition_key:("local-evidence-acceptance-positive-preconditions:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    positive_precondition_route:($precondition_route + "/" + hyphen_id(.source_blocker_id)),
    local_acceptance_authority_precondition_id:("local-evidence-acceptance-authority-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    operator_local_acceptance_approval_precondition_id:("operator-local-evidence-acceptance-approval-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    dev_evidence_acceptance_source_precondition_id:("dev-evidence-acceptance-source-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    evidence_payload_source_binding_precondition_id:("evidence-payload-source-binding-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    local_evidence_store_feature_gate_precondition_id:("local-evidence-store-feature-gate-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    local_receipt_store_feature_gate_precondition_id:("local-receipt-store-feature-gate-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    atomic_acceptance_append_precondition_id:("atomic-local-evidence-acceptance-append-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    post_acceptance_readback_precondition_id:("post-local-evidence-acceptance-readback-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    rollback_anchor_precondition_id:("local-evidence-acceptance-rollback-anchor-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    retention_policy_commit_precondition_id:("local-evidence-acceptance-retention-policy-commit-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    replay_idempotency_guard_precondition_id:("local-evidence-acceptance-replay-idempotency-guard-required:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    observed_state:"local_evidence_acceptance_positive_preconditions_projected_without_acceptance",
    ...
    and $live_mutation_allowed_count == 0) as $ready |
  {
    ...
    gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_positive_preconditions_readback_without_acceptance_gate",
    schema_version:"controlled_live_evidence_receipt_store_local_evidence_acceptance_positive_preconditions_readback_without_acceptance_v1",
    ...
    local_evidence_acceptance_positive_preconditions_readback_ready:$ready,
    local_evidence_acceptance_allowed:false,
    local_evidence_acceptance_recording_allowed:false,
    evidence_acceptance_recording_allowed:false,
    evidence_recording_allowed:false,
    ...
    next_actions:[
      ...
      "keep_local_evidence_acceptance_closed_until_all_positive_preconditions_are_present"
    ],
    recommended_next_gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_readback_without_acceptance",
    local_gate:$gate,
