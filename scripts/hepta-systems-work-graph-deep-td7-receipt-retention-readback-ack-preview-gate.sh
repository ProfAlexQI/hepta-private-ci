...
PRIOR="${BASE}_receipt_retention_expiry_readback_receipt_preview_gate"
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-ack-preview-report.sh"

report="$(capture_json_report "hepta-systems-work-graph-deep-td7-receipt-retention-readback-ack-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e --arg gate "$GATE" --arg next "$NEXT" --arg prior "$PRIOR" '
  ...
  and .source_probes.deep_td7_readback_ack.rust_module_present == true
  and .source_probes.prior_readback_receipt.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td7_receipt_retention_readback_ack_preview --lib

echo "Hepta WorkGraph deep td7 receipt retention readback acknowledgement preview gate passed"
