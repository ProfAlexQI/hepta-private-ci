...
STARTUP_TIMEOUT_SEC="${HEPTA_NATIVE_WINDOW_SECONDARY_SMOKE_STARTUP_TIMEOUT_SEC:-240}"
ALLOW_BLOCKED="${HEPTA_NATIVE_WINDOW_SECONDARY_SMOKE_ALLOW_BLOCKED:-0}"
WRAPPER_PREFLIGHT="${HEPTA_NATIVE_WINDOW_SECONDARY_SMOKE_PREFLIGHT:-1}"
ASSUME_PREFLIGHT_READY="${HEPTA_NATIVE_WINDOW_SECONDARY_SMOKE_ASSUME_PREFLIGHT_READY:-0}"
WRAPPER_PREBUILD="${HEPTA_NATIVE_WINDOW_SECONDARY_SMOKE_PREBUILD:-1}"
DESKTOP_BOUNDS="${HEPTA_NATIVE_WINDOW_SECONDARY_SMOKE_DESKTOP_BOUNDS:-40,120,1200,720}"
...
if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required for the Hepta Native secondary window smoke gate" >&2
  exit 2
fi
...
run_window_smoke_preflight_test() {
  ...
  if ! cargo_with_window_target test --manifest-path "$APP_MANIFEST" -q "$test_name" >&2; then
    echo "Hepta Native secondary window smoke preflight test failed: $test_name" >&2
    exit 1
  fi
}
...
prebuild_window_smoke_app() {
  ...
  if ! cargo_with_window_target build --manifest-path "$APP_MANIFEST" -q >&2; then
    echo "Hepta Native secondary window smoke prebuild failed" >&2
    exit 1
  fi
}
...
for surface in "${SURFACES[@]}"; do
  ...
  if ! env \
    ...
    HEPTA_NATIVE_WINDOW_SMOKE_CAPTURE_PROFILE=desktop-full-secondary \
    ...
    echo "Hepta Native secondary true-window smoke failed for surface=$surface" >&2
  ...
  jq -e \
    ...
    '
      .status == "ready"
      and .capture_profile == "desktop-full-secondary"
...
jq -s \
  ...
  '. as $reports | {
    ...
    native_makepad_secondary_surfaces_ready:(
      ...
      and ($reports | all(.capture_profile == "desktop-full-secondary"))
