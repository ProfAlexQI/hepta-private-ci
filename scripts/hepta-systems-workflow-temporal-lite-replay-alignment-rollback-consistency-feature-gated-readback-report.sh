...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-workflow-temporal-lite-replay-alignment-checkpoint-consistency-feature-gated-readback-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/workflow_temporal_lite_replay_alignment_rollback_consistency_feature_gated_readback.rs"
LIB_SOURCE="$ROOT/codex-rs/hepta-runtime/src/lib.rs"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_WORKFLOW_TEMPORAL_LITE_REPLAY_ALIGNMENT_ROLLBACK_CONSISTENCY_FEATURE_GATED_READBACK_2026-06-29.md"

fail() {
  printf 'hepta-systems-workflow-temporal-lite-replay-alignment-rollback-consistency-feature-gated-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable Temporal-lite replay-alignment checkpoint consistency report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing Temporal-lite replay-alignment rollback consistency Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing Temporal-lite replay-alignment rollback consistency architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the Temporal-lite replay-alignment rollback consistency report"
fi

lib_export_present=false
if grep -q 'hepta_workflow_temporal_lite_replay_alignment_rollback_consistency_feature_gated_readback_report' "$LIB_SOURCE"; then
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-workflow-temporal-lite-replay-alignment-rollback-consistency-feature-gated-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_WORKFLOW_TEMPORAL_LITE_REPLAY_ALIGNMENT_ROLLBACK_CONSISTENCY_FEATURE_GATED_READBACK_2026-06-29.md" \
  '
  def pad3($n):
  ...
  def consistency_entry($entry):
    ...
    keyed("temporal-lite.rollback.readback"; $entry) as $rollback_readback_key |
    {
      ...
      rollback_consistency_key:keyed("temporal-lite.replay-alignment.rollback-consistency.readback"; $entry),
      rollback_readback_key:$rollback_readback_key,
      rollback_consistency_digest:("replay-alignment-rollback-consistency-digest.v1." + pad3($entry.sequence) + "." + $entry.event_contract_id + "." + (($entry.checkpoint_consistency_digest | length) | tostring) + "." + (($entry.checkpoint_readback_key | length) | tostring)),
      expected_rollback_projection_key:$rollback_readback_key,
      consistency_state:"rollback_consistent_not_written",
      readback_state:"replay_alignment_rollback_consistency_contract_projected_in_memory_only",
  ...
  ($source_report.checkpoint_consistency_readback_ready == true
      ...
      and .live_execution_allowed == false))) as $readback_ready |
  {
    ...
    schema_version:"workflow_temporal_lite_replay_alignment_rollback_consistency_feature_gated_readback_v1",
    source_checkpoint_consistency_gate:$source_report.gate,
    source_checkpoint_consistency_ready:$source_report.checkpoint_consistency_readback_ready,
    ...
    rollback_consistency_readback_ready:$readback_ready,
    entries:$entries,
    blockers:[
    ...
    recommended_next_gate:"temporal_lite_replay_alignment_recovery_window_feature_gated_readback",
    local_gate:$gate,
