...
REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-packet-persistence-denial-terminal-no-persistence-report.sh"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-packet-persistence-denial-terminal-no-persistence-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable authority packet persistence-denial terminal no-persistence report: $REPORT"
...
target_json="$tmpdir/authority-packet-persistence-denial-terminal-no-persistence.json"

"$SOURCE_REPORT" >"$source_json" || fail "failed to render authority packet persistence-denial retention/replay source report"
jq -e . "$source_json" >/dev/null || fail "source report did not render valid JSON"
HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_PACKET_PERSISTENCE_DENIAL_RETENTION_REPLAY_JSON="$source_json" \
  "$REPORT" >"$target_json" || fail "failed to render authority packet persistence-denial terminal no-persistence report from cached source"
jq -e . "$target_json" >/dev/null || fail "target report did not render valid JSON"

rg -q 'Controlled Live Evidence Receipt Store Local Evidence Acceptance Authority Packet Persistence Denial Terminal No Persistence Readback' "$DOC" \
  || fail "architecture note must document the terminal no-persistence readback surface"
rg -q 'controlled live evidence receipt store local evidence acceptance authority packet persistence denial terminal no-persistence readback' "$DOC" \
  || fail "architecture note must document the plain-language local authority packet terminal no-persistence boundary"
rg -q 'no terminal closeout recording, terminal closeout persistence, terminal closeout acceptance, terminal closeout authority, packet persistence attempt recording, packet persistence denial receipt persistence, operator packet send, operator packet persistence, local evidence acceptance authority, authority decision recording, non-authority receipt persistence, local evidence acceptance, local evidence acceptance recording, evidence acceptance recording, evidence recording, receipt-store write-attempt recording, receipt store write, receipt persistence, ledger write, event-log write, SQLite write, credential read, Native POST mutation, Telegram transport mutation, gateway/auth mutation, channel send, provider call, model call, replay execution, rollback, kill-switch rehearsal execution, kill-switch mutation, package, release, Public GA promotion, or live execution' "$DOC" \
  || fail "architecture note must document the closed terminal no-persistence boundary"

jq -e '
  .runtime == "hepta"
  and .surface == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_terminal_no_persistence_readback"
  and .status == "ready_blocked"
  and .gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_terminal_no_persistence_readback_gate"
  and .schema_version == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_terminal_no_persistence_readback_v1"
  ...
  and .source_packet_persistence_denial_receipt_persisted_count == 0
  ...
  and .terminal_collection_id == "controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-packet-persistence-denial-terminal-no-persistence"
  and .terminal_collection_route == "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/persistence-denial/terminal-no-persistence"
  ...
  and (.blockers | index("local_evidence_acceptance_recording_disabled")) != null
  ...
  and (.entries | all(
    (.id | startswith("evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_terminal_no_persistence_"))
    and (.source_retention_replay_entry_id | startswith("evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_retention_replay_without_persistence_"))
    ...
    and (.source_retention_policy_id | startswith("local-evidence-acceptance-authority-packet-persistence-denial-retention-policy:"))
    and (.source_replay_idempotency_key | startswith("local-evidence-acceptance-authority-packet-persistence-denial-replay-idempotency:"))
    and (.source_zero_effect_digest | startswith("sha256:local-evidence-acceptance-authority-packet-persistence-denial-retention-replay-zero-effect:"))
    and (.terminal_closeout_id | startswith("local-evidence-acceptance-authority-packet-persistence-denial-terminal-no-persistence:"))
    and (.terminal_closeout_key | startswith("terminal-no-persistence:local-evidence-acceptance-authority-packet-persistence-denial:"))
    and (.terminal_closeout_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/persistence-denial/terminal-no-persistence/"))
    and .terminal_reason == "local_evidence_acceptance_authority_packet_persistence_denied_retention_replay_projected_no_persistence_authority"
    and .terminal_state == "terminal_no_persistence"
    and .observed_state == "local_evidence_acceptance_authority_packet_persistence_denial_terminal_no_persistence_closed"
  ...
  and any(.entries[]; .source_blocker_id == "operator_live_approval_missing" and .terminal_closeout_key == "terminal-no-persistence:local-evidence-acceptance-authority-packet-persistence-denial:controlled-live-evidence-receipt-store:operator_live_approval_missing")
  and .recommended_next_gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_boundary_readback_without_recording"
...
(
  ...
  cargo test -p hepta-runtime \
    local_authority_packet_persistence_denial_terminal --lib
)

printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-packet-persistence-denial-terminal-no-persistence-gate: PASS: local evidence acceptance authority packet persistence-denial branch is terminally read back without persistence or live execution\n'
