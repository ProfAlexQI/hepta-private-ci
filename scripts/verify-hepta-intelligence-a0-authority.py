...
def main() -> int:
    ...
    require(
        current.get("repository") == EXPECTED_REPOSITORY, "current repository mismatch"
    )
    require(current.get("active_phase", {}).get("id") == "A0", "A0 is not active")
    require(
        current.get("current_truth", {}).get("q0_executable_qualified") is True,
        "Q0 qualification missing",
    )
    require(
        current.get("current_truth", {}).get("qualified_candidate") is True,
        "qualified candidate missing",
    )
    require(
    ...
    require(
        current.get("current_truth", {}).get("qualified") is False,
        "product capability prematurely qualified",
    )
    require(
        current.get("current_truth", {}).get("full_repository_merge_green") is False,
        "merge-green was fabricated",
    )
    ...
    require(
        {job.get("job_id") for job in jobs} == {99101597686, 99101597800, 99105393694},
        "Q0 jobs drift",
    )
    require(
        all(
            job.get("runner_id", 0) > 0 and job.get("steps_non_empty") is True
            for job in jobs
        ),
        "Q0 has non-executable job",
    )
    artifacts = q0_observation.get("artifacts")
    require(
        isinstance(artifacts, list) and len(artifacts) == 3,
        "Q0 artifact surface incomplete",
    )
    require(
        {item.get("artifact_id") for item in artifacts}
        == {9715334789, 9715221566, 9715623771},
        "Q0 artifacts drift",
    )
    require(
        all(
            re.fullmatch(r"sha256:[0-9a-f]{64}", str(item.get("digest", "")))
            for item in artifacts
        ),
        "Q0 artifact digest invalid",
    )
    require(
        q0.get("conclusion", {}).get("q0_executable_qualified") is True,
        "Q0 pair conclusion missing",
    )
    require(
        q0.get("conclusion", {}).get("runtime_capability_qualified") is False,
        "Q0 crossed runtime boundary",
    )
    ...
    require(
        observed_binding == hashlib.sha256(canonical(binding)).hexdigest(),
        "Q0 summary binding mismatch",
    )

    require(
        document.get("repository") == EXPECTED_REPOSITORY,
        "document registry repository mismatch",
    )
    registered_inputs = {
    ...
    require(
        len(registered_inputs) == 4, "document registry canonical inputs incomplete"
    )
    require(
        all_false(document.get("authority")),
        "document registry authority must remain false",
    )
    ...
    require(
        p0.get("candidate_qualified") is True,
        "P0 source candidate qualification not recorded",
    )
    require(
        p0.get("wired") is False and p0.get("runtime_qualified") is False,
        "P0 gained runtime capability",
    )
    ...
    for capability_id, entry in by_id.items():
        require(
            ...
            f"{capability_id} authority must remain false",
        )
    require(
        all_false(capabilities.get("authority")),
        "capability registry authority must remain false",
    )
    ...
    require(
        [item.get("pr_number") for item in canonical_entries]
        == [7, 13, 14, 16, 21, 23, 29],
        "canonical stack order drift",
    )
    require(
        canonical_stack.get("candidate_head") == EXPECTED_PARENT,
        "canonical candidate head drift",
    )
    require(
        canonical_stack.get("candidate_tree") == EXPECTED_Q0_TREE,
        "canonical candidate tree drift",
    )
    require(
        all(
            item.get("merged") is False and item.get("draft") is True
            for item in canonical_entries
        ),
        "canonical PR state drift",
    )
    require(
        all(
            stack.get("canonical") is False for stack in pr_stack.get("side_stacks", [])
        ),
        "side stack gained canonical status",
    )
    for item in pr_stack.get("external_unmerged_dependencies", []):
        require(
            item.get("canonical") is False and item.get("authority") is False,
            "external PR gained authority",
        )
    require(
        all_false(pr_stack.get("authority")), "PR stack authority must remain false"
    )

    require(
        integration.get("repository") == EXPECTED_REPOSITORY,
        "integration repository mismatch",
    )
    require(integration.get("branch") == EXPECTED_BRANCH, "integration branch mismatch")
    require(
        integration.get("expected_parent") == EXPECTED_PARENT,
        "integration expected parent mismatch",
    )
    allowed = integration.get("allowed_changed_paths")
    require(
        isinstance(allowed, list) and allowed == sorted(set(allowed)),
        "integration allowlist invalid",
    )
    forbidden_prefixes = (
        "codex-rs/",
        "migrations/",
    ...
    require(
        not any(path.startswith(forbidden_prefixes) for path in allowed),
        "runtime/product path in A0 allowlist",
    )
    freeze = integration.get("source_freeze", {})
    require(
        freeze.get("rust_runtime_changes_allowed") is False,
        "runtime source freeze disabled",
    )
    ...
    require(
        all_false(integration.get("authority")),
        "integration authority must remain false",
    )

    require(
        evidence.get("q0_current_evidence", {}).get("q0_executable_qualified") is True,
        "evidence index lost Q0 qualification",
    )
    require(
        evidence.get("q0_current_evidence", {}).get("full_repository_merge_green")
        is False,
        "evidence index fabricated merge-green",
    )
    require(
    ...
    second = subprocess.check_output(
        [sys.executable, str(TRUTH), "--compact"], cwd=ROOT
    )
    require(first == second, "current-truth output is not byte deterministic")
    truth_value = json.loads(first)
    require(
        truth_value.get("schema") == "hepta_intelligence_current_truth_v1",
        "truth schema mismatch",
    )
    require(
        truth_value.get("q0", {}).get("qualified_candidate") is True,
        "truth lost Q0 candidate qualification",
    )
    require(
        truth_value.get("q0", {}).get("runtime_capability_qualified") is False,
        "truth crossed runtime boundary",
    )
    require(
        all_false(truth_value.get("authority")), "truth authority must remain false"
    )
    ...
    if git_dir.exists():
        ...
        require(
            changed_files == allowed, f"changed-path surface mismatch: {changed_files}"
        )
