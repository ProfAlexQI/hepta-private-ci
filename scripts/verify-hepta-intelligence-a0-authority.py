...
def load(path: Path) -> dict[str, Any]:
    try:
        ...
        fail(f'cannot parse {display_path(path)}: {exc}')
    need(isinstance(value, dict), f'{display_path(path)} must contain an object')
    return value
...
def _read_bounded(response: Any) -> bytes:
    ...
    need(len(data) <= limit, 'HTTP response exceeded 32 MiB')
    return data
...
def api(path_or_url: str) -> bytes:
    ...
    req = Request(
        ...
        headers={
            ...
            'User-Agent': 'hepta-a0-evidence-v2',
        },
    )
    ...
    try:
        ...
        need(exc.code in {301, 302, 303, 307, 308}, f'GitHub API request failed: {exc.code}')
        need(
            '/actions/artifacts/' in parsed.path and parsed.path.endswith('/zip'),
            'unexpected GitHub API redirect',
        )
        location = exc.headers.get('Location') if exc.headers is not None else None
        need(isinstance(location, str) and bool(location), 'artifact redirect missing')
        target = urlparse(location)
        target_host = (target.hostname or '').lower()
        allowed = (
            target_host.endswith('.blob.core.windows.net')
            or target_host.endswith('.actions.githubusercontent.com')
            or target_host.endswith('.githubusercontent.com')
        )
        need(
            target.scheme == 'https'
            and bool(target_host)
            and target_host != parsed.hostname
        ...
        with urlopen(signed, timeout=60) as response:
            return _read_bounded(response)
...
def validate_artifact(artifact: dict[str, Any], run_id: int, head: str) -> None:
    ...
    need(isinstance(size, int) and 0 < size <= 32 * 1024 * 1024, 'artifact archive size')
...
def output_root(raw: str) -> Path:
    root = Path(raw).resolve()
    need(ROOT in root.parents, 'output root escaped repository')
    return root
...
def main() -> int:
    ...
    if len(sys.argv) == 3 and sys.argv[1] == '--produce-executable-evidence':
        return produce(output_root(sys.argv[2]))
    if len(sys.argv) == 3 and sys.argv[1] == '--attest-executable-evidence':
        return attest(output_root(sys.argv[2]))
