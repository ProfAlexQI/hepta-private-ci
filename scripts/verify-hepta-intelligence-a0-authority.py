...
def main() -> int:
    ...
    if (root / ".git").exists():
        ...
        module.require(
            changed == module.ALLOWED_PATHS, f"changed-path surface mismatch: {changed}"
        )
        ...
        env_branch = os.environ.get("GITHUB_HEAD_REF") or os.environ.get(
            "GITHUB_REF_NAME"
        )
        if env_branch:
            module.require(
                env_branch == module.A0_BRANCH, f"branch mismatch: {env_branch}"
            )
    receipt = {
        ...
        "candidate": {
            "branch": module.A0_BRANCH,
            "head": head,
            "tree": tree,
            "parent": module.Q0_HEAD,
        },
