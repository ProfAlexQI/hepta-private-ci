...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing memory write execution activation command result receipt no-persistence boundary source text: $label" >&2
    exit 1
  fi
}
...
ENDPOINT="/api/hepta-memory-live-mutation-operator-write-execution-activation-command-result-receipt-no-persistence-boundary"
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes memory write execution activation command result receipt no-persistence boundary"
require_source_text "$ROUTE_REGISTRY_SOURCE" \
  ...
  "memory write execution activation command result receipt no-persistence boundary endpoint constant"
require_source_text "$ROUTE_REGISTRY_SOURCE" "$ENDPOINT" \
  "memory write execution activation command result receipt no-persistence boundary endpoint path"
require_source_text "$ROUTE_REGISTRY_SOURCE" "$SOURCE_COMMAND" \
  "memory write execution activation command result receipt no-persistence boundary source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "hepta_memory_live_mutation_operator_write_execution_activation_command_result_receipt_no_persistence_boundary_report" \
  "memory write execution activation command result receipt no-persistence boundary report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"memory_write_execution_activation_command_result_receipt_no_persistence_boundary_ready\"" \
  "memory write execution activation command result receipt no-persistence boundary ready field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"denied_by_activation_command_result_receipt_count\", 25" \
  "memory write execution activation command result receipt denied count"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "hepta_memory_write_execution_activation_command_result_receipt_no_persistence_boundary_endpoint_blocks_receipt_persistence" \
  "focused memory write execution activation command result receipt no-persistence boundary unit test"

TEST_LOG="$(mktemp /tmp/hepta-memory-write-execution-activation-command-result-receipt-no-persistence-boundary-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  ...
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_write_execution_activation_command_result_receipt_no_persistence_boundary_ready == true
    ...
    and .source_activation_command_noop_handoff_boundary_ready == true
    ...
    and .denied_by_activation_command_result_receipt_count == 25
    and (.denied_by_activation_command_result_receipt | length) == 25
    and .allowed_next_actions[0].action == "run_memory_write_execution_activation_command_result_receipt_no_persistence_boundary_require_live_gate"
    and .allowed_next_actions[1].action == "prepare_memory_write_execution_activation_command_result_receipt_replay_idempotency_denial_boundary"
...
jq -n \
  ...
  '{
    ...
    status:"ready",
    base_url:$base_url,
    gate:"hepta_memory_live_mutation_operator_write_execution_activation_command_result_receipt_no_persistence_boundary_route_gate",
    endpoint:$endpoint,
    ...
    memory_write_execution_activation_command_result_receipt_no_persistence_boundary_ready:true,
...
echo "Hepta memory write execution activation command result receipt no-persistence boundary route gate passed"
