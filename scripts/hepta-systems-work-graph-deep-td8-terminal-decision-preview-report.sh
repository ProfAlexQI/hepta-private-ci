...
prior_report_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-ack-replay-preview-report.sh"
prior_gate_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-ack-replay-preview-gate.sh"
...
rust_module="codex-rs/hepta-runtime/src/wg_deep_td8_terminal_decision_preview.rs"
report_script="scripts/hepta-systems-work-graph-deep-td8-terminal-decision-preview-report.sh"
gate_script="scripts/hepta-systems-work-graph-deep-td8-terminal-decision-preview-gate.sh"
...
jq -n \
  ...
  '
  def replay_ids: [range(0; 6) | "deep_td7_ack_replay_scenario_\(.)"];
  def surface_ids: [range(0; 6) | "deep_td8_terminal_decision_surface_\(.)"];
  def surface($i): {
    id: "deep_td8_terminal_decision_surface_\($i)",
    audience: (["operator", "release_owner", "auditor", "rollback_owner", "system", "external_delivery"][$i]),
    source_replay_scenario_ids: replay_ids,
    decision_visibility: (if $i == 5 then "external_delivery_echo_denied" else "local_deep_td8_terminal_decision_read_only" end),
  ...
  def denial($i): {
    id: "deep_td8_terminal_decision_non_promotion_denial_\($i)",
  ...
  def authority_guard($i): {
    id: "deep_td8_terminal_decision_authority_guard_\($i)",
    required_fields: ["authorityGuardId", "authorityHash", "authorityGranted"],
    authority_grant_allowed: false
  };
  def release_guard($i): {
    id: "deep_td8_terminal_decision_release_delivery_guard_\($i)",
  ...
  def local_view($i): {
    id: "deep_td8_terminal_decision_local_view_\($i)",
  ...
  def invariant($i): {
    id: "deep_td8_terminal_decision_invariant_\($i)",
    required: true,
