...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-td3-rcptret-rbackack-td-rcptret-rbackack-td-rcptret-expiry-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-dtd8-td3-rcptret-rbackack-td-rcptret-rbackack-td-rcptret-expiry-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .source_probes.deep_td8_td3_rbackack_td2_receipt_retention_expiry.rust_module_present == true
  and .source_probes.prior_deep_td8_td3_rbackack_td2_terminal_decision_receipt_ack_replay.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_retack_td3_rcptret_rbackack_td_rcptret_rbackack_td_rcptret_expiry_preview --lib
