...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-positive-preconditions-recording-denial-receipt-retention-replay-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_positive_preconditions.rs"
LIB_SOURCE="$ROOT/codex-rs/hepta-runtime/src/lib.rs"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_POSITIVE_PRECONDITIONS_RECORDING_DENIAL_RECEIPT_POSITIVE_PRECONDITIONS_2026-07-08.md"

fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-positive-preconditions-recording-denial-receipt-positive-preconditions-report: FAIL: %s\n' "$1" >&2
  exit 1
}
...
if grep -q 'controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_positive_preconditions_readback_without_persistence_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_POSITIVE_PRECONDITIONS_RECORDING_DENIAL_RECEIPT_RETENTION_REPLAY_JSON:-}"
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-positive-preconditions-recording-denial-receipt-positive-preconditions-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_POSITIVE_PRECONDITIONS_RECORDING_DENIAL_RECEIPT_POSITIVE_PRECONDITIONS_2026-07-08.md" \
  --arg route "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-decision-recording/recording-denial-receipts/positive-preconditions" \
  '
  def hyphen_id($id): $id | gsub("_"; "-");
  def count_true($entries; $key): $entries | map(select(.[$key] == true)) | length;
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_positive_preconditions_without_persistence_" + .source_blocker_id),
    source_blocker_id,
    ...
    positive_precondition_set_id:("local-evidence-acceptance-authority-decision-recording-positive-preconditions-recording-denial-receipt-positive-preconditions:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    positive_precondition_route:($route + "/" + hyphen_id(.source_blocker_id)),
    recording_denial_receipt_persistence_grant_precondition_id:("denial-receipt-persistence-grant-required:controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-decision-recording-positive-preconditions-recording-denial:" + .source_blocker_id),
    observed_state:"local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_positive_preconditions_projected_without_persistence",
  ...
  (count_true($entries; "persistence_authority_required")) as $persistence_authority_required_count |
  (count_true($entries; "persistence_authority_present")) as $persistence_authority_present_count |
  (count_true($entries; "operator_persistence_approval_required")) as $operator_persistence_approval_required_count |
  (count_true($entries; "operator_persistence_approval_present")) as $operator_persistence_approval_present_count |
  ...
  (count_true($entries; "recording_denial_receipt_persistence_grant_required")) as $recording_denial_receipt_persistence_grant_required_count |
  (count_true($entries; "recording_denial_receipt_persistence_grant_present")) as $recording_denial_receipt_persistence_grant_present_count |
  (count_true($entries; "atomic_append_required")) as $atomic_append_required_count |
  (count_true($entries; "atomic_append_enabled")) as $atomic_append_enabled_count |
  (count_true($entries; "post_persist_readback_required")) as $post_persist_readback_required_count |
  (count_true($entries; "post_persist_readback_persisted")) as $post_persist_readback_persisted_count |
  ...
  ($src.retention_replay_readback_ready == true
    ...
    and $recording_denial_receipt_persistence_grant_required_count == 7
    and $recording_denial_receipt_persistence_grant_present_count == 0
  ...
  {
    runtime:"hepta",
    surface:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_positive_preconditions_readback_without_persistence",
    status:(if $ready then "ready_blocked" else "blocked" end),
    gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_positive_preconditions_readback_without_persistence_gate",
    schema_version:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_positive_preconditions_readback_without_persistence_v1",
    ...
    recording_denial_receipt_persistence_grant_required_count:$recording_denial_receipt_persistence_grant_required_count,
    recording_denial_receipt_persistence_grant_present_count:$recording_denial_receipt_persistence_grant_present_count,
    ...
    entries:$entries,
    next_actions:["controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_persistence_denial_readback_without_persistence"],
    recommended_next_gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_positive_preconditions_recording_denial_receipt_persistence_denial_readback_without_persistence",
    local_gate:$gate,
