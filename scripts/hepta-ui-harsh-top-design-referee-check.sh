...
CONTROL_STYLE_SOURCE="$REPO_ROOT/apps/hepta-control-ui/styles.css"
...
if grep -Eq 'overflow-wrap:[[:space:]]*anywhere|word-break:[[:space:]]*(break-word|break-all)' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet still allows arbitrary word splitting in visible microcopy" >&2
  exit 1
fi

if ! grep -Fq '/*mslgtiblgbmslgstslgrslgbsprsp*/[data-view=chat] .telegram-chat-shell :is([data-control-ui-micro-surface],.tg-message small,.tg-thread-header p,[data-chat-shortcut-hint],.tg-chat-item__topline span,.tg-rail-status__item,.tg-bubble>span)' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing compact thread-header/composer/message-meta/chat-row readable text protection" >&2
  exit 1
fi

if ! grep -Fq 'input[data-control-ui-rail-search-input=light-glass]::placeholder{color:#456!important;opacity:1;text-shadow:var(--x)}' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing compact search/composer placeholder readability protection" >&2
  exit 1
fi

if ! grep -Fq '/*cmvctlgcplgcpsgttlg*/.tg-compose-footer .tg-thread-command-menu__panel' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing composer menu viewport guard marker" >&2
  exit 1
fi

if ! grep -Fq 'bottom:calc(100% + 8px)' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet does not force composer command menu to open upward inside the viewport" >&2
  exit 1
fi

if ! grep -Fq '[data-control-ui-thread-tools-panel=light-glass],[data-control-ui-composer-tools-panel=light-glass]{background:#f2fbff!important;color:#123!important}' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing compact chat-row glass contrast protection" >&2
  exit 1
fi

if ! grep -Fq '.command-palette[data-control-ui-command-palette-surface="light-glass"]' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing command palette light glass surface protection" >&2
  exit 1
fi

if ! grep -Fq '/*cptrmlg*/[data-view=chat] :is(.tg-icon-action[data-control-ui-command-palette-trigger=light-glass],[data-control-ui-thread-tools-trigger=light-glass],[data-control-ui-composer-tools-trigger=light-glass],[data-control-ui-row-menu-trigger=light-glass])' "$CONTROL_STYLE_SOURCE"; then
...
if ! grep -Fq 'data-control-ui-command-palette-input=light-glass' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing command palette input light-glass marker" >&2
  exit 1
fi

if ! grep -Fq 'data-control-ui-command-palette-result=light-glass' "$CONTROL_STYLE_SOURCE"; then
...
if ! grep -Fq 'min-height:44px' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing 44px folder chip touch target protection" >&2
  exit 1
fi

if grep -Eq '\.tg-folder-chip[^}]*min-height:[[:space:]]*30px' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI folder chips still use compact 30px touch targets" >&2
  exit 1
fi

if ! grep -Fq '.tg-pin-toggle,.tg-row-menu-toggle{width:44px;height:44px' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI row menu toggle is missing 44px touch target protection" >&2
  exit 1
fi

if ! grep -Fq '.tg-row-action{display:inline-flex;align-items:center;min-height:44px' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI row action items are missing 44px submenu item protection" >&2
  exit 1
fi

if ! grep -Fq '.tg-row-action-popover{position:absolute;top:54px;right:10px' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI row action popover is missing light glass stylesheet marker" >&2
  exit 1
fi

if ! grep -Fq 'background:#f2fbff' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI row action popover is missing light glass background protection" >&2
  exit 1
fi

if ! grep -Fq 'data-control-ui-thread-tools-panel=light-glass' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing thread tools light glass marker" >&2
  exit 1
fi

if ! grep -Fq 'data-control-ui-composer-tools-panel=light-glass' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing composer tools light glass marker" >&2
  exit 1
fi

if ! grep -Fq '.tg-composer-popover{' "$CONTROL_STYLE_SOURCE"; then
  echo "Control UI stylesheet is missing composer popover light glass marker" >&2
  exit 1
fi

if ! grep -Fq '.tg-composer-popover__search{' "$CONTROL_STYLE_SOURCE"; then
...
if ! grep -Fq 'data-chat-conversation="ui-chat-agent" role="option" aria-selected="true" tabindex="0" aria-label="Hepta conversation, local review ready" title="Hepta conversation, local review ready"' "$CONTROL_HTML_SOURCE"; then
  echo "Control UI static HTML is missing semantic title/aria coverage for the Hepta chat row option" >&2
  exit 1
fi

if ! grep -Fq 'data-chat-conversation="task-queue" role="option" aria-selected="false" tabindex="0" aria-label="Actions conversation, local approval queue" title="Actions conversation, local approval queue"' "$CONTROL_RUST_SOURCE"; then
  echo "Control UI Rust renderer is missing semantic title/aria coverage for the Actions chat row option" >&2
  exit 1
fi

if grep -Eq '\.tg-icon-action[^}]*width:[[:space:]]*3[0-9]px' "$CONTROL_STYLE_SOURCE"; then
...
for source in "$CONTROL_HTML_SOURCE" "$CONTROL_RUST_SOURCE"; do
  ...
  if ! grep -Fq 'data-chat-row-menu-panel="ui-chat-agent" role="menu" aria-label="Hepta conversation actions"' "$source"; then
  ...
  if ! grep -Fq 'data-chat-row-menu-item="archive" role="menuitem" aria-label="Archive Hepta conversation" title="Archive Hepta conversation"' "$source"; then
    echo "Control UI row menu items are missing menuitem semantics and tooltip parity: $source" >&2
    exit 1
  fi
  ...
  if ! grep -Fq 'data-control-ui-thread-tools-panel="light-glass" role="menu" aria-label="Thread tools"' "$source"; then
    echo "Control UI thread tools panel is missing light-glass menu semantics: $source" >&2
    exit 1
  fi
  if ! grep -Fq 'data-control-ui-menu-item="history" href="#transcript" role="menuitem" aria-label="Open thread history" title="Open thread history"' "$source"; then
    echo "Control UI thread tools History item is missing menuitem semantics and tooltip parity: $source" >&2
    exit 1
  fi
  if ! grep -Fq 'data-control-ui-menu-item="tasks" href="#tasks" role="menuitem" aria-label="Open thread tasks" title="Open thread tasks"' "$source"; then
    echo "Control UI thread tools Tasks item is missing menuitem semantics and tooltip parity: $source" >&2
    exit 1
  fi
  if ! grep -Fq 'data-control-ui-menu-item="sessions" href="#sessions" role="menuitem" aria-label="Open thread sessions" title="Open thread sessions"' "$source"; then
    echo "Control UI thread tools Sessions item is missing menuitem semantics and tooltip parity: $source" >&2
    exit 1
  fi
  ...
  if ! grep -Fq 'data-control-ui-composer-tools-panel="light-glass" role="menu" aria-label="Composer tools"' "$source"; then
    echo "Control UI composer tools panel is missing light-glass menu semantics: $source" >&2
    exit 1
  fi
  if ! grep -Fq 'data-control-ui-composer-tool-item="reply-mode" data-control-ui-menu-item="reply-mode" role="menuitem" aria-label="Set reply mode" title="Set reply mode"' "$source"; then
    echo "Control UI composer tools Reply item is missing menuitem semantics and tooltip parity: $source" >&2
    exit 1
  fi
  if ! grep -Fq 'data-control-ui-composer-tool-item="scroll-mode" data-control-ui-menu-item="scroll-mode" data-chat-autoscroll-persisted="local-storage-contract" role="menuitem" aria-label="Set auto-scroll mode" title="Set auto-scroll mode"' "$source"; then
    echo "Control UI composer tools Scroll item is missing menuitem semantics and tooltip parity: $source" >&2
    exit 1
  fi
  ...
  if ! grep -Fq 'data-control-ui-command-palette-trigger="light-glass" href="#command-palette" aria-label="Open command palette" title="Open command palette"' "$source"; then
    echo "Control UI command palette trigger is missing light-glass marker and tooltip parity: $source" >&2
    exit 1
  fi
  if ! grep -Fq 'data-control-ui-command-palette-close="light-glass" href="#commands" aria-label="Close command palette" title="Close command palette"' "$source"; then
