...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator canary activation command no-op handoff route source text: $label" >&2
    exit 1
  fi
}

ACTIVATION_REQUEST_ROUTE_JSON="$(
...
' >/dev/null <<<"$ACTIVATION_REQUEST_ROUTE_JSON"
...
jq -e '
  ...
  and .source_activation_request_performed_count == 0
  and .activation_command_fixture_count == 10
  and .activation_command_requested_fixture_count == 10
  ...
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$ACTIVATION_COMMAND_JSON"
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command no-op handoff endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command no-op handoff endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command no-op handoff source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway activation command no-op handoff report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "activation command no-op handoff route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"activation_command_performed_count": 0' \
  "activation command performed count remains zero"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "activation command provider invocation denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"live_kg_write_performed_count": 0' \
  "activation command live KG write denied"

TEST_LOG="$(mktemp /tmp/hepta-operator-canary-activation-command-noop-route-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_enablement_operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_noop_handoff_endpoint_blocks_activation_commands \
  -- --nocapture >"$TEST_LOG"
...
report="$(
  jq -n \
    ...
    --arg gate "hepta_memory_intelligence_kg_full_enablement_operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_noop_handoff_route_gate" \
    --arg endpoint "/api/hepta-memory-intelligence-kg-full-enablement-operator-canary-controlled-request-harness-operator-review-acknowledgement-activation-command-noop-handoff" \
    --arg source_command "/hepta-memory-intelligence-kg-full-enablement-operator-canary-controlled-request-harness-operator-review-acknowledgement-activation-command-noop-handoff --json" \
    --arg native_gateway_source "$NATIVE_GATEWAY_SOURCE" \
    --arg native_gateway_sha256 "$(sha256_file "$NATIVE_GATEWAY_SOURCE")" \
    --arg test_log "$TEST_LOG" \
    --argjson activation_request_route "$ACTIVATION_REQUEST_ROUTE_JSON" \
    --argjson activation_command "$ACTIVATION_COMMAND_JSON" \
    ...
    '{
      ...
      activation_mode:"operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_noop_handoff_native_route_status",
      source_activation_request_route_ready:($activation_request_route.status == "ready"),
      source_activation_command_noop_handoff_gate:$activation_command.gate,
      source_activation_command_noop_handoff_gate_ready:$activation_command.operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_noop_handoff_ready,
      source_activation_command_noop_handoff_gate_status:$activation_command.operator_canary_controlled_request_harness_operator_review_acknowledgement_activation_command_noop_handoff_status,
      ...
      source_contract:{
        ...
        test_log:$test_log,
        contract:"hepta-native-gateway-operator-canary-controlled-request-harness-operator-review-acknowledgement-activation-command-noop-handoff-route-v1",
        source_pattern_present:true,
        compile_checked_by_tests:true,
        route_handler_present:true,
        registers_activation_command:false,
        enables_activation_command:false,
        accepts_activation_command:false,
        invokes_activation_command:false,
        dispatches_activation_command:false,
        records_command_handoff:false,
        records_command_result_receipt:false,
...
jq -e '
  ...
  and .source_activation_request_route_ready == true
  and .source_activation_command_noop_handoff_gate_ready == true
...
printf '%s\n' "$report"
echo "Hepta Memory/Intelligence/KG operator canary controlled request harness operator review acknowledgement activation command no-op handoff route gate passed"
