...
canonical_guard_accepts() {
  ...
  [ "$(line_count "$approval_status")" = "47" ] || return 1
  [ "$(line_count "$negative_status")" = "4" ] || return 1
  [ "$(line_count "$combined_status")" = "51" ] || return 1
  [ "$(printf '%s\n' "$approval_status" | sha256_digest)" = "878b02b1ae4681940810c4344f2b3ba600f6e2ee8ae4e12f67b076217ac4a9a7" ] || return 1
  [ "$(printf '%s\n' "$negative_status" | sha256_digest)" = "06a70c53825a9a9d55573a2e108e2beb7a51f78ee4faf834918a656943e8aec2" ] || return 1
  [ "$(printf '%s\n' "$combined_status" | sha256_digest)" = "ea56c6a67703b1057cbfa47b89baf43e8abb1a2189509484240543bd5a57c062" ] || return 1
