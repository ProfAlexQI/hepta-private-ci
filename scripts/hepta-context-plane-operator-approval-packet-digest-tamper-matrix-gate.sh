...
canonical_guard_accepts() {
  ...
  [ "$(line_count "$approval_status")" = "30" ] || return 1
  [ "$(line_count "$negative_status")" = "4" ] || return 1
  [ "$(line_count "$combined_status")" = "34" ] || return 1
  [ "$(printf '%s\n' "$approval_status" | sha256_digest)" = "e9b35a9ebf1f9c8f712ab4401316a6da7ef9481b1d2072c2c736019156b823dc" ] || return 1
  [ "$(printf '%s\n' "$negative_status" | sha256_digest)" = "06a70c53825a9a9d55573a2e108e2beb7a51f78ee4faf834918a656943e8aec2" ] || return 1
  [ "$(printf '%s\n' "$combined_status" | sha256_digest)" = "cfd080ffc246e2fb94c8bff5bb68d3471c2cfe6a5655592b9de67fda5b003dab" ] || return 1
...
digest_value_tamper="$(printf '%s\n' "$digest_status" | sed 's/e9b35a9ebf1f9c8f712ab4401316a6da7ef9481b1d2072c2c736019156b823dc/7a20d497af07bc06a02339a94622ceec4c917e91b218ebf4df89aa899628d994/')"
