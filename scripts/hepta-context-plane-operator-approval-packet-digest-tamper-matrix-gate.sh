...
canonical_guard_accepts() {
  ...
  [ "$(line_count "$approval_status")" = "31" ] || return 1
  [ "$(line_count "$negative_status")" = "4" ] || return 1
  [ "$(line_count "$combined_status")" = "35" ] || return 1
  [ "$(printf '%s\n' "$approval_status" | sha256_digest)" = "66d08d2b96c57b4ba6f640f94d060b281d1ff5dbb5cd2f95b6c39555e20fae99" ] || return 1
  [ "$(printf '%s\n' "$negative_status" | sha256_digest)" = "06a70c53825a9a9d55573a2e108e2beb7a51f78ee4faf834918a656943e8aec2" ] || return 1
  [ "$(printf '%s\n' "$combined_status" | sha256_digest)" = "62391bd3e70e366e8735387481dcbedb951bcdcdc7254b07a659b9b5bd5f47aa" ] || return 1
...
digest_value_tamper="$(printf '%s\n' "$digest_status" | sed 's/66d08d2b96c57b4ba6f640f94d060b281d1ff5dbb5cd2f95b6c39555e20fae99/7a20d497af07bc06a02339a94622ceec4c917e91b218ebf4df89aa899628d994/')"
