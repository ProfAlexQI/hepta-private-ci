...
canonical_guard_accepts() {
  ...
  [ "$(line_count "$approval_status")" = "251" ] || return 1
  [ "$(line_count "$negative_status")" = "4" ] || return 1
  [ "$(line_count "$combined_status")" = "255" ] || return 1
  [ "$(printf '%s\n' "$approval_status" | sha256_digest)" = "c2c3973e8a132deefcc35adf90b3e590f012e0e5752bea24087c1b1bfaf8da1c" ] || return 1
  [ "$(printf '%s\n' "$negative_status" | sha256_digest)" = "06a70c53825a9a9d55573a2e108e2beb7a51f78ee4faf834918a656943e8aec2" ] || return 1
  [ "$(printf '%s\n' "$combined_status" | sha256_digest)" = "5792030f2a92e56de01d923993a2736e9007607af960de5231ddb111b732d430" ] || return 1
...
digest_value_tamper="$(printf '%s\n' "$digest_status" | sed 's/c2c3973e8a132deefcc35adf90b3e590f012e0e5752bea24087c1b1bfaf8da1c/d2c3973e8a132deefcc35adf90b3e590f012e0e5752bea24087c1b1bfaf8da1c/')"
