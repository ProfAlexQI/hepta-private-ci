...
for term in \
  ...
  "ranked recall routing diff counter tamper" \
...
context-plane-operator-approval-packet-canonical-export-digest.approval-report-lines=116
context-plane-operator-approval-packet-canonical-export-digest.approval-report-sha256=252c3cfcf928ba94673cd7713911325cfe446af9e133abe7c126439a22f14623
...
context-plane-operator-approval-packet-canonical-export-digest.combined-report-lines=120
context-plane-operator-approval-packet-canonical-export-digest.combined-report-sha256=0b4c1c1090289c823ae464b82cd22e47c28547f4e5a3e8598b39dbac8259cae0
...
canonical_guard_accepts() {
  ...
  [ "$(line_count "$approval_status")" = "116" ] || return 1
  [ "$(line_count "$negative_status")" = "4" ] || return 1
  [ "$(line_count "$combined_status")" = "120" ] || return 1
  [ "$(printf '%s\n' "$approval_status" | sha256_digest)" = "252c3cfcf928ba94673cd7713911325cfe446af9e133abe7c126439a22f14623" ] || return 1
  [ "$(printf '%s\n' "$negative_status" | sha256_digest)" = "06a70c53825a9a9d55573a2e108e2beb7a51f78ee4faf834918a656943e8aec2" ] || return 1
  [ "$(printf '%s\n' "$combined_status" | sha256_digest)" = "0b4c1c1090289c823ae464b82cd22e47c28547f4e5a3e8598b39dbac8259cae0" ] || return 1
...
digest_value_tamper="$(printf '%s\n' "$digest_status" | sed 's/252c3cfcf928ba94673cd7713911325cfe446af9e133abe7c126439a22f14623/352c3cfcf928ba94673cd7713911325cfe446af9e133abe7c126439a22f14623/')"
...
ranked_recall_routing_diff_counter_tamper="$(printf '%s\n' "$approval_status" | sed 's/context-plane-operator-approval-packet.ranked-recall.routing-diff-shadow-only-count=4/context-plane-operator-approval-packet.ranked-recall.routing-diff-shadow-only-count=3/')"
ranked_recall_real_workload_slo_counter_tamper="$(printf '%s\n' "$approval_status" | sed 's/context-plane-operator-approval-packet.ranked-recall.real-workload-trace-total-leak-count=0/context-plane-operator-approval-packet.ranked-recall.real-workload-trace-total-leak-count=1/')"
...
assert_rejected "ranked recall routing diff counter" "$ranked_recall_routing_diff_counter_tamper" "$negative_status" "$digest_status"
assert_rejected "ranked recall real workload SLO counter" "$ranked_recall_real_workload_slo_counter_tamper" "$negative_status" "$digest_status"
...
echo "context-plane-operator-approval-packet-digest-tamper-matrix.ranked-recall-routing-diff-counter=reject"
echo "context-plane-operator-approval-packet-digest-tamper-matrix.ranked-recall-real-workload-slo-counter=reject"
