...
REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-terminal-no-persistence-report.sh"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-terminal-no-persistence-gate: FAIL: %s\n' "$1" >&2
  exit 1
}
...
HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_DENIAL_RECEIPT_PERSISTENCE_DENIAL_RETENTION_REPLAY_JSON="$source_json" \
...
rg -q 'Controlled Live Evidence Receipt Store Local Evidence Acceptance Authority Decision Recording Denial Receipt Persistence Denial Terminal No Persistence Readback' "$DOC" \
...
rg -q 'no terminal closeout recording, terminal closeout persistence, terminal closeout acceptance, terminal closeout authority, denial receipt persistence attempt recording, denial receipt persistence, authority decision recording, authority decision persistence, evidence acceptance recording, evidence recording, receipt-store write-attempt recording, receipt store write, receipt persistence, ledger write, event-log write, SQLite write, credential read, Native POST mutation, Telegram transport mutation, gateway/auth mutation, channel send, provider call, model call, replay execution, rollback, kill-switch rehearsal execution, kill-switch mutation, package, release, Public GA promotion, or live execution' "$DOC" \
  || fail "architecture note must document the closed terminal no-persistence boundary"

jq -e '
  .runtime == "hepta"
  and .surface == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback"
  and .status == "ready_blocked"
  and .gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback_gate"
  and .schema_version == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback_v1"
  ...
  and .terminal_collection_id == "controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-terminal-no-persistence"
  and .terminal_collection_route == "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-decision-recording-denial-receipts/persistence-denial/terminal-no-persistence"
  ...
  and (.entries | all(
    (.id | startswith("evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence_"))
    and (.source_retention_replay_entry_id | startswith("evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_retention_replay_without_persistence_"))
    ...
    and (.source_retention_policy_id | startswith("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-retention-policy:"))
    and (.source_replay_idempotency_key | startswith("local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-replay-idempotency:"))
    and (.source_zero_effect_digest | startswith("sha256:local-evidence-acceptance-authority-decision-recording-denial-persistence-denial-retention-replay-zero-effect:"))
    and (.terminal_closeout_id | startswith("local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-terminal-no-persistence:"))
    and (.terminal_closeout_key | startswith("terminal-no-persistence:local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial:"))
    and (.terminal_closeout_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-decision-recording-denial-receipts/persistence-denial/terminal-no-persistence/"))
    and .terminal_reason == "local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denied_retention_replay_projected_no_persistence_authority"
    and .terminal_state == "terminal_no_persistence"
    and .observed_state == "local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence_closed"
  ...
  and any(.entries[]; .source_blocker_id == "dirty_worktree_boundary" and .terminal_closeout_route == "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-decision-recording-denial-receipts/persistence-denial/terminal-no-persistence/dirty-worktree-boundary")
  and any(.entries[]; .source_blocker_id == "operator_live_approval_missing" and .terminal_closeout_key == "terminal-no-persistence:local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial:controlled-live-evidence-receipt-store:operator_live_approval_missing")
...
(
  ...
  cargo test -p hepta-runtime \
    controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_persistence_denial_terminal_no_persistence::tests::local_denial_receipt_persistence_denial_terminal --lib
)

printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-persistence-denial-terminal-no-persistence-gate: PASS: local evidence acceptance authority decision recording denial receipt persistence-denial branch is terminally read back without persistence or live execution\n'
