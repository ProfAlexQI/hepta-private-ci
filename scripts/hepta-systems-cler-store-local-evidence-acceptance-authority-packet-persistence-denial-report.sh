...
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial.rs"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-packet-persistence-denial-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable local evidence acceptance authority packet non-send report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing local evidence acceptance authority packet persistence-denial Rust source: $RUST_SOURCE"
...
if grep -q 'controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_readback_without_persistence_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_PACKET_NON_SEND_JSON:-}"
source_cache_input_present=false
source_report_render_count=0
if [[ -n "$source_json" ]]; then
  [[ -f "$source_json" ]] || fail "missing cached local evidence acceptance authority packet non-send report: $source_json"
...
jq -e . "$source_json" >/dev/null || fail "local evidence acceptance authority packet non-send source report did not render valid JSON"
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-packet-persistence-denial-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_PACKET_PERSISTENCE_DENIAL_2026-07-07.md" \
  --arg denial_reason "operator_packet_persistence_disabled_local_evidence_acceptance_authority_missing_local_receipt_store_write_disabled" \
  '
  def hyphen_id($id): $id | gsub("_"; "-");
  def count_true($entries; $key): $entries | map(select(.[$key] == true)) | length;
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_without_persistence_" + .source_blocker_id),
    source_blocker_id,
    ...
    packet_persistence_denial_id:("local-evidence-acceptance-authority-packet-persistence-denial:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    packet_persistence_denial_route:("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/persistence-denial/" + hyphen_id(.source_blocker_id)),
    packet_persistence_denial_receipt_id:("local-evidence-acceptance-authority-packet-persistence-denial-receipt:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    packet_persistence_denial_reason:$denial_reason,
    observed_state:"local_evidence_acceptance_authority_packet_persistence_denied_without_persistence",
    source_packet_unsent:.packet_unsent,
    source_send_disabled:.send_disabled,
    non_send_projected:.non_send_projected,
    ...
    local_evidence_acceptance_authority_required:.local_evidence_acceptance_authority_required,
    local_evidence_acceptance_authority_present:.local_evidence_acceptance_authority_present,
  ...
  (count_true($entries; "packet_persistence_denial_projected")) as $persistence_denial_projected_count |
  (count_true($entries; "packet_persistence_denied")) as $packet_persistence_denied_count |
  (count_true($entries; "packet_persistence_disabled")) as $packet_persistence_disabled_count |
  (count_true($entries; "packet_persistence_allowed")) as $packet_persistence_allowed_count |
  (count_true($entries; "packet_persistence_attempt_recorded")) as $packet_persistence_attempt_recorded_count |
  (count_true($entries; "packet_persisted")) as $packet_persisted_count |
  ...
  (count_true($entries; "packet_persistence_attempt_recorded")) as $send_attempt_recorded_count |
  ...
  ($src.local_evidence_acceptance_authority_packet_non_send_readback_ready == true
    ...
    and $src.non_authority_receipt_projected_count == 7
    ...
    and ($entries | all(.observed_state == "local_evidence_acceptance_authority_packet_persistence_denied_without_persistence"
      ...
      and .live_mutation_allowed == false))) as $ready |
  {
    ...
    gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_readback_without_persistence_gate",
    schema_version:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_readback_without_persistence_v1",
    plugin_id:"hepta-system@hepta-local",
    source_non_send_readback_ready:$src.local_evidence_acceptance_authority_packet_non_send_readback_ready,
    ...
    source_send_attempt_recorded_count:$src.send_attempt_recorded_count,
    ...
    source_non_authority_receipt_projected_count:$src.non_authority_receipt_projected_count,
    ...
    local_evidence_acceptance_authority_packet_persistence_denial_readback_ready:$ready,
    ...
    next_actions:[
      ...
      "keep_local_evidence_acceptance_authority_packet_unpersisted_unaccepted_unsent"
    ],
    recommended_next_gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_persistence_denial_retention_replay_readback_without_persistence",
    local_gate:$gate,
