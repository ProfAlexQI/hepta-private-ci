...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-td2-rcptret-rbackack-td3-rcptret-rbackack-replay-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-dtd8-td2-rcptret-rbackack-td3-rcptret-rbackack-replay-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .source_probes.deep_td8_td3_terminal_decision_receipt_retention_readback_acknowledgement_replay.rust_module_present == true
  and .source_probes.prior_deep_td8_td3_terminal_decision_receipt_retention_readback_acknowledgement.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_retack_td3_receipt_retention_readback_ack_replay_preview --lib

echo "Hepta WorkGraph deep td8 td3 terminal decision receipt retention readback acknowledgement replay preview gate passed"
