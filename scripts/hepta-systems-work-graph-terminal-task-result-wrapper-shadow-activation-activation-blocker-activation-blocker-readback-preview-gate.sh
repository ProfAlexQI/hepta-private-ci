...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-terminal-task-result-wrapper-shadow-activation-activation-blocker-activation-blocker-readback-preview-report.sh"

report="$(capture_json_report "hepta-work-graph-terminal-task-result-wrapper-shadow-activation-activation-blocker-activation-blocker-readback-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .runtime == "hepta"
  and .status == "ready"
  and .gate == "hepta_work_graph_terminal_task_result_wrapper_shadow_activation_activation_blocker_activation_blocker_readback_preview_gate"
  and .schema_version == "work_graph_terminal_task_result_wrapper_shadow_activation_activation_blocker_activation_blocker_readback_preview_v1"
  and .preview_mode == "read_only_terminal_task_result_wrapper_shadow_activation_activation_blocker_activation_blocker_readback_preview_no_execution"
  ...
  and (.readback_plans | all(
    ...
    and .performs_shadow_activation_blocker_activation == false
  ...
  and (.required_prior_gates[-1] == "hepta_work_graph_terminal_task_result_wrapper_shadow_activation_activation_blocker_activation_blocker_preview_gate")
  and .recommended_next_gate == "hepta_work_graph_terminal_task_result_wrapper_shadow_activation_activation_blocker_activation_blocker_drift_budget_preview_gate"
  and .ready_for_shadow_activation_activation_blocker_activation_blocker_drift_budget_preview == true
  ...
  and .source_probes.terminal_task_result_wrapper_shadow_activation_activation_blocker_activation_blocker_readback.report_script_present == true
...
cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  work_graph_terminal_task_result_wrapper_shadow_activation_activation_blocker_activation_blocker_readback --lib

echo "Hepta WorkGraph terminal TaskResult wrapper shadow activation activation blocker activation blocker readback preview gate passed"
