...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-retack-rbackacktd-rbackacktd-rcptret-rbackack-td-receipt-ack-replay-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-dtd8-retack-rbackacktd-rbackacktd-rcptret-rbackack-td-receipt-ack-replay-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .source_probes.latest_terminal_decision_receipt_ack_replay.rust_module_present == true
  and .source_probes.prior_latest_terminal_decision_receipt_ack.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_ret_ack_td_rback_ack_td_receipt_retention_readback_ack_td_receipt_retention_readback_ack_td_receipt_retention_readback_ack_td_receipt_ack_replay_preview --lib

echo "Hepta WorkGraph deep td8 latest terminal decision receipt acknowledgement replay preview gate passed"
