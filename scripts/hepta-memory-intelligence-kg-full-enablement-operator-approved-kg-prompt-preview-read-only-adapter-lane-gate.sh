...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator-approved KG prompt-preview read-only adapter lane source text: $label" >&2
    exit 1
  fi
}

CONTEXT_LANE_JSON="$(
  curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-enablement-operator-approved-hepta-intelligence-context-attachment-lane"
)"
...
jq -e '
  ...
  and .kg_prompt_preview_lane_enabled == false
  and .kg_external_adapter_read_lane_enabled == false
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway operator-approved KG prompt-preview read-only adapter lane endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '/hepta-memory-intelligence-kg-full-enablement-operator-approved-kg-prompt-preview-read-only-adapter-lane --json' \
  "native gateway operator-approved KG prompt-preview read-only adapter lane source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'hepta_memory_intelligence_kg_full_enablement_operator_approved_kg_prompt_preview_read_only_adapter_lane_report' \
  "native gateway operator-approved KG prompt-preview read-only adapter lane report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'kg_prompt_preview_lane_enabled: true,' \
  "KG prompt-preview lane enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'kg_external_adapter_read_lane_enabled: true,' \
  "KG external adapter read lane enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
...
TEST_LOG="$(mktemp /tmp/hepta-operator-approved-kg-prompt-preview-read-only-adapter-lane-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p hepta-native-gateway --lib \
  hepta_memory_intelligence_kg_full_enablement_operator_approved_kg_prompt_preview_read_only_adapter_lane_endpoint_enables_preview_adapter_lane_only \
  -- --nocapture >"$TEST_LOG"
...
report="$(
  jq -n \
    ...
    --arg gate "hepta_memory_intelligence_kg_full_enablement_operator_approved_kg_prompt_preview_read_only_adapter_lane_gate" \
    --arg endpoint "/api/hepta-memory-intelligence-kg-full-enablement-operator-approved-kg-prompt-preview-read-only-adapter-lane" \
    --arg context_lane_endpoint "/api/hepta-memory-intelligence-kg-full-enablement-operator-approved-hepta-intelligence-context-attachment-lane" \
    --arg source_command "/hepta-memory-intelligence-kg-full-enablement-operator-approved-kg-prompt-preview-read-only-adapter-lane --json" \
    --arg native_gateway_source "$NATIVE_GATEWAY_SOURCE" \
    --arg native_gateway_sha256 "$(sha256_file "$NATIVE_GATEWAY_SOURCE")" \
    --arg test_log "$TEST_LOG" \
    --argjson context_lane "$CONTEXT_LANE_JSON" \
    '{
      product:$product,
      runtime:$runtime,
      status:"ready",
      ...
      gate:$gate,
      endpoint:$endpoint,
      hepta_intelligence_context_attachment_lane_endpoint:$context_lane_endpoint,
      source_command:$source_command,
      activation_mode:"operator_approved_kg_prompt_preview_read_only_adapter_lane_status",
      live_hepta_intelligence_context_attachment_lane_status:$context_lane.status,
      live_hepta_intelligence_context_attachment_lane_ready:$context_lane.hepta_intelligence_context_attachment_lane_enabled,
      source_route_wired:true,
      source_route_count_expected:83,
      ...
      operator_authorization_scope:"kg_prompt_preview_read_only_adapter_lane_no_kg_live_write_provider_model_channel_or_public_release",
      ...
      source_contract:{
        ...
        contract:"hepta-native-gateway-operator-approved-kg-prompt-preview-read-only-adapter-lane-route-v1",
        source_pattern_present:true,
        compile_checked_by_tests:true,
        route_handler_present:true,
        renders_prompt_preview_from_report_route:false,
      ...
      blocked_by_kg_prompt_preview_read_only_adapter_lane_boundary:[
...
jq -e '
  .status == "ready"
  and .source_route_count_expected == 83
  ...
  and .supported_kg_adapter_count == 3
  and (.supported_kg_adapters | index("graphiti") != null)
  and (.supported_kg_adapters | index("neo4j") != null)
  and (.supported_kg_adapters | index("cocoindex") != null)
  and .context_injection_allowed_by_lane == false
  and .kg_live_write_lane_enabled == false
...
printf '%s\n' "$report"
echo "Hepta operator-approved KG prompt-preview read-only adapter lane gate passed"
