...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator readiness packet template packet-acceptance receipt release publication result receipt terminal distribution delivery receipt query/export/observability route source text: $label" >&2
    exit 1
  fi
}
...
QUERY_EXPORT_OBSERVABILITY_JSON="$(
  capture_json_report \
    "hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-terminal-distribution-delivery-receipt-query-export-observability-denial-gate" \
...
' >/dev/null <<<"$QUERY_EXPORT_OBSERVABILITY_JSON"
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway terminal distribution delivery receipt query/export/observability endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway terminal distribution delivery receipt query/export/observability endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway terminal distribution delivery receipt query/export/observability source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway terminal distribution delivery receipt query/export/observability report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "terminal distribution delivery receipt query/export/observability route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "terminal distribution delivery receipt query/export/observability surface count emitted"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_terminal_distribution_delivery_receipt_query_export_observability_endpoint_blocks_observability_exports' \
  "terminal distribution delivery receipt query/export/observability focused endpoint test"

TEST_LOG="$(mktemp /tmp/hepta-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-terminal-distribution-delivery-receipt-query-export-observability-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-terminal-distribution-delivery-receipt-query-export-observability-denial"
  )"
  jq -e '
    .status == "ready"
    and .route_count == 159
...
source_query_export_observability_gate_sha256="$(printf '%s' "$QUERY_EXPORT_OBSERVABILITY_JSON" | shasum -a 256 | awk '{print $1}')"
...
jq -n \
  ...
  --arg gate "hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_terminal_distribution_delivery_receipt_query_export_observability_denial_route_gate" \
  --arg endpoint "/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-terminal-distribution-delivery-receipt-query-export-observability-denial" \
  --arg source_command "/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-terminal-distribution-delivery-receipt-query-export-observability-denial --json" \
  --arg source_query_export_observability_gate_sha256 "$source_query_export_observability_gate_sha256" \
  ...
  '{
    ...
    activation_mode:"full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_terminal_distribution_delivery_receipt_query_export_observability_native_route_status",
    side_effect_free:true,
    native_route:true,
    route_enabled:true,
    source_query_export_observability_gate_ready:true,
    source_query_export_observability_gate_sha256:$source_query_export_observability_gate_sha256,
    ...
    side_effects:{
      ...
      source_gate_side_effects:{
        ...
        delivery_receipt_export_file_written:false,
        delivery_receipt_observability_metric_recorded:false,
...
echo "Hepta memory/intelligence/KG full live activation operator readiness packet template packet acceptance receipt release/publication result receipt terminal distribution delivery receipt query/export/observability denial route gate passed"
