...
CONTRACT = SCRIPTS / "run_bazel_q028_execution_context.py"
...
def read(path: Path) -> str:
    if not path.is_file():
        fail(f"missing Q0.28 source path: {path.relative_to(ROOT)}")
...
def require(text: str, expected: str, owner: str) -> None:
    if expected not in text:
        fail(f"{owner} lacks Q0.28 contract text: {expected}")
...
def main() -> None:
    for path, expected in EXPECTED_BLOBS.items():
        ...
        if observed != expected:
            fail(
                f"immutable Q0.27 execution input drifted: "
    ...
    for path in (WRAPPER, TEST, FIXTURE):
        if not path.stat().st_mode & stat.S_IXUSR:
            fail(
                "required Q0.28 launcher lost executable mode: "
                f"{path.relative_to(ROOT)}"
            )

    for expected in (
        "Q0.17-Q0.28 qualification ratchets",
    ...
    for expected in (
        ...
        require(contract, expected, "Q0.28 execution-context policy")
    ...
    for test_name in required_tests:
        require(test, test_name, "Q0.28 regression suite")
    ...
    for expected in (
        ...
        require(boundary, expected, "qualification receipt")
    ...
    print("PASS_WINDOWS_GNULLVM_Q0_28_EXECUTION_CONTEXT_SOURCE")
