...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-deep-td8-retention-ack-terminal-decision-receipt-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-deep-td8-retention-ack-terminal-decision-receipt-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .source_probes.deep_td8_retention_ack_terminal_decision_receipt.rust_module_present == true
  and .source_probes.prior_deep_td8_retention_ack_terminal_decision.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_retention_ack_terminal_decision_receipt_preview --lib
