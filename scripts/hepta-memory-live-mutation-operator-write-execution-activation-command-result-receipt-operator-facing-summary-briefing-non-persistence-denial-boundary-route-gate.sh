...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing memory write execution activation command result receipt operator summary/briefing boundary source text: $label" >&2
    exit 1
  fi
}
...
ENDPOINT="/api/hepta-memory-live-mutation-operator-write-execution-activation-command-result-receipt-operator-facing-summary-briefing-non-persistence-denial-boundary"
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes memory write execution activation command result receipt operator summary/briefing denial boundary"
require_source_text "$ROUTE_REGISTRY_SOURCE" \
  ...
  "memory write execution activation command result receipt operator summary/briefing denial boundary endpoint constant"
require_source_text "$ROUTE_REGISTRY_SOURCE" "$ENDPOINT" \
  "memory write execution activation command result receipt operator summary/briefing denial boundary endpoint path"
require_source_text "$ROUTE_REGISTRY_SOURCE" "$SOURCE_COMMAND" \
  "memory write execution activation command result receipt operator summary/briefing denial boundary source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write execution activation command result receipt operator summary/briefing denial boundary report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write execution activation command result receipt operator summary/briefing denial boundary ready field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"denied_by_activation_command_result_receipt_operator_facing_summary_briefing_count\"" \
  "memory write execution activation command result receipt operator summary/briefing denied count"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "hepta_memory_write_execution_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial_boundary_endpoint_blocks_summary_delivery" \
  "focused memory write execution activation command result receipt operator summary/briefing boundary unit test"

TEST_LOG="$(mktemp /tmp/hepta-memory-write-execution-activation-command-result-receipt-operator-summary-briefing-boundary-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  ...
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_write_execution_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial_boundary_ready == true
    and .activation_command_result_receipt_operator_facing_summary_briefing_mode == "memory_write_execution_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial"
    and .source_activation_command_result_receipt_export_query_observability_boundary_ready == true
    and .source_activation_command_result_receipt_export_query_observability_ready == true
    and .source_activation_command_result_receipt_export_query_observability_boundary_report_sha256 != ""
    ...
    and .activation_command_result_receipt_operator_summary_denied_count == 10
    and .activation_command_result_receipt_operator_briefing_denied_count == 10
    ...
    and (.activation_command_result_receipt_operator_facing_summary_briefing_surfaces | length) == 12
    ...
    and ([.activation_command_result_receipt_operator_facing_summary_briefing_fixtures[] | select(.source_export_query_observability_present == false)] | length) == 1
    and ([.activation_command_result_receipt_operator_facing_summary_briefing_fixtures[] | select(.operator_summary_requested == true)] | length) == 7
    and ([.activation_command_result_receipt_operator_facing_summary_briefing_fixtures[] | select(.operator_briefing_requested == true)] | length) == 6
    and ([.activation_command_result_receipt_operator_facing_summary_briefing_fixtures[] | select(.channel_delivery_requested == true and .telegram_send_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_operator_facing_summary_briefing_fixtures[] | select(.activation_from_summary_briefing_requested == true and .memory_write_summary_requested == true and .rollback_summary_requested == true and .provider_prompt_summary_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_operator_facing_summary_briefing_fixtures[] | select(.external_send_summary_requested == true and .install_summary_requested == true and .active_binary_summary_requested == true)] | length) == 1
    and .denied_by_activation_command_result_receipt_operator_facing_summary_briefing_count == 20
    and (.denied_by_activation_command_result_receipt_operator_facing_summary_briefing | length) == 20
    and .allowed_next_actions[0].action == "run_memory_write_execution_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial_boundary_require_live_gate"
    and .allowed_next_actions[1].action == "prepare_memory_write_execution_activation_command_result_receipt_final_operator_acknowledgement_non_acceptance_denial_boundary"
...
jq -n \
  ...
  '{
    ...
    runtime: $runtime,
    status: "ready",
    gate: "hepta_memory_live_mutation_operator_write_execution_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial_boundary_route_gate",
    endpoint: $endpoint,
    ...
    records_summary_or_briefing: false,
