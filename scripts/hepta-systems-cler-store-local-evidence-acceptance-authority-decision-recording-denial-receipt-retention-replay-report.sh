...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_retention_replay.rs"
LIB_SOURCE="$ROOT/codex-rs/hepta-runtime/src/lib.rs"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_DENIAL_RECEIPT_RETENTION_REPLAY_2026-07-07.md"

fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-retention-replay-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable local authority decision-recording denial receipt report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing local authority decision-recording denial receipt retention/replay Rust source: $RUST_SOURCE"
...
if grep -q 'controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_retention_replay_readback_without_persistence_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_DENIAL_RECEIPT_JSON:-}"
source_cache_input_present=false
source_report_render_count=0
if [[ -n "$source_json" ]]; then
  [[ -f "$source_json" ]] || fail "missing cached local authority decision-recording denial receipt report: $source_json"
  source_cache_input_present=true
else
  source_json="$tmpdir/local-authority-decision-recording-denial-receipt.json"
  "$SOURCE_REPORT" >"$source_json" || fail "failed to render local authority decision-recording denial receipt source report"
...
jq -n \
  ...
  --arg gate_script "scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-retention-replay-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_AUTHORITY_DECISION_RECORDING_DENIAL_RECEIPT_RETENTION_REPLAY_2026-07-07.md" \
  --arg collection_id "controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-decision-recording-denial-receipt-retention-replay" \
  --arg collection_route "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-decision-recording-denial-receipts/retention-replay" \
  '
  def hyphen_id($id): $id | gsub("_"; "-");
  def count_true($entries; $key): $entries | map(select(.[$key] == true)) | length;
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_retention_replay_without_persistence_" + .source_blocker_id),
    source_blocker_id,
    ...
    source_recording_boundary_id,
    source_recording_boundary_route,
    ...
    source_authority_decision_record_schema_version,
    source_authority_decision_idempotency_key,
    retention_policy_id:("local-evidence-acceptance-authority-decision-recording-denial-receipt-retention-policy:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    retention_policy_route:($collection_route + "/retention/" + hyphen_id(.source_blocker_id)),
    expiry_guard_id:("local-evidence-acceptance-authority-decision-recording-denial-receipt-expiry-guard:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    replay_key:("local-evidence-acceptance-authority-decision-recording-denial-receipt-replay-key:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    replay_idempotency_key:("local-evidence-acceptance-authority-decision-recording-denial-receipt-replay-idempotency:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    ...
    garbage_collection_denial_id:("local-evidence-acceptance-authority-decision-recording-denial-receipt-gc-denial:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    supersession_guard_id:("local-evidence-acceptance-authority-decision-recording-denial-receipt-supersession-guard:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    zero_effect_digest:("sha256:local-evidence-acceptance-authority-decision-recording-denial-receipt-retention-replay-zero-effect:" + .source_blocker_id),
    retention_state:"projected_not_persisted",
    replay_state:"projected_not_written",
    observed_state:"local_evidence_acceptance_authority_decision_recording_denial_receipt_retention_replay_projected_without_persistence",
    source_denial_receipt_attached:.denial_receipt_projected,
    source_recording_boundary_attached,
    ...
    source_denial_receipt_projected:.denial_receipt_projected,
    source_denial_receipt_persisted:.denial_receipt_persisted,
  ...
  (count_true($entries; "source_recording_boundary_attached")) as $source_recording_boundary_attached_entry_count |
  ...
  (count_true($entries; "source_packet_persistence_denial_receipt_attached")) as $source_packet_persistence_denial_receipt_attached_entry_count |
  (count_true($entries; "source_non_send_readback_attached")) as $source_non_send_readback_attached_entry_count |
  (count_true($entries; "source_authority_packet_attached")) as $source_authority_packet_attached_entry_count |
  (count_true($entries; "source_authority_decision_request_attached")) as $source_authority_decision_request_attached_entry_count |
  (count_true($entries; "source_non_authority_receipt_attached")) as $source_non_authority_receipt_attached_entry_count |
  ...
  ($src.local_evidence_acceptance_authority_decision_recording_denial_receipt_readback_ready == true
    ...
    and $source_denial_receipt_attached_count == 7) as $ready |
  {
    runtime:"hepta",
    surface:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_retention_replay_readback_without_persistence",
    status:(if $ready then "ready_blocked" else "blocked" end),
    gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_retention_replay_readback_without_persistence_gate",
    gate_script:$gate_script,
    schema_version:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_retention_replay_readback_without_persistence_v1",
    plugin_id:"hepta-system@hepta-local",
    doc:$doc,
    source_denial_receipt_readback_ready:$src.local_evidence_acceptance_authority_decision_recording_denial_receipt_readback_ready,
    ...
    source_recording_boundary_attached_count:$src.source_recording_boundary_attached_count,
    ...
    source_packet_persistence_denial_receipt_attached_count:$src.source_packet_persistence_denial_receipt_attached_entry_count,
    source_non_send_readback_attached_count:$src.source_non_send_readback_attached_entry_count,
    source_authority_packet_attached_count:$src.source_authority_packet_attached_entry_count,
    source_authority_decision_request_attached_count:$src.source_authority_decision_request_attached_entry_count,
    source_non_authority_receipt_attached_count:$src.source_non_authority_receipt_attached_entry_count,
    source_authority_decision_record_id_attached_count:$src.source_authority_decision_record_id_attached_count,
    source_recording_denial_reason_projected_count:$src.recording_denial_reason_projected_count,
    source_recording_precondition_missing_count:$src.recording_precondition_missing_count,
    source_authority_decision_recording_disabled_count:$src.authority_decision_recording_disabled_count,
    ...
    source_cache_mode:(if $source_cache_input_present then "provided_source_json" else "rendered_source_report" end),
    ...
    source_recording_boundary_attached_entry_count:$source_recording_boundary_attached_entry_count,
    ...
    next_actions:[
      ...
      "keep_authority_decision_recording_denial_receipt_retention_replay_query_only_until_local_store_is_open"
    ],
    recommended_next_gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_decision_recording_denial_receipt_positive_preconditions_readback_without_persistence",
