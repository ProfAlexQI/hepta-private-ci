...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-agent-jobs-task-board-feature-flag-operator-review-precondition-non-request-readback-audit-index-non-persistence-readback-report.sh"
...
jq -e '
  ...
  and .runtime == "hepta"
  and .status == "ready"
  and .gate == "hepta_work_graph_agent_jobs_task_board_feature_flag_operator_review_precondition_non_request_readback_audit_index_non_persistence_readback_gate"
  and .schema_version == "work_graph_agent_jobs_task_board_feature_flag_operator_review_precondition_non_request_readback_audit_index_non_persistence_readback_v1"
  and .preview_mode == "operator_review_precondition_non_request_readback_audit_index_non_persistence_readback_only"
  and .source_audit_index_gate == "hepta_work_graph_agent_jobs_task_board_feature_flag_operator_review_precondition_non_request_readback_audit_index_gate"
  and .source_audit_index_entry_count == 6
  and .source_audit_index_blocker_count == 15
  and .source_required_prior_gate_count == 15
  and .readback_entry_count == 5
  and .readback_blocker_count == 16
  and .readback_scope.readback_mode == "operator_review_precondition_non_request_readback_audit_index_non_persistence_readback_only"
  ...
  and .recommended_next_gate == "hepta_work_graph_agent_jobs_task_board_feature_flag_operator_review_request_precondition_blocker_matrix_gate"
  ...
  and .ready_for_operator_review_request_precondition_blocker_matrix == true
  ...
  and .source_probes.audit_index_report_gate == "hepta_work_graph_agent_jobs_task_board_feature_flag_operator_review_precondition_non_request_readback_audit_index_gate"
...
cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  work_graph_agent_jobs_task_board_feature_flag_operator_review_precondition_non_request_readback_audit_index_non_persistence_readback --lib

echo "Hepta WorkGraph agent_jobs + task_board feature-flag operator review precondition non-request readback audit index non-persistence readback gate passed"
