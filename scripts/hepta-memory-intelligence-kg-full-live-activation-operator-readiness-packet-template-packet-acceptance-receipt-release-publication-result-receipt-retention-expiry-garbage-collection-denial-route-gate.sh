...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator readiness packet template packet-acceptance receipt release publication result receipt retention/expiry/garbage-collection route source text: $label" >&2
    exit 1
  fi
}
...
jq -e '
  ...
  and (.release_publication_result_receipt_retention_surfaces | length) == 18
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes packet acceptance receipt release publication result receipt retention/expiry/garbage-collection route"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt retention/expiry/garbage-collection endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt retention/expiry/garbage-collection endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt retention/expiry/garbage-collection source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway release publication result receipt retention/expiry/garbage-collection report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "packet acceptance receipt release publication result receipt retention/expiry/garbage-collection route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"release_publication_result_receipt_retention_surface_count": retention_surface_count' \
  "packet acceptance receipt release publication result receipt retention surface count emitted"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"release_publication_result_receipt_retention_policy_recorded_count": 0' \
  "packet acceptance receipt release publication result receipt retention policy recording denied"

TEST_LOG="$(mktemp /tmp/hepta-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-retention-route-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_retention_expiry_garbage_collection_endpoint_blocks_lifecycle_mutation \
  -- --nocapture >"$TEST_LOG"

LIVE_ROUTE_JSON='{}'
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-retention-expiry-garbage-collection-denial"
  )"
  jq -e '
    .status == "ready"
    and .route_count == 136
    ...
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_retention_expiry_garbage_collection_denial_route_enabled == true
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_retention_expiry_garbage_collection_denial_ready == true
    and .source_packet_acceptance_receipt_release_publication_result_receipt_audit_evidence_ready == true
    and .release_publication_result_receipt_retention_surface_count == 18
    and .release_publication_result_receipt_retention_attempt_count == 18
...
jq -n \
  ...
  --arg endpoint "/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-retention-expiry-garbage-collection-denial" \
  --arg source_command "/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-release-publication-result-receipt-retention-expiry-garbage-collection-denial --json" \
  ...
  '{
    ...
    status:"ready",
    base_url:$base_url,
    gate:"hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_retention_expiry_garbage_collection_denial_route_gate",
    endpoint:$endpoint,
    source_command:$source_command,
    activation_mode:"full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_retention_expiry_garbage_collection_denial_native_route_status",
    side_effect_free:true,
    native_route:true,
    route_enabled:true,
    source_retention_gate_ready:true,
    ...
    source_packet_acceptance_receipt_release_publication_result_receipt_audit_evidence_ready:$source.source_packet_acceptance_receipt_release_publication_result_receipt_audit_evidence_ready,
    release_publication_result_receipt_retention_surface_count:$source.release_publication_result_receipt_retention_surface_count,
    release_publication_result_receipt_retention_policy_recorded_count:$source.release_publication_result_receipt_retention_policy_recorded_count,
    release_publication_result_receipt_expiry_scheduler_registered_count:$source.release_publication_result_receipt_expiry_scheduler_registered_count,
    release_publication_result_receipt_garbage_collection_scan_performed_count:$source.release_publication_result_receipt_garbage_collection_scan_performed_count,
    release_publication_result_receipt_delete_performed_count:$source.release_publication_result_receipt_delete_performed_count,
    release_publication_result_receipt_compaction_performed_count:$source.release_publication_result_receipt_compaction_performed_count,
    release_publication_result_receipt_retention_activation_authority_derived_count:$source.release_publication_result_receipt_retention_activation_authority_derived_count,
    release_publication_result_receipt_retention_live_execution_allowed_count:$source.release_publication_result_receipt_retention_live_execution_allowed_count,
...
echo "Hepta memory/intelligence/KG full live activation operator readiness packet template packet acceptance receipt release/publication result receipt retention/expiry/garbage-collection denial route gate passed"
