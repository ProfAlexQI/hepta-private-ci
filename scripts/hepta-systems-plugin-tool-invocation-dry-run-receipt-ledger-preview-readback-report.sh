...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-plugin-tool-invocation-feature-gated-read-only-status-dry-run-readback-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/hepta_systems_plugin_tool_invocation_dry_run_receipt_ledger_preview_readback.rs"
...
fail() {
  printf 'hepta-systems-plugin-tool-invocation-dry-run-receipt-ledger-preview-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable plugin tool invocation feature-gated dry-run report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing plugin tool invocation dry-run receipt ledger preview Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing plugin tool invocation dry-run receipt ledger preview architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the plugin tool invocation dry-run receipt ledger preview report"
fi

tmpdir="$(mktemp -d)"
trap 'rm -rf "$tmpdir"' EXIT

"$SOURCE_REPORT" >"$tmpdir/source.json" || fail "failed to render plugin tool invocation feature-gated dry-run report"
jq -e . "$tmpdir/source.json" >/dev/null || fail "plugin tool invocation feature-gated dry-run report did not render valid JSON"

lib_export_present=false
if grep -q 'hepta_systems_plugin_tool_invocation_dry_run_receipt_ledger_preview_readback_report' "$LIB_SOURCE"; then
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-plugin-tool-invocation-dry-run-receipt-ledger-preview-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_PLUGIN_TOOL_INVOCATION_DRY_RUN_RECEIPT_LEDGER_PREVIEW_READBACK_2026-06-30.md" \
  '
  def suffix($kind):
  ...
  def receipt_preview_id($kind):
    if selected($kind) then "dry-run-receipt-preview:hepta-system:local-mcp:read-only-denied"
    else "dry-run-receipt-preview:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def receipt_preview_digest($kind):
    if selected($kind) then "dry-run-receipt-preview-digest:hepta-system:local-mcp:read-only-denied"
    else "dry-run-receipt-preview-digest:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def ledger_preview_id($kind):
    if selected($kind) then "ledger-preview:hepta-system:local-mcp:dry-run-read-only-denied"
    else "ledger-preview:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def ledger_preview_digest($kind):
    if selected($kind) then "ledger-preview-digest:hepta-system:local-mcp:dry-run-read-only-denied"
    else "ledger-preview-digest:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def policy_anchor_id($kind): "policy-denial-anchor:hepta-system:" + suffix($kind) + ":deny-no-invocation";
  def approval_anchor_id($kind): "approval-denial-anchor:hepta-system:" + suffix($kind) + ":no-request";
  def receipt_anchor_id($kind): "receipt-projection-anchor:hepta-system:" + suffix($kind) + ":no-persistence";
  def dry_run_idempotency_anchor_id($kind):
    if selected($kind) then "dry-run-idempotency-anchor:hepta-system:local-mcp:read-only-denied"
    else "dry-run-idempotency-anchor:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def preview_receipt_id($kind):
    if selected($kind) then "dry-run-ledger-preview-receipt:hepta-system:local-mcp:read-only-denied"
    else "dry-run-ledger-preview-receipt:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def preview_idempotency_key($kind):
    if selected($kind) then "dry-run-ledger-preview-idempotency:hepta-system:local-mcp:read-only-denied"
    else "dry-run-ledger-preview-idempotency:hepta-system:" + suffix($kind) + ":not-selected"
    end;
  def entry($source_entry):
    ($source_entry.contribution_kind) as $kind |
    {
      ...
      source_feature_gate_id:$source_entry.feature_gate_id,
      source_dry_run_receipt_id:$source_entry.first_dry_run_receipt_id,
      source_receipt_projection_id:$source_entry.receipt_projection_id,
      source_policy_denial_id:$source_entry.policy_denial_id,
      source_dry_run_idempotency_key:$source_entry.first_dry_run_idempotency_key,
      dry_run_receipt_preview_id:receipt_preview_id($kind),
      dry_run_receipt_preview_digest:receipt_preview_digest($kind),
      ledger_preview_id:ledger_preview_id($kind),
      ledger_preview_digest:ledger_preview_digest($kind),
      policy_denial_anchor_id:policy_anchor_id($kind),
      approval_denial_anchor_id:approval_anchor_id($kind),
      receipt_projection_anchor_id:receipt_anchor_id($kind),
      dry_run_idempotency_anchor_id:dry_run_idempotency_anchor_id($kind),
      first_preview_receipt_id:preview_receipt_id($kind),
      second_preview_receipt_id:preview_receipt_id($kind),
      stable_preview_receipt:true,
      unique_preview_receipt:true,
      first_preview_idempotency_key:preview_idempotency_key($kind),
      second_preview_idempotency_key:preview_idempotency_key($kind),
  ...
  ($entries | length) as $preview_entry_count |
  ...
  ($entries | map(select(.dry_run_receipt_preview_id_projected == true)) | length) as $dry_run_receipt_preview_id_projected_count |
  ($entries | map(select(.dry_run_receipt_preview_digest_projected == true)) | length) as $dry_run_receipt_preview_digest_projected_count |
  ($entries | map(select(.ledger_preview_id_projected == true)) | length) as $ledger_preview_id_projected_count |
  ($entries | map(select(.ledger_preview_digest_projected == true)) | length) as $ledger_preview_digest_projected_count |
  ($entries | map(select(.policy_denial_anchor_projected == true)) | length) as $policy_denial_anchor_projected_count |
  ($entries | map(select(.approval_denial_anchor_projected == true)) | length) as $approval_denial_anchor_projected_count |
  ($entries | map(select(.receipt_projection_anchor_projected == true)) | length) as $receipt_projection_anchor_projected_count |
  ($entries | map(select(.dry_run_idempotency_anchor_projected == true)) | length) as $dry_run_idempotency_anchor_projected_count |
  ($entries | map(select(.stable_preview_receipt == true)) | length) as $stable_preview_receipt_count |
  ($entries | map(.first_preview_receipt_id) | unique | length) as $unique_preview_receipt_count |
  ($entries | map(select(.preview_idempotency_key_projected == true)) | length) as $preview_idempotency_key_projected_count |
  ($entries | map(select(.stable_preview_idempotency_key == true)) | length) as $stable_preview_idempotency_key_count |
  ($entries | map(.first_preview_idempotency_key) | unique | length) as $unique_preview_idempotency_key_count |
  ($entries | map(select(.stable_preview_receipt == false)) | length) as $preview_receipt_mismatch_count |
  ($preview_entry_count - $unique_preview_receipt_count) as $duplicate_preview_receipt_count |
  ($entries | map(select(.stable_preview_idempotency_key == false)) | length) as $preview_idempotency_key_mismatch_count |
  ($preview_entry_count - $unique_preview_idempotency_key_count) as $duplicate_preview_idempotency_key_count |
  ...
  ($source_report.feature_gated_read_only_status_dry_run_readback_ready == true
    ...
    and $source_report.receipt_projection_count == 2
    and $source_report.stable_dry_run_receipt_count == 2
    and $source_report.unique_dry_run_receipt_count == 2
    ...
    and $live_execution_started_count == 0) as $ready |
  {
    ...
    schema_version:"hepta_systems_plugin_tool_invocation_dry_run_receipt_ledger_preview_readback_v1",
    plugin_id:"hepta-system@hepta-local",
    manifest_name:$source_report.manifest_name,
    manifest_version:$source_report.manifest_version,
    source_feature_gated_dry_run_ready:$source_report.feature_gated_read_only_status_dry_run_readback_ready,
    ...
    dry_run_receipt_preview_id_projected_count:$dry_run_receipt_preview_id_projected_count,
    dry_run_receipt_preview_digest_projected_count:$dry_run_receipt_preview_digest_projected_count,
    ledger_preview_id_projected_count:$ledger_preview_id_projected_count,
    ledger_preview_digest_projected_count:$ledger_preview_digest_projected_count,
    policy_denial_anchor_projected_count:$policy_denial_anchor_projected_count,
    approval_denial_anchor_projected_count:$approval_denial_anchor_projected_count,
    receipt_projection_anchor_projected_count:$receipt_projection_anchor_projected_count,
    dry_run_idempotency_anchor_projected_count:$dry_run_idempotency_anchor_projected_count,
    stable_preview_receipt_count:$stable_preview_receipt_count,
    unique_preview_receipt_count:$unique_preview_receipt_count,
    preview_idempotency_key_projected_count:$preview_idempotency_key_projected_count,
    stable_preview_idempotency_key_count:$stable_preview_idempotency_key_count,
    unique_preview_idempotency_key_count:$unique_preview_idempotency_key_count,
    preview_receipt_mismatch_count:$preview_receipt_mismatch_count,
    duplicate_preview_receipt_count:$duplicate_preview_receipt_count,
    preview_idempotency_key_mismatch_count:$preview_idempotency_key_mismatch_count,
    duplicate_preview_idempotency_key_count:$duplicate_preview_idempotency_key_count,
    ...
    dry_run_receipt_ledger_preview_readback_ready:$ready,
    ...
    recommended_next_gate:"hepta_systems_plugin_tool_invocation_read_only_status_dry_run_operator_packet_readback",
    local_gate:$gate,
