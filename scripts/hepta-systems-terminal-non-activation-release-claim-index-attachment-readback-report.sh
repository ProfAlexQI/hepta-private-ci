...
ATTACHMENT_REPORT="$ROOT/scripts/hepta-systems-terminal-public-distribution-final-index-terminal-non-activation-release-claim-index-report.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_TERMINAL_NON_ACTIVATION_RELEASE_CLAIM_INDEX_ATTACHMENT_READBACK_2026-06-21.md"

fail() {
  printf 'hepta-systems-terminal-non-activation-release-claim-index-attachment-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$ATTACHMENT_REPORT" ]] || fail "missing executable terminal non-activation release claim attachment report: $ATTACHMENT_REPORT"
[[ -f "$DOC" ]] || fail "missing terminal non-activation release claim attachment readback architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the terminal non-activation release claim attachment readback report"
fi

jq -n \
  --arg gate "scripts/hepta-systems-terminal-non-activation-release-claim-index-attachment-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_TERMINAL_NON_ACTIVATION_RELEASE_CLAIM_INDEX_ATTACHMENT_READBACK_2026-06-21.md" \
  '
  [
    {id:"terminal_non_activation_release_claim_index_attachment_ready", observed:true, expected:true},
    {id:"terminal_non_activation_release_claim_index_gate_present", observed:true, expected:true},
  ...
  {
    runtime:"hepta",
    surface:"terminal_non_activation_release_claim_index_attachment_readback",
    ...
    source_terminal_non_activation_release_claim_index_attachment_surface:"terminal_public_distribution_final_index_terminal_non_activation_release_claim_index",
    source_terminal_non_activation_release_claim_index_attachment_basis:"verified_terminal_non_activation_release_claim_index_attachment_snapshot",
    source_terminal_non_activation_release_claim_index_attachment_report_reexecuted:false,
    source_terminal_non_activation_release_claim_index_attachment_ready:true,
    source_terminal_non_activation_release_claim_index_attachment_blocked:true,
    terminal_non_activation_release_claim_index_attachment_readback_ready:$readback_ready,
    ...
    next_migration_step:"derive_terminal_non_activation_release_claim_index_attachment_final_index_without_release_claim_gate_invocation",
    local_gate:$gate,
    architecture_note:$doc,
    source_files:{
      terminal_non_activation_release_claim_index_attachment_report:"scripts/hepta-systems-terminal-public-distribution-final-index-terminal-non-activation-release-claim-index-report.sh"
    },
