...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-controlled-live-evidence-receipt-store-acceptance-authority-packet-readback-without-acceptance-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback.rs"
LIB_SOURCE="$ROOT/codex-rs/hepta-runtime/src/lib.rs"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_ACCEPTANCE_AUTHORITY_PACKET_NON_SEND_READBACK_2026-07-07.md"

fail() {
  printf 'hepta-systems-controlled-live-evidence-receipt-store-acceptance-authority-packet-non-send-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable acceptance authority packet source report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing acceptance authority packet non-send Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the acceptance authority packet non-send report"
fi
...
if grep -q 'controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_ACCEPTANCE_AUTHORITY_PACKET_JSON:-}"
if [[ -n "$source_json" ]]; then
  [[ -f "$source_json" ]] || fail "missing cached acceptance authority packet report: $source_json"
else
  source_json="$tmpdir/acceptance-authority-packet.json"
  "$SOURCE_REPORT" >"$source_json" || fail "failed to render acceptance authority packet report"
fi
jq -e . "$source_json" >/dev/null || fail "acceptance authority packet report did not render valid JSON"

jq -n \
  --slurpfile source "$source_json" \
  --argjson lib_export_present "$lib_export_present" \
  --arg gate "scripts/hepta-systems-controlled-live-evidence-receipt-store-acceptance-authority-packet-non-send-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_ACCEPTANCE_AUTHORITY_PACKET_NON_SEND_READBACK_2026-07-07.md" \
  --arg denial_reason "operator_packet_send_disabled_acceptance_authority_missing_evidence_acceptance_missing" \
  '
  def hyphen_id($id):
    $id | gsub("_"; "-");
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_acceptance_authority_packet_non_send_readback_" + .source_blocker_id),
    source_blocker_id,
    source_acceptance_authority_packet_id:.acceptance_authority_packet_id,
    source_acceptance_authority_packet_route:.acceptance_authority_packet_route,
    ...
    packet_non_send_readback_id:("packet-non-send-readback:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    packet_non_send_readback_route:("readback://controlled-live/evidence-receipt-store/acceptance-authority-packet/non-send/" + hyphen_id(.source_blocker_id)),
    ...
    acceptance_authority_required:.acceptance_authority_required,
    acceptance_authority_present:.acceptance_authority_present,
  ...
  ($entries | map(select(.packet_unsent == true)) | length) as $unsent_packet_count |
  ($entries | map(select(.send_disabled == true)) | length) as $send_disabled_count |
  ($entries | map(select(.send_allowed == true)) | length) as $send_allowed_count |
  ...
  ($src.acceptance_authority_packet_readback_ready == true
    ...
    and ($entries | all(.observed_state == "acceptance_authority_packet_confirmed_unsent_without_acceptance"
  ...
  {
    runtime:"hepta",
    surface:"controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback",
    status:(if $ready then "ready_blocked" else "blocked" end),
    gate:"controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback_gate",
    schema_version:"controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback_v1",
    plugin_id:"hepta-system@hepta-local",
    source_acceptance_authority_packet_ready:$src.acceptance_authority_packet_readback_ready,
    ...
    acceptance_authority_packet_non_send_readback_ready:$ready,
    ...
    recommended_next_gate:"controlled_live_evidence_receipt_store_acceptance_authority_packet_persistence_denial_readback_without_persistence",
    local_gate:$gate,
