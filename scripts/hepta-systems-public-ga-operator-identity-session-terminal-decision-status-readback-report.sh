...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-final-acknowledgement-final-index-terminal-decision-status-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session terminal decision/status attachment report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session terminal decision/status readback report" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    ...
    source_public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_surface: $source.surface,
    source_public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_ready: $source.public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_ready,
    source_public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_blocked: $source.public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_blocked,
    public_ga_operator_identity_session_terminal_decision_status_promotion_readback_ready: true,
    public_ga_operator_identity_session_terminal_decision_status_promotion_readback_blocked: true,
    public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_attached: true,
    ...
    readback_mode: "static_operator_identity_session_terminal_decision_status_snapshot_only",
    readback_check_count: 52,
    ...
    readback_blocker_count: 46,
    next_migration_step: "derive_public_ga_operator_identity_session_terminal_decision_status_promotion_final_index_without_acknowledgement",
    local_gate: "scripts/hepta-systems-public-ga-operator-identity-session-terminal-decision-status-readback-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_TERMINAL_DECISION_STATUS_READBACK_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_terminal_decision_status_attachment_report: "scripts/hepta-systems-public-ga-operator-final-acknowledgement-final-index-terminal-decision-status-report.sh"
    },
