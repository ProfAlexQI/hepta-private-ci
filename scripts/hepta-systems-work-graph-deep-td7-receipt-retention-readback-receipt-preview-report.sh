...
GATE="${BASE}_receipt_retention_expiry_readback_receipt_preview_gate"
SCHEMA="${GATE#hepta_}"
...
rust_module="codex-rs/hepta-runtime/src/wg_deep_td7_receipt_retention_readback_receipt_preview.rs"
report_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-receipt-preview-report.sh"
gate_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-receipt-preview-gate.sh"
prior_report_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-expiry-preview-report.sh"
prior_gate_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-expiry-preview-gate.sh"
...
jq -n \
  ...
  def items($prefix; $count): [range(0; $count) | {id: "\($prefix)_\(.)", required_fields: ["retentionGate", "readbackDigest", "zeroEffectHash"], hash_only: true, blocks_recording: true, blocks_external_delivery: true, required: true}];
  {
    ...
    receipts: items("deep_td7_readback_receipt"; 6),
    digest_checks: items("deep_td7_readback_digest_check"; 6),
    mismatch_denials: items("deep_td7_readback_mismatch_denial"; 7),
    receipt_guards: items("deep_td7_readback_receipt_guard"; 5),
    local_views: items("deep_td7_readback_receipt_view"; 4),
    invariants: items("deep_td7_readback_receipt_invariant"; 6),
