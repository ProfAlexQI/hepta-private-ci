...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing controlled readback receipt trusted operator packet intake precondition source text: $label" >&2
    exit 1
  fi
}

PACKET_SEPARATION_JSON="$(
  curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-enablement-runtime-provider-router-operator-approved-shadow-context-activation-execution-controlled-readback-receipt-trusted-operator-packet-separation"
)"
...
jq -e '
  ...
  and .operator_packet_identity_required == true
  and .operator_packet_intent_required == true
  and .operator_packet_signature_required == true
  and .operator_packet_session_required == true
  and .operator_packet_freshness_required == true
  and .operator_packet_scope_required == true
...
' >/dev/null <<<"$PACKET_SEPARATION_JSON"
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway controlled readback receipt trusted operator packet intake precondition endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway controlled readback receipt trusted operator packet intake precondition endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway controlled readback receipt trusted operator packet intake precondition source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway controlled readback receipt trusted operator packet intake precondition report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'trusted_operator_packet_intake_precondition_ready' \
  "trusted operator packet intake precondition readiness field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'independent_trusted_operator_packet_shape_declared: true,' \
  "trusted operator packet shape declared"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'operator_packet_signature_verified: false,' \
  "trusted operator packet signature verification denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'operator_packet_session_bound: false,' \
  "trusted operator packet session binding denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'operator_packet_freshness_verified: false,' \
  "trusted operator packet freshness verification denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'operator_packet_scope_validated: false,' \
  "trusted operator packet scope validation denied"

TEST_LOG="$(mktemp /tmp/hepta-shadow-context-activation-execution-controlled-readback-receipt-trusted-operator-packet-intake-precondition-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_shadow_execution_controlled_readback_receipt_trusted_operator_packet_intake_precondition_endpoint_is_report_only \
  -- --nocapture >"$TEST_LOG"
...
report="$(
  jq -n \
    ...
    --arg gate "hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_operator_approved_shadow_context_activation_execution_controlled_readback_receipt_trusted_operator_packet_intake_precondition_gate" \
    --arg endpoint "/api/hepta-memory-intelligence-kg-full-enablement-runtime-provider-router-operator-approved-shadow-context-activation-execution-controlled-readback-receipt-trusted-operator-packet-intake-precondition" \
    --arg packet_separation_endpoint "/api/hepta-memory-intelligence-kg-full-enablement-runtime-provider-router-operator-approved-shadow-context-activation-execution-controlled-readback-receipt-trusted-operator-packet-separation" \
    --arg source_command "/hepta-memory-intelligence-kg-full-enablement-runtime-provider-router-operator-approved-shadow-context-activation-execution-controlled-readback-receipt-trusted-operator-packet-intake-precondition --json" \
    --arg native_gateway_source "$NATIVE_GATEWAY_SOURCE" \
    --arg native_gateway_sha256 "$(sha256_file "$NATIVE_GATEWAY_SOURCE")" \
    --arg test_log "$TEST_LOG" \
    --argjson packet_separation "$PACKET_SEPARATION_JSON" \
    '{
      product:$product,
      runtime:$runtime,
      status:"ready",
      ...
      gate:$gate,
      endpoint:$endpoint,
      packet_separation_endpoint:$packet_separation_endpoint,
      source_command:$source_command,
      activation_mode:"operator_approved_shadow_context_activation_execution_controlled_readback_receipt_trusted_operator_packet_intake_precondition_report",
      ...
      source_contract:{
        ...
        contract:"hepta-native-gateway-controlled-shadow-readback-receipt-trusted-operator-packet-intake-precondition-route-v1",
      ...
      denied_by_controlled_readback_receipt_trusted_operator_packet_intake_precondition_gate:[
        "trusted_operator_packet_identity_precondition_missing",
        "trusted_operator_packet_intent_precondition_missing",
        "trusted_operator_packet_signature_precondition_missing",
        "trusted_operator_packet_session_precondition_missing",
        "trusted_operator_packet_freshness_precondition_missing",
        "trusted_operator_packet_scope_precondition_missing",
        "trusted_operator_packet_recording_denied",
        "trusted_operator_packet_persistence_denied",
        "trusted_operator_packet_acceptance_denied",
        "trusted_operator_packet_activation_authority_denied",
        "trusted_operator_packet_activation_request_denied",
        "trusted_operator_packet_activation_command_exposure_denied",
        "trusted_operator_packet_live_mutation_denied",
        "trusted_operator_packet_public_claim_denied",
...
jq -e '
  ...
  and .operator_packet_identity_verified == false
  and .operator_packet_intent_confirmed == false
  and .operator_packet_signature_verified == false
  and .operator_packet_session_bound == false
  and .operator_packet_freshness_verified == false
  and .operator_packet_scope_validated == false
...
printf '%s\n' "$report"
echo "Hepta controlled shadow readback receipt trusted operator packet intake precondition gate passed"
