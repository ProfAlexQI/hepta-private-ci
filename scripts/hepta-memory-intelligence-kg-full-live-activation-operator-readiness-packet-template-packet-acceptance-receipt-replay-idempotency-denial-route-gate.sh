...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing operator readiness packet template packet-acceptance receipt replay/idempotency route source text: $label" >&2
    exit 1
  fi
}
...
' >/dev/null <<<"$SOURCE_REPLAY_JSON"

NATIVE_GATEWAY_SOURCE="codex-rs/cli/src/native_gateway.rs"

require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'const NATIVE_GATEWAY_SOURCE_COMMAND_COUNT: usize = 126;' \
  "native gateway route/source command count includes operator readiness packet acceptance receipt replay/idempotency route"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway operator readiness packet acceptance receipt replay/idempotency endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway operator readiness packet acceptance receipt replay/idempotency endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway operator readiness packet acceptance receipt replay/idempotency source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway operator readiness packet acceptance receipt replay/idempotency report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "operator readiness packet acceptance receipt replay/idempotency route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"replay_surface_count": replay_surface_count' \
  "packet acceptance receipt replay surface count emitted"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"packet_acceptance_receipt_replayed": false' \
  "packet acceptance receipt replay denied"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  '"packet_acceptance_receipt_idempotency_key_registered": false' \
  "packet acceptance receipt idempotency key registration denied"

TEST_LOG="$(mktemp /tmp/hepta-operator-readiness-packet-template-packet-acceptance-receipt-replay-idempotency-route-tests.XXXXXX)"
cargo test --offline --manifest-path "$MANIFEST" -q -p codex-cli --lib \
  hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_replay_idempotency_endpoint_blocks_replay \
  -- --nocapture >"$TEST_LOG"

if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-replay-idempotency-denial"
  )"
  jq -e '
    .status == "ready"
    and .route_count == 126
    ...
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_replay_idempotency_denial_route_enabled == true
    and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_replay_idempotency_denial_ready == true
    and .source_packet_acceptance_receipt_ready == true
    and .source_receipt_surface_count == 8
    and .source_receipt_generated_count == 8
...
jq -e '
  ...
  and .required_marker_count == 266
  and .present_required_marker_count == 266
...
source_replay_gate_sha256="$(printf '%s' "$SOURCE_REPLAY_JSON" | shasum -a 256 | awk '{print $1}')"
...
jq -n \
  ...
  --arg gate "hepta_memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_replay_idempotency_denial_route_gate" \
  --arg endpoint "/api/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-replay-idempotency-denial" \
  --arg source_command "/hepta-memory-intelligence-kg-full-live-activation-operator-readiness-packet-template-packet-acceptance-receipt-replay-idempotency-denial --json" \
  --arg source_replay_gate_sha256 "$source_replay_gate_sha256" \
  --arg native_gateway_sha256 "$native_gateway_sha256" \
  --arg test_log "$TEST_LOG" \
  --arg terminal_coverage_sha256 "$terminal_coverage_sha256" \
  --argjson source "$SOURCE_REPLAY_JSON" \
  ...
  '{
    ...
    activation_mode:"full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_replay_idempotency_denial_native_route_status",
    side_effect_free:true,
    native_route:true,
    route_enabled:true,
    source_packet_acceptance_receipt_replay_idempotency_gate_ready: $source.memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_replay_idempotency_denial_ready,
    ...
    source_route_count_expected:126,
    terminal_required_marker_count_expected:266,
    source_packet_acceptance_receipt_ready: $source.source_packet_acceptance_receipt_ready,
    source_receipt_surface_count: $source.source_receipt_surface_count,
    source_receipt_generated_count: $source.source_receipt_generated_count,
    source_receipt_recorded_count: $source.source_receipt_recorded_count,
    source_receipt_persisted_count: $source.source_receipt_persisted_count,
    replay_surface_count: $source.replay_surface_count,
    replay_attempt_count: $source.replay_attempt_count,
    replay_recorded_count: $source.replay_recorded_count,
    replay_persisted_count: $source.replay_persisted_count,
    idempotency_key_registered_count: $source.idempotency_key_registered_count,
    idempotency_cache_written_count: $source.idempotency_cache_written_count,
    replay_activation_authority_derived_count: $source.replay_activation_authority_derived_count,
    replay_live_execution_allowed_count: $source.replay_live_execution_allowed_count,
...
echo "Hepta memory/intelligence/KG full live activation operator readiness packet template packet acceptance receipt replay/idempotency denial route gate passed"
