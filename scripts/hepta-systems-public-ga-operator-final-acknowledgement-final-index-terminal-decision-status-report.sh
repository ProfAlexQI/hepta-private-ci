...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-final-acknowledgement-final-index-report.sh"
TERMINAL_DECISION_GATE="$ROOT/scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-terminal-decision-status-promotion-denial-gate.sh"
TERMINAL_DECISION_DOC="$ROOT/docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_REPLAY_REINSTATEMENT_TERMINAL_DECISION_STATUS_PROMOTION_DENIAL_GATE.md"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session final acknowledgement final index report: $SOURCE_REPORT" >&2
  exit 1
}
[[ -f "$TERMINAL_DECISION_GATE" ]] || {
  echo "missing operator identity/session terminal decision/status promotion denial gate: $TERMINAL_DECISION_GATE" >&2
  exit 1
}
[[ -f "$TERMINAL_DECISION_DOC" ]] || {
  echo "missing operator identity/session terminal decision/status promotion denial doc: $TERMINAL_DECISION_DOC" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session terminal decision/status report" >&2
  exit 1
fi
...
jq -e '
  .surface == "public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index"
  and .public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_ready == true
  and .public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_blocked == true
...
jq -n \
  ...
  '{
    runtime: "hepta",
    surface: "public_ga_operator_identity_session_terminal_decision_status_promotion_attachment",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_surface: $source.surface,
    source_public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_ready: $source.public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_ready,
    source_public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_blocked: $source.public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_blocked,
    public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_attached: true,
    public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_ready: true,
    public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_blocked: true,
    ...
    operator_identity_session_terminal_decision_status_promotion_denial_gate_present: true,
    operator_identity_session_terminal_decision_status_promotion_denial_doc_present: true,
    operator_identity_session_terminal_decision_status_static_mention_count: $terminal_decision_status_static_mention_count,
    ...
    long_soak_required_by_source_terminal_decision_status_gate: true,
    ...
    attachment_blocker_count: 46,
    manual_operator_live_cutover_approval_required: true,
    public_ga_claim_allowed: false,
    rollback_execution_allowed: false,
    next_migration_step: "derive_public_ga_operator_identity_session_terminal_decision_status_promotion_readback_without_acknowledgement",
    local_gate: "scripts/hepta-systems-public-ga-operator-final-acknowledgement-final-index-terminal-decision-status-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_FINAL_ACKNOWLEDGEMENT_FINAL_INDEX_TERMINAL_DECISION_STATUS_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_final_acknowledgement_final_index_report: "scripts/hepta-systems-public-ga-operator-identity-session-final-acknowledgement-final-index-report.sh",
      operator_identity_session_terminal_decision_status_promotion_denial_gate: "scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-terminal-decision-status-promotion-denial-gate.sh",
      operator_identity_session_terminal_decision_status_promotion_denial_doc: "docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_REPLAY_REINSTATEMENT_TERMINAL_DECISION_STATUS_PROMOTION_DENIAL_GATE.md"
    },
