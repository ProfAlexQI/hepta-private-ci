...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-export-query-observability-final-index-summary-briefing-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-export-query-observability-final-index-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_EXPORT_QUERY_OBSERVABILITY_FINAL_INDEX_SUMMARY_BRIEFING_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-export-query-observability-final-index-summary-briefing-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session summary briefing attachment report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Public GA operator identity/session export query observability final index gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session summary briefing attachment architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session summary briefing attachment report"
fi

grep -q 'Public GA Operator Identity/Session Operator-Facing Summary Briefing Attachment' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Operator-Facing Summary Briefing Attachment"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_operator_facing_summary_briefing_attachment"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_export_query_observability_final_index_surface == "public_ga_operator_identity_session_export_query_observability_final_index"
  and .source_public_ga_operator_identity_session_export_query_observability_final_index_ready == true
  and .source_public_ga_operator_identity_session_export_query_observability_final_index_blocked == true
  and .public_ga_operator_identity_session_export_query_observability_final_index_attached == true
  and .public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_ready == true
  and .public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_blocked == true
  ...
  and .operator_identity_session_operator_facing_summary_briefing_static_mention_count >= 20
  ...
  and .attachment_blocker_count == 42
  and .manual_operator_live_cutover_approval_required == true
  and .public_ga_claim_allowed == false
  and .public_ga_claimed == false
  and .public_release_published == false
  and .rollback_execution_allowed == false
  and .next_migration_step == "derive_public_ga_operator_identity_session_operator_facing_summary_briefing_readback_without_export"
...
printf 'hepta-systems-public-ga-operator-export-query-observability-final-index-summary-briefing-gate: PASS: Public GA operator identity/session summary briefing attachment is ready but blocked without export\n'
