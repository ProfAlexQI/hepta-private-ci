...
def validate_review_envelope(path)
  ...
  raise QualificationError, "review envelope not eligible" unless receipt.dig("qualification", "implementationReviewEligible") == true
  false_wiring.each_key do |key|
    next if key == "deviceValidation"
    value = receipt.dig("qualification", key)
    raise QualificationError, "review envelope #{key} escaped" unless value == false
  end
...
def validate_device_receipt(path, candidate, tree, review)
  ...
  raise QualificationError, "device qualification claimed product device validation" unless receipt.dig("qualification", "deviceValidation") == false
...
def qualification_payload(status, candidate, tree, evidence, failures)
  ...
  compile_ok = PLATFORMS.all? do |platform|
    MODES.all? { |mode| evidence.dig("compile", platform, mode, "qualified") == true }
  end
  ...
  final_ok = status == "PASS_WINDOWS_PRODUCT_HOST_IMPLEMENTATION_QUALIFICATION"
  {
    ...
    "qualification" => {
      "source" => source_ok,
      "defaultOffCompile" => compile_ok,
      "explicitFeatureCompile" => compile_ok,
