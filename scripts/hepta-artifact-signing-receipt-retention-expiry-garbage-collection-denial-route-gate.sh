...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing artifact signing receipt retention/expiry/garbage-collection route source text: $label" >&2
    exit 1
  fi
}
...
SOURCE_RETENTION_GATE="scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-operator-intent-consent-evidence-artifact-signing-retention-gc-denial-gate.sh"
NATIVE_GATEWAY_SOURCE="codex-rs/hepta-native-gateway/src/native_gateway.rs"

SOURCE_GATE_JSON="$(
  capture_json_report \
    "hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-operator-intent-consent-evidence-artifact-signing-retention-gc-denial-gate" \
...
jq -e '
  ...
  .runtime == "hepta"
  and .status == "ready"
  and .memory_intelligence_kg_full_live_activation_operator_readiness_packet_template_packet_acceptance_receipt_release_publication_result_receipt_terminal_distribution_delivery_receipt_artifact_download_install_affordance_result_receipt_operator_identity_session_revocation_logout_replay_reinstatement_operator_intent_consent_evidence_artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_denial_ready == true
  and .source_artifact_distribution_signing_notarization_receipt_audit_evidence_ready == true
  and .source_artifact_distribution_signing_notarization_receipt_audit_evidence_surface_count == 18
  and .source_artifact_distribution_signing_notarization_receipt_audit_evidence_denied_count == 18
  and .artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_surface_count == 18
  and .artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_attempt_count == 18
  and .artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_denied_count == 18
  and (.denied_by_artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection | length) == 20
  and zero_fields(.; [
  ...
  and (.artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_surfaces | length) == 18
  and (.artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_surfaces | all(
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes artifact signing receipt retention/expiry/garbage-collection route"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "artifact signing receipt retention/expiry/garbage-collection endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "artifact signing receipt retention/expiry/garbage-collection endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "artifact signing receipt retention/expiry/garbage-collection source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "artifact signing receipt retention/expiry/garbage-collection report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "artifact signing receipt retention/expiry/garbage-collection route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_surface_count\": surface_count" \
  "artifact signing receipt retention/expiry/garbage-collection surface count emitted"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "hepta_memory_intelligence_kg_full_live_activation_artifact_signing_receipt_retention_expiry_garbage_collection_endpoint_blocks_lifecycle_mutation" \
  "artifact signing receipt retention/expiry/garbage-collection focused endpoint test"

TEST_LOG="$(mktemp /tmp/hepta-artifact-signing-receipt-retention-expiry-garbage-collection-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_ROUTE_JSON="$(
    curl -fsS "$BASE_URL/api/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-operator-intent-consent-evidence-artifact-signing-receipt-retention-expiry-garbage-collection-denial"
  )"
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_intelligence_kg_full_live_activation_artifact_download_install_affordance_result_receipt_operator_identity_session_revocation_logout_replay_reinstatement_operator_intent_consent_evidence_artifact_signing_receipt_retention_expiry_garbage_collection_denial_route_enabled == true
    and .memory_intelligence_kg_full_live_activation_artifact_download_install_affordance_result_receipt_operator_identity_session_revocation_logout_replay_reinstatement_operator_intent_consent_evidence_artifact_signing_receipt_retention_expiry_garbage_collection_denial_ready == true
    and .source_artifact_distribution_signing_notarization_receipt_audit_evidence_ready == true
    and .artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_surface_count == 18
    and .artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_attempt_count == 18
    and .artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_denied_count == 18
    and (.denied_by_artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection | length) == 20
    and .artifact_distribution_signing_notarization_receipt_retention_policy_recorded_count == 0
    and .artifact_distribution_signing_notarization_receipt_expiry_scheduler_recorded_count == 0
    and .artifact_distribution_signing_notarization_receipt_garbage_collection_scan_performed_count == 0
    and .artifact_distribution_signing_notarization_receipt_archive_recorded_count == 0
    and .artifact_distribution_signing_notarization_receipt_compaction_recorded_count == 0
...
source_gate_sha256="$(sha256_file "$SOURCE_RETENTION_GATE")"
...
jq -n \
  ...
  --arg gate "hepta_memory_intelligence_kg_full_live_activation_artifact_signing_receipt_retention_expiry_garbage_collection_denial_route_gate" \
  --arg endpoint "/api/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-operator-intent-consent-evidence-artifact-signing-receipt-retention-expiry-garbage-collection-denial" \
  --arg source_command "/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-revocation-logout-replay-reinstatement-operator-intent-consent-evidence-artifact-signing-receipt-retention-expiry-garbage-collection-denial --json" \
  ...
  '{
    ...
    activation_mode:"artifact_signing_receipt_retention_expiry_garbage_collection_native_route_status",
    ...
    artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_surface_count:18,
    artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_denied_count:18,
    denied_by_artifact_distribution_signing_notarization_receipt_retention_expiry_garbage_collection_count:20,
    artifact_distribution_signing_notarization_receipt_retention_policy_recorded_count:0,
    artifact_distribution_signing_notarization_receipt_expiry_scheduler_recorded_count:0,
    artifact_distribution_signing_notarization_receipt_garbage_collection_scan_performed_count:0,
    artifact_distribution_signing_notarization_receipt_archive_recorded_count:0,
    artifact_distribution_signing_notarization_receipt_compaction_recorded_count:0,
...
echo "Hepta artifact signing receipt retention/expiry/garbage-collection route gate passed: endpoint ready, 18 surfaces, terminal markers ${terminal_present_required_marker_count}/${terminal_required_marker_count}" >&2
