...
def main() -> int:
    ...
    require(
        bootstrap.get("document_authority_registry")
        == "plans/hepta-intelligence/HEPTA_INTELLIGENCE_DOCUMENT_AUTHORITY_REGISTRY_V1.json",
        "registry pointer drift",
    )
    require(
        bootstrap.get("time_bounded_evidence_index")
    ...
    require(
        ...
        == "DOC-Q0.1_RESTORE_FROZEN_QUALIFICATION_COMPATIBILITY",
        "active compatibility task drift",
    )
    ...
    require(
        rules.get("single_current_machine_authority") is True,
        "single machine authority disabled",
    )
    require(
        rules.get("single_current_human_authority") is True,
        "single human authority disabled",
    )
    require(
        rules.get("compatibility_contracts_are_current_authority") is False,
    ...
    require(
        registry.get("current_authority", {}).get("human", {}).get("path")
        == "plans/hepta-intelligence/HEPTA_INTELLIGENCE_MASTER_PLAN.md",
    ...
    require(
        ...
        "compatibility contract registry incomplete",
    )
    registered_paths = {
    ...
    require(
        ...
        in registered_paths,
        "legacy V2 contract not registered",
    )
    require(
        "plans/hepta-intelligence/HEPTA_INTELLIGENCE_EXECUTION_STATUS_V3.json"
        in registered_paths,
        "legacy V3 contract not registered",
    )
    for item in compatibility:
        require(isinstance(item, dict), "compatibility entry must be an object")
        require(
            item.get("current_authority") is False,
            f"compatibility entry gained authority: {item.get('path')}",
        )
        require(
            item.get("schema_preserved") is True,
            f"compatibility schema not preserved: {item.get('path')}",
        )
        consumers = item.get("registered_consumers")
        require(
            isinstance(consumers, list) and consumers,
    ...
    require(
        isinstance(policy_documents, list) and policy_documents,
        "policy document registry missing",
    )
    claim_entry = next(
        (
            item
    ...
    require(
        ...
        "claim ladder gained current-state authority",
    )
    require(
        claim_entry.get("current_state_source")
        == "plans/hepta-intelligence/HEPTA_INTELLIGENCE_CURRENT_PLAN.json",
    ...
    require(
        claims.get("classification") == "POLICY_SCHEMA_AND_TRANSITION_RULES",
        "claim ladder classification drift",
    )
    require(
        claims.get("current_state_authority") is False,
        "claim ladder gained current-state authority",
    )
    require(
        claims.get("current_state_source")
        == "plans/hepta-intelligence/HEPTA_INTELLIGENCE_CURRENT_PLAN.json",
    ...
    for tranche, path in SNAPSHOTS.items():
        ...
        require(
            ...
            f"snapshot classification drift: {tranche}",
        )
        require(
            snapshot.get("current_authority") is False,
            f"snapshot gained authority: {tranche}",
        )
        require(
            snapshot.get("current_tranche", {}).get("id") == tranche,
            f"snapshot current tranche drift: {tranche}",
        )
        require(
            snapshot.get("current_tranche", {}).get("qualified") is False,
            f"snapshot unexpectedly qualified: {tranche}",
        )
        require(
            all_false(snapshot.get("authority")),
            f"snapshot authority must remain false: {tranche}",
        )
        require(
            isinstance(snapshot.get("source_ref"), str)
            and len(snapshot["source_ref"]) == 40,
            f"snapshot source ref invalid: {tranche}",
        )
        require(
            isinstance(snapshot.get("source_status_blob_sha"), str)
            and len(snapshot["source_status_blob_sha"]) == 40,
            f"snapshot source blob invalid: {tranche}",
        )
