...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-replay-denial-readback-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session replay denial readback report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session replay denial final index report" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    ...
    source_public_ga_operator_identity_session_replay_denial_readback_surface: $source.surface,
    source_public_ga_operator_identity_session_replay_denial_readback_ready: $source.public_ga_operator_identity_session_replay_denial_readback_ready,
    source_public_ga_operator_identity_session_replay_denial_readback_blocked: $source.public_ga_operator_identity_session_replay_denial_readback_blocked,
    ...
    public_ga_operator_identity_session_replay_denial_final_index_ready: true,
    public_ga_operator_identity_session_replay_denial_final_index_blocked: true,
    public_ga_operator_identity_session_replay_denial_readback_attached: true,
    ...
    next_migration_step: "attach_public_ga_operator_identity_session_replay_denial_final_index_to_public_ga_operator_identity_session_revocation_logout_without_replay_acceptance",
    local_gate: "scripts/hepta-systems-public-ga-operator-identity-session-replay-denial-final-index-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_REPLAY_DENIAL_FINAL_INDEX_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_replay_denial_readback_report: "scripts/hepta-systems-public-ga-operator-identity-session-replay-denial-readback-report.sh"
    },
