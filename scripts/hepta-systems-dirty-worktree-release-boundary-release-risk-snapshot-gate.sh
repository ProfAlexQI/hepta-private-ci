...
"$REPORT" | jq -e '
  ...
  and .source_evidence_recording_boundary_gate == "dirty_worktree_release_boundary_clean_worktree_strategy_operator_evidence_recording_boundary_readback_gate"
  ...
  and .release_risk_snapshot_scope.snapshot_id == "dirty-worktree.release-boundary.release-risk-snapshot.v1"
  and .release_risk_snapshot_scope.snapshot_route == "readback://release-boundary/dirty-worktree/release-risk-snapshot/v1"
  and .release_risk_snapshot_scope.source_evidence_recording_boundary_route == "readback://release-boundary/dirty-worktree/clean-worktree-strategy/operator-evidence-recording-boundary/v1"
  ...
  and .high_or_critical_risk_count == 5
  ...
  and any(.entries[]; .source_bucket == "codex-rs" and .release_risk_tier == "high" and .snapshot_key == "dirty_worktree.release_risk_snapshot.top_level.codex_rs")
  and any(.entries[]; .source_bucket == "plugins" and .release_risk_tier == "high" and .rehearsal_action == "test_only_plugin_surface_rehearsal")
  and any(.entries[]; .source_bucket == "scripts" and .release_risk_tier == "high" and .rehearsal_action == "test_only_script_gate_rehearsal")
  and any(.entries[]; .source_bucket == "hepta_systems_owned" and .release_risk_tier == "high" and .snapshot_route == "readback://release-boundary/dirty-worktree/release-risk-snapshot/scope/hepta-systems-owned")
  and any(.entries[]; .source_bucket == "artifacts" and .release_risk_tier == "medium")
  and any(.entries[]; .source_bucket == "docs" and .release_risk_tier == "medium")
  and (.entries | all(
    (.source_evidence_boundary_key | startswith("dirty_worktree.evidence_recording_boundary."))
    and (.source_evidence_boundary_route | startswith("readback://release-boundary/dirty-worktree/clean-worktree-strategy/operator-evidence-recording-boundary/"))
    and (.snapshot_key | startswith("dirty_worktree.release_risk_snapshot."))
    and (.snapshot_route | startswith("readback://release-boundary/dirty-worktree/release-risk-snapshot/"))
...
printf 'hepta-systems-dirty-worktree-release-boundary-release-risk-snapshot-gate: PASS: dirty-worktree release risk is collapsed into a visible-only snapshot with git mutation, cleanup, evidence recording, approval, release, canary, and live blocked\n'
