#!/usr/bin/env python3
"""Verify the exact Bazelisk and direct Bazel used by the Q0.41 query smoke."""
...
SPEC = importlib.util.spec_from_file_location("_hepta_q041_query_vector_base", BASE_PATH)
...
PASS_SOURCE: Final = "PASS_WINDOWS_GNULLVM_Q0_41_QUERY_EXECUTABLE_SOURCE"
PASS_EXECUTED: Final = "PASS_WINDOWS_GNULLVM_Q0_41_DIRECT_BAZEL_QUERY_EXECUTED"
...
def _validate_bazelisk_inputs(
    ...
    required_folded = {name.casefold() for name in REQUIRED_BAZELISK_ENV}
    forbidden_folded = {name.casefold() for name in FORBIDDEN_BAZELISK_ENV}
    for name, value in list(env.items()):
        folded = name.casefold()
        if not value:
            continue
        if folded in forbidden_folded:
            fail(f"Bazelisk override {name} is forbidden")
        if folded.startswith("bazelisk_") and folded not in required_folded:
            fail(f"unreviewed Bazelisk control {name} is forbidden")
...
def execute_parser_smoke(
    *,
    ...
    with TemporaryDirectory(prefix="hepta-q041-query-") as temporary:
        workspace = Path(temporary).resolve()
        (workspace / "MODULE.bazel").write_text(
            'module(name = "hepta_q041_query_probe", version = "1.0")\n',
            encoding="utf-8",
        )
