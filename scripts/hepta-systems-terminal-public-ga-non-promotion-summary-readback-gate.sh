...
REPORT="$ROOT/scripts/hepta-systems-terminal-public-ga-non-promotion-summary-readback-report.sh"
SUMMARY_GATE="$ROOT/scripts/hepta-systems-public-ga-readiness-final-index-terminal-public-ga-non-promotion-summary-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_TERMINAL_PUBLIC_GA_NON_PROMOTION_SUMMARY_READBACK_2026-06-21.md"

fail() {
  printf 'hepta-systems-terminal-public-ga-non-promotion-summary-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable terminal Public GA non-promotion summary readback report: $REPORT"
[[ -x "$SUMMARY_GATE" ]] || fail "missing executable terminal Public GA non-promotion summary gate: $SUMMARY_GATE"
[[ -f "$DOC" ]] || fail "missing terminal Public GA non-promotion summary readback architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the terminal Public GA non-promotion summary readback report"
fi

grep -q 'Terminal Public GA Non-Promotion Summary Readback' "$DOC" \
  || fail "architecture note must document Terminal Public GA Non-Promotion Summary Readback"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "terminal_public_ga_non_promotion_summary_readback"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_terminal_public_ga_non_promotion_summary_surface == "terminal_public_ga_non_promotion_summary"
  and .source_terminal_public_ga_non_promotion_summary_ready == true
  and .source_terminal_public_ga_non_promotion_summary_blocked == true
  ...
  and .terminal_public_ga_non_promotion_summary_readback_ready == true
  and .terminal_public_ga_non_promotion_summary_readback_blocked == true
  and .terminal_public_ga_non_promotion_summary_attached == true
  and .readback_mode == "static_terminal_public_ga_non_promotion_summary_snapshot_only"
  and .readback_check_count == 22
  ...
  and .next_migration_step == "derive_terminal_public_ga_non_promotion_summary_final_index_without_public_ga_readiness_invocation"
...
printf 'hepta-systems-terminal-public-ga-non-promotion-summary-readback-gate: PASS: terminal Public GA non-promotion summary readback is static without readiness invocation\n'
