...
GATE="${BASE}_receipt_retention_expiry_readback_acknowledgement_preview_gate"
SCHEMA="${GATE#hepta_}"
...
rust_module="codex-rs/hepta-runtime/src/wg_deep_td7_receipt_retention_readback_ack_preview.rs"
report_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-ack-preview-report.sh"
gate_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-ack-preview-gate.sh"
prior_report_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-receipt-preview-report.sh"
prior_gate_script="scripts/hepta-systems-work-graph-deep-td7-receipt-retention-readback-receipt-preview-gate.sh"
...
jq -n \
  ...
  '
  def items($prefix; $count): [range(0; $count) | {id: "\($prefix)_\(.)", source_receipt_ids: [range(0; 6) | "deep_td7_readback_receipt_\(.)"], required_fields: ["readbackReceiptHash", "scopeEpoch", "zeroEffectHash"], hash_only: true, blocks_recording: true, blocks_acceptance: true, blocks_authority: true, blocks_external_delivery: true, required: true}];
  {
    product: "Hepta",
    runtime: "hepta",
    status: "ready",
    ...
    acknowledgement_contracts: items("deep_td7_readback_ack_contract"; 6),
    non_acceptance_reasons: items("deep_td7_readback_ack_non_acceptance"; 7),
    recording_denials: items("deep_td7_readback_ack_recording_denial"; 7),
    expiry_replay_guards: items("deep_td7_readback_ack_expiry_replay_guard"; 5),
    local_views: items("deep_td7_readback_ack_view"; 4),
    invariants: items("deep_td7_readback_ack_invariant"; 6),
