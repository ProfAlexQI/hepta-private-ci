...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-non-acceptance-final-index-report.sh"
SESSION_GATE="$ROOT/scripts/hepta-memory-intelligence-kg-full-live-activation-artifact-download-install-affordance-result-receipt-operator-identity-session-replay-cross-binding-denial-gate.sh"
SESSION_DOC="$ROOT/docs/architecture/HEPTA_MEMORY_INTELLIGENCE_KG_FULL_LIVE_ACTIVATION_ARTIFACT_DOWNLOAD_INSTALL_AFFORDANCE_RESULT_RECEIPT_OPERATOR_IDENTITY_SESSION_REPLAY_CROSS_BINDING_DENIAL_GATE.md"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity non-acceptance final index report: $SOURCE_REPORT" >&2
  exit 1
}
[[ -f "$SESSION_GATE" ]] || {
  echo "missing operator identity/session replay cross-binding denial gate: $SESSION_GATE" >&2
  exit 1
}
[[ -f "$SESSION_DOC" ]] || {
  echo "missing operator identity/session replay cross-binding denial doc: $SESSION_DOC" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator session non-binding report" >&2
  exit 1
fi
...
jq -e '
  .surface == "public_ga_operator_identity_non_acceptance_final_index"
  and .public_ga_operator_identity_non_acceptance_final_index_ready == true
  and .public_ga_operator_identity_non_acceptance_final_index_blocked == true
...
session_static_mention_count="$(
  grep -Ec 'session|cross-binding|replay|operator_identity|operator identity|operator_session|operator session' "$SESSION_GATE" || true
)"

jq -n \
  --argjson source "$source_json" \
  --argjson session_static_mention_count "$session_static_mention_count" \
  '{
    runtime: "hepta",
    surface: "public_ga_operator_session_non_binding_attachment",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_identity_non_acceptance_final_index_surface: $source.surface,
    source_public_ga_operator_identity_non_acceptance_final_index_ready: $source.public_ga_operator_identity_non_acceptance_final_index_ready,
    source_public_ga_operator_identity_non_acceptance_final_index_blocked: $source.public_ga_operator_identity_non_acceptance_final_index_blocked,
    ...
    public_ga_operator_identity_non_acceptance_final_index_attached: true,
    public_ga_operator_session_non_binding_attachment_ready: true,
    public_ga_operator_session_non_binding_attachment_blocked: true,
    ...
    operator_session_replay_cross_binding_static_mention_count: $session_static_mention_count,
    ...
    long_soak_required_by_source_session_gate: true,
    ...
    next_migration_step: "derive_public_ga_operator_session_non_binding_readback_without_identity_acceptance",
    local_gate: "scripts/hepta-systems-public-ga-operator-identity-final-index-operator-session-non-binding-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_FINAL_INDEX_OPERATOR_SESSION_NON_BINDING_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_non_acceptance_final_index_report: "scripts/hepta-systems-public-ga-operator-identity-non-acceptance-final-index-report.sh",
