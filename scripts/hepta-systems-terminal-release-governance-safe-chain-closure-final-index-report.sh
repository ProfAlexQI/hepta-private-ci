...
READBACK_REPORT="$ROOT/scripts/hepta-systems-terminal-release-governance-safe-chain-closure-readback-report.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_TERMINAL_RELEASE_GOVERNANCE_SAFE_CHAIN_CLOSURE_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-terminal-release-governance-safe-chain-closure-final-index-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$READBACK_REPORT" ]] || fail "missing executable terminal release governance safe chain closure readback report: $READBACK_REPORT"
[[ -f "$DOC" ]] || fail "missing terminal release governance safe chain closure final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the terminal release governance safe chain closure final index report"
fi

jq -n \
  --slurpfile readback <("$READBACK_REPORT") \
  --arg gate "scripts/hepta-systems-terminal-release-governance-safe-chain-closure-final-index-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_TERMINAL_RELEASE_GOVERNANCE_SAFE_CHAIN_CLOSURE_FINAL_INDEX_2026-06-21.md" \
  '
  ...
  ($readback.terminal_release_governance_safe_chain_closure_readback_ready == true
    and $readback.terminal_release_governance_safe_chain_closure_readback_blocked == true
    and $readback.safe_chain_ready_source_count == 5
  ...
  {
    runtime:"hepta",
    surface:"terminal_release_governance_safe_chain_closure_final_index",
    plugin_id:$readback.plugin_id,
    status:(if $final_index_ready then "ready_blocked" else "blocked" end),
    source_terminal_release_governance_safe_chain_closure_readback_surface:$readback.surface,
    source_terminal_release_governance_safe_chain_closure_readback_ready:$readback.terminal_release_governance_safe_chain_closure_readback_ready,
    source_terminal_release_governance_safe_chain_closure_readback_blocked:$readback.terminal_release_governance_safe_chain_closure_readback_blocked,
    terminal_release_governance_safe_chain_closure_final_index_ready:$final_index_ready,
    ...
    safe_chain_source_count:$readback.safe_chain_source_count,
    safe_chain_ready_source_count:$readback.safe_chain_ready_source_count,
    ...
    next_migration_step:"attach_terminal_release_governance_safe_chain_closure_final_index_to_terminal_final_public_ga_denial_summary_without_live_gate_invocation",
    local_gate:$gate,
    architecture_note:$doc,
    source_files:{
      terminal_release_governance_safe_chain_closure_readback_report:"scripts/hepta-systems-terminal-release-governance-safe-chain-closure-readback-report.sh"
    },
