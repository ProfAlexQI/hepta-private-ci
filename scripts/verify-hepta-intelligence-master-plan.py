...
def main() -> int:
    ...
    require(
        canonical.get("human_document") == expected_master,
        "canonical human document drift",
    )
    require(
        canonical.get("plan_id") == "HEPTA_INTELLIGENCE_MASTER_PLAN_V4", "plan id drift"
    )
    ...
    require(
        bootstrap.get("on_mismatch") == "FAIL_CLOSED",
        "session mismatch policy must fail closed",
    )
    require(
    ...
    require(
        claims.get("system_learning") == "L0_STATIC_SHADOW_WITH_PARTIAL_L1_FOUNDATIONS",
        "system claim drift",
    )
    ...
    require(
        stack.get("runtime_source_freeze") is True,
        "runtime source freeze must remain true",
    )
    ...
    for path in plan_candidates:
        ...
        if "CANONICAL_CURRENT" in body:
            ...
            require(
                path == MASTER,
                f"non-master plan declares CANONICAL_CURRENT: {path.name}",
            )
        elif path in HISTORICAL_PLANS:
            require(
                "HISTORICAL_REDIRECT" in body,
                f"historical plan lacks redirect marker: {path.name}",
            )
    require(
        canonical_count == 1,
        f"expected exactly one canonical plan, found {canonical_count}",
    )
    agents = AGENTS.read_text(encoding="utf-8")
    require(
        "HEPTA_INTELLIGENCE_CURRENT_PLAN.json" in agents,
        "AGENTS missing machine pointer",
    )
    require(
        "HEPTA_INTELLIGENCE_MASTER_PLAN.md" in agents, "AGENTS missing master pointer"
    )
    require(
        "Q0" in agents and "fail closed" in agents.lower(),
        "AGENTS missing Q0/fail-closed rules",
    )
    print(PASS)
    return 0
