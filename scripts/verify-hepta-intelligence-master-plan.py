...
PASS = "PASS_HEPTA_INTELLIGENCE_MASTER_PLAN_V4_1_SOURCE_ONLY"


def fail(message: str) -> NoReturn:
    raise SystemExit(f"FAIL_HEPTA_INTELLIGENCE_MASTER_PLAN_V4_1: {message}")
...
def main() -> int:
    for path in [MASTER, CURRENT, AGENTS, CAPABILITIES, PR_STACK, INTEGRATION, *HISTORICAL_PLANS]:
        require(path.is_file(), f"missing {path.relative_to(ROOT)}")

    current = load_json(CURRENT)
    require(current.get("schema") == "hepta_intelligence_current_plan_v2", "current schema mismatch")
    require(current.get("repository") == "ProfHepta/hepta-private-ci", "repository identity drift")
    ...
    require(canonical.get("plan_version") == "4.1.0", "plan version drift")
    ...
    require(q0.get("status") == "EXECUTABLE_EXACT_CANDIDATE_QUALIFIED", "Q0 exact qualification missing")
    require(q0.get("paired_receipt_qualified_candidate") is True, "paired Q0 qualification missing")
    require(q0.get("runtime_capability_qualified") is False, "Q0 gained runtime capability")
    ...
    require(active.get("status") == "ACTIVE_BLOCKING", "A0 blocking status drift")
    require(active.get("active_task") == "A0.1_BUILD_CANONICAL_REGISTRIES_AND_CURRENT_TRUTH", "A0 task drift")
    ...
    require(stack.get("expected_parent") == "c768bcbeb4c1168088d2499828c24da521a2a73a", "A0 expected parent drift")
    ...
    for marker in [
        ...
        require(marker.lower() in agents.lower(), f"AGENTS missing marker: {marker}")
