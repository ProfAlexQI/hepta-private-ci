...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-session-non-binding-readback-report.sh"
ATTACHMENT_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-final-index-operator-session-non-binding-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_SESSION_NON_BINDING_READBACK_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-session-non-binding-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator session non-binding readback report: $REPORT"
[[ -x "$ATTACHMENT_GATE" ]] || fail "missing executable Public GA operator session non-binding attachment gate: $ATTACHMENT_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator session non-binding readback architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator session non-binding readback report"
fi

grep -q 'Public GA Operator Session Non-Binding Readback' "$DOC" \
  || fail "architecture note must document Public GA Operator Session Non-Binding Readback"
grep -q 'ready-but-blocked' "$DOC" \
  || fail "architecture note must document ready-but-blocked status"
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that readback does not invoke session gates"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_session_non_binding_readback"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_session_non_binding_attachment_surface == "public_ga_operator_session_non_binding_attachment"
  and .source_public_ga_operator_session_non_binding_attachment_ready == true
  and .source_public_ga_operator_session_non_binding_attachment_blocked == true
  ...
  and .public_ga_operator_session_non_binding_readback_ready == true
  and .public_ga_operator_session_non_binding_readback_blocked == true
  and .public_ga_operator_session_non_binding_attachment_attached == true
  and .readback_mode == "static_operator_session_non_binding_snapshot_only"
  and .readback_check_count == 30
  ...
  and .readback_blocker_count == 24
  and .public_ga_claim_allowed == false
  and .public_ga_claimed == false
  and .public_release_published == false
  and .rollback_execution_allowed == false
  and .next_migration_step == "derive_public_ga_operator_session_non_binding_final_index_without_identity_acceptance"
...
printf 'hepta-systems-public-ga-operator-session-non-binding-readback-gate: PASS: Public GA operator session non-binding readback is ready but blocked\n'
