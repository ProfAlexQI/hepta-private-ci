...
idempotence_gate_script="hepta-context-source-aware-compression-front-door-report-status-artifact-export-idempotence-gate.sh"
...
tmp_dir="$(mktemp -d -t hepta-context-source-aware-status-artifact-idempotence.XXXXXX)"
artifact_out="$tmp_dir/source-aware-contracts.status"
first_artifact="$tmp_dir/first-source-aware-contracts.status"
...
fail() {
  echo "hepta-context-source-aware-compression-front-door-report-status-artifact-export-idempotence-gate: $*" >&2
  if [ -f "$artifact_out" ]; then
    echo "persisted source-aware status artifact:" >&2
    cat "$artifact_out" >&2
  fi
...
assert_persisted_artifact_contract() {
  ...
  if [ ! -s "$artifact_out" ]; then
    fail "$label persisted status artifact was not written"
  fi

  line_count="$(wc -l <"$artifact_out" | tr -d ' ')"
  if [ "$line_count" != "5" ]; then
    fail "$label persisted status artifact expected exactly five source-aware-contracts lines, got $line_count"
  fi

  if grep -F "front-door:" "$artifact_out" >/dev/null; then
    fail "$label persisted status artifact must not contain front-door diagnostic noise"
  fi

  if grep -F "hepta-context-source-aware-compression-front-door-gate" "$artifact_out" >/dev/null; then
    fail "$label persisted status artifact must not contain front-door gate diagnostic noise"
  fi

  if grep -F "stale-source-aware-status" "$artifact_out" >/dev/null; then
    fail "$label persisted status artifact must overwrite stale caller-provided content"
  fi

  if grep -F "source-aware-contracts.runtime-activation=enabled" "$artifact_out" >/dev/null; then
    fail "$label persisted status artifact must not enable runtime activation"
  fi

  if grep -v '^source-aware-contracts' "$artifact_out" >/dev/null; then
    fail "$label persisted status artifact must contain only source-aware-contracts lines"
  fi
  ...
  case "$runtime_dirty_line" in
    ...
    source-aware-contracts.runtime-dirty-classifier=non-blocking)
      ;;
    *)
      fail "$label persisted status artifact runtime dirty classifier line malformed: $runtime_dirty_line"
      ;;
  esac
...
required_contract_terms=(
  ...
  "avoid early-closing head pipelines under"
...
for term in "${required_contract_terms[@]}"; do
  assert_file_contains \
    "$context_contracts" \
    "$term" \
    "source-aware compression persisted status artifact export overwrite/idempotence contract"
done

assert_file_contains \
  "$front_door_gate" \
  "$idempotence_gate_script" \
  "source-aware compression status artifact export idempotence front-door wiring"

assert_file_contains \
  "$debug_gate" \
  "$idempotence_gate_script" \
  "source-aware compression status artifact export idempotence debug gate"

assert_file_contains \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export overwrite/idempotence" \
  "source-aware compression status artifact export idempotence preflight stage"

assert_line_before \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export precheck" \
  "source-aware compression front-door report persisted status artifact export overwrite/idempotence" \
  "source-aware compression status artifact export idempotence preflight stage order"
...
if grep -E '\|[[:space:]]*head -n 20' "$front_door_gate" >/dev/null; then
  fail "front-door runtime dirty preview must not use early-closing head under pipefail"
fi

{
  echo "front-door: stale-source-aware-status diagnostic"
  echo "source-aware-contracts=stale-source-aware-status"
  echo "source-aware-contracts.runtime-activation=enabled"
  echo "stale-source-aware-status tail"
} >"$artifact_out"

if ! env "$artifact_out_env=$artifact_out" bash "$artifact_gate" >"$gate_output" 2>&1; then
  fail "status artifact gate failed on first persisted overwrite run"
fi

assert_persisted_artifact_contract "first run"
cp "$artifact_out" "$first_artifact"

{
  echo "front-door: stale-source-aware-status appended diagnostic"
  echo "source-aware-contracts.runtime-activation=enabled"
  echo "stale-source-aware-status appended tail"
} >>"$artifact_out"

if ! env "$artifact_out_env=$artifact_out" bash "$artifact_gate" >"$gate_output" 2>&1; then
  fail "status artifact gate failed on second persisted overwrite run"
fi

assert_persisted_artifact_contract "second run"

if ! cmp -s "$first_artifact" "$artifact_out"; then
  fail "repeated successful persisted artifact export must be idempotent"
fi

echo "Hepta context source-aware compression persisted status artifact export idempotence gate passed"
