...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-final-index-operator-session-non-binding-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator session non-binding attachment report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator session non-binding readback report" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    runtime: "hepta",
    surface: "public_ga_operator_session_non_binding_readback",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_session_non_binding_attachment_surface: $source.surface,
    source_public_ga_operator_session_non_binding_attachment_ready: $source.public_ga_operator_session_non_binding_attachment_ready,
    source_public_ga_operator_session_non_binding_attachment_blocked: $source.public_ga_operator_session_non_binding_attachment_blocked,
    ...
    public_ga_operator_session_non_binding_readback_ready: true,
    public_ga_operator_session_non_binding_readback_blocked: true,
    public_ga_operator_session_non_binding_attachment_attached: true,
    public_ga_operator_identity_non_acceptance_final_index_attached: $source.public_ga_operator_identity_non_acceptance_final_index_attached,
    readback_mode: "static_operator_session_non_binding_snapshot_only",
    readback_check_count: 30,
    ...
    long_soak_required_by_source_session_gate: $source.long_soak_required_by_source_session_gate,
    ...
    readback_blocker_count: 24,
    public_ga_claim_allowed: false,
    public_ga_claimed: false,
    public_release_published: false,
    rollback_execution_allowed: false,
    next_migration_step: "derive_public_ga_operator_session_non_binding_final_index_without_identity_acceptance",
    local_gate: "scripts/hepta-systems-public-ga-operator-session-non-binding-readback-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_SESSION_NON_BINDING_READBACK_2026-06-21.md",
    source_files: {
      public_ga_operator_session_non_binding_attachment_report: "scripts/hepta-systems-public-ga-operator-identity-final-index-operator-session-non-binding-report.sh"
    },
