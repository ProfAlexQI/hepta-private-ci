...
atomic_gate_script="hepta-context-source-aware-compression-front-door-report-status-artifact-export-atomic-gate.sh"
...
tmp_dir="$(mktemp -d -t hepta-context-source-aware-status-artifact-atomic.XXXXXX)"
...
fail() {
  echo "hepta-context-source-aware-compression-front-door-report-status-artifact-export-atomic-gate: $*" >&2
...
assert_persisted_artifact_contract() {
  ...
  if grep -F "hepta-context-source-aware-compression-front-door-gate" "$artifact_out" >/dev/null; then
    fail "persisted status artifact must not contain front-door gate diagnostic noise"
  fi

  if grep -F "stale-source-aware-status" "$artifact_out" >/dev/null; then
    fail "persisted status artifact must replace stale caller-provided content"
  fi
...
for term in "${required_contract_terms[@]}"; do
  assert_file_contains \
    "$context_contracts" \
    "$term" \
    "source-aware compression persisted status artifact export atomic replace contract"
done

assert_file_contains \
  "$front_door_gate" \
  "$atomic_gate_script" \
  "source-aware compression status artifact export atomic front-door wiring"

assert_file_contains \
  "$debug_gate" \
  "$atomic_gate_script" \
  "source-aware compression status artifact export atomic debug gate"

assert_file_contains \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export atomic replace" \
  "source-aware compression status artifact export atomic preflight stage"

assert_line_before \
  "$preflight_script" \
  "source-aware compression front-door report persisted status artifact export overwrite/idempotence" \
  "source-aware compression front-door report persisted status artifact export atomic replace" \
  "source-aware compression status artifact export atomic preflight stage order"
...
assert_file_contains \
  "$artifact_gate" \
  'mktemp "$artifact_out_dir/.${artifact_out_base}.tmp.XXXXXX"' \
  "source-aware compression status artifact export atomic implementation"

assert_line_before \
  "$artifact_gate" \
  'cp "$status_artifact" "$artifact_out_tmp"' \
  'mv -f "$artifact_out_tmp" "$artifact_out_path"' \
  "source-aware compression status artifact export atomic implementation order"

if grep -F 'cp "$status_artifact" "$artifact_out_path"' "$artifact_gate" >/dev/null; then
  fail "status artifact export must not copy directly to the caller-provided output path"
fi
...
if ! env "$artifact_out_env=$artifact_out" bash "$artifact_gate" >"$gate_output" 2>&1; then
  fail "status artifact gate failed with atomic persisted artifact output enabled"
fi

assert_persisted_artifact_contract

temp_residue="$(find "$tmp_dir" -name ".source-aware-contracts.status.tmp.*" -print -quit)"
if [ -n "$temp_residue" ]; then
  fail "status artifact export must not leave temporary artifact residue: $temp_residue"
fi

echo "Hepta context source-aware compression persisted status artifact export atomic gate passed"
