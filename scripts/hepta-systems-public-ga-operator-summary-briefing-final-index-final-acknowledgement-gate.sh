...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-summary-briefing-final-index-final-acknowledgement-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-operator-facing-summary-briefing-final-index-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_SUMMARY_BRIEFING_FINAL_INDEX_FINAL_ACKNOWLEDGEMENT_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-summary-briefing-final-index-final-acknowledgement-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session final acknowledgement attachment report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Public GA operator identity/session summary briefing final index gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session final acknowledgement attachment architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session final acknowledgement attachment report"
fi

grep -q 'Public GA Operator Identity/Session Final Acknowledgement Non-Acceptance Attachment' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Final Acknowledgement Non-Acceptance Attachment"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_final_acknowledgement_non_acceptance_attachment"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_surface == "public_ga_operator_identity_session_operator_facing_summary_briefing_final_index"
  and .source_public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_ready == true
  and .source_public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_blocked == true
  ...
  and .operator_identity_session_final_acknowledgement_static_mention_count >= 20
  ...
  and .long_soak_required_by_source_final_acknowledgement_gate == true
  ...
  and .attachment_blocker_count == 44
  and .manual_operator_live_cutover_approval_required == true
  and .public_ga_claim_allowed == false
  and .public_ga_claimed == false
  and .public_release_published == false
  and .rollback_execution_allowed == false
  and .next_migration_step == "derive_public_ga_operator_identity_session_final_acknowledgement_readback_without_summary"
...
printf 'hepta-systems-public-ga-operator-summary-briefing-final-index-final-acknowledgement-gate: PASS: Public GA operator identity/session final acknowledgement attachment is ready but blocked without summary acceptance\n'
