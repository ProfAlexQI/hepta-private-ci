...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing runtime provider-router activation command result receipt terminal operator decision public-claim route source text: $label" >&2
    exit 1
  fi
}
...
SOURCE_JSON="$(
  HEPTA_LIVE_URL="$BASE_URL" \
    ...
    capture_json_report \
      "hepta-memory-intelligence-kg-full-enablement-runtime-provider-router-activation-command-result-receipt-terminal-operator-decision-public-claim-non-promotion-denial-gate" \
...
jq -e '
  .runtime == "hepta"
  and .status == "ready"
  and .gate == "hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_gate"
  and .activation_command_result_receipt_terminal_operator_decision_public_claim_mode == "runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial"
  ...
  and .activation_command_result_receipt_terminal_operator_decision_allowed == false
  and .activation_command_result_receipt_terminal_operator_decision_request_accepted == false
  ...
  and (.activation_command_result_receipt_terminal_operator_decision_public_claim_surfaces | length) == 12
  ...
  and ([.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures[] | select(.terminal_operator_decision_requested == true)] | length) == 10
  and ([.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures[] | select(.public_claim_promotion_requested == true)] | length) == 1
  and ([.activation_command_result_receipt_terminal_operator_decision_public_claim_fixtures[] | select(.release_artifact_write_requested == true)] | length) == 1
  ...
  and (.denied_by_activation_command_result_receipt_terminal_operator_decision_public_claim | length) == 18
...
require_source_text "$ROUTE_REGISTRY_SOURCE" \
  ...
  "runtime provider-router activation command result receipt terminal operator decision public-claim endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "runtime provider-router activation command result receipt terminal operator decision public-claim endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "runtime provider-router activation command result receipt terminal operator decision public-claim source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "runtime provider-router activation command result receipt terminal operator decision public-claim report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "runtime provider-router activation command result receipt terminal operator decision public-claim route enabled"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  'hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_endpoint_blocks_public_claim_and_authority' \
  "runtime provider-router activation command result receipt terminal operator decision public-claim focused test"

TEST_LOG="$(mktemp /tmp/hepta-runtime-provider-router-activation-command-result-receipt-terminal-public-claim-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  LIVE_JSON="$(
    curl -fsS \
      "$BASE_URL/api/hepta-memory-intelligence-kg-full-enablement-runtime-provider-router-activation-command-result-receipt-terminal-operator-decision-public-claim-non-promotion-denial"
  )"
  jq -e --argjson expected_route_count "$EXPECTED_ROUTE_COUNT" '
    ...
    and .runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_route_enabled == true
    and .runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_ready == true
    and .runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_status == "blocked"
    and .source_activation_command_result_receipt_final_operator_acknowledgement_ready == true
    and .source_activation_command_result_receipt_final_operator_acknowledgement_status == "blocked"
    and .required_activation_command_result_receipt_terminal_operator_decision_public_claim_surface_count == 12
    and .activation_command_result_receipt_terminal_operator_decision_public_claim_fixture_count == 10
    and .accepted_activation_command_result_receipt_terminal_operator_decision_public_claim_fixture_count == 0
    and .activation_command_result_receipt_terminal_operator_decision_performed_count == 0
    ...
    and .active_binary_mutated == false
    and (.denied_by_activation_command_result_receipt_terminal_operator_decision_public_claim | length) == 56
    and .source_final_operator_acknowledgement_denial_count == 38
    and (.side_effects | to_entries | all(.value == false))
  ' >/dev/null <<<"$LIVE_JSON"
  live_checked=true
fi
...
jq -n \
  --arg status "ready" \
  --arg gate "hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_route_gate" \
  --arg source_gate "hepta_memory_intelligence_kg_full_enablement_runtime_provider_router_activation_command_result_receipt_terminal_operator_decision_public_claim_non_promotion_denial_gate" \
  ...
  --argjson accepted_terminal_operator_decision_public_claim_fixture_count "$(jq -r '.accepted_activation_command_result_receipt_terminal_operator_decision_public_claim_fixture_count // 0' <<<"$SOURCE_JSON")" \
  --argjson terminal_operator_decision_performed_count "$(jq -r '.activation_command_result_receipt_terminal_operator_decision_performed_count // 0' <<<"$SOURCE_JSON")" \
  --argjson public_claim_promotion_performed_count "$(jq -r '.activation_command_result_receipt_public_claim_promotion_performed_count // 0' <<<"$SOURCE_JSON")" \
  --argjson source_terminal_operator_decision_public_claim_denial_count "$(jq -r '(.denied_by_activation_command_result_receipt_terminal_operator_decision_public_claim // []) | length' <<<"$SOURCE_JSON")" \
  '{
    status:$status,
    gate:$gate,
    ...
    accepted_terminal_operator_decision_public_claim_fixture_count:$accepted_terminal_operator_decision_public_claim_fixture_count,
    terminal_operator_decision_performed_count:$terminal_operator_decision_performed_count,
    public_claim_promotion_performed_count:$public_claim_promotion_performed_count,
    source_terminal_operator_decision_public_claim_denial_count:$source_terminal_operator_decision_public_claim_denial_count
  }'

echo "Hepta memory/intelligence/KG full enablement runtime provider-router activation command result receipt terminal operator decision public-claim non-promotion denial route gate passed"
