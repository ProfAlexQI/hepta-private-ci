...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-intent-consent-evidence-artifact-signing-replay-idempotency-readback-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session intent consent evidence artifact signing replay/idempotency readback report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session intent consent evidence artifact signing replay/idempotency final index report" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    ...
    source_public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_readback_surface: $source.surface,
    source_public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_readback_ready: $source.public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_readback_ready,
    source_public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_readback_blocked: $source.public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_readback_blocked,
    public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_final_index_ready: true,
    public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_final_index_blocked: true,
    public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_readback_attached: true,
    ...
    next_migration_step: "attach_public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_replay_idempotency_final_index_to_public_ga_operator_identity_session_operator_intent_consent_evidence_artifact_signing_ordering_monotonicity_without_receipt_replay",
    local_gate: "scripts/hepta-systems-public-ga-operator-identity-session-intent-consent-evidence-artifact-signing-replay-idempotency-final-index-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_INTENT_CONSENT_EVIDENCE_ARTIFACT_SIGNING_REPLAY_IDEMPOTENCY_FINAL_INDEX_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_intent_consent_evidence_artifact_signing_replay_idempotency_readback_report: "scripts/hepta-systems-public-ga-operator-identity-session-intent-consent-evidence-artifact-signing-replay-idempotency-readback-report.sh"
    },
