...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing memory write execution activation command result receipt ordering/monotonicity boundary source text: $label" >&2
    exit 1
  fi
}

NATIVE_GATEWAY_SOURCE="codex-rs/cli/src/native_gateway.rs"
ENDPOINT="/api/hepta-memory-live-mutation-operator-write-execution-activation-command-result-receipt-ordering-monotonicity-denial-boundary"
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes memory write execution activation command result receipt ordering/monotonicity denial boundary"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write execution activation command result receipt ordering/monotonicity denial boundary endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" "$ENDPOINT" \
  "memory write execution activation command result receipt ordering/monotonicity denial boundary endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" "$SOURCE_COMMAND" \
  "memory write execution activation command result receipt ordering/monotonicity denial boundary source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write execution activation command result receipt ordering/monotonicity denial boundary report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"memory_write_execution_activation_command_result_receipt_ordering_monotonicity_denial_boundary_ready\"" \
  "memory write execution activation command result receipt ordering/monotonicity denial boundary ready field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"denied_by_activation_command_result_receipt_ordering_monotonicity_count\"" \
  "memory write execution activation command result receipt ordering/monotonicity denied count"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "hepta_memory_write_execution_activation_command_result_receipt_ordering_monotonicity_denial_boundary_endpoint_blocks_ordering_and_monotonicity" \
  "focused memory write execution activation command result receipt ordering/monotonicity boundary unit test"

TEST_LOG="$(mktemp /tmp/hepta-memory-write-execution-activation-command-result-receipt-ordering-monotonicity-boundary-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  ...
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_write_execution_activation_command_result_receipt_ordering_monotonicity_denial_boundary_ready == true
    ...
    and .source_activation_command_result_receipt_replay_idempotency_boundary_ready == true
    and .source_activation_command_result_receipt_replay_idempotency_ready == true
    and .source_activation_command_result_receipt_replay_idempotency_boundary_report_sha256 != ""
    ...
    and .denied_by_activation_command_result_receipt_ordering_monotonicity_count == 24
    and (.denied_by_activation_command_result_receipt_ordering_monotonicity | length) == 24
    and .allowed_next_actions[0].action == "run_memory_write_execution_activation_command_result_receipt_ordering_monotonicity_denial_boundary_require_live_gate"
    and .allowed_next_actions[1].action == "prepare_memory_write_execution_activation_command_result_receipt_cancellation_supersession_denial_boundary"
...
jq -n \
  ...
  '{
    ...
    status:"ready",
    base_url:$base_url,
    gate:"hepta_memory_live_mutation_operator_write_execution_activation_command_result_receipt_ordering_monotonicity_denial_boundary_route_gate",
    endpoint:$endpoint,
    ...
    memory_write_execution_activation_command_result_receipt_ordering_monotonicity_denial_boundary_ready:true,
    ...
    denied_by_activation_command_result_receipt_ordering_monotonicity_count:24,
