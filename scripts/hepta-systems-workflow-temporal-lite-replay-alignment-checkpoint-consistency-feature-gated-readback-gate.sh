...
REPORT="$ROOT/scripts/hepta-systems-workflow-temporal-lite-replay-alignment-checkpoint-consistency-feature-gated-readback-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-workflow-temporal-lite-work-graph-projection-replay-alignment-feature-gated-readback-gate.sh"
...
fail() {
  printf 'hepta-systems-workflow-temporal-lite-replay-alignment-checkpoint-consistency-feature-gated-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Temporal-lite replay-alignment checkpoint consistency report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Temporal-lite WorkGraph projection replay-alignment gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Temporal-lite replay-alignment checkpoint consistency architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Temporal-lite replay-alignment checkpoint consistency report"
fi

grep -q 'Temporal-Lite Replay Alignment Checkpoint Consistency Feature-Gated Readback' "$DOC" \
  || fail "architecture note must document Temporal-Lite Replay Alignment Checkpoint Consistency Feature-Gated Readback"
grep -q 'test-only replay alignment checkpoint consistency readback' "$DOC" \
  || fail "architecture note must document test-only replay alignment checkpoint consistency readback"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "workflow_temporal_lite_replay_alignment_checkpoint_consistency_feature_gated_readback"
  and .status == "ready_blocked"
  and .gate == "workflow_temporal_lite_replay_alignment_checkpoint_consistency_feature_gated_readback_gate"
  and .schema_version == "workflow_temporal_lite_replay_alignment_checkpoint_consistency_feature_gated_readback_v1"
  and .source_replay_alignment_ready == true
  and .source_replay_alignment_entry_count == 9
  and .lib_export_present == true
  and .consistency_scope == "test_only_replay_alignment_checkpoint_consistency_readback_no_execution"
  ...
  and (.next_actions | index("temporal_lite_replay_alignment_rollback_consistency_feature_gated_readback")) != null
  and .recommended_next_gate == "temporal_lite_replay_alignment_rollback_consistency_feature_gated_readback"
...
(
  cd "$ROOT/codex-rs"
  cargo test -p hepta-runtime workflow_temporal_lite_replay_alignment_checkpoint_consistency_feature_gated_readback --lib
)

printf 'hepta-systems-workflow-temporal-lite-replay-alignment-checkpoint-consistency-feature-gated-readback-gate: PASS: Temporal-lite replay-alignment checkpoint consistency is projected, feature-gated, not executed, not persisted, and runtime-write/live blocked\n'
