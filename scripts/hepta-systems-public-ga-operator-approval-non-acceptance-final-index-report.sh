...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-approval-non-acceptance-readback-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator approval non-acceptance readback report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator approval non-acceptance final index report" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    ...
    source_public_ga_operator_approval_non_acceptance_readback_surface: $source.surface,
    source_public_ga_operator_approval_non_acceptance_readback_ready: $source.public_ga_operator_approval_non_acceptance_readback_ready,
    source_public_ga_operator_approval_non_acceptance_readback_blocked: $source.public_ga_operator_approval_non_acceptance_readback_blocked,
    ...
    public_ga_operator_approval_non_acceptance_final_index_ready: true,
    public_ga_operator_approval_non_acceptance_final_index_blocked: true,
    public_ga_operator_approval_non_acceptance_readback_attached: true,
    ...
    next_migration_step: "attach_public_ga_operator_approval_non_acceptance_final_index_to_public_ga_operator_identity_non_acceptance_without_approval_send",
    local_gate: "scripts/hepta-systems-public-ga-operator-approval-non-acceptance-final-index-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_APPROVAL_NON_ACCEPTANCE_FINAL_INDEX_2026-06-21.md",
    source_files: {
      public_ga_operator_approval_non_acceptance_readback_report: "scripts/hepta-systems-public-ga-operator-approval-non-acceptance-readback-report.sh"
    },
