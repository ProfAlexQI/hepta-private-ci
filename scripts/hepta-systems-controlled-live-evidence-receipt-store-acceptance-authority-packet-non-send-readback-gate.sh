...
REPORT="$ROOT/scripts/hepta-systems-controlled-live-evidence-receipt-store-acceptance-authority-packet-non-send-readback-report.sh"
...
fail() {
  printf 'hepta-systems-controlled-live-evidence-receipt-store-acceptance-authority-packet-non-send-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable acceptance authority packet non-send report: $REPORT"
...
if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the acceptance authority packet non-send report"
fi

rg -q 'Controlled Live Evidence Receipt Store Acceptance Authority Packet Non-Send Readback' "$DOC" \
  || fail "architecture note must document Controlled Live Evidence Receipt Store Acceptance Authority Packet Non-Send Readback"
rg -q 'controlled live evidence receipt store acceptance authority packet non-send readback' "$DOC" \
  || fail "architecture note must document acceptance authority packet non-send readback"
rg -q 'no operator packet send, send attempt record, operator packet persistence, acceptance authority acceptance, acceptance recording, evidence recording, receipt store write, receipt persistence, ledger write, event-log write, SQLite write, credential read, Native POST mutation, Telegram transport mutation, gateway/auth mutation, channel send, provider call, model call, replay execution, rollback, kill-switch rehearsal execution, kill-switch mutation, package, release, Public GA promotion, or live execution' "$DOC" \
  || fail "architecture note must document the closed non-send boundary"

"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback"
  and .status == "ready_blocked"
  and .gate == "controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback_gate"
  and .schema_version == "controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback_v1"
  and .plugin_id == "hepta-system@hepta-local"
  and .source_acceptance_authority_packet_ready == true
  ...
  and (.entries | all(
    (.id | startswith("evidence_receipt_store_acceptance_authority_packet_non_send_readback_"))
    ...
    and (.source_authority_decision_request_id | startswith("acceptance-authority-decision-request:controlled-live-evidence-receipt-store:"))
    and (.source_authority_decision_request_route | startswith("readback://controlled-live/evidence-receipt-store/acceptance-authority-packet/decision-request/"))
    and (.packet_non_send_readback_id | startswith("packet-non-send-readback:controlled-live-evidence-receipt-store:"))
    and (.packet_non_send_readback_route | startswith("readback://controlled-live/evidence-receipt-store/acceptance-authority-packet/non-send/"))
    and .packet_send_denial_reason == "operator_packet_send_disabled_acceptance_authority_missing_evidence_acceptance_missing"
    and .operator_status == "blocked_missing_evidence"
    and .observed_state == "acceptance_authority_packet_confirmed_unsent_without_acceptance"
  ...
  and any(.entries[]; .source_blocker_id == "operator_live_approval_missing" and .packet_non_send_readback_route == "readback://controlled-live/evidence-receipt-store/acceptance-authority-packet/non-send/operator-live-approval-missing")
  and (.next_actions | index("controlled_live_evidence_receipt_store_acceptance_authority_packet_persistence_denial_readback_without_persistence")) != null
  and .recommended_next_gate == "controlled_live_evidence_receipt_store_acceptance_authority_packet_persistence_denial_readback_without_persistence"
...
(
  cd "$ROOT/codex-rs"
  cargo test -p hepta-runtime controlled_live_evidence_receipt_store_acceptance_authority_packet_non_send_readback --lib
)

printf 'hepta-systems-controlled-live-evidence-receipt-store-acceptance-authority-packet-non-send-readback-gate: PASS: acceptance authority packet remains unsent without acceptance, persistence, or live execution\n'
