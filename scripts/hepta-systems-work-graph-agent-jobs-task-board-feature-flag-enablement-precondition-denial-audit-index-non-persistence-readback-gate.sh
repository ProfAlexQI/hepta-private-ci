...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-agent-jobs-task-board-feature-flag-enablement-precondition-denial-audit-index-non-persistence-readback-report.sh"
...
jq -e '
  ...
  and .runtime == "hepta"
  and .status == "ready"
  and .gate == "hepta_work_graph_agent_jobs_task_board_feature_flag_enablement_precondition_denial_audit_index_non_persistence_readback_gate"
  and .schema_version == "work_graph_agent_jobs_task_board_feature_flag_enablement_precondition_denial_audit_index_non_persistence_readback_v1"
  and .preview_mode == "denial_audit_index_non_persistence_readback_no_record_no_acceptance_no_write"
  and .source_denial_audit_index_gate == "hepta_work_graph_agent_jobs_task_board_feature_flag_enablement_precondition_denial_audit_index_gate"
  and .source_audit_index_entry_count == 6
  and .source_audit_index_blocker_count == 11
  and .source_required_prior_gate_count == 11
  and .readback_entry_count == 5
  and .readback_blocker_count == 12
  and .readback_scope.readback_mode == "denial_audit_index_non_persistence_readback_only"
  ...
  and .ready_for_operator_review_precondition_matrix == true
...
cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  work_graph_agent_jobs_task_board_feature_flag_enablement_precondition_denial_audit_index_non_persistence_readback --lib

echo "Hepta WorkGraph agent_jobs + task_board feature-flag enablement precondition denial audit index non-persistence readback gate passed"
