...
TOP_LEVEL_KEYS = {
    "current": {"a0_previous_exact_head_provenance", "active_phase", "authority", "canonical", "claim_levels", "current_truth", "evidence_model", "external_stop_conditions", "generated_at_utc", "new_required_contracts", "next_actions", "operational_execution", "q0_qualification", "repository", "roadmap", "schema", "session_bootstrap", "source_snapshot_policy", "stack_budget"},
    "document": {"authority", "claim_ladder_policy", "compatibility_contracts", "consumer_migration_receipts", "generated_at_utc", "immutable_tranche_snapshots", "precedence", "registered_canonical_inputs", "repository", "rules", "schema", "source_snapshot_policy"},
    "evidence": {"append_only_policy", "as_of_utc", "authority", "current_plan_source", "entries", "historical_evidence_policy", "index_id", "not_current_plan_authority", "q0_current_evidence", "repository", "schema", "source_snapshot_timestamp_policy"},
    "capability": {"as_of_utc", "authority", "capabilities", "classification", "current_plan_authority", "current_state_source", "invariants", "lifecycle", "registry_id", "repository", "schema", "source_snapshot_timestamp_policy"},
    "pr_stack": {"as_of_utc", "authority", "canonical_stack", "classification", "current_plan_authority", "external_unmerged_dependencies", "registry_id", "repository", "rules", "schema", "side_stacks", "source_snapshot_timestamp_policy"},
    "integration": {"allowed_changed_paths", "authority", "base", "branch", "candidate_head_binding", "candidate_id", "candidate_provenance_policy", "classification", "excluded_side_stack_pr_numbers", "expected_changed_path_count", "expected_parent", "external_unmerged_dependency_pr_numbers", "gap_closure_ledger", "gap_identity_registry", "multimodal_memory_gap_ledger", "operational_documents", "repository", "repository_check_classifications", "schema", "source_freeze", "source_snapshot_timestamp_policy"},
    "q0": {"as_of_utc", "authority", "classification", "conclusion", "current_plan_authority", "evidence_observation", "raw_artifact_receipts_embedded", "raw_artifact_receipts_location", "receipt_binding_sha256", "receipt_id", "schema", "source_writeback", "verifier_contract"},
}
EVIDENCE_STATES = {"CURRENT", "RETAINED", "HISTORICAL_FAILURE_RETAINED", "SUPERSEDED_PROVENANCE"}
EVIDENCE_CLASSIFICATIONS = {"E2_INDEPENDENT_RUNNER_REGRESSION", "DIAGNOSTIC_NOT_TRANCHE_QUALIFICATION", "QUALIFIED_NON_CANONICAL_SIDE_STACK", "E2_INDEPENDENT_RUNNER_EXECUTABLE_FAILURE", "E2_INDEPENDENT_RUNNER_SOURCE_ONLY", "PAIRED_EXECUTABLE_QUALIFICATION", "EXECUTABLE_EXACT_CANDIDATE", "INDEPENDENT_EXECUTABLE_EXACT_CANDIDATE"}
...
def validate_gap_entries(
    ...
    for raw in entries:
        ...
        exact_keys(raw, required | ({"payload_status"} if require_payload_status else set()), allowed, f"{label} entry")
        gap_id = raw["gap_id"]
        ...
        require(all(isinstance(dep, str) and dep for dep in raw["blocked_by"]), f"{gap_id} invalid dependency ID")
        require(len(raw["blocked_by"]) == len(set(raw["blocked_by"])), f"{gap_id} duplicate dependency")
    ...
    for gap_id, raw in seen.items():
        for dep in raw["blocked_by"]:
            require(dep in seen or dep in allowed_external, f"{gap_id} unknown dependency {dep}")
...
def validate_and_build() -> dict[str, Any]:
    ...
    normative = {"current": current, "document": document, "evidence": evidence, "capability": capability, "pr_stack": pr_stack, "integration": integration, "q0": q0}
    for label, doc in normative.items():
        exact_keys(doc, TOP_LEVEL_KEYS[label], TOP_LEVEL_KEYS[label], f"{label} top level")
    ...
    for label, mapping in (("current", current.get("authority")), ("document", document.get("authority")), ("evidence", evidence.get("authority")), ("capability", capability.get("authority")), ("pr_stack", pr_stack.get("authority")), ("integration", integration.get("authority")), ("q0", q0.get("authority"))):
        require(mapping == AUTHORITY_FALSE, f"{label} authority schema/value drift")
    require(all_false(claims.get("authority")), "claims authority must remain all false")
    ...
    require(current.get("a0_previous_exact_head_provenance") == ["241daa85253a6224a9473a1a2b2967f71e9b46af", "88b355931456603bd7a1dd9e0cdfd4d0d035dc0e", "0b80caff91010f40a79c795c20487ff9d773d229", "0a93b172e575670d20c30e2efa7f312097466fe6", "d93050c48e2943bb2be46e5f610c1bc109498194", "1146f0290fbbff7e6a26f6b91c80ec1e1daa0b60", "f09972580e7a10781bf48890f7eeeb2d9b9e945e"], "A0 provenance drift")
    ...
    require(claim_policy.get("source_repository") == "ProfAlexQI/hepta-private-ci", "claim policy source repository")
    require(claim_policy.get("repository_alias_classification") == "LEGACY_PRE_RENAME_NO_CURRENT_STATE_AUTHORITY", "claim policy alias classification")
    require(claim_policy.get("repository_alias_migration_required") is True, "claim policy alias migration")
    require(claims.get("repository") == claim_policy.get("source_repository"), "claim policy repository binding")
    require(claims.get("plan_id") == "HEPTA_INTELLIGENCE_MASTER_PLAN_V4", "claim policy plan")
    require(claims.get("current_state_authority") is False, "claim current state authority")
    require(claims.get("current_state_source") == READ_ORDER[0], "claim current state source")
    reference = claims.get("reference_baseline_claims", {})
    claim_levels = current.get("claim_levels", {})
    require(reference.get("system_learning_level") == claim_levels.get("system_learning"), "claim system level drift")
    ...
    for key in ("self_evolution", "longitudinal_learning_efficacy", "closed_loop_learning", "structural_plasticity", "neuromorphic_mechanism", "biological_mechanism_replication"):
        require(reference.get(key) == claim_levels.get(key), f"claim baseline drift: {key}")
    ...
    require(evidence.get("append_only_policy", {}) == {
    ...
    for entry in entries:
        ...
        require(entry.get("classification") in EVIDENCE_CLASSIFICATIONS, f"{entry_id} classification")
        require(type(entry.get("current")) is bool, f"{entry_id} current flag")
        require(entry.get("grants_capability_authority") is not True, f"{entry_id} grants authority")
        require(entry.get("canonical") is not True, f"{entry_id} side evidence became canonical")
        if entry["current"]:
            current_ids.add(entry_id)
            require(entry["entry_state"] == "CURRENT" and entry.get("conclusion") == "success", f"{entry_id} invalid current evidence")
        else:
            require(entry["entry_state"] != "CURRENT", f"{entry_id} stale current state")
        if entry["entry_state"] == "HISTORICAL_FAILURE_RETAINED":
            require(entry.get("conclusion") == "failure", f"{entry_id} retained failure conclusion")
        ...
        if digest is not None:
            require(isinstance(digest, str) and re.fullmatch(r"sha256:[0-9a-f]{64}", digest) is not None, f"{entry_id} artifact digest")
        for key in ("run_id", "workflow_run", "job_id", "runner_id", "artifact_id"):
            if key in entry:
                require(type(entry[key]) is int and entry[key] > 0, f"{entry_id}.{key}")
    require(current_ids == CURRENT_EVIDENCE_IDS, f"current evidence set drift: {sorted(current_ids)}")
    require(evidence.get("historical_evidence_policy", {}) == {
    ...
    require(isinstance(canonical_entries, list) and [entry.get("pr_number") for entry in canonical_entries] == [7, 13, 14, 16, 21, 23, 29], "canonical PR order")
    require(canonical_stack.get("candidate_head") == Q0_HEAD and canonical_stack.get("candidate_tree") == Q0_TREE, "canonical stack candidate")
    require(canonical_stack.get("candidate_qualified") is True, "canonical stack qualification")
    previous_head: str | None = None
    for index, entry in enumerate(canonical_entries):
        require(entry.get("canonical_inclusion") is True, f"canonical PR {index} inclusion")
        require(entry.get("draft") is True and entry.get("merged") is False and entry.get("state") == "open", f"canonical PR {index} state")
        require(entry.get("individual_runtime_authority") is False, f"canonical PR {index} runtime authority")
        require(isinstance(entry.get("head_sha"), str) and re.fullmatch(r"[0-9a-f]{40}", entry["head_sha"]) is not None, f"canonical PR {index} head")
        if previous_head is not None:
            require(entry.get("base_sha") == previous_head, f"canonical PR {index} linear base")
        previous_head = entry["head_sha"]
        qualification = entry.get("qualification", {})
        require(qualification.get("candidate_head") == Q0_HEAD and qualification.get("run_id") == 33252922404, f"canonical PR {index} qualification binding")
    require(previous_head == Q0_HEAD, "canonical stack terminal head")
    require(pr_stack.get("rules", {}) == {
    ...
    for stack in pr_stack.get("side_stacks", []):
        require(stack.get("canonical") is False, f"side stack {stack.get('stack_id')} canonical")
        ...
        for entry in stack.get("entries", []):
            require(entry.get("wired") is not True, f"side PR {entry.get('pr_number')} wired")
            require(entry.get("canonical") is not True and entry.get("authority") is not True, f"side PR {entry.get('pr_number')} authority")
    for entry in pr_stack.get("external_unmerged_dependencies", []):
        require(entry.get("canonical") is False and entry.get("authority") is False and entry.get("merged") is False, f"external PR {entry.get('pr_number')} authority/state")
    ...
    require(global_ledger.get("schema") == "hepta_intelligence_gap_closure_ledger_v1", "global ledger schema")
    require(global_ledger.get("allowed_classifications") == ["CLOSED_SOURCE_CONTROLLED", "OPEN_SOURCE_CONTROLLED", "BLOCKED_EXTERNAL_EVIDENCE", "BLOCKED_UPSTREAM", "STOP_CONDITION"], "global ledger classifications")
    require(global_ledger.get("as_of_utc") is None, "global ledger source timestamp")
    require(global_ledger.get("invariants", {}) == {
    ...
    global_entries = validate_gap_entries(global_ledger.get("entries"), GLOBAL_GAP_IDS, "global")
    global_ids = {item["gap_id"] for item in global_entries}
    mm = integration.get("multimodal_memory_gap_ledger", {})
    require(mm.get("schema") == "hepta_multimodal_memory_gap_ledger_v2" and mm.get("version") == "2.0.0", "multimodal ledger identity")
    require(mm.get("allowed_classifications") == ["CLOSED_SOURCE_CONTROLLED", "OPEN_SOURCE_CONTROLLED", "BLOCKED_EXTERNAL_EVIDENCE", "BLOCKED_UPSTREAM", "STOP_CONDITION"], "multimodal classifications")
    ...
    require(mm.get("duplicate_definitions_allowed") is False, "multimodal duplicate policy")
    require(mm.get("claim_boundary", {}) == {
    ...
    require(mm_contract.get("required_fields") == ["gap_id", "title", "classification", "status", "owner_class", "blocked_by", "acceptance_tests", "receipt_type", "closure_evidence", "next_action", "authority_effect", "rollback_pointer", "resume_predicate", "payload_status"], "multimodal entry contract")
    require(mm_contract.get("classification_field") == "classification" and mm_contract.get("status_field") == "status", "multimodal status fields")
    require(mm_contract.get("external_dependency_namespace") == "inherited_global_gap_ids_only", "multimodal dependency namespace contract")
    inherited = mm.get("inherited_global_gap_ids")
    require(isinstance(inherited, list), "multimodal inherited refs")
    require(inherited == sorted(set(inherited)), "multimodal inherited refs duplicate/order")
    require(set(inherited).issubset(global_ids), "unknown inherited global gap")
    mm_entries = validate_gap_entries(mm.get("entries"), None, "multimodal", set(inherited), True)
    ...
    for gap_id, receipt_type in MM_RECEIPT_TYPES.items():
        require(by_mm_id.get(gap_id, {}).get("receipt_type") == receipt_type, f"{gap_id} receipt type")
    ...
    require(registry.get("all_blocked_by_refs_registered") is True, "dependency registration invariant")
    require(registry.get("multimodal_external_dependency_namespace") == "inherited_global_gap_ids_only", "dependency namespace invariant")
    require(registry.get("dependency_lists_unique") is True, "dependency uniqueness invariant")
    require(registry.get("combined_dependency_graph_acyclic") is True, "combined DAG invariant")
    require(registry.get("receipt_type_mapping_enforced") is True, "receipt mapping invariant")

    def pull_request_paths(workflow_text: str) -> list[str]:
        lines = workflow_text.splitlines()
        try:
            start = next(i for i, line in enumerate(lines) if line == "  pull_request:")
            paths_start = next(i for i in range(start + 1, len(lines)) if lines[i] == "    paths:")
        ...
        for line in lines[paths_start + 1:]:
            if line.startswith("      - "):
                ...
                require(len(value) >= 2 and value[0] == value[-1] and value[0] in {"\"", "'"}, "workflow path quoting")
    ...
    for path in workflow_paths:
        ...
        require(pull_request_paths(text) == ALLOWED_PATHS, f"workflow trigger path/order drift in {path.name}")
        ...
        require(re.search(r"a0_candidate_qualified[\"']?\s*[:=]\s*(?:true|True)\b", text) is None, f"workflow self qualification in {path.name}")
