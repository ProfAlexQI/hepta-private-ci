...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-operator-facing-summary-briefing-readback-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-export-query-observability-final-index-summary-briefing-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_OPERATOR_FACING_SUMMARY_BRIEFING_READBACK_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-identity-session-operator-facing-summary-briefing-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session summary briefing readback report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Public GA operator identity/session summary briefing attachment gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session summary briefing readback architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session summary briefing readback report"
fi

grep -q 'Public GA Operator Identity/Session Operator-Facing Summary Briefing Readback' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Operator-Facing Summary Briefing Readback"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_operator_facing_summary_briefing_readback"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_surface == "public_ga_operator_identity_session_operator_facing_summary_briefing_attachment"
  and .source_public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_ready == true
  and .source_public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_blocked == true
  ...
  and .public_ga_operator_identity_session_operator_facing_summary_briefing_attachment_attached == true
  and .public_ga_operator_identity_session_export_query_observability_final_index_attached == true
  and .readback_mode == "static_operator_identity_session_summary_briefing_snapshot_only"
  and .readback_check_count == 48
  ...
  and .next_migration_step == "derive_public_ga_operator_identity_session_operator_facing_summary_briefing_final_index_without_export"
...
printf 'hepta-systems-public-ga-operator-identity-session-operator-facing-summary-briefing-readback-gate: PASS: Public GA operator identity/session summary briefing readback is static and blocked\n'
