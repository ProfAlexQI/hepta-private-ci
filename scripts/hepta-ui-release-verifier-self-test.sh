...
if [[ "$PROFILE" == bridge ]]; then
  ...
  CHECKS="{\"canonical_loopback_endpoint\":true,\"exact_get_request\":true,\"http_status_200\":true,\"response_deserialized\":true,\"matrix_session_authenticated\":true,\"explicit_user_opt_in\":true,\"session_match\":true,\"correlation_match\":true,\"authoritative_origin_valid\":true,\"redaction_valid\":true,\"provenance_valid\":true,\"raw_source_payload_rejected\":true,\"logout_transport_dropped\":true,\"login_failure_transport_dropped\":true,\"subscribe\":false,\"prepare\":false,\"confirm\":false,\"reject\":false,\"cancel\":false,\"provider_invocation\":false,\"channel_delivery\":false,\"cursor_write\":false,\"gateway_mutation\":false,\"external_mutation\":false,\"endpoint\":\"/api/hepta-native-bridge/v1/snapshot\",\"method\":\"GET\",\"content_type\":\"application/json\",\"request_descriptor_sha256\":\"8888888888888888888888888888888888888888888888888888888888888888\",\"response_sha256\":\"8989898989898989898989898989898989898989898989898989898989898989\",\"session_identifier_sha256\":\"6666666666666666666666666666666666666666666666666666666666666666\",\"correlation_identifier_sha256\":\"9090909090909090909090909090909090909090909090909090909090909090\",\"matrix_attestation_sha256\":\"$(sha "$MATRIX_RECEIPT")\",\"response_byte_count\":64,\"bridge_get_observed_unix_ms\":$BRIDGE_MS,\"mutation_capability_count\":0}"
fi
...
if [[ "$PROFILE" == bridge ]]; then
  /usr/bin/jq -e --arg parent "$(sha "$MATRIX_RECEIPT")" '.live_chain_binding.matrix_attestation_sha256==$parent and .live_chain_binding.parent_signature_verified==true and .live_chain_binding.session_match_verified==true and .live_chain_binding.run_match_verified==true' "$TEST_DIR/ready.json" >/dev/null || fail "parent Matrix provenance missing"
fi
