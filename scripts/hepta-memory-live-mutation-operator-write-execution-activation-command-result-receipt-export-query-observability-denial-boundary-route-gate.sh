...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing memory write execution activation command result receipt export/query/observability boundary source text: $label" >&2
    exit 1
  fi
}

NATIVE_GATEWAY_SOURCE="codex-rs/cli/src/native_gateway.rs"
ENDPOINT="/api/hepta-memory-live-mutation-operator-write-execution-activation-command-result-receipt-export-query-observability-denial-boundary"
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes memory write execution activation command result receipt export/query/observability denial boundary"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write execution activation command result receipt export/query/observability denial boundary endpoint constant"
require_source_text "$NATIVE_GATEWAY_SOURCE" "$ENDPOINT" \
  "memory write execution activation command result receipt export/query/observability denial boundary endpoint path"
require_source_text "$NATIVE_GATEWAY_SOURCE" "$SOURCE_COMMAND" \
  "memory write execution activation command result receipt export/query/observability denial boundary source command"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write execution activation command result receipt export/query/observability denial boundary report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"memory_write_execution_activation_command_result_receipt_export_query_observability_denial_boundary_ready\"" \
  "memory write execution activation command result receipt export/query/observability denial boundary ready field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"denied_by_activation_command_result_receipt_export_query_observability_count\"" \
  "memory write execution activation command result receipt export/query/observability denied count"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "hepta_memory_write_execution_activation_command_result_receipt_export_query_observability_denial_boundary_endpoint_blocks_visibility" \
  "focused memory write execution activation command result receipt export/query/observability boundary unit test"

TEST_LOG="$(mktemp /tmp/hepta-memory-write-execution-activation-command-result-receipt-export-query-observability-boundary-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  ...
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_write_execution_activation_command_result_receipt_export_query_observability_denial_boundary_ready == true
    and .activation_command_result_receipt_export_query_observability_mode == "memory_write_execution_activation_command_result_receipt_export_query_observability_denial"
    and .source_activation_command_result_receipt_retention_expiry_garbage_collection_boundary_ready == true
    and .source_activation_command_result_receipt_retention_expiry_garbage_collection_ready == true
    and .source_activation_command_result_receipt_retention_expiry_garbage_collection_boundary_report_sha256 != ""
    ...
    and .activation_command_result_receipt_export_denied_count == 10
    and .activation_command_result_receipt_query_denied_count == 10
    and .activation_command_result_receipt_observability_denied_count == 10
    ...
    and .activation_command_result_receipt_query_result_materialized == false
    ...
    and (.activation_command_result_receipt_export_query_observability_surfaces | length) == 12
    and (.activation_command_result_receipt_export_query_observability_fixtures | length) == 10
    and (.activation_command_result_receipt_export_query_observability_fixtures | all((.export_query_observability_status == "blocked_noop" or .export_query_observability_status == "blocked_export_noop" or .export_query_observability_status == "blocked_query_noop" or .export_query_observability_status == "blocked_observability_noop") and .export_request_accepted == false and .export_artifact_written == false and .export_stream_opened == false and .query_registered == false and .query_endpoint_materialized == false and .query_index_recorded == false and .query_cache_written == false and .observability_metric_emitted == false and .observability_log_recorded == false and .observability_trace_recorded == false and .observability_dashboard_materialized == false and .observability_alert_registered == false and .observability_slo_recorded == false and .receipt_recorded == false and .receipt_persisted == false and .receipt_accepted == false and .activation_allowed == false and .live_mutation_execution_performed == false and .memory_store_write_performed == false and .memory_store_mutated == false and .rollback_executed == false and .receipt_noop_confirmed == true))
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.source_retention_expiry_gc_present == false)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.export_file_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.export_stream_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.query_endpoint_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.query_index_requested == true and .query_cache_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.metric_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.trace_requested == true and .span_requested == true and .log_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.dashboard_requested == true and .alert_requested == true and .slo_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.activation_from_observability_requested == true and .memory_write_observability_requested == true and .rollback_observability_requested == true and .provider_prompt_observability_requested == true)] | length) == 1
    and ([.activation_command_result_receipt_export_query_observability_fixtures[] | select(.external_send_observability_requested == true and .install_observability_requested == true and .active_binary_observability_requested == true)] | length) == 1
    and .denied_by_activation_command_result_receipt_export_query_observability_count == 30
    and (.denied_by_activation_command_result_receipt_export_query_observability | length) == 30
    and .allowed_next_actions[0].action == "run_memory_write_execution_activation_command_result_receipt_export_query_observability_denial_boundary_require_live_gate"
    and .allowed_next_actions[1].action == "prepare_memory_write_execution_activation_command_result_receipt_operator_facing_summary_briefing_non_persistence_denial_boundary"
...
jq -n \
  ...
  '{
    ...
    runtime: $runtime,
    status: "ready",
    gate: "hepta_memory_live_mutation_operator_write_execution_activation_command_result_receipt_export_query_observability_denial_boundary_route_gate",
    endpoint: $endpoint,
