...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-td3-rbtd5-td-rt-td-rt-td-rt-td-rt-td-rt-td-rt-td-rba-preview-report.sh"
...
jq -e '
  ...
  and .source_probes.deep_td8_td3_rbackack_td18_receipt_retention_readback_ack.rust_module_present == true
  and .source_probes.prior_deep_td8_td3_rbackack_td18_receipt_retention_readback_receipt.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_dtd8_td3_rbtd_rbtd_rbtd_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_rbackack_preview --lib

echo "Hepta WorkGraph deep td8 td3 rbackack td18 receipt retention readback ack preview gate passed"
