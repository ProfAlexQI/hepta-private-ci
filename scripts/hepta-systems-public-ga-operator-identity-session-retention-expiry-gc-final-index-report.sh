...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-retention-expiry-gc-readback-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session retention expiry GC readback report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session retention expiry GC final index report" >&2
  exit 1
fi
...
jq -e '
  .surface == "public_ga_operator_identity_session_retention_expiry_gc_readback"
  and .public_ga_operator_identity_session_retention_expiry_gc_readback_ready == true
  and .public_ga_operator_identity_session_retention_expiry_gc_readback_blocked == true
...
jq -n \
  ...
  '{
    ...
    source_public_ga_operator_identity_session_retention_expiry_gc_readback_surface: $source.surface,
    source_public_ga_operator_identity_session_retention_expiry_gc_readback_ready: $source.public_ga_operator_identity_session_retention_expiry_gc_readback_ready,
    source_public_ga_operator_identity_session_retention_expiry_gc_readback_blocked: $source.public_ga_operator_identity_session_retention_expiry_gc_readback_blocked,
    public_ga_operator_identity_session_retention_expiry_gc_final_index_ready: true,
    public_ga_operator_identity_session_retention_expiry_gc_final_index_blocked: true,
    public_ga_operator_identity_session_retention_expiry_gc_readback_attached: true,
    public_ga_operator_identity_session_audit_evidence_final_index_attached: $source.public_ga_operator_identity_session_audit_evidence_final_index_attached,
    ...
    operator_identity_session_retention_expiry_gc_denial_gate_present: $source.operator_identity_session_retention_expiry_gc_denial_gate_present,
    operator_identity_session_retention_expiry_gc_denial_doc_present: $source.operator_identity_session_retention_expiry_gc_denial_doc_present,
    ...
    long_soak_required_by_source_retention_gate: $source.long_soak_required_by_source_retention_gate,
    ...
    next_migration_step: "attach_public_ga_operator_identity_session_retention_expiry_gc_final_index_to_public_ga_operator_identity_session_export_query_observability_without_retention",
    local_gate: "scripts/hepta-systems-public-ga-operator-identity-session-retention-expiry-gc-final-index-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_RETENTION_EXPIRY_GC_FINAL_INDEX_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_retention_expiry_gc_readback_report: "scripts/hepta-systems-public-ga-operator-identity-session-retention-expiry-gc-readback-report.sh"
    },
