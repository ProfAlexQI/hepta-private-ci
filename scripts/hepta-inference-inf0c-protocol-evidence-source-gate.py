#!/usr/bin/env python3
"""Fail-closed source gate for the restacked INF-0C protocol-evidence tranche."""
...
def git(*args: str, check: bool = True) -> str:
    run = subprocess.run(["git", *args], cwd=ROOT, check=False, capture_output=True, text=True)
...
def main() -> int:
    protocol, self_test, contract, workflow = map(text, (PROTOCOL, SELF_TEST, CONTRACT, WORKFLOW))
    ...
    for label, value in (("status", status), ("matrix", matrix), ("receipt", receipt)):
        ...
        closed(value, label)
    require(status.get("protocol_development_branch") == PROTOCOL_BRANCH, "status branch drift")
    require(matrix.get("protocol_branch") == PROTOCOL_BRANCH, "matrix branch drift")
    require(status.get("status") == matrix.get("overall_status") == "SOURCE_PRESENT_NOT_RUN", "status drift")
    require(status.get("qualified") is False and receipt.get("qualified") is False, "qualified early")

    markers(protocol, (
        'TOOL_NAME = "hepta_probe"', 'TOOL_NONCE = "HEPTA_INF0C_TOOL"', "TOOL_VALUE = 7",
        "tool_choice", "parallel_tool_calls", "additionalProperties", "extract_exact_tool_call",
        "must return exactly one function call", "raw_arguments_persisted", "canonical_inventory",
        "model inventory changed during qualification", "raw_model_ids_persisted", "implicit_download",
        "MAX_SSE_EVENT_BYTES", "MAX_SSE_TOTAL_BYTES", "MAX_SSE_EVENTS", "ALLOWED_SSE_EVENT_TYPES",
        "read_strict_sse", "SSE sequence number is not monotonic",
        "SSE stream ended without response.completed", "SSE event appeared after response.completed",
        "legacy [DONE] sentinel is not accepted", "raw_events_persisted", '"qualified": False',
    ), "protocol harness")
    for banned in ('"qualified": True', '"implicit_download": True', '"raw_events_persisted": True', "shell=True"):
        require(banned not in protocol, f"banned protocol path: {banned}")
    markers(self_test, (
        "ThreadingHTTPServer", "/malformed-json", "/unknown-event", "/truncated",
        "/duplicate-completion", "/event-after-completion", "/bad-sequence", "/legacy-done",
        "/oversized", "/bad-media", "/slow", "PASS_HEPTA_INFERENCE_INF0C_PROTOCOL_SSE_SELF_TEST",
    ), "protocol self-test")
    markers(contract, ("hepta_probe", "HEPTA_INF0C_TOOL", "model inventory", "text/event-stream",
                       "raw_arguments_persisted=false", "SOURCE_PRESENT_NOT_RUN", "INF-1=NOT_STARTED"),
            "protocol contract")
    markers(workflow, (
        "ref: ${{ github.event.pull_request.head.sha || github.sha }}", "fetch-depth: 2",
        ...
        "python3 scripts/hepta-inference-inf0-source-gate.py", "--execute", "--ollama-model", "--lmstudio-model",
    ), "protocol workflow")

    implemented = status.get("implemented", {})
    for flag in (
        "real_e2e_tool_call_harness", "real_e2e_exact_tool_choice",
        "real_e2e_tool_arguments_digest_only", "real_e2e_model_inventory_stability_fence",
        "real_e2e_no_implicit_download_inventory_fence", "real_e2e_strict_sse_parser",
        "real_e2e_sse_sequence_fence", "real_e2e_sse_terminal_fence",
        "real_e2e_sse_total_and_event_bounds", "real_e2e_malformed_event_fixture", "real_e2e_timeout_fixture",
    ):
    ...
    for flag in (
        "tool_call_harness_source_complete", "model_inventory_stability_source_complete",
        "strict_sse_parser_source_complete", "malformed_event_fixture_source_complete",
        "timeout_fixture_source_complete", "no_implicit_download_inventory_fence_source_complete",
    ):
        require(inf0c.get(flag) is True, f"INF-0C source flag false: {flag}")
    for flag in ("real_tool_call_executed", "real_model_inventory_stability_verified",
                 "real_strict_sse_executed", "backend_cancellation_acknowledged", "qualified"):
        require(inf0c.get(flag) is False, f"execution claimed early: {flag}")
    ...
    require(stack_base == STACK_BASE, "protocol source not stacked on current evidence-v2 receipt")
    require(receipt.get("source_candidate_commit") == source, "receipt source mismatch")
    require(receipt.get("source_candidate_tree") == source_tree, "receipt tree mismatch")
    require(receipt.get("parent_stack_receipt_commit") == STACK_BASE, "receipt stack-base drift")
    changed_source = {line for line in git("diff", "--name-only", STACK_BASE, source).splitlines() if line}
    require(changed_source == SOURCE_FILES, "unexpected protocol source file set")
    changed_receipt = {line for line in git("diff", "--name-only", source, candidate).splitlines() if line}
    require(changed_receipt == {str(RECEIPT.relative_to(ROOT))}, "receipt commit must be receipt-only")
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL_HEPTA_INFERENCE_INF0C_PROTOCOL_SOURCE_GATE: {error}", file=sys.stderr)
