...
prior_report_script="scripts/hepta-systems-work-graph-deep-td8-receipt-retention-readback-ack-preview-report.sh"
prior_gate_script="scripts/hepta-systems-work-graph-deep-td8-receipt-retention-readback-ack-preview-gate.sh"
...
rust_module="codex-rs/hepta-runtime/src/wg_deep_td8_receipt_retention_readback_ack_replay_preview.rs"
report_script="scripts/hepta-systems-work-graph-deep-td8-receipt-retention-readback-ack-replay-preview-report.sh"
gate_script="scripts/hepta-systems-work-graph-deep-td8-receipt-retention-readback-ack-replay-preview-gate.sh"
...
jq -n \
  ...
  '
  def ack_ids: [range(0; 6) | "deep_td8_readback_ack_\(.)"];
  def items($prefix; $count): [range(0; $count) | {id: "\($prefix)_\(.)", source_acknowledgement_ids: ack_ids, required_fields: ["idempotencyKey", "priorGateDigest", "zeroEffectHash"], blocks_replay_mutation: true, blocks_acknowledgement_recording: true, blocks_acceptance: true, blocks_authority: true, blocks_rollout: true, blocks_release_publication: true, blocks_public_claim: true, blocks_external_delivery: true, required: true}];
  {
    ...
    replay_scenarios: items("deep_td8_readback_ack_replay_scenario"; 6),
    idempotency_guards: items("deep_td8_readback_ack_replay_idempotency_guard"; 7),
    replay_denials: items("deep_td8_readback_ack_replay_denial"; 7),
    monotonicity_checks: items("deep_td8_readback_ack_replay_monotonicity_check"; 5),
    local_views: items("deep_td8_readback_ack_replay_view"; 4),
    invariants: items("deep_td8_readback_ack_replay_invariant"; 6),
