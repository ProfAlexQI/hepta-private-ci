...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-revocation-logout-final-index-report.sh"
READBACK_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-revocation-logout-readback-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_REVOCATION_LOGOUT_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-identity-session-revocation-logout-final-index-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session revocation logout final index report: $REPORT"
[[ -x "$READBACK_GATE" ]] || fail "missing executable Public GA operator identity/session revocation logout readback gate: $READBACK_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session revocation logout final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session revocation logout final index report"
fi

grep -q 'Public GA Operator Identity/Session Revocation Logout Final Index' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Revocation Logout Final Index"
grep -q 'ready-but-blocked' "$DOC" \
  || fail "architecture note must document ready-but-blocked status"
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that final index does not invoke revocation/logout gates"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_revocation_logout_final_index"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_revocation_logout_readback_surface == "public_ga_operator_identity_session_revocation_logout_readback"
  and .source_public_ga_operator_identity_session_revocation_logout_readback_ready == true
  and .source_public_ga_operator_identity_session_revocation_logout_readback_blocked == true
  ...
  and .public_ga_operator_identity_session_revocation_logout_readback_attached == true
  ...
  and .next_migration_step == "attach_public_ga_operator_identity_session_revocation_logout_final_index_to_public_ga_operator_identity_session_reinstatement_denial_without_revocation_or_logout"
...
printf 'hepta-systems-public-ga-operator-identity-session-revocation-logout-final-index-gate: PASS: Public GA operator identity/session revocation logout final index is ready but blocked\n'
