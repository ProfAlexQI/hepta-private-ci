...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-final-acknowledgement-final-index-terminal-decision-status-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-final-acknowledgement-final-index-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_FINAL_ACKNOWLEDGEMENT_FINAL_INDEX_TERMINAL_DECISION_STATUS_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-final-acknowledgement-final-index-terminal-decision-status-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session terminal decision/status attachment report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Public GA operator identity/session final acknowledgement final index gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session terminal decision/status attachment architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session terminal decision/status attachment report"
fi

grep -q 'Public GA Operator Identity/Session Terminal Decision/Status Promotion Attachment' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Terminal Decision/Status Promotion Attachment"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_terminal_decision_status_promotion_attachment"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_surface == "public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index"
  and .source_public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_ready == true
  and .source_public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_blocked == true
  and .public_ga_operator_identity_session_final_acknowledgement_non_acceptance_final_index_attached == true
  and .public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_ready == true
  and .public_ga_operator_identity_session_terminal_decision_status_promotion_attachment_blocked == true
  ...
  and .operator_identity_session_terminal_decision_status_static_mention_count >= 20
  ...
  and .revocation_replay_terminal_decision_recorded == false
  and .logout_replay_status_promoted == false
  ...
  and .attachment_blocker_count == 46
  and .public_ga_claim_allowed == false
  and .public_ga_claimed == false
  and .public_release_published == false
  and .rollback_execution_allowed == false
  and .next_migration_step == "derive_public_ga_operator_identity_session_terminal_decision_status_promotion_readback_without_acknowledgement"
...
printf 'hepta-systems-public-ga-operator-final-acknowledgement-final-index-terminal-decision-status-gate: PASS: Public GA operator identity/session terminal decision/status attachment is ready but blocked without acknowledgement\n'
