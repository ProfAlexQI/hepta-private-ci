...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-retention-expiry-gc-final-index-export-query-observability-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-retention-expiry-gc-final-index-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_RETENTION_EXPIRY_GC_FINAL_INDEX_EXPORT_QUERY_OBSERVABILITY_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-retention-expiry-gc-final-index-export-query-observability-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session export query observability attachment report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Public GA operator identity/session retention expiry GC final index gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session export query observability attachment architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session export query observability attachment report"
fi

grep -q 'Public GA Operator Identity/Session Export Query Observability Attachment' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Export Query Observability Attachment"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_export_query_observability_attachment"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_retention_expiry_gc_final_index_surface == "public_ga_operator_identity_session_retention_expiry_gc_final_index"
  and .source_public_ga_operator_identity_session_retention_expiry_gc_final_index_ready == true
  and .source_public_ga_operator_identity_session_retention_expiry_gc_final_index_blocked == true
  ...
  and .operator_identity_session_export_query_observability_static_mention_count >= 20
  ...
  and .attachment_blocker_count == 40
  and .manual_operator_live_cutover_approval_required == true
  and .public_ga_claim_allowed == false
  and .public_ga_claimed == false
  and .public_release_published == false
  and .rollback_execution_allowed == false
  and .next_migration_step == "derive_public_ga_operator_identity_session_export_query_observability_readback_without_retention"
...
printf 'hepta-systems-public-ga-operator-retention-expiry-gc-final-index-export-query-observability-gate: PASS: Public GA operator identity/session export query observability attachment is ready but blocked without retention\n'
