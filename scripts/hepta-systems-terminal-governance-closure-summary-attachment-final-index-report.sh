...
READBACK_REPORT="$ROOT/scripts/hepta-systems-terminal-governance-closure-summary-attachment-readback-report.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_TERMINAL_GOVERNANCE_CLOSURE_SUMMARY_ATTACHMENT_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-terminal-governance-closure-summary-attachment-final-index-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$READBACK_REPORT" ]] || fail "missing executable terminal governance closure summary attachment readback report: $READBACK_REPORT"
[[ -f "$DOC" ]] || fail "missing terminal governance closure summary attachment final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the terminal governance closure summary attachment final index report"
fi

jq -n \
  --slurpfile readback <("$READBACK_REPORT") \
  --arg gate "scripts/hepta-systems-terminal-governance-closure-summary-attachment-final-index-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_TERMINAL_GOVERNANCE_CLOSURE_SUMMARY_ATTACHMENT_FINAL_INDEX_2026-06-21.md" \
  '
  ...
  ($readback.terminal_governance_closure_summary_attachment_readback_ready == true
    and $readback.terminal_governance_closure_summary_attachment_readback_blocked == true
    and $readback.terminal_governance_closure_summary_gate_present == true
  ...
  {
    runtime:"hepta",
    surface:"terminal_governance_closure_summary_attachment_final_index",
    plugin_id:$readback.plugin_id,
    status:(if $final_index_ready then "ready_blocked" else "blocked" end),
    source_terminal_governance_closure_summary_attachment_readback_surface:$readback.surface,
    source_terminal_governance_closure_summary_attachment_readback_ready:$readback.terminal_governance_closure_summary_attachment_readback_ready,
    source_terminal_governance_closure_summary_attachment_readback_blocked:$readback.terminal_governance_closure_summary_attachment_readback_blocked,
    terminal_governance_closure_summary_attachment_final_index_ready:$final_index_ready,
    terminal_governance_closure_summary_attachment_final_index_blocked:true,
    terminal_denial_summary_final_index_attached:$readback.terminal_denial_summary_final_index_attached,
    ...
    next_migration_step:"attach_terminal_governance_closure_summary_attachment_final_index_to_terminal_release_governance_without_live_gate_invocation",
    local_gate:$gate,
    architecture_note:$doc,
    source_files:{
      terminal_governance_closure_summary_attachment_readback_report:"scripts/hepta-systems-terminal-governance-closure-summary-attachment-readback-report.sh"
    },
