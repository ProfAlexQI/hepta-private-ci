...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-recording-denial-persistence-denial-retention-replay-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_terminal_no_persistence.rs"
LIB_SOURCE="$ROOT/codex-rs/hepta-runtime/src/lib.rs"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_RECORDING_DENIAL_PERSISTENCE_DENIAL_TERMINAL_NO_PERSISTENCE_2026-07-07.md"

fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-recording-denial-persistence-denial-terminal-no-persistence-report: FAIL: %s\n' "$1" >&2
  exit 1
}
...
if grep -q 'controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

source_json="${HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_RECORDING_DENIAL_RECEIPT_PERSISTENCE_DENIAL_RETENTION_REPLAY_JSON:-}"
...
jq -n \
  ...
  --arg gate "scripts/hepta-systems-cler-store-local-evidence-acceptance-recording-denial-persistence-denial-terminal-no-persistence-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_CLER_STORE_LOCAL_EVIDENCE_ACCEPTANCE_RECORDING_DENIAL_PERSISTENCE_DENIAL_TERMINAL_NO_PERSISTENCE_2026-07-07.md" \
  --arg collection_id "controlled-live-evidence-receipt-store-local-evidence-acceptance-recording-denial-receipt-persistence-denial-terminal-no-persistence" \
  --arg collection_route "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/recording-denial-receipts/persistence-denial/terminal-no-persistence" \
  '
  def hyphen_id($id): $id | gsub("_"; "-");
  def count_true($entries; $key): $entries | map(select(.[$key] == true)) | length;
  ($source[0]) as $src |
  ($src.entries | map({
    id:("evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_terminal_no_persistence_" + .source_blocker_id),
    source_blocker_id,
    ...
    terminal_closeout_id:("local-evidence-acceptance-recording-denial-receipt-persistence-denial-terminal-no-persistence:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    terminal_closeout_key:("terminal-no-persistence:local-evidence-acceptance-recording-denial-receipt-persistence-denial:controlled-live-evidence-receipt-store:" + .source_blocker_id),
    terminal_closeout_route:($collection_route + "/" + hyphen_id(.source_blocker_id)),
    terminal_reason:"local_evidence_acceptance_recording_denial_receipt_persistence_denied_retention_replay_projected_no_persistence_authority",
    terminal_state:"terminal_no_persistence",
    observed_state:"local_evidence_acceptance_recording_denial_receipt_persistence_denial_terminal_no_persistence_closed",
  ...
  ($src.retention_replay_readback_ready == true
    ...
    and $src.source_acceptance_source_record_attached_count == 7
    ...
    and $live_mutation_allowed_count == 0) as $ready |
  {
    ...
    gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback_gate",
    schema_version:"controlled_live_evidence_receipt_store_local_evidence_acceptance_recording_denial_receipt_persistence_denial_terminal_no_persistence_readback_v1",
    ...
    source_acceptance_source_record_attached_count:$src.source_acceptance_source_record_attached_count,
    ...
    entries:$entries,
    next_actions:["controlled_live_evidence_receipt_store_local_evidence_acceptance_positive_preconditions_readback_without_acceptance"],
    recommended_next_gate:"controlled_live_evidence_receipt_store_local_evidence_acceptance_positive_preconditions_readback_without_acceptance",
    local_gate:$gate,
