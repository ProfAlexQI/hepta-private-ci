...
for term in \
  ...
  "approval report 58 lines" \
  "negative export report 4 lines" \
  "combined report 62 lines" \
...
if [ "$(line_count "$approval_status")" != "58" ]; then
...
if [ "$(line_count "$combined_status")" != "62" ]; then
  fail "combined canonical line count changed"
fi

if [ "$(printf '%s\n' "$approval_status" | sha256_digest)" != "e85aad630100faabcda6f14dfce9e96d17cefdd7a6d8c96e3f29e05a97858fc7" ]; then
...
if [ "$(printf '%s\n' "$combined_status" | sha256_digest)" != "56ea3c7291e544029ef08709616ce834e29e6978db4398b3b4d456866922d47e" ]; then
...
echo "context-plane-operator-approval-packet-canonical-export-digest.approval-report-lines=58"
echo "context-plane-operator-approval-packet-canonical-export-digest.negative-export-report-lines=4"
echo "context-plane-operator-approval-packet-canonical-export-digest.combined-report-lines=62"
