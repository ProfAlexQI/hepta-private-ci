...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-workflow-temporal-lite-deterministic-replay-validator-local-persistence-readback-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/workflow_temporal_lite_checkpoint_and_rollback_anchor_local_persistence_readback.rs"
...
fail() {
  printf 'hepta-systems-workflow-temporal-lite-checkpoint-and-rollback-anchor-local-persistence-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable Temporal-lite local persistence replay validator report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing Temporal-lite local checkpoint/rollback anchor Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing Temporal-lite local checkpoint/rollback anchor architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the Temporal-lite local checkpoint/rollback anchor report"
fi

lib_export_present=false
if grep -q 'hepta_workflow_temporal_lite_checkpoint_and_rollback_anchor_local_persistence_readback_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

reopened_sqlite_anchor_test_present=false
if grep -q 'local_checkpoint_and_rollback_anchor_readback_uses_reopened_sqlite_history' "$RUST_SOURCE"; then
  reopened_sqlite_anchor_test_present=true
fi

jq -n \
  --slurpfile source <("$SOURCE_REPORT") \
  --argjson lib_export_present "$lib_export_present" \
  --argjson reopened_sqlite_anchor_test_present "$reopened_sqlite_anchor_test_present" \
  --arg gate "scripts/hepta-systems-workflow-temporal-lite-checkpoint-and-rollback-anchor-local-persistence-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_WORKFLOW_TEMPORAL_LITE_CHECKPOINT_AND_ROLLBACK_ANCHOR_LOCAL_PERSISTENCE_READBACK_2026-06-30.md" \
  '
  def pad3($n):
  ...
  def anchor_entry($entry): {
    ...
    checkpoint_anchor_key:("temporal-lite.local-checkpoint-anchor." + pad3($entry.replay_order) + "." + $entry.event_contract_id),
    rollback_anchor_key:("temporal-lite.local-rollback-anchor." + pad3($entry.replay_order) + "." + $entry.event_contract_id),
    checkpoint_source_key:$entry.checkpoint_key,
    rollback_source_anchor:$entry.rollback_anchor,
    replay_batch_digest:$entry.replay_batch_digest,
    checkpoint_readback_digest:("temporal-lite.local-checkpoint-anchor-digest.v1." + pad3($entry.replay_order) + "." + $entry.event_contract_id + "." + (($entry.replay_checksum | length) | tostring) + "." + (($entry.replay_batch_digest | length) | tostring)),
    rollback_readback_digest:("temporal-lite.local-rollback-anchor-digest.v1." + pad3($entry.replay_order) + "." + $entry.event_contract_id + "." + (($entry.replay_checksum | length) | tostring) + "." + (($entry.replay_batch_digest | length) | tostring)),
    anchor_pair_state:"projected_from_local_persistence_readback_without_anchor_writes",
    ...
    checkpoint_digest_validated:($entry.replay_checksum_validated == true and $entry.replay_batch_digest_validated == true),
    rollback_digest_validated:($entry.replay_checksum_validated == true and $entry.replay_batch_digest_validated == true),
      ...
      and .live_execution_allowed == false))) as $anchor_ready |
  {
    ...
    schema_version:"workflow_temporal_lite_checkpoint_and_rollback_anchor_local_persistence_readback_v1",
    ...
    replay_readback_projection_count:$source_report.replay_readback_projection_count,
    ...
    checkpoint_and_rollback_anchor_local_persistence_readback_ready:$anchor_ready,
    entries:$entries,
    blockers:[
    ...
    recommended_next_gate:"workflow_temporal_lite_lease_idempotency_index_local_persistence_readback",
    local_gate:$gate,
