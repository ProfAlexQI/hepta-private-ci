...
def emit(checks: dict[str, bool], *, dependency_qualified: bool = False) -> int:
...
def main() -> int:
    ...
    qualification_runner = FILES["qualification_runner"].read_text(encoding="utf-8")
    ...
    checks["workflow.repository_toolchain_and_scoped_fmt"] = (
        ...
        and "skip_children=true" in qualification_runner
        and "--check" in qualification_runner
    )
    ...
    stack_base = status.get("stack_base", {})
    ...
    stack_head = stack_base.get("head")
    stack_head_is_sha = (
        isinstance(stack_head, str)
        and len(stack_head) == 40
        and all(character in "0123456789abcdef" for character in stack_head)
    )
    blocked_dependency = (
        dependency.get("id") == "P0.3.2"
        and dependency.get("qualified") is False
        and dependency.get("activation_blocking") is True
    )
    qualified_dependency = (
        dependency.get("id") == "P0.3.2"
        and dependency.get("qualified") is True
        and dependency.get("implemented_in_repository") is True
        and dependency.get("activation_blocking") is False
        ...
        and stack_base.get("branch")
            == "codex/hepta-intelligence-shared-projection-planner-v5-20260828"
        and stack_head_is_sha
    )
    checks["status.p0_3_2_dependency_state"] = (
        blocked_dependency or qualified_dependency
    )
    ...
    return emit(checks, dependency_qualified=dependency_qualified)
