...
REPORT="$ROOT/scripts/hepta-systems-cler-store-local-evidence-acceptance-authority-packet-report.sh"
...
fail() {
  printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-packet-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable local evidence acceptance authority packet report: $REPORT"
...
rg -q 'Controlled Live Evidence Receipt Store Local Evidence Acceptance Authority Packet Readback Without Acceptance' "$DOC" \
  || fail "architecture note must document the local evidence acceptance authority packet surface"
rg -q 'controlled live evidence receipt store local evidence acceptance authority packet readback without acceptance' "$DOC" \
  || fail "architecture note must document the plain-language local evidence acceptance authority packet boundary"
rg -q 'no operator packet send, operator packet persistence, local evidence acceptance authority, authority decision recording, non-authority receipt persistence, local evidence acceptance, local evidence acceptance recording, evidence acceptance recording, evidence recording, receipt-store write-attempt recording, receipt store write, receipt persistence, ledger write, event-log write, SQLite write, credential read, Native POST mutation, Telegram transport mutation, gateway/auth mutation, channel send, provider call, model call, replay execution, rollback, kill-switch rehearsal execution, kill-switch mutation, package, release, Public GA promotion, or live execution' "$DOC" \
  || fail "architecture note must document the closed local evidence acceptance authority packet boundary"

tmpdir="$(mktemp -d)"
trap 'rm -rf "$tmpdir"' EXIT

source_json="$tmpdir/local-evidence-acceptance-positive-preconditions.json"
target_json="$tmpdir/local-evidence-acceptance-authority-packet.json"
...
HEPTA_CONTROLLED_LIVE_EVIDENCE_RECEIPT_STORE_LOCAL_EVIDENCE_ACCEPTANCE_POSITIVE_PRECONDITIONS_JSON="$source_json" \
  "$REPORT" >"$target_json" || fail "failed to render local evidence acceptance authority packet report from cached source"
jq -e . "$target_json" >/dev/null || fail "target report did not render valid JSON"

jq -e '
  .runtime == "hepta"
  and .surface == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_readback_without_acceptance"
  and .status == "ready_blocked"
  and .gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_readback_without_acceptance_gate"
  and .schema_version == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_readback_without_acceptance_v1"
  ...
  and .authority_packet_id == "controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-packet"
  and .authority_packet_route == "operator-packet://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority"
  and .authority_packet_payload_fingerprint == "sha256:controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-packet-no-acceptance"
  ...
  and .local_evidence_acceptance_authority_packet_readback_ready == true
  ...
  and (.blockers | index("non_authority_receipt_persistence_disabled")) != null
  and (.blockers | index("local_evidence_acceptance_disabled")) != null
  and (.blockers | index("live_execution_disabled")) != null
  and (.entries | length) == 7
  and (.entries | all(
    (.id | startswith("evidence_receipt_store_local_evidence_acceptance_authority_packet_without_acceptance_"))
    and (.source_positive_precondition_set_id | startswith("local-evidence-acceptance-positive-preconditions:controlled-live-evidence-receipt-store:"))
    and (.source_positive_precondition_key | startswith("local-evidence-acceptance-positive-preconditions:controlled-live-evidence-receipt-store:"))
    and (.source_positive_precondition_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/positive-preconditions/"))
    ...
    and (.source_local_acceptance_authority_precondition_id | startswith("local-evidence-acceptance-authority-required:controlled-live-evidence-receipt-store:"))
    and (.source_operator_local_acceptance_approval_precondition_id | startswith("operator-local-evidence-acceptance-approval-required:controlled-live-evidence-receipt-store:"))
    and (.source_dev_evidence_acceptance_source_precondition_id | startswith("dev-evidence-acceptance-source-required:controlled-live-evidence-receipt-store:"))
    and (.source_evidence_payload_source_binding_precondition_id | startswith("evidence-payload-source-binding-required:controlled-live-evidence-receipt-store:"))
    and (.source_local_evidence_store_feature_gate_precondition_id | startswith("local-evidence-store-feature-gate-required:controlled-live-evidence-receipt-store:"))
    and (.source_local_receipt_store_feature_gate_precondition_id | startswith("local-receipt-store-feature-gate-required:controlled-live-evidence-receipt-store:"))
    and (.source_atomic_acceptance_append_precondition_id | startswith("atomic-local-evidence-acceptance-append-required:controlled-live-evidence-receipt-store:"))
    and (.source_post_acceptance_readback_precondition_id | startswith("post-local-evidence-acceptance-readback-required:controlled-live-evidence-receipt-store:"))
    and (.source_rollback_anchor_precondition_id | startswith("local-evidence-acceptance-rollback-anchor-required:controlled-live-evidence-receipt-store:"))
    and (.source_retention_policy_commit_precondition_id | startswith("local-evidence-acceptance-retention-policy-commit-required:controlled-live-evidence-receipt-store:"))
    and (.source_replay_idempotency_guard_precondition_id | startswith("local-evidence-acceptance-replay-idempotency-guard-required:controlled-live-evidence-receipt-store:"))
    and .authority_packet_id == "controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-packet"
    and .authority_packet_route == "operator-packet://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority"
    and .authority_packet_payload_fingerprint == "sha256:controlled-live-evidence-receipt-store-local-evidence-acceptance-authority-packet-no-acceptance"
    and (.authority_packet_key | startswith("local-evidence-acceptance-authority-packet:controlled-live-evidence-receipt-store:"))
    and (.authority_decision_request_id | startswith("local-evidence-acceptance-authority-decision-request:controlled-live-evidence-receipt-store:"))
    and (.authority_decision_request_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/decision-request/"))
    and (.non_authority_receipt_id | startswith("local-evidence-acceptance-non-authority-receipt:controlled-live-evidence-receipt-store:"))
    and (.non_authority_receipt_route | startswith("readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/non-authority-receipts/"))
    and .observed_state == "local_evidence_acceptance_authority_packet_projected_without_acceptance"
    and .packet_projected == true
    and .packet_ready == true
    and .authority_packet_key_projected == true
    and .authority_checklist_projected == true
    and .authority_item_required_count == 11
    and .authority_item_present_count == 0
  ...
  and any(.entries[]; .source_blocker_id == "dirty_worktree_boundary" and .authority_decision_request_route == "readback://controlled-live/evidence-receipt-store/local-evidence-acceptance/authority-packet/decision-request/dirty-worktree-boundary")
  and any(.entries[]; .source_blocker_id == "operator_live_approval_missing" and .source_operator_local_acceptance_approval_precondition_id == "operator-local-evidence-acceptance-approval-required:controlled-live-evidence-receipt-store:operator_live_approval_missing")
  and .recommended_next_gate == "controlled_live_evidence_receipt_store_local_evidence_acceptance_authority_packet_non_send_readback"
  and .side_effect_free == true
  and (.side_effects | to_entries | all(.value == false))
' "$target_json" >/dev/null

jq -e '
  .surface == "controlled_live_evidence_receipt_store_local_evidence_acceptance_positive_preconditions_readback_without_acceptance"
  and .status == "ready_blocked"
...
(
  cd "$ROOT/codex-rs"
  cargo test -p hepta-runtime local_evidence_acceptance_authority_packet --lib
)

printf 'hepta-systems-cler-store-local-evidence-acceptance-authority-packet-gate: PASS: local evidence acceptance authority packet is read back without send, acceptance, persistence, or live execution\n'
