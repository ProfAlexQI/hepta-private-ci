...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-td3-rbtd-rbtd-rbtd-rt-rba-td-rt-rba-td-rt-rba-td-rt-rba-td-rt-rba-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-dtd8-td3-rbtd-rbtd-rbtd-rt-rba-td-rt-rba-td-rt-rba-td-rt-rba-td-rt-rba-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .source_probes.deep_td8_td3_rbackack_td7_receipt_retention_readback_ack.rust_module_present == true
  and .source_probes.prior_deep_td8_td3_rbackack_td7_receipt_retention_readback_receipt.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_dtd8_td3_rbtd_rbtd_rbtd_rt_td_rt_td_rt_td_rt_td_rt_rbackack_preview --lib

echo "Hepta WorkGraph deep td8 td3 rbackack td7 receipt retention readback ack preview gate passed"
