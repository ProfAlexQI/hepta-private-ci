...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-workflow-temporal-lite-replay-alignment-recovery-window-local-persistence-readback-report.sh"
RUST_SOURCE="$ROOT/codex-rs/hepta-runtime/src/workflow_temporal_lite_replay_alignment_recovery_receipt_local_persistence_readback.rs"
...
fail() {
  printf 'hepta-systems-workflow-temporal-lite-replay-alignment-recovery-receipt-local-persistence-readback-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$SOURCE_REPORT" ]] || fail "missing executable Temporal-lite local replay-alignment recovery window report: $SOURCE_REPORT"
[[ -f "$RUST_SOURCE" ]] || fail "missing Temporal-lite local replay-alignment recovery receipt Rust source: $RUST_SOURCE"
[[ -f "$LIB_SOURCE" ]] || fail "missing hepta-runtime lib source: $LIB_SOURCE"
[[ -f "$DOC" ]] || fail "missing Temporal-lite local replay-alignment recovery receipt architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the Temporal-lite local replay-alignment recovery receipt report"
fi

lib_export_present=false
if rg -q 'hepta_workflow_temporal_lite_replay_alignment_recovery_receipt_local_persistence_readback_report' "$LIB_SOURCE"; then
  lib_export_present=true
fi

reopened_sqlite_recovery_receipt_test_present=false
if rg -q 'local_recovery_receipt_uses_reopened_sqlite_event_history' "$RUST_SOURCE"; then
  reopened_sqlite_recovery_receipt_test_present=true
fi

jq -n \
  --slurpfile source <("$SOURCE_REPORT") \
  --argjson lib_export_present "$lib_export_present" \
  --argjson reopened_sqlite_recovery_receipt_test_present "$reopened_sqlite_recovery_receipt_test_present" \
  --arg gate "scripts/hepta-systems-workflow-temporal-lite-replay-alignment-recovery-receipt-local-persistence-readback-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_WORKFLOW_TEMPORAL_LITE_REPLAY_ALIGNMENT_RECOVERY_RECEIPT_LOCAL_PERSISTENCE_READBACK_2026-06-30.md" \
  '
  def pad3($n):
  ...
  def recovery_receipt_entry($entry):
    ...
    keyed("temporal-lite.local-recovery-receipt.ack-readback"; $entry) as $recovery_receipt_ack_key |
    {
      ...
      recovery_receipt_key:keyed("temporal-lite.local-replay-alignment.recovery-receipt-readback"; $entry),
      recovery_receipt_ack_key:$recovery_receipt_ack_key,
      recovery_receipt_digest:("temporal-lite.local-replay-alignment-recovery-receipt-digest.v1." + pad3($entry.replay_order) + "." + $entry.event_contract_id + "." + (($entry.recovery_window_digest | length) | tostring) + "." + (($entry.recovery_window_end_key | length) | tostring) + "." + (($entry.local_sequence | tostring))),
      ...
      readback_state:"projected_from_sqlite_wal_local_persistence_readback_without_recovery_receipt_writes",
  ...
  ($source_report.recovery_window_local_persistence_readback_ready == true
      ...
      and .live_execution_allowed == false))) as $readback_ready |
  {
    ...
    gate:"workflow_temporal_lite_replay_alignment_recovery_receipt_local_persistence_readback_gate",
    schema_version:"workflow_temporal_lite_replay_alignment_recovery_receipt_local_persistence_readback_v1",
    source_recovery_window_gate:$source_report.gate,
    source_recovery_window_ready:$source_report.recovery_window_local_persistence_readback_ready,
    source_recovery_window_entry_count:($source_report.entries | length),
    lib_export_present:$lib_export_present,
    reopened_sqlite_recovery_receipt_test_present:$reopened_sqlite_recovery_receipt_test_present,
    ...
    recovery_receipt_local_persistence_readback_ready:$readback_ready,
    entries:$entries,
    blockers:[
