...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-td3-rbtd5-td-rtx-preview-report.sh"
...
jq -e '
  ...
  and .source_probes.deep_td8_td3_rbackack_td9_receipt_retention_expiry.rust_module_present == true
  and .source_probes.prior_deep_td8_td3_rbackack_td9_terminal_decision_receipt_ack_replay.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_dtd8_td3_rbtd_rbtd_rbtd_rt_td_rt_td_rt_td_rt_td_rt_td_rt_td_rt_expiry_preview --lib

echo "Hepta WorkGraph deep td8 td3 rbackack td9 receipt retention expiry preview gate passed"
