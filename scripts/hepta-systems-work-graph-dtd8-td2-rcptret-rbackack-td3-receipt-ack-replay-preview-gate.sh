...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-td2-rcptret-rbackack-td3-receipt-ack-replay-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-dtd8-td2-rcptret-rbackack-td3-receipt-ack-replay-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and (.recommended_next_gate | endswith("_terminal_decision_non_promotion_receipt_retention_expiry_readback_acknowledgement_terminal_decision_non_promotion_receipt_retention_expiry_preview_gate"))
  ...
  and .source_probes.deep_td8_td3_terminal_decision_receipt_ack_replay.rust_module_present == true
  and .source_probes.prior_deep_td8_td3_terminal_decision_receipt_ack.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_retack_td3_receipt_ack_replay_preview --lib

echo "Hepta WorkGraph deep td8 td3 terminal decision receipt acknowledgement replay preview gate passed"
