...
PINNED_MANIFEST_SHA256="4e0ad42fe7edc0f073d840457f48cf579befbbfd0abb73cc0845778f3122eca6"
...
MANIFEST="${HEPTA_UPSTREAM_CODEX_R3_INTEGRITY_MANIFEST:-$PINNED_MANIFEST}"
CUTOFF_REF="${HEPTA_UPSTREAM_CODEX_R3_INTEGRITY_CUTOFF_REF:-$PINNED_CUTOFF_REF}"
CUTOFF_HEAD="${HEPTA_UPSTREAM_CODEX_R3_INTEGRITY_CUTOFF_HEAD:-$PINNED_CUTOFF_HEAD}"
ALLOW_FIXTURE_MANIFEST="${HEPTA_UPSTREAM_CODEX_R3_INTEGRITY_ALLOW_FIXTURE_MANIFEST:-0}"

fail() {
  echo "hepta upstream Codex R3 historical integrity gate failed: $*" >&2
  exit 1
}
...
if [[ "$MANIFEST" == "$PINNED_MANIFEST" ]]; then
  [[ "$(shasum -a 256 "$MANIFEST" | awk '{print $1}')" == "$PINNED_MANIFEST_SHA256" ]] || fail "R3 manifest hash drifted"
fi
...
[[ "$normalized_commit_digest" == "$PINNED_NORMALIZED_COMMIT_DIGEST" ]] || fail "R3 normalized commit inventory digest drifted"
[[ "$normalized_file_digest" == "$PINNED_NORMALIZED_FILE_DIGEST" ]] || fail "R3 normalized file surface digest drifted"

jq -n \
  --arg schema hepta_upstream_codex_r3_historical_integrity_v1 \
  --arg status ready \
