...
READBACK_REPORT="$ROOT/scripts/hepta-systems-public-ga-readiness-non-live-attachment-readback-report.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_READINESS_NON_LIVE_ATTACHMENT_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-readiness-non-live-attachment-final-index-report: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$READBACK_REPORT" ]] || fail "missing executable Public GA readiness non-live attachment readback report: $READBACK_REPORT"
[[ -f "$DOC" ]] || fail "missing Public GA readiness non-live attachment final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to render the Public GA readiness non-live attachment final index report"
fi

jq -n \
  --slurpfile readback <("$READBACK_REPORT") \
  --arg gate "scripts/hepta-systems-public-ga-readiness-non-live-attachment-final-index-gate.sh" \
  --arg doc "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_READINESS_NON_LIVE_ATTACHMENT_FINAL_INDEX_2026-06-21.md" \
  '
  ($readback[0]) as $readback |
  ($readback.public_ga_readiness_non_live_attachment_readback_ready == true
    and $readback.public_ga_readiness_non_live_attachment_readback_blocked == true
    and $readback.public_ga_readiness_attachment_basis == "preflight_non_live_adapter_migration_final_index"
    and $readback.public_ga_readiness_preflight_non_live_adapter_migration_final_index_attached == true
    ...
    and $readback.public_ga_readiness_live_endpoint_read_required_by_attachment == false
  ...
  {
    runtime:"hepta",
    surface:"public_ga_readiness_non_live_attachment_final_index",
    plugin_id:$readback.plugin_id,
    status:(if $final_index_ready then "ready_blocked" else "blocked" end),
    source_public_ga_readiness_non_live_attachment_readback_surface:$readback.surface,
    source_public_ga_readiness_non_live_attachment_readback_ready:$readback.public_ga_readiness_non_live_attachment_readback_ready,
    source_public_ga_readiness_non_live_attachment_readback_blocked:$readback.public_ga_readiness_non_live_attachment_readback_blocked,
    public_ga_readiness_non_live_attachment_final_index_ready:$final_index_ready,
    ...
    public_ga_readiness_attachment_basis:$readback.public_ga_readiness_attachment_basis,
    ...
    next_migration_step:"attach_public_ga_readiness_non_live_attachment_final_index_to_terminal_public_ga_non_promotion_summary_without_readiness_invocation",
    local_gate:$gate,
    architecture_note:$doc,
    source_files:{
      public_ga_readiness_non_live_attachment_readback_report:"scripts/hepta-systems-public-ga-readiness-non-live-attachment-readback-report.sh"
    },
