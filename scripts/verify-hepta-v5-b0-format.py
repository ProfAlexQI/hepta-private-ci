#!/usr/bin/env python3
"""Format or verify only the source-owned V5/B0 verified-use Rust package."""
...
def main() -> int:
    ...
    if sys.argv[1:] not in ([], ["--check"]):
        raise SystemExit("usage: verify-hepta-v5-b0-format.py [--check]")
    missing = [path for path in FILES if not (ROOT / path).is_file()]
    if missing:
        raise SystemExit(f"missing V5/B0 Rust source files: {missing}")
    command = [
        "rustfmt",
        "--edition",
        "2024",
