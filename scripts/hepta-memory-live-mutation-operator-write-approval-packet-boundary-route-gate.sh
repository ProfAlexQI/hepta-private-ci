...
require_source_text() {
  ...
  if ! rg -Fq "$source_text" "$source_file"; then
    echo "missing memory write approval packet boundary source text: $label" >&2
    exit 1
  fi
}
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "native gateway route/source command count includes memory write approval packet boundary"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
...
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  ...
  "memory write approval packet boundary report function"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"memory_write_approval_packet_boundary_ready\"," \
  "memory write approval packet boundary ready field"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "\"denied_by_memory_write_approval_packet_boundary_count\", 22" \
  "memory write approval packet boundary denied count"
require_source_text "$NATIVE_GATEWAY_SOURCE" \
  "hepta_memory_write_approval_packet_boundary_endpoint_exposes_packet_shape_without_execution" \
  "focused memory write approval packet boundary unit test"

TEST_LOG="$(mktemp /tmp/hepta-memory-write-approval-packet-boundary-route-tests.XXXXXX)"
...
if [[ "$REQUIRE_LIVE_ENDPOINT" == "1" ]]; then
  ...
  jq -e --argjson expected "$EXPECTED_ROUTE_COUNT" '
    ...
    and .memory_write_approval_packet_boundary_ready == true
    and .approval_packet_mode == "memory_write_operator_approval_packet_shape_no_recording_no_execution"
    and .source_full_live_activation_closure_index_ready == true
    ...
    and .denied_by_memory_write_approval_packet_boundary_count == 22
    and .required_before_memory_write_approval_packet_acceptance_count == 14
    ...
    and .allowed_next_actions[0].action == "run_memory_write_approval_packet_boundary_require_live_gate"
    and .allowed_next_actions[1].action == "prepare_memory_write_execution_preflight_denial_boundary"
...
jq -n \
  ...
  '{
    ...
    status:"ready",
    base_url:$base_url,
    gate:"hepta_memory_live_mutation_operator_write_approval_packet_boundary_route_gate",
    endpoint:$endpoint,
    ...
    memory_write_approval_packet_boundary_ready:true,
...
echo "Hepta memory live mutation operator write approval packet boundary route gate passed"
