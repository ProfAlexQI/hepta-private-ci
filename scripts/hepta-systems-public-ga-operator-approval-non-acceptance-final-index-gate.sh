...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-approval-non-acceptance-final-index-report.sh"
READBACK_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-approval-non-acceptance-readback-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_APPROVAL_NON_ACCEPTANCE_FINAL_INDEX_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-approval-non-acceptance-final-index-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator approval non-acceptance final index report: $REPORT"
[[ -x "$READBACK_GATE" ]] || fail "missing executable Public GA operator approval non-acceptance readback gate: $READBACK_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator approval non-acceptance final index architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator approval non-acceptance final index report"
fi

grep -q 'Public GA Operator Approval Non-Acceptance Final Index' "$DOC" \
  || fail "architecture note must document Public GA Operator Approval Non-Acceptance Final Index"
grep -q 'ready-but-blocked' "$DOC" \
  || fail "architecture note must document ready-but-blocked status"
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that final index does not invoke operator approval gates"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_approval_non_acceptance_final_index"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_approval_non_acceptance_readback_surface == "public_ga_operator_approval_non_acceptance_readback"
  and .source_public_ga_operator_approval_non_acceptance_readback_ready == true
  and .source_public_ga_operator_approval_non_acceptance_readback_blocked == true
  ...
  and .public_ga_operator_approval_non_acceptance_readback_attached == true
  ...
  and .next_migration_step == "attach_public_ga_operator_approval_non_acceptance_final_index_to_public_ga_operator_identity_non_acceptance_without_approval_send"
...
printf 'hepta-systems-public-ga-operator-approval-non-acceptance-final-index-gate: PASS: Public GA operator approval non-acceptance final index is ready but blocked\n'
