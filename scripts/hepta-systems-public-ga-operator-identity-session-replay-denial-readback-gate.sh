...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-replay-denial-readback-report.sh"
ATTACHMENT_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-session-final-index-identity-session-replay-denial-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_REPLAY_DENIAL_READBACK_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-identity-session-replay-denial-readback-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session replay denial readback report: $REPORT"
[[ -x "$ATTACHMENT_GATE" ]] || fail "missing executable Public GA operator identity/session replay denial attachment gate: $ATTACHMENT_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session replay denial readback architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session replay denial readback report"
fi

grep -q 'Public GA Operator Identity/Session Replay Denial Readback' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Replay Denial Readback"
grep -q 'ready-but-blocked' "$DOC" \
  || fail "architecture note must document ready-but-blocked status"
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that readback does not invoke replay gates"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_replay_denial_readback"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_replay_denial_attachment_surface == "public_ga_operator_identity_session_replay_denial_attachment"
  and .source_public_ga_operator_identity_session_replay_denial_attachment_ready == true
  and .source_public_ga_operator_identity_session_replay_denial_attachment_blocked == true
  ...
  and .public_ga_operator_identity_session_replay_denial_readback_ready == true
  and .public_ga_operator_identity_session_replay_denial_readback_blocked == true
  and .public_ga_operator_identity_session_replay_denial_attachment_attached == true
  and .readback_mode == "static_operator_identity_session_replay_denial_snapshot_only"
  and .readback_check_count == 32
  ...
  and .next_migration_step == "derive_public_ga_operator_identity_session_replay_denial_final_index_without_replay_acceptance"
...
printf 'hepta-systems-public-ga-operator-identity-session-replay-denial-readback-gate: PASS: Public GA operator identity/session replay denial readback is ready but blocked\n'
