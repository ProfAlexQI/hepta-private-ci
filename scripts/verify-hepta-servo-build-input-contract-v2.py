#!/usr/bin/env python3
"""Canonical verifier for the Servo build-input sealing contract."""
...
def verify_tests() -> None:
    ...
    names = (
        ...
        "test_recipe_and_source_digests_bind_exact_canonical_bytes",
...
def verify_workflow() -> None:
    ...
    for token in (
        ...
        "python3 scripts/verify-hepta-servo-build-input-contract-v2.py",
        "build_run=false",
        "artifact_created=false",
        "servo_runtime_qualified=false",
    ...
    if "scripts/hepta-servo-build-input-seal.py --source-verification" in text:
...
def main() -> int:
    try:
        ...
        print(f"HEPTA_SERVO_BUILD_INPUT_CONTRACT_V2=FAIL: {error}", file=sys.stderr)
        return 1
    print(
        json.dumps(
            {
                "schema": "hepta.browser.servo_build_input_contract_check.v2",
