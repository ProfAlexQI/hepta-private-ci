...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-revocation-logout-final-index-identity-session-reinstatement-denial-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session reinstatement denial attachment report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session reinstatement denial readback report" >&2
  exit 1
fi
...
jq -e '
  ...
  and .public_ga_operator_identity_session_reinstatement_denial_attachment_ready == true
  and .public_ga_operator_identity_session_reinstatement_denial_attachment_blocked == true
...
jq -n \
  ...
  '{
    runtime: "hepta",
    surface: "public_ga_operator_identity_session_reinstatement_denial_readback",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_identity_session_reinstatement_denial_attachment_surface: $source.surface,
    source_public_ga_operator_identity_session_reinstatement_denial_attachment_ready: $source.public_ga_operator_identity_session_reinstatement_denial_attachment_ready,
    source_public_ga_operator_identity_session_reinstatement_denial_attachment_blocked: $source.public_ga_operator_identity_session_reinstatement_denial_attachment_blocked,
    ...
    public_ga_operator_identity_session_reinstatement_denial_readback_ready: true,
    public_ga_operator_identity_session_reinstatement_denial_readback_blocked: true,
    public_ga_operator_identity_session_reinstatement_denial_attachment_attached: true,
    public_ga_operator_identity_session_revocation_logout_final_index_attached: $source.public_ga_operator_identity_session_revocation_logout_final_index_attached,
    readback_mode: "static_operator_identity_session_reinstatement_denial_snapshot_only",
    readback_check_count: 36,
    operator_identity_session_revocation_logout_replay_reinstatement_denial_gate_present: $source.operator_identity_session_revocation_logout_replay_reinstatement_denial_gate_present,
    operator_identity_session_revocation_logout_replay_reinstatement_denial_doc_present: $source.operator_identity_session_revocation_logout_replay_reinstatement_denial_doc_present,
    ...
    long_soak_required_by_source_reinstatement_gate: $source.long_soak_required_by_source_reinstatement_gate,
    ...
    readback_blocker_count: 30,
    public_ga_claim_allowed: false,
    public_ga_claimed: false,
    public_release_published: false,
    rollback_execution_allowed: false,
    next_migration_step: "derive_public_ga_operator_identity_session_reinstatement_denial_final_index_without_reinstatement",
    local_gate: "scripts/hepta-systems-public-ga-operator-identity-session-reinstatement-denial-readback-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_REINSTATEMENT_DENIAL_READBACK_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_reinstatement_denial_attachment_report: "scripts/hepta-systems-public-ga-operator-revocation-logout-final-index-identity-session-reinstatement-denial-report.sh"
    },
