...
REPORT_SCRIPT="$ROOT/scripts/hepta-systems-work-graph-dtd8-td3-rcptret-rbackack-td-rcptret-rbackack-preview-report.sh"
report="$(capture_json_report "hepta-systems-work-graph-dtd8-td3-rcptret-rbackack-td-rcptret-rbackack-preview-report" "$REPORT_SCRIPT")"
printf '%s\n' "$report"
...
jq -e '
  ...
  and .source_probes.deep_td8_td3_rcptret_rbackack_td_receipt_retention_readback_ack.rust_module_present == true
  and .source_probes.prior_deep_td8_td3_rcptret_rbackack_td_retention_readback_receipt.report_script_present == true
  and (.side_effects | to_entries | all(.value == false))
' >/dev/null <<<"$report"

cargo test --manifest-path "$ROOT/codex-rs/Cargo.toml" -p hepta-runtime \
  wg_deep_td8_retack_td3_rcptret_rbackack_td_rcptret_rbackack_preview --lib

echo "Hepta WorkGraph deep td8 td3 rbackack terminal decision retention readback ack preview gate passed"
