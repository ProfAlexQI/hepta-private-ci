...
REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-revocation-logout-final-index-identity-session-reinstatement-denial-report.sh"
SOURCE_GATE="$ROOT/scripts/hepta-systems-public-ga-operator-identity-session-revocation-logout-final-index-gate.sh"
DOC="$ROOT/docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_REVOCATION_LOGOUT_FINAL_INDEX_IDENTITY_SESSION_REINSTATEMENT_DENIAL_2026-06-21.md"

fail() {
  printf 'hepta-systems-public-ga-operator-revocation-logout-final-index-identity-session-reinstatement-denial-gate: FAIL: %s\n' "$1" >&2
  exit 1
}

[[ -x "$REPORT" ]] || fail "missing executable Public GA operator identity/session reinstatement denial attachment report: $REPORT"
[[ -x "$SOURCE_GATE" ]] || fail "missing executable Public GA operator identity/session revocation logout final index gate: $SOURCE_GATE"
[[ -f "$DOC" ]] || fail "missing Public GA operator identity/session reinstatement denial architecture note: $DOC"

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required to validate the Public GA operator identity/session reinstatement denial attachment report"
fi

grep -q 'Public GA Operator Identity/Session Reinstatement Denial Attachment' "$DOC" \
  || fail "architecture note must document Public GA Operator Identity/Session Reinstatement Denial Attachment"
grep -q 'ready-but-blocked' "$DOC" \
  || fail "architecture note must document ready-but-blocked status"
grep -q 'does not invoke' "$DOC" \
  || fail "architecture note must document that attachment does not invoke reinstatement gates"
...
"$REPORT" | jq -e '
  .runtime == "hepta"
  and .surface == "public_ga_operator_identity_session_reinstatement_denial_attachment"
  and .plugin_id == "hepta-system@hepta-local"
  and .status == "ready_blocked"
  and .source_public_ga_operator_identity_session_revocation_logout_final_index_surface == "public_ga_operator_identity_session_revocation_logout_final_index"
  and .source_public_ga_operator_identity_session_revocation_logout_final_index_ready == true
  and .source_public_ga_operator_identity_session_revocation_logout_final_index_blocked == true
  ...
  and .operator_identity_session_reinstatement_static_mention_count >= 10
  ...
  and .next_migration_step == "derive_public_ga_operator_identity_session_reinstatement_denial_readback_without_reinstatement"
...
printf 'hepta-systems-public-ga-operator-revocation-logout-final-index-identity-session-reinstatement-denial-gate: PASS: Public GA operator identity/session reinstatement denial attachment is ready but blocked without revocation or logout\n'
