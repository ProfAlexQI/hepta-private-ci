...
SOURCE_REPORT="$ROOT/scripts/hepta-systems-public-ga-operator-cancellation-final-index-audit-evidence-report.sh"

[[ -x "$SOURCE_REPORT" ]] || {
  echo "missing executable Public GA operator identity/session audit evidence attachment report: $SOURCE_REPORT" >&2
  exit 1
}

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required to build the Public GA operator identity/session audit evidence readback report" >&2
  exit 1
fi
...
jq -n \
  ...
  '{
    runtime: "hepta",
    surface: "public_ga_operator_identity_session_audit_evidence_readback",
    plugin_id: "hepta-system@hepta-local",
    status: "ready_blocked",
    source_public_ga_operator_identity_session_audit_evidence_attachment_surface: $source.surface,
    source_public_ga_operator_identity_session_audit_evidence_attachment_ready: $source.public_ga_operator_identity_session_reinstatement_audit_evidence_attachment_ready,
    source_public_ga_operator_identity_session_audit_evidence_attachment_blocked: $source.public_ga_operator_identity_session_reinstatement_audit_evidence_attachment_blocked,
    public_ga_operator_identity_session_audit_evidence_readback_ready: true,
    public_ga_operator_identity_session_audit_evidence_readback_blocked: true,
    public_ga_operator_identity_session_audit_evidence_attachment_attached: true,
    public_ga_operator_identity_session_reinstatement_cancellation_final_index_attached: $source.public_ga_operator_identity_session_reinstatement_cancellation_final_index_attached,
    readback_mode: "static_operator_identity_session_audit_evidence_snapshot_only",
    readback_check_count: 42,
    ...
    operator_identity_session_reinstatement_audit_evidence_denial_gate_present: $source.operator_identity_session_reinstatement_audit_evidence_denial_gate_present,
    operator_identity_session_reinstatement_audit_evidence_denial_doc_present: $source.operator_identity_session_reinstatement_audit_evidence_denial_doc_present,
    ...
    long_soak_required_by_source_audit_gate: $source.long_soak_required_by_source_audit_gate,
    ...
    readback_blocker_count: 36,
    public_ga_claim_allowed: false,
    public_ga_claimed: false,
    public_release_published: false,
    rollback_execution_allowed: false,
    next_migration_step: "derive_public_ga_operator_identity_session_audit_evidence_final_index_without_evidence",
    local_gate: "scripts/hepta-systems-public-ga-operator-identity-session-audit-evidence-readback-gate.sh",
    architecture_note: "docs/architecture/HEPTA_SYSTEMS_PUBLIC_GA_OPERATOR_IDENTITY_SESSION_AUDIT_EVIDENCE_READBACK_2026-06-21.md",
    source_files: {
      public_ga_operator_identity_session_audit_evidence_attachment_report: "scripts/hepta-systems-public-ga-operator-cancellation-final-index-audit-evidence-report.sh"
    },
