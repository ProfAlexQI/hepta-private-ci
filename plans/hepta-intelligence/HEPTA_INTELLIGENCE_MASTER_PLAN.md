...
V4.5 在 V4.3 上完成以下 source-controlled 深化：

1. 把强制读取顺序统一为 8 个输入，机器 truth、AGENTS、master 与 verifier 不再分裂；
2. 将 subordinate execution specification 的 canonical parent 从 V4.2 修正为 V4.5；
...
## 0.1 V4.5 A0 source-contract closure
...
V4.5 closes review findings `A0-RV-010` through `A0-RV-018` without
crossing the A0 source freeze:

- source/observer workflows emit exact-head **evidence only** and always retain
  `a0_candidate_qualified=false`; candidate qualification is reserved for the
  distinct review, selection and merge-admission chain;
- downloaded artifact archives are byte-hashed, size-bounded and safely
  inspected before extraction; workflow blob identity, jobs, runners, steps,
  artifact creation/expiry and extracted-file hashes are bound by the observer;
- the eight mandatory inputs are registered under one precedence model; the
  document registry binds every input, including a canonical self-digest with
  its self-binding field nulled;
- legacy V2/V3 contracts, immutable P0.2–P0.4c snapshots, registered consumers,
  claim-ladder policy and migration receipts are content-sealed and verified;
- evidence history is append-only: superseded PASS and real failure evidence is
  retained with stable IDs and explicit state;
- every capability has the complete lifecycle and receipt slots; no capability
  is `selected`, `wired`, `runtime_qualified`, `operator_accepted`, `promoted`
  or `released` without the corresponding external receipt;
- one authoritative global gap registry owns shared IDs. The multimodal view
  references shared gaps and defines only unique modality-specific gaps;
- all normative JSON uses duplicate-key rejecting decoders, bounded shapes,
  exact required fields, cross-reference checks and an acyclic dependency DAG;
- all three read-only workflows use the same canonical 17-path trigger surface
  and PR/ref-scoped concurrency, so mirror PRs cannot cancel canonical evidence.

The source producer, observer and exact-head evidence artifacts do not select,
merge or activate the candidate. A distinct reviewer and selector remain
mandatory.
...
- exact-head source producer and independent observer evidence PASS；
...
```text
1. Publish V4.5 as one exact-parent A0 replacement
2. Obtain fresh A0 exact-head source and observer evidence (qualification remains false)
