...
## V4.5 mandatory closure rules

- The canonical read order is exactly the eight paths in
  `HEPTA_INTELLIGENCE_CURRENT_PLAN.json`; no separate hidden input list exists.
- Source and observer workflows never set `a0_candidate_qualified=true`.
- Normative JSON must reject duplicate keys and unknown required structures.
- Evidence history is append-only; failure evidence is never deleted merely
  because a newer head exists.
- Shared gap IDs are defined only once. Multimodal projections reference shared
  IDs instead of redefining them.
- Every workflow path filter must contain the exact 17-path canonical manifest.
- A distinct reviewer, selector and operator remain external roles even when the
  implementer has repository write permission.
