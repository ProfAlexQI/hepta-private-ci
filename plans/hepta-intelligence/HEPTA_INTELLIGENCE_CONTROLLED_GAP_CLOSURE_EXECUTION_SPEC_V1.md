...
> Version: `1.3.0`
>
> Canonical parent plan: `HEPTA_INTELLIGENCE_MASTER_PLAN_V4` version `4.5.1`
>
...
## 16. V4.5/1.3.0 evidence, registry and parser closure

### 16.1 Non-self-qualifying evidence chain

`A0SourceEvidenceReceiptV2` and `A0ExactHeadEvidenceReceiptV2` are evidence
artifacts only. Both contain `a0_candidate_qualified=false`. The producer may
observe its completed prerequisite job; it may not claim its own final artifact
or terminal status. A later independent reviewer binds the completed producer
and observer artifacts by ID, digest and expiry. `CanonicalSelectionReceiptV1`
and `RepositoryCheckAttributionReceiptV1` then govern selection and merge
admission.

### 16.2 Safe artifact boundary

The observer recomputes the downloaded ZIP SHA-256 against the GitHub artifact
digest, rejects absolute paths, `..`, links, duplicate normalized targets,
excessive file count and excessive expanded bytes, and hashes every extracted
file. It binds the exact workflow blob SHA/Git object, workflow content SHA-256,
run/attempt/event, source job runner and successful steps, artifact
created/expires timestamps, archive digest and extracted manifest.

### 16.3 Single input and gap authority

The document authority registry contains exactly the eight mandatory inputs.
Each entry includes schema/role, digest, digest scope, mutation policy, conflict
rule and negative current-plan authority. The registry self-entry uses the
domain `CANONICAL_JSON_WITH_SELF_DIGEST_NULL` to avoid a recursive digest.

The 17 shared program gaps are defined once in `gap_closure_ledger`. The
multimodal ledger carries only unique multimodal definitions plus
`inherited_global_gap_ids`; the projected surface remains 42 entries without
conflicting duplicate definitions.

### 16.4 Strict parsing and compatibility

All normative JSON is decoded with duplicate-key rejection. Verifiers enforce
exact lifecycle fields and implications, stable evidence IDs, append-only
failure retention, exact gap IDs/classes/required fields, an acyclic `blocked_by`
DAG, sealed legacy V2/V3 contracts, immutable tranche snapshots, registered
consumers, claim-ladder authority and Q0 receipt/index cross-hashes.
