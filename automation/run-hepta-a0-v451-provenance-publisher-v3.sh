...
if not all(
    isinstance(item, str) and len(item) == 40 for item in required_predecessors
):
...
verifier_path.write_text("".join(lines), encoding="utf-8")
...
if len(current_entries) != 1 or len(self_entries) != 1:
    raise SystemExit("canonical registry entry uniqueness failure")
current_entries[0]["content_sha256"] = hashlib.sha256(plan_bytes).hexdigest()
...
receipt = {
    "schema": "hepta_a0_v451_provenance_replacement_source_publisher_v3",
    "status": "PASS_HEPTA_A0_V451_PROVENANCE_REPLACEMENT_SOURCE_PUBLISHER",
...
printf 'PASS_HEPTA_A0_V451_PROVENANCE_REPLACEMENT_SOURCE_PUBLISHER commit=%s tree=%s staging=%s\n' \
