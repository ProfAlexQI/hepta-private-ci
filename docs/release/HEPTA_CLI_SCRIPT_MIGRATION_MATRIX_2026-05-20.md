...
- machine-readable legacy CLI/script compatibility closure gate
...
### `hepta-codex-legacy-compatibility-closure.sh`

Purpose: validate the old CLI/script family compatibility closure exposed by
...
- ops file family coverage is `65`
- release/hardening script family status gates are `12/12`
- old CLI command breadth is claimed migrated as local route/script coverage
- old release/hardening script execution compatibility is claimed as local
  status-gate coverage
- dangerous live execution, credentialed live smoke, and external release remain
  deferred
- process spawn, filesystem read/write, artifact write, credential reads,
  provider/model invocation, external network read, channel read/send, Telegram
...
live-production equivalence: provider/search/runtime-session/channel/local
tooling/memory-capability families remain represented through machine-readable
inventories, dry-run plans, status gates, and approval boundaries before any
live handoff, process, filesystem, network, file-transfer, or channel-delivery
smoke.
