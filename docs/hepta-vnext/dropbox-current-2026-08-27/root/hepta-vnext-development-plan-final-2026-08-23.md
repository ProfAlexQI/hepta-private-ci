...
<a id="development-docs-sync-e45"></a>
## E.45 开发文档全量同步绑定（2026-08-27）

本节是对 E.44 的**文档层 successor binding**，不改变 `AUTHBUS.11` v1.3 的语义版本，
也不把任何 qualification receipt 提升为实现或生产授权。当前开发输入必须先读取本节、
`AUTHBUS_CANONICAL_CONTRACT_REGISTRY_v1.yaml#/registry` 与
`AUTHBUS_STAGE_MATRIX_v1.yaml#/execution_closure_v1_3/phase_map`；旧 E.42/E.43 crosswalk、
`implementation_overlay`、旧 E21 manifest 和旧 rebind package 只允许 decode/provenance，
不得驱动 codegen、CI、运行时路由或权限判断。

### 当前开发文档边界

- 全局阶段仍由 E.41 决定：`active_profile=development_internal_test`、`phase=DEVELOPMENT`、
  `development_blockers=implementation_backlog_only`；`production_caller`、`production_writer`、
  `effect_authority`、`execute_allowed`、`g5_allowed`、`operator_acceptance`、`promotion` 全为
  `false`。
- AuthBus v1.3 的唯一语义源是 canonical registry；四个窄合同和四个 broad domain 文件是
  generated/domain projections。任何 source digest 缺失、过期或与 registry 不一致都必须
  `FAIL_CLOSED`，不能通过手工 crosswalk 修复。
- HNL federated B7/B8/B9/B10 继续受 `HNL-GATE0-DECISIONS=NOT_READY_FAIL_CLOSED` 约束；
  local loopback lane 可按 Stage Matrix 继续做静态/沙箱工作。INF lane 仍是独立 stale
  qualification lane，不是 AuthBus implementation source。
- 所有模型、NPU、provider、外部 KMS/HSM、真实 effect、公开结算和 operator acceptance
  证据在本次文档同步中均为 `NOT_RUN`/`DEFERRED_PRE_RELEASE`；不得把静态 contract PASS
  写成行为 efficacy 或 release approval。

### 同步清单与生成关系

机器可读的当前清单和变更收据位于
`OpenClaw/HEPTA_DEVELOPMENT_DOCS_CURRENT_BINDING_V1.json` 与
`OpenClaw/HEPTA-DEVELOPMENT-DOCS-SYNC-RECEIPT-2026-08-27.json`。清单覆盖：主计划、
qualification/effective/authority 指针、AuthBus registry/stage/narrow/domain/policy/
protocol/TaskFlow 文档，以及 H8/H9、NDU、Neuron、资源、sensorimotor、topology、推理、
HNL/INF 计划和矩阵。receipt/index 自身不进入彼此的 digest 集合，避免自引用哈希环；
任何后续修改都必须生成新的 successor binding，不得改写 E.44 receipt 或历史包。

开发者在实现前必须执行：

1. 读取当前清单并核对 plan/index/registry/stage digest；
2. 用 duplicate-key rejecting YAML loader、JSON schema、projection map、active DAG、
   terminal/non-terminal guard 和 negative-authority guard 做静态检查；
3. 只在 B0–B6 与 local B8–B10 使用已绑定的 registry/projection 输入；federated lane 在
   HNL Gate-0 新 receipt 到达前拒绝；
4. 将行为测试、硬件/真实 provider 证据明确记录为 `NOT_RUN`，并保留旧 receipt 的 provenance。

本节仍属于 `PLANNING_ONLY / IMPLEMENTATION_BACKLOG`，不修改运行态，不生成密钥，不调用
外部 effect，也不授予任何 production authority。

## WEB-PLAN-2026-08-27 — hepta-browserd Browser-as-Agent 执行层（plan-only append）

> **SUPERSEDED — archive/provenance only.** 本历史段及其旧附件不再是实现、CI、路由或发布
> 输入；当前唯一实现入口是文末 `WEB-PLAN-2026-08-27D`、D runtime、v1.3 matrix 和
> `HEPTA_BROWSER_ACTIVE_PLAN.md`。不要按本段的旧引擎/阶段绑定实施。

本节是在 E.45 之后追加的独立 WEB capability 开发计划。它不改写 E.45、AUTHBUS.11、
HNL/INF、G1–G5 或 H0–H9 的历史语义，也不把 browserd 提升成第六个 authority plane。
本节及三个附件只属于 `PLANNING_ONLY / IMPLEMENTATION_BACKLOG`；在当前
`development_internal_test` profile 下继续开发，但不改变任何 production/effect/CALLERS/
G5/operator/promotion flag。

### WEB.0 Source binding、附件与当前状态

- pre-append parent plan SHA-256：
  `f0a98ae0e6cb1ff3c4bb660bfe50ca921cda434926a755d127dcc7399c4a5620`
- append anchor：`E.45` 结束处，pre-append EOF line `4729`
- detailed plan：`OpenClaw/hepta-browser-runtime-plan-2026-08-27.md`
  - SHA-256：`d90da74d3732fcc74b952530a3d523c7a369bb48a8c5f371d6ec15606e2b5c7b`
- stage matrix：`OpenClaw/HEPTA_BROWSER_STAGE_MATRIX_v1.yaml`
  - SHA-256：`45b080ed7e8dd62bbb3b4288cefb9426ba6ebe6f0d485d9e32ff2094f59aaafd`
- receipt schema：`OpenClaw/hepta.browser_receipt.v1.schema.json`
  - SHA-256：`596b5c235e02d38875bdc55927d912f58c3b98332b4defa3ec3ec813f5789d79`
- source repo/head/tree、Cargo.lock、Servo/Chromium commit、sandbox image、operator/provider
  状态在实现前必须重新采集；本节不把旧 Trillionnium/Servo smoke 或当前 dirty tree 当作
  实现绑定。
- 当前状态：`PLANNING_ONLY / STALE_SOURCE_BINDING`，等待 E.45 current-binding/index 的
  append-only successor receipt；stale 只限制 promotion/authority，不阻止本地 RFC、fixture、
  fake worker 和未发布 qualification 继续。
- `production_caller=false`、`production_writer=false`、`effect_authority=false`、
  `execute_allowed=false`、`g5_allowed=false`、`CALLERS_touched=false`、
  `operator_acceptance=false`、`promotion=false`。

本页不能嵌入自己的最终 SHA-256；最终 plan digest 只能由后续 successor binding/receipt
外部记录，避免自引用哈希环。附件 digest 或 parent/source 发生漂移时必须 fail-closed 并生成
新 binding，不得改写本节已有 digest。

### WEB.1 定位和长期维护决策

最终决策：`hepta-browserd` 是 Hepta 长期一级、可选的 Web Capability / Browser Execution
模块，与 Hepta 同一产品 roadmap、contracts 和 governance 共同维护，但保持独立进程、故障域、
package/version、CI、安全响应和 release lane。

“独立”分为三层：

1. **架构/authority 独立**：必须；browserd 不取得 Thread、Flow、Effect、Memory/KG、AuthBus、
   Net/HNL、Inferd 或 Fleet 的写权；
2. **进程/安全独立**：必须；不可信 HTML/JS、Servo/Chromium 崩溃、OOM 或安全事件不能拖垮
   Codex/TaskFlow/agentd；
3. **Git 仓库独立**：初期不必；先作为 monorepo sibling，只有 Servo patch stack、许可证、
   构建/安全发版频率或权限边界需要时才拆分 engine fork/artifact。

Hepta 长期拥有的是 browser control plane：semantic API、policy、sandbox/egress、session/ref
状态机、worker supervisor、receipt/evidence bridge、站点 recipe、corpus 和 qualification；
不是承诺从零维护完整现代浏览器内核。Servo 以 pinned upstream + 最小 patch 为主，Chromium
作为显式 fallback；引擎与 browserd contract 分开 pin、升级和回滚。

browserd 不是自建 search provider。Google/Bing 继续提供索引和排序；
`search(engine, query)` 只是版本化 allowlisted URL template、URL encoding、等待和结果抽取
recipe。Codex/OpenAI hosted `web_search` 是 WebCapabilityBroker 的并列 adapter，用于公开事实、
候选 URL 和 citations；browserd 用于指定 Google/Bing、真实 DOM/JS、多步操作、隔离 profile 和
可审计快照。两者互不依赖，禁止 shell-out `codex --search`。

### WEB.2 在 Hepta 五平面中的位置

~~~text
Hepta Planner / Intelligence
        │ approved WebIntent / WorkflowDecision
Agent-local TaskFlow
        │ typed BrowserActivity / EffectIntent / CAS / reconcile
Codex ToolRouter / ActivityAdapter
        │
WebCapabilityBroker
   ├── HostedSearchAdapter（可选 hosted discovery/citations）
   └── hepta-browser-client（loopback UDS）
          │
       hepta-browserd control daemon
          ├── sandboxed Servo worker（primary）
          └── sandboxed Chromium worker（explicit fallback）
                    │
             Net egress policy proxy/netns

agentd/fleet supervisor → lifecycle / generation / fence / drain / kill / upgrade / rollback
browserd → BrowserActivityReceipt / PageSnapshot / WebEvidenceReceipt
TaskFlow/evidence writer → canonical run/effect/trajectory append
~~~

browserd 是 Codex/Agent execution spine 边界上的受治理 activity executor，不是新顶层平面。
Governance/Safety 对 URL、egress、动作 capability、预算、审批、签名和 rollback 做硬 gate；
NDU 只能离线提议 selector/routing/retry/abstain artifact，不能在线改权限、拓扑或 effect。

### WEB.3 进程拓扑、transport 与 package 边界

P0 默认每个受管物理安装/host 有一个由 agentd 管理的 browserd control daemon；每个 task/
session 单独 worker、process group、临时 profile、netns/egress policy 和 cgroup，不共享 renderer、
cookie、profile、localStorage、cache 或 service-worker 状态。高风险多租户部署可按 workspace/
generation 分片 daemon，但保持同一合同。browserd/worker 重启会使旧 session、ref、operation
token 和 permit 失效，不能静默续用。

主通道使用权限收紧的 loopback UDS、OS peer credentials、agent generation/capability fence、
长度前缀 canonical CBOR；JSON 只用于 edge/debug。MCP 是薄 adapter，不取得 authority；Servo
WebDriver/Chromium CDP 端口和 raw protocol 永不向模型或公网暴露。
...
hepta-browser-core/
hepta-browser-client/
hepta-browserd/
hepta-browser-servo/
hepta-browser-chromium/
ext/hepta-browser-codex/
ext/hepta-browser-taskflow/
ext/hepta-browser-mcp/
~~~

依赖必须单向：contracts → core/client/daemon/backends；Codex/TaskFlow/MCP 只依赖 client/
contracts。browserd 不反向依赖 Codex core、OpenBao、CognitiveStore、Wallet/Market、通用业务 DB
或第二 event bus。browserd 可有小型 bounded session/worker journal，但 FlowRun/Step、
EffectIntent/Receipt、Memory、quota 和 fleet 状态仍由各自 owner 单写。

### WEB.4 Authority matrix

| 状态/动作 | 唯一权威 | browserd 权限 |
|---|---|---|
| Thread/turn/session/model/tool dispatch | Codex/agentd | 接收 typed call；不得创建第二 Thread/router |
| FlowRun/Step/retry/timer/join/cancel/reconcile | Agent-local TaskFlow store | 返回 activity outcome；不得推进 DAG |
| EffectIntent/EffectReceipt/compensation | TaskFlow + owning executor | Prepare/Execute 受 permit；ACK 不等于 effect success |
| daemon lifecycle/generation/fence | agentd/fleet supervisor | 管理有界 renderer child；不得管理 Agent/fleet |
| browser session/page/worker/ref/policy | hepta-browserd | 唯一 browser-control writer；仅短命状态/journal |
| URL/DNS/egress/node identity | Net/node-linkd + Governance | 消费 policy/permit；不得创建 route/TUN/relay authority |
| quota/lease/capability/SecretRef | AuthBus | 消费短期 opaque permit；不得读 OpenBao/刷新 OAuth/铸权 |
| Memory/KG/facts | CognitiveStore + validator | 只提交 untrusted observation/candidate |
| Evidence/trajectory append | Agent evidence writer/outbox | 生成 digest/ref；不得写共享 evidence authority |
| model/risk signal | Inferd | 可选 typed extraction；不得规划、点击或授予权限 |
| NDU artifact/promotion | Registry/NDU/governance | 仅输出脱敏 trajectory；不接受在线自改 policy |

### WEB.5 Semantic API、refs 与 capability modes

Agent 只看到：

~~~text
session.create / status / close / cancel
page.navigate
page.observe
page.act(click | type | fill | press | select | scroll)
page.wait
page.extract(schema)
page.open_result
browser.search(engine, query)
operation.get / operation.ack
health / capabilities
~~~

`observe` 返回 semantic tree：`snapshot_id`、`page_revision`、`session_generation`、origin、role/
name/text 摘要、有限 attributes 和短期 opaque ref；browser-trusted metadata 与页面 text/attrs
分栏，页面字段全部 `untrusted_web_content=true`。`act` 必须带 snapshot/ref、expected revision、
owner/generation/policy digest 和 idempotency key；stale/cross-session/cross-generation ref 全部
fail-closed。Agent 不使用截图/OCR/坐标；raw WebDriver/CDP 和任意 `eval` 默认不存在。

`extract` 只接受 bounded JSON Schema/allowlisted recipe，不接受任意 CSS/JS。P0 capability：

- `ObserveOnly`：navigate/observe/extract；默认；
- `InteractiveRead`：click/type/press/select/scroll；网络层默认仅 GET/HEAD/OPTIONS；
- `Prepare`：生成表单动作和 EffectIntent，不发送；
- `Execute`：需明确 capability、approval、permit、EffectReceipt/reconcile；仅 WEB-7/
  FINAL_RELEASE 后评估。

download、popup、新窗口、跨 origin frame、service worker、WebSocket、设备访问、持久 profile、
登录和私网默认 deny，后续逐项 feature/capability gate；DOM click 不能天然视为只读。

### WEB.6 Receipt、CAS 和失败语义

必须区分：

1. `BrowserActivityReceipt`：导航/观察/动作/等待的 worker outcome；不声称外部 effect；
2. `PageSnapshot/WebEvidenceReceipt`：URL/redirect/status/MIME/title/canonical、semantic/DOM/text/
   link/extract digest、engine commit、UA/locale/profile/policy、resource/network/challenge；页面内容
   是不可信 observation，不是 fact；
3. `EffectIntent/EffectReceipt`：submit/post/purchase 等外部写操作；canonical writer 是
   TaskFlow，browser 字段通过 typed sidecar/ref 关联，不向闭合 E21 top-level 随意加字段。

TaskFlow 先事务写 BrowserActivity/EffectIntent、outbox 和 owner fence，再 at-least-once 派发；
browserd 以 `(idempotency_key,payload_digest)` 去重并保留有界 result lookup。UDS ACK、Queued、
DispatchAccepted、HTTP 202 不是 terminal success。超时、断线、worker/daemon 崩溃、迟到 callback、
外部结果不明一律 `Indeterminate`，由 TaskFlow lookup/reconcile/compensate；禁止盲重试和猜成功。

receipt 至少绑定 exact source/head/tree、parent/stage/contracts/schema digest、request/command/run/
step/attempt、tenant/workspace/agent/thread、session/page/generation/revision、URL/redirect、engine/
SBOM/license、policy/capability/permit、network/resource、challenge/error、causal digest 和全部
negative-authority flags。raw HTML/header/body 如保留只能进加密、TTL、权限受控 CAS；cookie、
password、Authorization/raw secret 永不进入 receipt/log/普通 SQLite/LLM context。

### WEB.7 安全和反滥用

所有 worker 必须经过 egress policy proxy/netns，并在初始 URL、DNS、实际 connect、redirect、
iframe、subresource、worker、service worker、WebSocket、fetch/XHR 和下载逐跳重检。默认拒绝
loopback、RFC1918、CGNAT、IPv6 私网/link-local、multicast、云 metadata、Unix socket、未批准
端口/方法以及 `file:`、`data:`、`javascript:`、扩展协议；DNS rebinding 校验必须绑定解析结果
与实际连接。

worker 使用低权限 UID、只读 rootfs、临时目录、bubblewrap/landlock/seccomp/container、
cgroup/pidfd/进程组，限制 wall/CPU/RSS/PID/FD、action/navigation/redirect、请求/字节、DOM/
snapshot 大小和 profile TTL；Chromium sandbox 不能替代 OS/network sandbox。禁宿主文件、
credential、摄像头、麦克风、位置、剪贴板、扩展和默认下载；日志和 metrics 必须脱敏。

页面文本始终是不可信 prompt-injection 输入，不能直接触发 Memory/KG/Effect。consent、CAPTCHA、
429、anti-bot、unsupported 返回 typed `Challenge`/warning，不把挑战页当成功，不提供验证码求解、
指纹规避或站点条款绕过。

### WEB.8 Engine strategy

- **Servo primary**：WEB-3 固定 upstream commit，先以 `servoshell --headless --webdriver` 或窄
  native hook worker 验证；只维护 UDS/loopback、policy、receipt、lifecycle 所需最小 patch；
  不先深 fork renderer。MPL-2.0、SpiderMonkey 和依赖许可证/SBOM 单独审查。
- **Chromium fallback**：复杂 SPA、登录、Canvas/WebGL、兼容性缺口时由 capability matrix
  显式 `NeedFallback`；CDP 对模型隐藏。Servo→Chromium 不隐式复制 cookie、profile、localStorage
  或 JS state，需重新导航或受授权 checkpoint。
- Oxi/Aginx/Plasmate 等只作为 benchmark/API/recipe 参考；不能未经独立 security/capability
  receipt 进入默认路由。旧 Trillionnium ServoShell 只复用回归矩阵和集成经验，不是代码基线。

### WEB.9 WEB-0..WEB-8 实施阶段与 H/G gate 对齐

| 阶段 | 范围 | 最小退出证据 | 与既有 gate 的关系 |
|---|---|---|---|
| WEB-0 | authority/威胁/API/receipt/engine provenance/ownership | ADR、schema、golden vectors、negative authority | 可在 H0/G4 期间 RFC/shadow；不替代 H0 |
| WEB-1 | fake worker、UDS、ACL、idempotency、generation/fence、kill/restart | 两租户隔离、stale/duplicate/Indeterminate fixture | 类似 H1 seam，但不取得 H1 writer |
| WEB-2 | sandbox/egress/SSRF/DNS/资源/脱敏 | 全部网络和资源负例矩阵 | local qualification；无公网 listener |
| WEB-3 | pinned Servo；Google/Bing navigate→observe→extract | 20–50 URL/task corpus、capability/SBOM/license receipt | read-only shadow；不把 demo 当 efficacy |
| WEB-4 | Chromium 同合同 fallback | 同一 corpus、NeedFallback、无隐式状态迁移 | read-only shadow/known-gap |
| WEB-5 | Broker、Codex、TaskFlow、Evidence、Hosted adapter、MCP | typed caller/outbox/reconcile/provenance/fail-soft | 依赖 H1/H2/H3 typed seam；不绕过 spine |
| WEB-6 | opaque auth permit、isolated profile、login/cookie/challenge | raw-secret=0、跨租户泄漏=0、challenge policy | H8 前保持 gated/read-only |
| WEB-7 | submit/post/purchase 等网页写 effect | approval、effect key、lookup/reconcile、rollback | 对齐 H8/FINAL_RELEASE；不自动 grant |
| WEB-8 | HNL 远程 node browser service | HNL Gate0、identity/revoke/partition、无 global session store | 对齐 H9/R2-G5，future P2 |

WEB-1..WEB-4 可在 DEVELOPMENT/INTERNAL_TEST 独立推进，不阻塞 Hepta core；WEB-5 才接正式
typed activity；H8 仅单 Agent/单 workspace/read-only 或 simulated effect canary；H9 才评估多
Agent/node/remote。任何 WEB qualification 都不能替代 HNL/INF/AuthBus 各自 gate，不能自动改变
production/effect/CALLERS/G5/operator/promotion flags。

### WEB.10 Qualification DoD、维护与下一步

必测：canonical bytes/schema/version、stale ref/generation、duplicate payload conflict、cancel sticky、
kill/OOM/disk-full/timeout/restart/late callback、Indeterminate reconcile、SSRF/DNS rebinding/redirect/
subresource/iframe/worker/WebSocket、prompt injection/伪 ref/超深 DOM、两 tenant/profile/cookie/log/
receipt 隔离、Servo/Chromium parity/fallback、browserd 缺失时核心 fail-soft、reproducible build、
SBOM/license/MSRV、x86_64/aarch64 Linux。测试必须绑定命令、fixture/corpus digest、次数、耗时、
资源和 artifact digest；`NOT_TESTED/KNOWN_GAP/BLOCKED` 不得写成 PASS。

provisional targets（待实机/corpus receipt 后冻结）：安全不变量失败、跨租户泄漏、raw-secret
泄漏、stale-generation 接受、duplicate external effect 均为 0；read-only corpus 的一般任务目标
成功率 ≥90%、关键站点目标 ≥95%，但 Google/Bing 的地区/AB/CAPTCHA/429 单独记 Challenge；p95
延迟、冷/热启动、RSS/CPU/bytes/FD/PID 和 crash/timeout 目标按 engine/device profile 冻结。

长期维护内容：contracts/API、policy/sandbox/egress、worker supervisor、receipt/trajectory、site
recipe/corpus、CI/CVE/SBOM/license 和 rollback；指定 browser-runtime maintainer、security reviewer、
platform/build reviewer。浏览器安全补丁可以独立发版，但必须维持 N/N-1 contract compatibility；
Hepta core 构建和运行不得强依赖 Servo 源码或 browserd 存在。

第一条 vertical slice：WEB-0 contract → WEB-1 fake daemon/UDS/fence → WEB-2 controlled fixture
sandbox/egress → WEB-3 pinned Servo → Google/Bing URL navigate → semantic observe/ref → extract
result cards → open_result → PageSnapshot/WebEvidenceReceipt → TaskFlow reconcile fixture。验收必须
证明 Agent 未使用截图/OCR/坐标，raw WebDriver/CDP/eval 未进入模型面，网页内容未直接写 Memory/
KG/Effect，旧 ref/generation 被拒绝，browserd 被杀后 Codex/TaskFlow 仍可恢复。

本节仍然只是实现计划和机器合同输入；它不表示 crates 已创建、Servo/Chromium 已构建、目标
语料通过、外部站点放行、真实 effect 执行、operator acceptance 或 production release。

## WEB-PLAN-2026-08-27B — Shared live Obscura session / Human-Agent UI successor（append-only）

> **SUPERSEDED — archive/provenance only.** 本 B successor 的 Obscura runtime 选择已被
> `WEB-PLAN-2026-08-27D` 取代；只保留历史审计。当前实现必须遵循 Servo-only D 方案和
> 唯一活动阶段 `WEB-C0`…`WEB-C7`。

本 successor 根据用户对人机协同的进一步约束，修订 WEB-PLAN-2026-08-27A 的“Obscura→Servo
人工交接”方向：P0 不迁移页面、不切换引擎，Agent 和真人共同控制**同一个 Obscura
BrowserContext/Page**。A 中的跨引擎 checkpoint 只保留为未来显式例外，不再是普通人工接管路径。
本节仍为 `PLANNING_ONLY / IMPLEMENTATION_BACKLOG`，不创建 production caller/writer/effect。

### WEB-B.1 绑定与决策

- parent plan pre-append SHA-256：`83b8a7de2802a4c5ccb2fafb6b0a0e972cc433364f8c508c800f7228b2aa9e65`
- preceding detailed runtime plan SHA-256：`d90da74d3732fcc74b952530a3d523c7a369bb48a8c5f371d6ec15606e2b5c7b`
- shared-session detailed plan：`OpenClaw/hepta-browser-shared-session-plan-2026-08-27.md`
- shared-session plan SHA-256：`9c62ed3055861a4dfb39227c13f1314f57e722cc34248e6bd3fb2cc8785e3e24`
- reviewed Obscura source：`h4ckf0r0day/obscura` `v0.2.1` / `2810cb478696885e0d44d1741cbf586f1cc98bb5`
- current status：`CONTINUE_PRE_RELEASE / SHARED_SESSION_DESIGN_ONLY`

没有名为 `ObscuraBackend` 的现成单 crate，也没有现成 headed GUI/window crate。最低成本
路线是 fork/vendor Obscura workspace，复用 `obscura-browser`（Page/Context）、
`obscura-mcp`（semantic tool/ref）、`obscura-cdp`（协议/截图/输入）和
`obscura-render`（layout/raster），由 Hepta 新增薄的 `SessionBroker/PageOwner`、双端
仲裁、policy/receipt 和 UI adapter。预计核心代码约 60–75% 可复用；不是把 Servo fork
塞入 Hepta core。

### WEB-B.2 单浏览器双控制端拓扑

~~~text
Hepta Planner/TaskFlow -> WebCapabilityBroker -> hepta-browserd (loopback UDS)
                                                   |
                                  SharedSessionBroker / PageOwner
                                  one BrowserContext + one Page
                                  one LocalSet + one V8 isolate
                                      /                 \
                         Agent semantic API          Human UI/events
                                      \                 /
                                  ordered command arbiter
                                                   |
                                           Obscura runtime
~~~

`Page` 是唯一 browser-control writer。Agent 与 UI 不各自连接 stock `obscura-cdp`、不各自
创建 context，也不跨线程持有 V8；所有 navigate/observe/act/keyboard/mouse/wheel 都排入
同一 owner 的有界队列。这样真人输入之后 Agent 读取的是同一 DOM、JS heap、cookie、
localStorage、history 和 network state。

Obscura v0.2.1 的 CDP server 当前按 WebSocket 创建隔离 `CdpContext`/页面/V8 线程，不能
直接靠两个连接实现共享 live page。上游 [PR #634](https://github.com/h4ckf0r0day/obscura/pull/634)
提出 global target registry/cross-connection routing/page persistence，但截至本计划日期仍
未合并；它只能作为后续移植参考，不能作为 released dependency。P0 采用 browserd 内部
单 owner + channel multiplexing，避免把复杂 routing 暴露给模型。

### WEB-B.3 人机仲裁和 UI

控制状态固定为 `SHARED_READ`、`AGENT_TURN`、`HUMAN_TURN`、`CHALLENGE_PAUSED`：

- observe 可读取最近稳定快照；mutation 同时只能有一个 owner；
- 人类鼠标/键盘活动或 `human.take_control` 获得短 lease，挂起未执行的 Agent mutation；
- `human.release_control` 后必须刷新 snapshot/revision，旧 ref fail-closed；
- lease TTL、owner epoch、session/page revision、generation 和 actor 写入 receipt；
- UI 不得绕过 TaskFlow 的 Prepare/Execute、审批、EffectReceipt 或 reconcile。

P0 先做本机 local web UI：复用 `Page.captureScreenshot`/`Page.screenshot` 和
`tools/live-view.mjs` 的 frame polling，再通过 loopback WebSocket/UDS 把鼠标、键盘、wheel
事件送进同一 owner。它是同一个 Obscura page 的显示/输入端，不是第二个页面。Obscura
当前没有 winit/egui/tao、OS event loop 或独立 window compositor；原生窗口需后续新增
`hepta-browser-ui`（winit + pixels/softbuffer 等），并单独做 DPI/IME/剪贴板/resize/drag
qualification。当前 Input handler 是 page-side JS synthetic/marked events，不能预先承诺
WebAuthn、文件选择器、复杂拖放、Canvas/WebGL 或站点反爬所需的真正 OS-trusted input。

### WEB-B.4 角色和阶段 successor

| 阶段 | 工作 | 最小退出证据 | 状态 |
|---|---|---|---|
| `WEB-S0` | pin/vendor Obscura、Cargo.lock、SBOM、license；抽 crate API | source/artifact receipt | planning |
| `WEB-S1` | 从 `obscura-mcp` 抽 semantic/ref control 与 `hepta-browser-contracts` | golden vectors、stale-ref fixture | qualification |
| `WEB-S2` | 单 PageOwner/LocalSet、Agent UDS、queue、lease、generation/fence | 双端同 DOM/cookie；无第二 isolate | qualification |
| `WEB-S3` | local UI frame stream、mouse/key/wheel、pause/resume、脱敏 | Agent→UI→Agent same-page test | internal test |
| `WEB-S4` | Google/Bing、JS、React/Vue、分页、frame/popup、storage corpus | 20–50 task parity/challenge receipt | read-only shadow |
| `WEB-S5` | sandbox/egress/SSRF、receipt、TaskFlow/Evidence adapter | negative matrix、raw-secret=0、reconcile | internal test |
| `WEB-S6` | 可选 native UI shell；不改变 PageOwner | window/input qualification | optional |
| `WEB-S7` | Servo/Chromium session-from-start backend | capability/fallback receipt；无 live migration | gated |
| `WEB-S8` | login/profile/remote UI/node | AuthBus/H8/H9/HNL successor | future |

第一条 vertical slice 改为：`WEB-S0 → WEB-S1 → WEB-S2 → WEB-S3` 的本地 fixture，先证明
`Agent navigate → 真人 UI 看到 → 真人输入 → Agent observe/extract`；通过后再跑公网
Google/Bing。Servo 仅作独立 standards/reference 或任务开始即选定的 headed backend，不是
Obscura 人工接管的热切换目标。Chromium 同理只在 session 创建时选择，或由显式
`NeedFallback` 开新 session；Challenge/Indeterminate 不自动切换。

### WEB-B.5 维护和安全边界

Hepta 长期维护 browserd contract、PageOwner/arbiter、UI adapter、policy/sandbox/egress、
receipt/evidence、corpus、CI/SBOM/CVE 和 Obscura 最小 patch queue；不承诺维护完整通用
浏览器 fork。页面内容是不可信 observation，不能直接写 Memory/KG/Effect；cookie/password/
Authorization/clipboard/file path 不进入模型、普通日志或 receipt。browserd 仍是独立进程
和故障域，browserd 缺失时 Hepta 必须可退化到 HostedSearchAdapter/静态读取。

本 successor 不授权生产监听、真实网页写操作、持久 profile、远程 GUI、验证码/反爬绕过、
Obscura fork 发布或任何现有 H8/H9/G5/operator/promotion flag；旧 WEB-A 文字保留为审计
provenance，shared-session plan 是当前人机协同实现输入。

## WEB-PLAN-2026-08-27A — ObscuraBackend qualification overlay (append-only)

> **SUPERSEDED — archive/provenance only.** 本 A overlay 不再授权或指导当前实现；Obscura
> 仅保留合同/fixture 参考，当前 live runtime 只能是 Servo，详见 D successor。

This is the successor append to WEB-PLAN-2026-08-27 and E.45. It corrects the
effective engine qualification route without rewriting the historical WEB
section or its v1 attachments. All authority, production, effect, CALLERS,
G5 and operator flags remain false.

### WEB-A.1 Current binding and new attachment set

- parent plan SHA-256 before this append:
  47a0f501754a0448c9f5bb5c26d951365539d1d189bb57f776283317c9491edb
- parent pre-append EOF line: 4994
- successor companion:
  OpenClaw/hepta-browser-obscura-qualification-plan-2026-08-27.md
  SHA-256: 0da9b069113dbe92441c647da03bd8f698c503557a5661cac8ccb8e6d53d212e
- successor stage matrix:
  OpenClaw/HEPTA_BROWSER_STAGE_MATRIX_v1_1.yaml
  SHA-256: 4d6df4f02efd5e8429a85843263f026667f33acd069aff9c88e48ae2466e961f
- historical v1 matrix and receipt schema remain immutable provenance:
  matrix SHA-256 45b080ed7e8dd62bbb3b4288cefb9426ba6ebe6f0d485d9e32ff2094f59aaafd;
  schema SHA-256 596b5c235e02d38875bdc55927d912f58c3b98332b4defa3ec3ec813f5789d79
- decision: CONTINUE_PRE_RELEASE; no production caller/writer/effect grant.

If the parent plan or any attachment changes again, a new successor binding is
required. The old WEB section is retained for audit but is superseded for
engine routing by this overlay.

### WEB-A.2 Obscura finding and decision

The evaluated project is h4ckf0r0day/obscura, release v0.2.1 (tag commit
2810cb478696885e0d44d1741cbf586f1cc98bb5), reviewed main commit
f449e6fb3183138eb3e80f11fe44af31cefe0fae, Apache-2.0. It is a Rust
agent-oriented runtime with html5ever/custom DOM and CSS/layout, deno_core/V8,
optional native rendering, CDP, MCP and a direct Rust API. It is not a Servo
fork or a standards-complete browser.

Obscura is a strong candidate for the Hepta use case of direct Google/Bing URL
navigation, semantic observation, ordinary JavaScript settlement, bounded
result extraction and limited DOM actions. Its project-reported performance
figures are claims only until reproduced. Service workers, some Web APIs,
long-tail CSS/compositor behavior and cross-document edge cases remain
qualification items. Pages in one process share a single V8 isolate.

The effective policy is therefore:

  WebCapabilityBroker -> hepta-browserd
    -> ObscuraBackend (candidate ObserveOnly fast path)
    -> ServoBackend (standards/reference backend)
    -> ChromiumBackend (explicit compatibility backend)

No engine is promoted by README claims, star count or a single smoke test.

### WEB-A.3 Effective stage and fallback changes

The successor matrix adds WEB-3A (Obscura read-only qualification) in parallel
with WEB-3 (Servo), WEB-4A (semantic-action parity and side-effect
qualification), and WEB-4R (Chromium explicit fallback). WEB-5 now consumes a
backend-neutral capability matrix. WEB-6 requires a fresh AuthBus permit and
profile gate. WEB-7 explicitly requires H8/H9, R2-G5, the TaskFlow E21 effect
seam and operator approval. WEB-8 is split into WEB-8R remote read and WEB-8E
remote effect; WEB-8R does not depend on WEB-7.

Automatic fallback is allowed only for a typed NeedFallback caused by an
unsupported capability or renderer failure before an external effect. It uses
a fresh session/profile and records lineage. Consent, CAPTCHA, 429, anti-bot,
policy denial and an unknown external outcome return Challenge,
PolicyDenied or Indeterminate; they never trigger automatic Chromium fallback.

Obscura MCP/CDP/raw evaluate, raw cookie/header access, upload/download,
stealth, allow-private-network, allow-file-access and persistent storage are
not model-facing defaults. Every Obscura worker remains subject to the
browserd UDS/peer ACL, per-task process/container, egress proxy, DNS/redirect/
subresource/WebSocket checks, resource budgets and profile wipe.

### WEB-A.4 Qualification and maintenance boundary

The same 20–50 task corpus must run on pinned Obscura, Servo and Chromium and
record typed outcomes, action semantics, extraction, p95 latency, RSS/CPU,
crashes/timeouts, challenges, SSRF and cross-tenant/secret leakage. Stale-ref
rejection is 100%; security, raw-secret, cross-tenant and duplicate-effect
failures are zero; read-only success targets remain provisional at >=90%
(key sites >=95%), with challenges counted separately.

Hepta owns the browserd contract, policy, supervisor, receipt/evidence bridge,
corpus and security CI. Engine artifacts are independently pinned, scanned,
licensed, upgraded and rolled back. Obscura may become the default ObserveOnly
fast path only after qualification; Servo remains the standards/reference
baseline and Chromium the explicit compatibility path. This overlay authorizes
no Obscura fork, login, external write, remote listener or production release.

References:

- Obscura repository: https://github.com/h4ckf0r0day/obscura
- Obscura architecture: https://github.com/h4ckf0r0day/obscura/blob/main/docs/Architecture-overview.md
- Obscura security: https://github.com/h4ckf0r0day/obscura/blob/main/SECURITY.md
- Obscura production/auth limits: https://github.com/h4ckf0r0day/obscura/blob/main/docs/Run-in-production-at-scale.md
- Servo WebDriver: https://github.com/servo/servo/blob/main/components/webdriver_server/lib.rs

## WEB-PLAN-2026-08-27C — Single-engine live browser decision (append-only)

> **SUPERSEDED — archive/provenance only.** 本 C decision 已由可执行的 D successor 取代；
> C 的 Servo-only 结论保留作 provenance，但实现、CI 和发布只读取 D 及活动索引。

本 successor 进一步收敛引擎选择。用户的硬约束是：只保留一个 browser，真人 UI 与 Agent
API 共享同一个 live page，同时尽量降低 `hepta-browserd` 的开发成本。结论是：**只编译和
运行一个 Servo WebView；从 Obscura 复用语义合同、ref/snapshot 规则和测试设计，不把
Obscura 的 DOM/JS/layout/render/net runtime 链进 Servo。**

### WEB-C.1 Binding and supersession

- parent plan SHA-256 before this append: `2b8ddcd96ffe0eb7a8ec13f9c1d0b1f2b35af0031c4a8be354387a3368b2d3c2`
- detailed decision memo: `OpenClaw/hepta-browser-engine-selection-2026-08-27.md`
- decision memo claim level: `PLANNING_ONLY / IMPLEMENTATION_INPUT`
- source Servo: public `servo` embedding API plus pinned/forked `servoshell` pattern
- source Obscura: `v0.2.1` / `2810cb478696885e0d44d1741cbf586f1cc98bb5`, Apache-2.0
- status: `CONTINUE_PRE_RELEASE`; no production caller, writer, login or external effect

This successor supersedes the **engine-choice** portions of WEB-PLAN-2026-08-27A/B and
WEB-R-PLAN-2026-08-27B. Their shared-session, authority, safety and audit history remains valid;
the old Obscura-primary text is retained as provenance, not as the current implementation route.

### WEB-C.2 Why the engine cores cannot be composed

Obscura's `obscura-dom`/`obscura-browser` use a custom `DomTree`/`NodeId`, `obscura-js` uses
`deno_core`/V8, `obscura-render` uses Taffy/tiny-skia, and `obscura-net` owns reqwest/cookie state.
Servo uses its script DOM/SpiderMonkey heap, Stylo/layout/WebRender and Servo net/IPC state.
The shared `html5ever` dependency is only a parser dependency; it does not make DOM nodes,
JavaScript heaps, layout trees, display lists, cookies or event loops interchangeable. A Servo
`RenderingContext` cannot directly display an Obscura raster surface. Linking both stacks would
create two page owners or a new translation-heavy browser engine and violates the one-live-page
invariant.

There is no released `ObscuraBackend` crate. `obscura-mcp` and `obscura-cdp` handlers are tied to
concrete Obscura `Page`/`CdpContext`; stock CDP connections also create isolated contexts/pages/V8
threads. They may be mined for schemas and fixtures, but are not drop-in Servo adapters.

### WEB-C.3 Reuse boundary

Allowed reuse from Obscura:

- tool vocabulary and JSON shapes: `navigate`, `observe`, `act`, `wait`, `extract`;
- semantic refs, snapshot/page-revision and stale-ref rejection rules;
- bounded markdown/structured-extract behavior and golden fixtures;
- generic serde/URL/HTTP/Tokio dependencies where independently useful.

Not imported into the Servo runtime:

- `obscura-browser`, `obscura-dom`, `obscura-js`, `obscura-render`, `obscura-net`;
- concrete `obscura-cdp` domain handlers or a second V8/page context.

The portable layer becomes `hepta-browser-contracts`; its Servo implementation maps semantic
operations to WebDriver/native hooks and bounded script evaluation. Apache notices are preserved
if code is copied. Servo MPL-2.0, SpiderMonkey, WebRender and all transitive licenses receive a
separate SBOM/license review.

### WEB-C.4 Single-browser topology and UI

```text
Hepta TaskFlow/Planner
          | authenticated loopback UDS
          v
hepta-browserd / BrowserActor (one owner, one event loop)
          Servo + one WebView + one RenderingContext
             /                         \\
      AgentPort (semantic RPC)     HumanPort (headed servoshell/winit UI)
             \\                         /
             ordered arbiter / lease / policy / receipt
```

P0 may launch a pinned headed `servoshell` without `--headless`, with `--webdriver`, and proxy
semantic calls through a local authenticated channel. The human window and WebDriver target are
the same WebView; it may start hidden/minimized and be revealed for takeover, subject to platform
qualification. Patch the current `0.0.0.0` listener to loopback/UDS and peer authentication.
P1 embeds `BrowserActor`/semantic hooks in the Servo event loop to remove the WebDriver hop and
make arbitration authoritative in one queue. A pure headless Servo process must not later be
converted into a different GUI page; the headed-capable session is created from the start.

Human and Agent mutations are serialized. Human input can preempt a pending Agent mutation;
navigation, UI input and DOM rerender advance page revision and invalidate old refs. TaskFlow
retains approval, external-effect, retry and reconcile authority; UI input is never a permission
bypass. Raw WebDriver/CDP/eval is not model-facing.

### WEB-C.5 Revised stage order

| Stage | Scope | Exit evidence | Status |
|---|---|---|---|
| `WEB-C0` | decision memo, authority/threat/API contracts, Servo/Obscura provenance | ADR, schema, source/license receipt | planning |
| `WEB-C1` | pinned/forked headed servoshell, loopback/UDS WebDriver, one WebView | same-window smoke; no public listener | qualification |
| `WEB-C2` | `hepta-browser-contracts` from Obscura semantics; Servo adapter | golden observe/ref/extract vectors; stale-ref fail-closed | qualification |
| `WEB-C3` | BrowserActor/arbiter, AgentPort + HumanPort, lease/revision/fence | human and Agent same DOM/JS/cookie/history; ordered mutation | internal test |
| `WEB-C4` | local fixtures then Google/Bing and 20–50-site read-only corpus | same-page receipts, challenge/compatibility matrix | read-only shadow |
| `WEB-C5` | sandbox/egress/SSRF, evidence/TaskFlow bridge, fail-soft | negative matrix; raw-secret/cross-tenant zero | internal test |
| `WEB-C6` | native window/IME/clipboard/show-hide qualification | platform input receipt; no state copy | gated |
| `WEB-C7` | auth/profile and external web effects | AuthBus, approval, idempotency, reconcile | H8/H9/FINAL_RELEASE only |

The first vertical slice is `WEB-C1 → WEB-C3`: `Agent navigate → human sees the same page →
human click/type → Agent observe/extract`. Obscura remains a benchmark/reference source; it is not
loaded beside Servo in the production browser process. If a future product explicitly accepts a
local web viewer with synthetic input instead of a native window, that is a separate Obscura-only
product choice, not a hybrid engine.

### WEB-C.6 Cost and decision gate

- Native OS window, native focus/IME or manual login/challenge is a hard requirement: Servo-only is
  the lowest total complexity because the window, compositor and event loop already exist.
- A local screenshot/web viewer and synthetic input are sufficient: Obscura-only can be the
  cheaper experiment, but Servo must not be added to that live runtime.
- Obscura+Servo engine-crate composition: rejected; it has the highest implementation and
  maintenance cost and cannot preserve one live DOM/JS state.

All effort and success-rate numbers remain provisional until source-bound, machine-generated
qualification receipts exist. This successor changes no existing production/effect/promotion gate.

## WEB-PLAN-2026-08-27D — Canonical Servo-only browserd implementation plan（append-only）

本 successor 是当前 Hepta 浏览器工作的唯一活动实现入口，专门消除前面 A/B/C successor
并存造成的歧义。**实现者、CI、路由和发布脚本只读本 D successor、D runtime、v1.3
matrix 和 active-plan index；A/B/C 及旧附件仅作归档 provenance。**

### WEB-D.1 唯一决策

- 一个 `BrowserSession` 只运行一个 Servo live runtime、一个 `WebView`、一个
  `RenderingContext`、一个 event loop/PageOwner；真人 UI 与 Agent API 共享同一页面状态。
- 规范部署是一 session 一 `hepta-browserd` 进程，由 agentd/fleet supervisor 管理；并发
  session 只能是多个隔离进程，不能共用 actor、renderer、profile 或 cookie。
- `hepta-browserd` 直接拥有 Servo/servoshell-derived headed event loop；Agent UDS 请求和
  winit 人工输入都进入同一 BrowserActor/arbiter。P0 可在同一进程内复用 Servo WebDriver
  handler 作为内部转换代码，但不得开放第二个 TCP listener 或第二个 page owner。
- Obscura 只贡献 `navigate/observe/act/wait/extract` 合同、semantic ref/revision 规则和
  golden fixtures；`obscura-browser/dom/js/render/net` 及具体 CDP handler 不进入 Servo
  dependency graph。Obscura+Servo 混合内核、live migration 和 Chromium backend 不属于本
  活动计划。
- Codex/OpenAI hosted `web_search` 只能作为 WebCapabilityBroker 的并列可选检索后端，
  不是 browserd 依赖；browserd 不自建索引或 search provider。

### WEB-D.2 规范文件与唯一阶段命名

| 作用 | 唯一活动文件 |
|---|---|
| 主计划实现入口 | `hepta-vnext-development-plan-final-2026-08-23.md` 本 D successor |
| 详细 browserd 设计 | `hepta-browser-servo-development-plan-2026-08-27.md` |
| runtime 合同 | `hepta-browser-runtime-plan-2026-08-27.md` 本 D runtime successor |
| 阶段矩阵 | `HEPTA_BROWSER_STAGE_MATRIX_v1_3.yaml` |
| receipt 合同 | `hepta.browser_receipt.v2.schema.json` |
| 活动索引 | `HEPTA_BROWSER_ACTIVE_PLAN.md` |
| 绑定 receipt | `HEPTA_BROWSER_SERVO_PLAN_RECEIPT-2026-08-27.json` |

活动阶段只有 `WEB-C0`…`WEB-C7`：C0 决策/合同，C1 headed Servo，C2 semantic contracts，
C3 BrowserActor/双控制端/仲裁，C4 Google/Bing 与 20–50 站点只读 corpus，C5 安全与
Hepta 集成，C6 本机 UI/IME 平台资格，C7 AuthBus/profile 及 H8/H9 审批后的外部 effect。
不得再创建或引用 `WEB-S*`、`WEB-A*`、`WEB-B*` 或另一套 D 阶段编号作为活动实施状态。

### WEB-D.3 实施硬约束

`hepta-browserd` 必须保持独立进程/故障域和 fail-soft；TaskFlow 保持 retry/cancel/effect/
reconcile 权威，agentd 保持 lifecycle/generation/fence 权威，网页内容保持 untrusted
observation。Agent 只获得语义 API，不获得 raw WebDriver/CDP/unrestricted eval。所有出网、
DNS rebinding、redirect、iframe、worker、WebSocket、下载和资源预算按 C5 安全门处理；
consent/CAPTCHA/429/anti-bot/unknown outcome 返回 typed Challenge/Indeterminate，绝不自动
切换 runtime 或绕过。

首条 vertical slice 固定为：`C0 → C1 → C2 → C3 → local fixture navigate → human sees/
clicks/types → agent observe/extract → ActivityReceipt + WebEvidenceReceipt`。在此通过前不
接真实账号、生产 listener、持久凭证或外部网页写操作。

### WEB-D.4 历史附件清理

以下文件已移至 `OpenClaw/archive/2026-08-27-browser-plan-superseded/`，保留原始 hash
供审计但禁止作为当前输入：旧 runtime、Obscura qualification、Obscura shared-session、
v1/v1.1/v1.2 matrices、旧 shared-session receipt 和 C 版本旧绑定 receipt。归档目录的
`README-SUPERSEDED.md` 是唯一恢复说明；重新启用任何历史方案必须创建新的 successor
decision/receipt，不能直接移回活动目录。

本 D successor 将 C 的 Servo-only 决策落实为可执行的单一计划，但不改变 Hepta 既有
H8/H9、G5、operator、promotion、effect 或 production gate；所有实现和 qualification
仍为 `PLANNING_ONLY`，直到后续 receipt 明确授权。
