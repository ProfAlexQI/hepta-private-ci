...
`existing_legacy_aggregate` is a source-location fact. For a target or unbound module, development begins with `P0.7B-B1A-PROVIDER-BOUNDARY` and materializes the declared root before source completion. Aggregate evidence roots are read-only decomposition evidence; they transfer no ownership and cannot become a second permanent implementation. A source move updates `MODULES.json`, `SOURCE_BINDINGS.json` and this guide in one candidate.
