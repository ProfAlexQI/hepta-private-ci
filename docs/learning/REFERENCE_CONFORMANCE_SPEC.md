...
`PAPER-NDU-FOUNDATIONS-2024`, `PAPER-NDU-UPA-2025`, `PAPER-NDU-EU-2025` and `PAPER-HOLDER-Q-2026` are tracked by exact title, author, date, publication and claim/non-claim scope in `PAPER_TRACEABILITY.json`. A theorem or approximation statement is used only when its assumptions are represented by a current certificate.

Fixed-point durability, counter-based reproducibility, typed authority separation, causal outcome ownership, exact Git identity, synthetic merge verification, rollback, unlearning and claim ladders are Hepta conformance extensions. They must not be attributed to the papers.
