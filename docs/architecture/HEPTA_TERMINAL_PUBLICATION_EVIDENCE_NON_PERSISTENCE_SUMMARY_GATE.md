...
`scripts/hepta-terminal-publication-evidence-non-persistence-summary-gate.sh` is a schema-only terminal summary over the public-distribution non-publication lock and watchdog evidence.
...
It is an additional terminal denial summary, not an activation path.
