...
`BridgeSnapshot`. The Native production facade therefore remains disabled by
default and cannot become live without an explicit authenticated executor.
...
contains a snapshot-only transport seam whose executor accepts only a body-free
GET descriptor. `HeptaBridge::try_live` captures a concrete authenticated
...
2. Authenticate the caller and bind the request to the already-authenticated
   ...
   proof.
3. Accept caller-issued run, session, correlation, and expected-sequence
...
orchestration must remain absent until the backend contract exists and an
actual live integration test can exercise the lifecycle.

## Live Receipt Contract

A future live receipt must be generated by the Native process from an actual
request and response. The first promotion target is the exact-source macOS
authenticated post-login surface. Its receipt must include:
...
- `fixture_or_mock_absent=true`, attested by the independent runtime lane;
- expected and observed run/session/correlation/sequence match booleans;
