...
The route gate validates the source replay/idempotency denial gate, native gateway route wiring, source command count `102`, the focused codex-cli native unit test, optional live endpoint parity, and terminal coverage through `scripts/hepta-preflight.sh`.
