...
| project-robius/robrix | b2bb6cf | MIT | `packaging/icon_google_play_512.png` | `apps/hepta-native/packaging/icon_google_play_512.png` | exact duplicate retired from Git; deterministically materialized at build time from canonical `apps/hepta-native/resources/icon_512.png` with SHA-256 and byte-count enforcement | deduplicated_generated |
...
| project-robius/robrix | b2bb6cf | MIT | `src/home/main_mobile_ui.rs` | — | retired after the adaptive `HomeScreen` became the sole instantiated desktop/mobile root; upstream provenance remains recorded here | retired |
| project-robius/robrix | b2bb6cf | MIT | `src/home/mod.rs` | `apps/hepta-native/src/home/mod.rs` | copied then narrowed to register only active widgets after legacy mobile-root and no-action search cleanup | heavily_modified |
...
| project-robius/robrix | b2bb6cf | MIT | `src/home/rooms_list.rs` | `apps/hepta-native/src/home/rooms_list.rs` | copied then hardened so inactive shared header categories are ignored instead of panicking | heavily_modified |
...
| project-robius/robrix | b2bb6cf | MIT | `src/home/search_messages.rs` | — | retired because its disabled button was never instantiated and its click handler emitted no action | retired |
...
- `apps/hepta-native/src/home/main_mobile_ui.rs`: historically added a mobile safety wrapper during M2/M3; it was later retired when `HomeScreen` became the sole authoritative adaptive root.
